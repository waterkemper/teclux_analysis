...
- Certificado A1 é custodiado por Contribuinte Fiscal e associado explicitamente a uma ou mais Filiais, com histórico, validação, ativação, alertas, criptografia e Auditoria; uma Filial pode usar certificado compartilhado ou próprio.
...
- [Definir a custódia e a operação dos Certificados A1](issues/06-definir-seguranca-certificados-a1.md) — adota versões custodiadas por Contribuinte Fiscal e vínculos compartilhados ou exclusivos por Filial, com PFX/senha cifrados no PostgreSQL, validação/ativação administrativa, uso efêmero pelos workers, alertas, recuperação testada e expurgo do material privado.
...
- [Consolidar a especificação da plataforma fiscal no Laravel](issues/11-consolidar-especificacao-fiscal.md) — reúne as decisões em uma especificação rastreável com 91 requisitos verificáveis, matriz de aceite, runbooks e sequência pronta para decomposição em tickets de implementação.
- [Definir a associação de Certificados A1 a grupos de CNPJ e Filiais](issues/15-definir-associacao-certificado-grupo-cnpj.md) — separa Contribuinte Fiscal de Estabelecimento Fiscal, permite certificado compartilhado ou próprio por associação explícita e exige compatibilidade oficial, escopo e seleção determinística.
- [Definir o monitoramento de atualizações de leiaute e regras da SEFAZ](issues/16-definir-monitoramento-atualizacoes-layout-sefaz.md) — cria Catálogo Normativo Fiscal e Observador Regulatório com polling HTTP, snapshots/hash/diff, escopo por UF/modelo/ambiente e aprovação humana antes de homologação e produção.
