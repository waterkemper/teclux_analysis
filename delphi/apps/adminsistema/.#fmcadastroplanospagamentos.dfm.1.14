inherited frmCadastroPlanosPagamentos: TfrmCadastroPlanosPagamentos
  Left = 450
  Top = 156
  Action = actHabilitar
  Caption = 'Cadastro de planos de pagamentos'
  ClientHeight = 547
  ClientWidth = 612
  FormStyle = fsMDIChild
  Position = poDesktopCenter
  PixelsPerInch = 96
  TextHeight = 16
  inherited pnlBarra: TPanel
    Width = 521
  end
  inherited pnlTopMenu: TPanel
    Width = 612
    TabOrder = 2
    inherited tblBarra: TToolBar
      Width = 387
    end
  end
  object pnlFundoJanela: TPanel
    Left = 0
    Top = 45
    Width = 612
    Height = 502
    Align = alClient
    BevelOuter = bvNone
    TabOrder = 1
    inline fraComissao: TfraComissao
      Left = 339
      Top = 123
      Width = 171
      Height = 83
      HorzScrollBar.Range = 171
      VertScrollBar.Range = 77
      AutoScroll = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      TabOrder = 11
      inherited gbxComissao: TGroupBox
        Top = 6
        Width = 140
        Height = 68
        Caption = ' COMISS'#195'O '
        Font.Height = -11
        Font.Style = [fsBold]
        inherited lblComissao: TLabel
          Left = 65
          Top = 29
          Width = 70
          Height = 14
          Font.Height = -11
          Font.Style = [fsBold]
          ParentFont = False
        end
        inherited dtxComissao: TDBEditNumero
          Left = 56
          Top = 42
          Width = 81
          Height = 23
          Font.Height = -12
          Alignment = taRightJustify
          DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
        end
        inherited rgpTipoComissao: TtecDBRadioGroup
          Left = 2
          Top = 13
          Width = 56
          Caption = ' TIPO '
          Font.Height = -11
          Font.Style = [fsBold]
          ParentFont = False
          DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
          inherited rbnPercentual: TtecRadioButton
            Left = 9
            Font.Height = -12
          end
          inherited rbnValor: TtecRadioButton
            Left = 9
            Font.Height = -12
          end
        end
      end
    end
    object gbxCodigo: TGroupBox
      Left = 6
      Top = 10
      Width = 60
      Height = 36
      Caption = ' C'#211'DIGO '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -9
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      TabOrder = 0
    end
    object gbxDescricao: TGroupBox
      Left = 76
      Top = 10
      Width = 370
      Height = 36
      Caption = ' DESCRI'#199#195'O '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -9
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      object edtDescricao: TDBEditTexto
        Left = 2
        Top = 10
        Width = 365
        Height = 23
        DataField = 'descricao'
        DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        MaxLength = -1
        ParentFont = False
        TabOrder = 0
        CharCasenoDefault = False
        Alignment = taLeftJustify
      end
    end
    object gbxJuros: TGroupBox
      Left = 456
      Top = 10
      Width = 64
      Height = 36
      Caption = ' JUROS '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -9
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      TabOrder = 2
      object Label3: TLabel
        Left = 48
        Top = 14
        Width = 11
        Height = 15
        Caption = '%'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
      end
      object edtTaxaMensalJuros: TDBEditNumero
        Left = 2
        Top = 10
        Width = 43
        Height = 23
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        MaxLength = -1
        ParentFont = False
        TabOrder = 0
        Mascara = True
        TipoMascara = tmGERAL
        NrDecimal = 2
        Decimais = True
        Negativo = False
        Tamanho = 2
        DataField = 'taxamensaljuros'
        DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
      end
    end
    object gbxMultiplo: TGroupBox
      Left = 529
      Top = 10
      Width = 60
      Height = 36
      Caption = 'M'#218'LTIPLO'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -9
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      TabOrder = 3
      object edtMultiplo: TDBEditNumero
        Left = 2
        Top = 10
        Width = 55
        Height = 23
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        MaxLength = -1
        ParentFont = False
        TabOrder = 0
        Mascara = False
        TipoMascara = tmGERAL
        NrDecimal = 2
        Decimais = True
        Negativo = False
        Tamanho = 3
        DataField = 'multiplo'
        DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
      end
    end
    object gbxAgente: TGroupBox
      Left = 6
      Top = 50
      Width = 514
      Height = 36
      Caption = ' AGENTE '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -9
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      TabOrder = 4
      object sbnConsultaAgente: TSpeedButton
        Left = 63
        Top = 10
        Width = 23
        Height = 23
        Hint = 'Procurar Agente'
        Flat = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -13
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        Glyph.Data = {
          
          DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
        NumGlyphs = 2
        ParentFont = False
        OnClick = sbnConsultaAgenteClick
      end
      object flkAgente: TtecDBFindLookup
        Left = 2
        Top = 10
        Width = 60
        Height = 23
        Alignment = taLeftJustify
        DataField = 'agente'
        DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        MaxLength = 4
        Maximo = 0
        Minimo = 37353
        Adicional = 0
        Opcional = True
        ParentFont = False
        TabOrder = 0
        PermitirZero = False
        PermitirNulo = False
        Operacao = opATRIBUICAO
        LookupField = 'codigo'
        LookupSource = dtmCadastroPlanosPagamentos.dsrProcuraAgentes
        LookupQueryParameter = 'codigo'
        Parameter = ' '
        ActiveSetControls = False
        DenyInsert = False
        NoSetControls = <>
        SetControls = <>
        LookupParameter = 'codigo'
      end
      object dtxAgente: TtecDBText
        Left = 88
        Top = 10
        Width = 423
        Height = 23
        TabStop = False
        Color = clBtnFace
        DataField = 'descricao'
        DataSource = dtmCadastroPlanosPagamentos.dsrProcuraAgentes
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        ReadOnly = True
        TabOrder = 1
        Alignment = taLeftJustify
      end
    end
    object gbxAposDia: TGroupBox
      Left = 529
      Top = 50
      Width = 60
      Height = 36
      Caption = 'AP'#211'S DIA'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -9
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      TabOrder = 5
      object edtAposDia: TDBEditNumero
        Left = 16
        Top = 10
        Width = 26
        Height = 23
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        MaxLength = -1
        ParentFont = False
        TabOrder = 0
        Text = 'edtAposDia'
        Mascara = False
        Alignment = taLeftJustify
        TipoMascara = tmGERAL
        NrDecimal = 2
        Decimais = False
        Negativo = False
        Tamanho = 2
        DataField = 'aposdia'
        DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
      end
    end
    object gbxDiaBase: TGroupBox
      Left = 529
      Top = 90
      Width = 60
      Height = 36
      Caption = 'DIA BASE'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -9
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      TabOrder = 8
      object edtDiaBase: TDBEditNumero
        Left = 15
        Top = 10
        Width = 26
        Height = 23
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        MaxLength = -1
        ParentFont = False
        TabOrder = 0
        Text = 'edtDiaBase'
        Mascara = False
        Alignment = taLeftJustify
        TipoMascara = tmGERAL
        NrDecimal = 2
        Decimais = False
        Negativo = False
        Tamanho = 2
        DataField = 'diabase'
        DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
      end
    end
    object gbxDesagio: TGroupBox
      Left = 456
      Top = 90
      Width = 64
      Height = 36
      Caption = ' DES'#193'GIO '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -9
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      TabOrder = 7
      object Label4: TLabel
        Left = 48
        Top = 14
        Width = 11
        Height = 15
        Caption = '%'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
      end
      object edtDesagio: TDBEditNumero
        Left = 2
        Top = 10
        Width = 43
        Height = 23
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        MaxLength = -1
        ParentFont = False
        TabOrder = 0
        Text = 'edtDesagio'
        Mascara = True
        Alignment = taLeftJustify
        TipoMascara = tmGERAL
        NrDecimal = 2
        Decimais = True
        Negativo = False
        Tamanho = 2
        DataField = 'desagio'
        DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
      end
    end
    object gbxRecebimento: TGroupBox
      Left = 6
      Top = 90
      Width = 440
      Height = 36
      Caption = ' TIPO DE RECEBIMENTO '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -9
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      TabOrder = 6
      object sbnConsultaTipoRecebimento: TSpeedButton
        Left = 65
        Top = 10
        Width = 23
        Height = 23
        Hint = 'Procurar Agente'
        Flat = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -13
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        Glyph.Data = {
          
          DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
        NumGlyphs = 2
        ParentFont = False
        OnClick = sbnConsultaTipoRecebimentoClick
      end
      object flkTipoRecebimento: TtecDBFindLookup
        Left = 2
        Top = 10
        Width = 60
        Height = 23
        Alignment = taLeftJustify
        DataField = 'tiporecebimento'
        DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        MaxLength = 4
        Maximo = 0
        Minimo = 37353
        Adicional = 0
        Opcional = True
        ParentFont = False
        TabOrder = 0
        PermitirZero = False
        PermitirNulo = False
        Operacao = opATRIBUICAO
        LookupField = 'codigo'
        LookupSource = dtmCadastroPlanosPagamentos.dsrProcuraTiposRecebimentos
        LookupQueryParameter = 'codigo'
        Parameter = ' '
        ActiveSetControls = False
        DenyInsert = False
        NoSetControls = <>
        SetControls = <>
        LookupParameter = 'codigo'
      end
      object dtxTipoRecebimento: TtecDBText
        Left = 88
        Top = 10
        Width = 349
        Height = 23
        TabStop = False
        Color = clBtnFace
        DataField = 'descricao'
        DataSource = dtmCadastroPlanosPagamentos.dsrProcuraTiposRecebimentos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        ReadOnly = True
        TabOrder = 1
        Alignment = taLeftJustify
      end
    end
    object gbxDataValidade: TGroupBox
      Left = 6
      Top = 130
      Width = 165
      Height = 36
      Caption = ' VALIDADE '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -9
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      TabOrder = 9
      object lblA: TLabel
        Left = 78
        Top = 14
        Width = 7
        Height = 15
        Caption = 'a'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
      end
      object edtDataInicial: TDBEditData
        Left = 2
        Top = 10
        Width = 70
        Height = 23
        Alignment = taCenter
        DataField = 'validadeinicial'
        DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        MaxLength = -1
        Maximo = 37353
        Minimo = 37353
        Adicional = 0
        ParentFont = False
        TabOrder = 0
        Opcional = True
      end
      object edtDataFinal: TDBEditData
        Left = 92
        Top = 10
        Width = 70
        Height = 23
        Alignment = taCenter
        DataField = 'validadefinal'
        DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        MaxLength = -1
        Maximo = 37353
        Minimo = 37353
        Adicional = 0
        ParentFont = False
        TabOrder = 1
        Opcional = True
      end
    end
    object gbxPermiteAlterar: TGroupBox
      Left = 181
      Top = 130
      Width = 147
      Height = 72
      Caption = ' P&ERMITIDO ALTERAR '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 10
      object ckbAlterarValores: TDBCheckBox
        Left = 6
        Top = 12
        Width = 135
        Height = 18
        Caption = 'Valor da parcela'
        DataField = 'alterarvalor'
        DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object ckbAlterarVencimentos: TDBCheckBox
        Left = 6
        Top = 30
        Width = 135
        Height = 18
        Caption = 'Data de vencimento'
        DataField = 'alterarvencimento'
        DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 1
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object ckbAlterarTipoRecebimento: TDBCheckBox
        Left = 6
        Top = 48
        Width = 135
        Height = 18
        Caption = 'Tipo de recebimento'
        DataField = 'alterartiporecebimento'
        DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 2
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
    end
    object gbxParcelas: TGroupBox
      Left = 6
      Top = 210
      Width = 151
      Height = 128
      Caption = ' PARCELAS '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 12
      object lblDeTrintaDias: TLabel
        Left = 26
        Top = 104
        Width = 71
        Height = 15
        Caption = 'de trinta dias'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
      end
      object ckbMesTrintaDias: TDBCheckBox
        Left = 8
        Top = 89
        Width = 130
        Height = 15
        Caption = 'Considerar meses'
        DataField = 'MesTrintaDias'
        DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 4
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object gbxQtdeParcelas: TGroupBox
        Left = 2
        Top = 12
        Width = 43
        Height = 36
        Caption = 'QTDE'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        object edtQuantidadeParcelas: TDBEditNumero
          Left = 2
          Top = 10
          Width = 38
          Height = 23
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = -1
          ParentFont = False
          TabOrder = 0
          Text = 'edtQuantidadeParcelas'
          OnExit = edtQuantidadeParcelasExit
          Mascara = False
          Alignment = taLeftJustify
          TipoMascara = tmGERAL
          NrDecimal = 2
          Decimais = False
          Negativo = False
          Tamanho = 2
          DataField = 'quantidadeparcelas'
          DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
        end
      end
      object gbxIntervalo: TGroupBox
        Left = 2
        Top = 50
        Width = 73
        Height = 36
        Caption = 'INTERVALO'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 2
        object lblDias1: TLabel
          Left = 42
          Top = 16
          Width = 24
          Height = 15
          Caption = 'dias'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
        end
        object edtIntervaloParcelas: TDBEditNumero
          Left = 2
          Top = 10
          Width = 35
          Height = 23
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = -1
          ParentFont = False
          TabOrder = 0
          Text = 'edtIntervaloParcelas'
          Mascara = False
          Alignment = taLeftJustify
          TipoMascara = tmGERAL
          NrDecimal = 2
          Decimais = False
          Negativo = False
          Tamanho = 2
          DataField = 'intervaloparcelas'
          DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
        end
      end
      object gbxToleranciaParcela: TGroupBox
        Left = 73
        Top = 50
        Width = 76
        Height = 36
        Caption = 'TOLER'#194'NCIA'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 3
        object lblDias2: TLabel
          Left = 42
          Top = 16
          Width = 24
          Height = 15
          Caption = 'dias'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
        end
        object edtToleranciaParcelas: TDBEditNumero
          Left = 2
          Top = 10
          Width = 35
          Height = 23
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = -1
          ParentFont = False
          TabOrder = 0
          Text = 'edtToleranciaParcelas'
          Mascara = False
          Alignment = taLeftJustify
          TipoMascara = tmGERAL
          NrDecimal = 2
          Decimais = False
          Negativo = False
          Tamanho = 2
          DataField = 'toleranciaentreparcelas'
          DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
        end
      end
      object gbxMinimoParcela: TGroupBox
        Left = 43
        Top = 12
        Width = 106
        Height = 36
        Caption = ' VALOR M'#205'NIMO '
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 1
        object edtValorMinimoParcelas: TDBEditNumero
          Left = 3
          Top = 10
          Width = 100
          Height = 23
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = -1
          ParentFont = False
          TabOrder = 0
          Text = 'edtValorMinimoParcelas'
          Mascara = True
          Alignment = taLeftJustify
          TipoMascara = tmGERAL
          NrDecimal = 2
          Decimais = True
          Negativo = False
          Tamanho = 9
          DataField = 'valorminimoparcela'
          DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
        end
      end
    end
    object gbxEntrada: TGroupBox
      Left = 163
      Top = 210
      Width = 153
      Height = 85
      Caption = ' ENTRADA '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 13
      object gbxPrazoEntrada: TGroupBox
        Left = 2
        Top = 12
        Width = 71
        Height = 36
        Caption = 'PRAZO '
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        object Label1: TLabel
          Left = 40
          Top = 16
          Width = 24
          Height = 15
          Caption = 'dias'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
        end
        object edtPrazoVenctoEntrada: TDBEditNumero
          Left = 2
          Top = 10
          Width = 35
          Height = 23
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = -1
          ParentFont = False
          TabOrder = 0
          Text = 'edtPrazoVenctoEntrada'
          Mascara = False
          Alignment = taLeftJustify
          TipoMascara = tmGERAL
          NrDecimal = 2
          Decimais = False
          Negativo = False
          Tamanho = 3
          DataField = 'prazovenctoentrada'
          DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
        end
      end
      object gbxToleranciaEntrada: TGroupBox
        Left = 71
        Top = 12
        Width = 80
        Height = 36
        Caption = 'TOLER'#194'NCIA'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 1
        object Label2: TLabel
          Left = 48
          Top = 16
          Width = 24
          Height = 15
          Caption = 'dias'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
        end
        object edtToleranciaEntrada: TDBEditNumero
          Left = 2
          Top = 10
          Width = 40
          Height = 23
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = -1
          ParentFont = False
          TabOrder = 0
          Text = 'edtToleranciaEntrada'
          Mascara = False
          Alignment = taLeftJustify
          TipoMascara = tmGERAL
          NrDecimal = 2
          Decimais = False
          Negativo = False
          Tamanho = 2
          DataField = 'toleranciaentrada'
          DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
        end
      end
      object gbxDataVencto: TGroupBox
        Left = 72
        Top = 50
        Width = 79
        Height = 36
        Caption = 'VENCIMENTO'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 3
        object edtVenctoEntrada: TDBEditData
          Left = 2
          Top = 10
          Width = 75
          Height = 23
          Alignment = taCenter
          DataField = 'diavenctoentrada'
          DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = -1
          Maximo = 0
          Minimo = 37353
          Adicional = 0
          ParentFont = False
          ParentShowHint = False
          ShowHint = True
          TabOrder = 0
          OnExit = edtVenctoEntradaExit
          Opcional = True
        end
      end
      object gbxPercentualEntrada: TGroupBox
        Left = 2
        Top = 50
        Width = 72
        Height = 36
        Caption = 'PERCENTUAL'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 2
        object lblPercEntrada: TLabel
          Left = 53
          Top = 14
          Width = 11
          Height = 15
          Caption = '%'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
        end
        object edtPorcentagemEntrada: TDBEditNumero
          Left = 2
          Top = 10
          Width = 47
          Height = 23
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = -1
          ParentFont = False
          TabOrder = 0
          Text = 'edtPorcentagemEntrada'
          Mascara = True
          Alignment = taLeftJustify
          TipoMascara = tmGERAL
          NrDecimal = 2
          Decimais = True
          Negativo = False
          Tamanho = 2
          DataField = 'porcentagementrada'
          DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
        end
      end
    end
    object gbxLimites: TGroupBox
      Left = 323
      Top = 210
      Width = 114
      Height = 125
      Caption = ' LIMITES COMPRA'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 14
      object gbxCompraMinima: TGroupBox
        Left = 2
        Top = 12
        Width = 110
        Height = 36
        Caption = 'VALOR M'#205'NIMO'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        object edtCompraMinima: TDBEditNumero
          Left = 2
          Top = 10
          Width = 105
          Height = 23
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = -1
          ParentFont = False
          TabOrder = 0
          Text = 'edtCompraMinima'
          Mascara = True
          Alignment = taLeftJustify
          TipoMascara = tmGERAL
          NrDecimal = 2
          Decimais = True
          Negativo = False
          Tamanho = 9
          DataField = 'compraminima'
          DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
        end
      end
      object gbxCompraMaximo: TGroupBox
        Left = 2
        Top = 50
        Width = 110
        Height = 36
        Caption = 'VALOR M'#193'XIMO'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 1
        object edtCompraMaxima: TDBEditNumero
          Left = 2
          Top = 10
          Width = 105
          Height = 23
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = -1
          ParentFont = False
          TabOrder = 0
          Text = 'edtCompraMaxima'
          Mascara = True
          Alignment = taLeftJustify
          TipoMascara = tmGERAL
          NrDecimal = 2
          Decimais = True
          Negativo = False
          Tamanho = 9
          DataField = 'compramaxima'
          DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
        end
      end
      object gbxDesctoMaximo: TGroupBox
        Left = 44
        Top = 87
        Width = 68
        Height = 36
        Caption = 'DESCONTO'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 2
        object lblPercDescto: TLabel
          Left = 46
          Top = 14
          Width = 11
          Height = 15
          Caption = '%'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
        end
        object edtDescontoMaximo: TDBEditNumero
          Left = 2
          Top = 10
          Width = 39
          Height = 23
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = -1
          ParentFont = False
          TabOrder = 0
          Text = 'edtDescontoMaximo'
          Mascara = True
          Alignment = taLeftJustify
          TipoMascara = tmGERAL
          NrDecimal = 2
          Decimais = True
          Negativo = False
          Tamanho = 2
          DataField = 'descontomaximo'
          DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
        end
      end
    end
    object gbxOpcoes: TGroupBox
      Left = 6
      Top = 347
      Width = 235
      Height = 147
      Caption = ' OP'#199#213'ES '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 15
      object ckbImprimirParcelas: TDBCheckBox
        Left = 6
        Top = 15
        Width = 200
        Height = 23
        Caption = 'Imprimir parcelas na NF/Cupom'
        DataField = 'imprparcelacarne'
        DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object ckbEnderecoCompleto: TDBCheckBox
        Left = 6
        Top = 36
        Width = 200
        Height = 23
        Caption = 'Endere'#231'o completo do cliente'
        DataField = 'enderecocompleto'
        DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 1
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object ckbDisponivelSite: TDBCheckBox
        Left = 7
        Top = 57
        Width = 200
        Height = 20
        Caption = 'Dispon'#237'vel no Site'
        DataField = 'aparecernosite'
        DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 2
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object DBCheckBox1: TDBCheckBox
        Left = 6
        Top = 79
        Width = 200
        Height = 20
        Caption = 'Enviar Boleto por Email'
        DataField = 'EnviarBoletoEmail'
        DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 3
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object ckbPermitirImpressaodoBoleto: TDBCheckBox
        Left = 6
        Top = 100
        Width = 200
        Height = 23
        Caption = 'Permitir Impress'#227'o do Boleto'
        DataField = 'permitirimpressaodoboleto'
        DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 4
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object DBCheckBox2: TDBCheckBox
        Left = 6
        Top = 122
        Width = 227
        Height = 23
        Caption = 'Tipo de Recebimento para c'#225'lculos'
        DataField = 'minimoparcelaaotipodereceb'
        DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 5
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
    end
    object pgcGruposClasses: TPageControl
      Left = 243
      Top = 351
      Width = 365
      Height = 143
      ActivePage = tstGrupos
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -12
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      TabOrder = 16
      object tstGrupos: TTabSheet
        Caption = ' &1 - Grupos'
        object sbnIncluirGrupos: TSpeedButton
          Left = 333
          Top = 22
          Width = 23
          Height = 22
          Hint = 'Incluir novo c'#243'digo'
          Flat = True
          Glyph.Data = {
            
            DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
          NumGlyphs = 2
          OnClick = sbnIncluirGruposClick
        end
        object sbnExcluirGrupos: TSpeedButton
          Left = 333
          Top = 44
          Width = 23
          Height = 22
          Hint = 'Excluir c'#243'digo atual'
          Flat = True
          Glyph.Data = {
            
            DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
          NumGlyphs = 2
          OnClick = sbnExcluirGruposClick
        end
        object dbgGrupos: TtecDBGrid
          Left = 0
          Top = 0
          Width = 330
          Height = 113
          Align = alLeft
          DataSource = dtmCadastroPlanosPagamentos.dsrPlanosGrupos
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'MS Sans Serif'
          Font.Pitch = fpVariable
          Font.Style = []
          Options = [dgTitles, dgIndicator, dgColLines, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
          ParentFont = False
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clBlack
          TitleFont.Height = -12
          TitleFont.Name = 'helvetica'
          TitleFont.Pitch = fpVariable
          TitleFont.Style = []
          OnEnter = dbgGruposEnter
          OnExit = dbgGridsExit
          RowWrap = 60
          Large = False
          DoubleRowColor = False
          MsgDelete = 'o GRUPO'
          TitleMinHeight = 100
          CellHeights = 100
          StrippedColor = 16054260
          CanDelete = False
          CanNotInsertFromGrid = False
          PostOnEnter = False
          PostOnSetUpDown = False
          DenySort = False
          DefaultRowHeight = 16
          ExibirNumerodaLinha = False
          NaoAtribuirDadosaTabelaaoDigitar = False
          ShowWhenFieldInvisible = False
          CanClickWhenEditing = False
          Columns = <
            item
              Expanded = False
              FieldName = 'grupo'
              Title.Alignment = taCenter
              Title.Caption = 'C'#211'DIGO'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlack
              Title.Font.Height = -9
              Title.Font.Name = 'helvetica'
              Title.Font.Pitch = fpVariable
              Title.Font.Style = []
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'descricao'
              Title.Alignment = taCenter
              Title.Caption = 'DESCRI'#199#195'O'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlack
              Title.Font.Height = -9
              Title.Font.Name = 'helvetica'
              Title.Font.Pitch = fpVariable
              Title.Font.Style = []
              Width = 250
              Visible = True
            end>
        end
      end
      object tstClasses: TTabSheet
        Caption = ' &2 - Classes'
        ImageIndex = 1
        object sbnIncluirClasses: TSpeedButton
          Left = 333
          Top = 22
          Width = 23
          Height = 22
          Flat = True
          Glyph.Data = {
            
            DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
          NumGlyphs = 2
          OnClick = sbnIncluirClassesClick
        end
        object sbnExcluirClasses: TSpeedButton
          Left = 333
          Top = 44
          Width = 23
          Height = 22
          Flat = True
          Glyph.Data = {
            
            DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
          NumGlyphs = 2
          OnClick = sbnExcluirClassesClick
        end
        object dbgClasses: TtecDBGrid
          Left = 0
          Top = 0
          Width = 330
          Height = 113
          Align = alLeft
          DataSource = dtmCadastroPlanosPagamentos.dsrPlanosClasses
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -12
          Font.Name = 'MS Sans Serif'
          Font.Pitch = fpVariable
          Font.Style = []
          Options = [dgTitles, dgIndicator, dgColLines, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
          ParentFont = False
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clBlack
          TitleFont.Height = -12
          TitleFont.Name = 'helvetica'
          TitleFont.Pitch = fpVariable
          TitleFont.Style = []
          OnEnter = dbgClassesEnter
          OnExit = dbgGridsExit
          RowWrap = 60
          Large = False
          DoubleRowColor = False
          MsgDelete = 'a CLASSE'
          TitleMinHeight = 100
          CellHeights = 100
          StrippedColor = 16054260
          CanDelete = False
          CanNotInsertFromGrid = False
          PostOnEnter = False
          PostOnSetUpDown = False
          DenySort = False
          DefaultRowHeight = 16
          ExibirNumerodaLinha = False
          NaoAtribuirDadosaTabelaaoDigitar = False
          ShowWhenFieldInvisible = False
          CanClickWhenEditing = False
          Columns = <
            item
              Expanded = False
              FieldName = 'classe'
              Title.Alignment = taCenter
              Title.Caption = 'C'#211'DIGO'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlack
              Title.Font.Height = -9
              Title.Font.Name = 'helvetica'
              Title.Font.Pitch = fpVariable
              Title.Font.Style = []
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'descricao'
              Title.Alignment = taCenter
              Title.Caption = 'DESCRI'#199#195'O'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlack
              Title.Font.Height = -9
              Title.Font.Name = 'helvetica'
              Title.Font.Pitch = fpVariable
              Title.Font.Style = []
              Width = 250
              Visible = True
            end>
        end
      end
    end
    object edfCodigoPlano: TtecDbEditFind
      Left = 9
      Top = 20
      Width = 55
      Height = 23
      Alignment = taLeftJustify
      DataField = 'codigo'
      DataSource = dtmCadastroPlanosPagamentos.dsrPlanos
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -12
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      Group = 'Plano'
      MaxLength = 6
      Maximo = 0
      Minimo = 37353
      Adicional = 0
      Opcional = True
      OnExit = edfCodigoPlanoExit
      ParentFont = False
      TabOrder = 17
      PermitirZero = False
      PermitirNulo = False
      Operacao = opATRIBUICAO
      Parameter = 'codigo'
      ActiveSetControls = True
      DenyInsert = True
      NoSetControls = <
        item
          Control = gbxCodigo
        end>
      SetControls = <>
    end
  end
  object aclHabilitar: TActionList
    Left = 432
    Top = 8
    object actHabilitar: TAction
      OnUpdate = actHabilitarUpdate
    end
  end
end
