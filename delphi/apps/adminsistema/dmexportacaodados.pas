...
type
  TdtmExportacaoDados = class(TdtmBasico)
    ...
    qryParametrosSelecaolistadecontascontabeis: TStringField;
    ...
    procedure qryExportacaoTabelasConsultaBeforeOpen(DataSet: TDataSet);
    procedure qryExportacaoDadosBeforeOpen(DataSet: TDataSet);
    procedure qryParametrosSelecaoAfterPost(DataSet: TDataSet);
  private
    FExtensao: String;
  ...
  public
    ...
    procedure SalvarData(Tipo: Integer; DataFinal: tDateTime);
    ...
    procedure SalvarListadeContasContabeis(Lista: String);
...
begin
  ...
  RefazConsulta(qryParametrosSelecao,[0,1],[self.name, inttostr(qryExportacaocodigo.AsInteger)]);
...
//    qryExportacaoDados.sql.text := qryExportacaoTabelassql.Text;
    ...
    begin
      ...
      if qryExportacaoTabelasConsulta.Macros[i].name = 'sql_Intervalo_de_Datas_2' then
      begin
        if not qryExportacaoFiltros.Locate('descricao','INTERVALO DE DATAS 2',[]) then
          if qryfiltros.Locate('macro',qryExportacaoTabelasConsulta.Macros[i].Name,[]) then
            ExibirFiltro(true);
      end
      else
      ...
      begin
        if not qryExportacaoFiltros.Locate('descricao','PARÂMETRO DATA',[]) then
          if qryfiltros.Locate('macro',qryExportacaoTabelasConsulta.Macros[i].Name,[]) then
            ExibirFiltro(true);
      end
      else
      ...
      if qryExportacaoTabelasConsulta.Macros[i].Name = 'sql_Lista_de_Contas_Contabeis' then
      begin
        if not qryExportacaoFiltros.Locate('descricao','LISTA DE CONTAS CONTÁBEIS',[]) then
          if qryfiltros.Locate('macro',qryExportacaoTabelasConsulta.Macros[i].Name,[]) then
            ExibirFiltro(true);
      end
...
begin
  ...
  if (qryParametrosSelecaointervalodatasdatafinal.AsString<>'') then
    qryParametrosSelecaoextensao.AsString :=
      '_ate_'+
       FormatDateTime('dd-mm-yyyy', strtodate(qryParametrosSelecaointervalodatasdatafinal.AsString))+'.txt'
  else        
...
begin
  ...
  if qryExportacaoFiltrosdescricao.AsString = 'INTERVALO DE DATAS 2' then
    SalvarIntervaloDatas(3,date,date)
  else
  ...
  begin
    ...
    try
      ...
      begin
        ...
        if qryexportacaodados.Fields[i].FieldName = qryExportacaoCamposcampo.AsString then
        begin
          qryExportacaoDados.Fields[i].Origin := qryExportacaoCamposdelimitador.AsString+' '+qryExportacaoCamposmascara.AsString;
          break;
        end;
        qryexportacaocampos.Next;
      end;
      ...
      begin
        ...
        begin
          ...
          begin
            if copy(qryExportacaoDados.Fields[i].Origin,1,1)=' ' then
            begin
              vdelimitador :='';
              vmascara := copy(qryExportacaoDados.Fields[i].Origin,2,length(qryExportacaoDados.Fields[i].Origin));
            end
            else
            begin
              vdelimitador :=copy(qryExportacaoDados.Fields[i].Origin,1, pos(' ',qryExportacaoDados.Fields[i].Origin)-1);
              vmascara := copy(qryExportacaoDados.Fields[i].Origin,2,
                              length(qryExportacaoDados.Fields[i].Origin));
            end;
            ...
            begin
              ...
              if not qryExportacaoDados.Fields[i].IsNull then
                Linha := Linha +  vdelimitador +
                         formatfloat(vmascara,abs(qryExportacaoDados.Fields[i].value)) +
                                       vdelimitador
              else
                linha := linha + vdelimitador + vdelimitador;

              ThousandSeparator := '.';
              DecimalSeparator := ',';
            end
            else
              if qryExportacaoModelosseparadorcampos.AsString='' then
              Linha := Linha + vdelimitador +
                       preencheString(qryExportacaoDados.Fields[i].asstring,' ',qryExportacaoDados.Fields[i].Size)+
                       vdelimitador
              else
              Linha := Linha + vdelimitador +
...
begin
  ...
  GuardarRegistroAtual(qryExportacaoFiltros, false);
  qryExportacaoFiltros.First;
  while not qryExportacaoFiltros.Eof do
  begin
...
begin
  ...
  for i:=0 to qryExportacaoTabelasConsulta.ParamCount-1 do
  begin
    if qryExportacaoTabelasConsulta.params[i].Name = 'datainicial' then
    begin
      if DataInicial<>'' then
        qryExportacaoTabelasConsulta.params[i].AsString := DataInicial
    end
    else
    if qryExportacaoTabelasConsulta.params[i].Name = 'datafinal' then
    begin
      if DataFinal<>'' then
        qryExportacaoTabelasConsulta.params[i].AsString := DataFinal;
    end
    else
    if qryExportacaoTabelasConsulta.params[i].Name = 'filial' then
    begin
      if ListadeFiliais.Count<>0 then
        qryExportacaoTabelasConsulta.params[i].AsInteger := strtoint(ListadeFiliais[0])
      else
        qryExportacaoTabelasConsulta.params[i].AsInteger := 0;
    end
  end;
...
procedure TdtmExportacaoDados.qryExportacaoDadosBeforeOpen(
  DataSet: TDataSet);
var
 i: integer;
...
begin
  inherited;
  for i:=0 to qryExportacaoDados.MacroCount-1 do
    if qryExportacaoDados.Macros[i].Name = 'param_data' then
      if qryExportacaoDados.MacroByName('param_data').AsString = '' then
      begin
         qryExportacaoDados.MacroByName('param_data').AsString := quotedstr('1980-01-01');
         break;
      end;

  for i:=0 to qryExportacaoDados.ParamCount-1 do
  begin
    if qryExportacaoDados.params[i].Name = 'datainicial' then
    begin
      if DataInicial<>'' then
        qryExportacaoDados.params[i].AsString := DataInicial
    end
    else
    if qryExportacaoDados.params[i].Name = 'datafinal' then
    begin
      if DataFinal<>'' then
        qryExportacaoDados.params[i].AsString := DataFinal;
    end;
  end;
...
procedure TdtmExportacaoDados.SalvarListadeContasContabeis(Lista: String);
begin
  with qryParametrosSelecao do
  begin
    if qryParametrosSelecaolistadecontascontabeis.AsString <> Lista then
    begin
      Edit;
      if Lista<>'' then
        qryParametrosSelecaolistadecontascontabeis.AsString := Lista
      else
        qryParametrosSelecaolistadecontascontabeis.clear;
      Post;
    end;
  end;
end;

procedure TdtmExportacaoDados.qryParametrosSelecaoAfterPost(
  DataSet: TDataSet);
begin
  inherited;
  EditarExportacao;
