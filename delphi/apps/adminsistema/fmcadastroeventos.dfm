inherited frmcadastroeventos: Tfrmcadastroeventos
  Left = 364
  Top = 85
  BorderIcons = [biSystemMenu, biMinimize, biMaximize]
  Caption = 'Cadastro de Eventos'
  ClientHeight = 700
  ClientWidth = 831
  FormStyle = fsMDIChild
  Visible = True
  OnActivate = FormActivate
  PixelsPerInch = 96
  TextHeight = 16
  inherited pnlBarra: TPanel
    Width = 479
  end
  inherited pnlTopMenu: TPanel
    Width = 831
    TabOrder = 2
    inherited tblBarra: TToolBar
      Width = 606
      inherited sbnAjuda: TSpeedButton
        Action = Action1
      end
      object sbnLogClientes: TSpeedButton
        Left = 84
        Top = 2
        Width = 35
        Height = 39
        Hint = 'Exibir o hist'#243'rico de altera'#231#245'es'
        Anchors = [akTop, akRight]
        Constraints.MaxHeight = 39
        Constraints.MinHeight = 39
        Caption = 'Log.'
        Flat = True
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        NumGlyphs = 2
        ParentFont = False
        Transparent = False
        OnClick = sbnLogClientesClick
      end
    end
  end
  object pnlFundoJanela: TPanel
    Left = 0
    Top = 45
    Width = 831
    Height = 655
    Align = alClient
    BevelOuter = bvNone
    TabOrder = 1
    object rgbTipoMovimentacao: TtecDBRadioGroup
      Left = 281
      Top = 6
      Width = 115
      Height = 72
      Caption = ' TIPO '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -9
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      TabOrder = 4
      TabStop = False
      DataField = 'tipomovimentacao'
      DataSource = dtmcadastroeventos.dsrEventos
      object rbnEntrada: TtecRadioButton
        Left = 8
        Top = 12
        Width = 100
        Height = 18
        Caption = '&Entrada'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        Value = 'E'
      end
      object rbnSaida: TtecRadioButton
        Left = 8
        Top = 30
        Width = 100
        Height = 18
        Caption = '&Sa'#237'da'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 1
        Value = 'S'
      end
      object rbnTransferencia: TtecRadioButton
        Left = 8
        Top = 48
        Width = 100
        Height = 18
        Caption = '&Transfer'#234'ncia'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 2
        Value = 'T'
      end
    end
    object gbxClassificacao: TGroupBox
      Left = 6
      Top = 47
      Width = 170
      Height = 36
      Caption = ' CLASSIFICA'#199#195'O '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -9
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      object edtClassificacao: TDBEditmascara
        Left = 2
        Top = 10
        Width = 165
        Height = 23
        DataField = 'classificacao'
        DataSource = dtmcadastroeventos.dsrEventos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        MaxLength = 16
        ParentFont = False
        TabOrder = 0
        OnChange = edtClassificacaoChange
        OnExit = edtClassificacaoExit
        OnKeyDown = edtClassificacaoKeyDown
        TipodeMascara = tMascaraBanco
      end
    end
    object gbxContasContabeis: TGroupBox
      Left = 6
      Top = 271
      Width = 779
      Height = 163
      Caption = ' CONTAS CONT'#193'BEIS '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 6
      object dbgEventosContasContabeis: TtecDBGrid
        Left = 2
        Top = 16
        Width = 775
        Height = 145
        Align = alClient
        DataSource = dtmcadastroeventos.dsrEventosContasContabeis
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clBlack
        TitleFont.Height = -9
        TitleFont.Name = 'helvetica'
        TitleFont.Pitch = fpVariable
        TitleFont.Style = []
        OnDblClick = dbgEventosContasContabeisDblClick
        OnKeyDown = dbgEventosContasContabeisKeyDown
        RowWrap = 60
        Large = False
        DoubleRowColor = False
        MsgDelete = 'esta conta cont'#225'bil?'
        TitleMinHeight = 100
        CellHeights = 100
        StrippedColor = 16054260
        CanDelete = True
        CanNotInsertFromGrid = False
        PostOnEnter = False
        PostOnSetUpDown = False
        DenySort = False
        DefaultRowHeight = 19
        ExibirNumerodaLinha = False
        NaoAtribuirDadosaTabelaaoDigitar = False
        ShowWhenFieldInvisible = False
        CanClickWhenEditing = False
        Columns = <
          item
            Expanded = False
            FieldName = 'contacontabil'
            Title.Alignment = taCenter
            Title.Caption = 'CONTA'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'descricaoplanocontas'
            Title.Alignment = taCenter
            Title.Caption = 'DESCRI'#199#195'O DA CONTA'
            Width = 300
            Visible = True
          end
          item
            Alignment = taLeftJustify
            Expanded = False
            FieldName = 'historicocontabil'
            Title.Alignment = taCenter
            Title.Caption = 'HIST'#211'RICO'
            Width = 60
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'descricaohistorico'
            Title.Alignment = taCenter
            Title.Caption = 'DESCRI'#199#195'O DO HIST'#211'RICO'
            Width = 250
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'percentual'
            Title.Alignment = taCenter
            Title.Caption = '% RATEIO'
            Width = 50
            Visible = True
          end>
      end
    end
    object gbxDescricao: TGroupBox
      Left = 7
      Top = 88
      Width = 517
      Height = 36
      Caption = ' DESCRI'#199#195'O '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -9
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      TabOrder = 2
      object edtDescricaoEvento: TDBEditTexto
        Left = 2
        Top = 10
        Width = 512
        Height = 23
        DataField = 'Descricao'
        DataSource = dtmcadastroeventos.dsrEventos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        MaxLength = 18
        ParentFont = False
        TabOrder = 0
        OnKeyDown = edtDescricaoEventoKeyDown
        CharCasenoDefault = False
        Alignment = taLeftJustify
      end
    end
    object gbxInativo: TGroupBox
      Left = 93
      Top = 6
      Width = 83
      Height = 36
      Caption = ' INATIVO '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -9
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      TabOrder = 9
      object edtInativo: TDBEditData
        Left = 2
        Top = 10
        Width = 78
        Height = 23
        Alignment = taCenter
        DataField = 'inativo'
        DataSource = dtmcadastroeventos.dsrEventos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        MaxLength = -1
        Maximo = 365
        Minimo = 37353
        Adicional = 0
        ParentFont = False
        TabOrder = 0
        OnExit = edtInativoExit
        OnKeyDown = edtInativoKeyDown
        Opcional = True
      end
    end
    object gbxOpcoes: TGroupBox
      Left = 407
      Top = 6
      Width = 418
      Height = 72
      Caption = ' OP'#199#213'ES '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -9
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      TabOrder = 5
      object ckbCentroCusto: TDBCheckBox
        Left = 166
        Top = 12
        Width = 137
        Height = 17
        Caption = '&Centro de custo'
        DataField = 'centrocusto'
        DataSource = dtmcadastroeventos.dsrEventos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 3
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object ckbRecibo: TDBCheckBox
        Left = 167
        Top = 32
        Width = 105
        Height = 17
        Caption = '&Recibo'
        DataField = 'recibo'
        DataSource = dtmcadastroeventos.dsrEventos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 4
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object ckbSaldoCliente: TDBCheckBox
        Left = 167
        Top = 51
        Width = 125
        Height = 17
        Caption = '&Saldo clientes'
        DataField = 'saldocliente'
        DataSource = dtmcadastroeventos.dsrEventos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 5
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object ckbPlanilhaCustos: TDBCheckBox
        Left = 285
        Top = 11
        Width = 125
        Height = 17
        Caption = '&Planilha de custos'
        DataField = 'planilhacustos'
        DataSource = dtmcadastroeventos.dsrEventos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 6
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object ckbIncluirNaDRE: TDBCheckBox
        Left = 8
        Top = 14
        Width = 153
        Height = 17
        Caption = '&Incluir na DRE Normal'
        DataField = 'IncluirNaDRE'
        DataSource = dtmcadastroeventos.dsrEventos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object DBCheckBox1: TDBCheckBox
        Left = 8
        Top = 50
        Width = 129
        Height = 17
        Caption = '&Incluir no Bal. Fin.'
        DataField = 'incluirnobf'
        DataSource = dtmcadastroeventos.dsrEventos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 2
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object DBCheckBox2: TDBCheckBox
        Left = 8
        Top = 32
        Width = 153
        Height = 17
        Caption = '&Incluir na DRE Gerencial'
        DataField = 'incluirnadregerencial'
        DataSource = dtmcadastroeventos.dsrEventos
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 1
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
    end
    object gbxVinculacao: TGroupBox
      Left = 7
      Top = 130
      Width = 517
      Height = 135
      Caption = ' VINCULA'#199#195'O '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 7
      object dbgPlanoContasVinculados: TtecDBGrid
        Left = 2
        Top = 13
        Width = 513
        Height = 120
        TabStop = False
        Align = alBottom
        DataSource = dtmcadastroeventos.dsrEventosVinculado_Sinteticas
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        Options = [dgTitles, dgIndicator, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        ParentFont = False
        ReadOnly = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clBlack
        TitleFont.Height = -9
        TitleFont.Name = 'helvetica'
        TitleFont.Pitch = fpVariable
        TitleFont.Style = []
        RowWrap = 60
        Large = False
        DoubleRowColor = True
        TitleMinHeight = 100
        CellHeights = 100
        StrippedColor = 16054260
        CanDelete = False
        CanNotInsertFromGrid = False
        PostOnEnter = False
        PostOnSetUpDown = False
        DenySort = False
        DefaultRowHeight = 18
        ExibirNumerodaLinha = False
        NaoAtribuirDadosaTabelaaoDigitar = False
        ShowWhenFieldInvisible = False
        CanClickWhenEditing = False
        Columns = <
          item
            Expanded = False
            FieldName = 'classificacao'
            Title.Caption = 'CLASSIFICA'#199#195'O'
            Width = 160
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'tipo'
            Title.Caption = 'TIPO'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'descricao'
            Title.Caption = 'DESCRI'#199#195'O'
            Width = 295
            Visible = True
          end>
      end
    end
    object pnlEventos: TPanel
      Left = 6
      Top = 440
      Width = 780
      Height = 209
      BevelOuter = bvNone
      TabOrder = 8
      object gbxHistoricos: TGroupBox
        Left = 0
        Top = 0
        Width = 780
        Height = 209
        Align = alClient
        Caption = ' HIST'#211'RICOS DO EVENTO '
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 0
        object pnlBotesHistorico: TPanel
          Left = 749
          Top = 16
          Width = 29
          Height = 191
          Align = alRight
          TabOrder = 1
          object sbnExcluirHistoricos: TSpeedButton
            Left = 5
            Top = 54
            Width = 23
            Height = 22
            Hint = 'Excluir hist'#243'rico do evento'
            Glyph.Data = {
              
              DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
            NumGlyphs = 2
            OnClick = sbnExcluirHistoricosClick
          end
          object sbnInserirHistoricos: TSpeedButton
            Left = 5
            Top = 25
            Width = 23
            Height = 22
            Hint = 'Incluir hist'#243'rico no evento'
            Glyph.Data = {
              
              C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3}
            NumGlyphs = 2
            OnClick = sbnInserirHistoricosClick
          end
        end
        object dbgHistorico: TtecDBGrid
          Left = 2
          Top = 16
          Width = 747
          Height = 191
          Align = alClient
          DataSource = dtmcadastroeventos.dsrHistoricosEventos
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
          ParentFont = False
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clBlack
          TitleFont.Height = -9
          TitleFont.Name = 'helvetica'
          TitleFont.Pitch = fpVariable
          TitleFont.Style = []
          OnDblClick = dbgHistoricoDblClick
          OnKeyDown = dbgHistoricoKeyDown
          RowWrap = 60
          Large = False
          DoubleRowColor = False
          MsgDelete = 'o HIST'#211'RICO'
          TitleMinHeight = 100
          CellHeights = 100
          StrippedColor = 16054260
          CanDelete = False
          CanNotInsertFromGrid = False
          PostOnEnter = False
          PostOnSetUpDown = False
          DenySort = False
          DefaultRowHeight = 20
          ExibirNumerodaLinha = False
          NaoAtribuirDadosaTabelaaoDigitar = False
          ShowWhenFieldInvisible = False
          CanClickWhenEditing = False
          Columns = <
            item
              Expanded = False
              FieldName = 'historico'
              Title.Alignment = taCenter
              Title.Caption = 'C'#211'DIGO'
              Width = 45
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'descricao'
              Title.Alignment = taCenter
              Title.Caption = 'DESCRI'#199#195'O'
              Width = 497
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'menu'
              Title.Alignment = taCenter
              Title.Caption = 'MENU'
              Width = 120
              Visible = True
            end>
        end
      end
    end
    object rgpTipo: TtecDBRadioGroup
      Left = 185
      Top = 6
      Width = 86
      Height = 55
      Caption = ' EVENTO '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -9
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      TabOrder = 3
      TabStop = False
      DataField = 'Tipo'
      DataSource = dtmcadastroeventos.dsrEventos
      object rbnAnalitica: TtecRadioButton
        Left = 8
        Top = 12
        Width = 70
        Height = 17
        Caption = '&Anal'#237'tico'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        Value = 'A'
      end
      object rbnSintetica: TtecRadioButton
        Left = 8
        Top = 30
        Width = 70
        Height = 17
        Caption = '&Sint'#233'tico'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 1
        Value = 'S'
      end
    end
    object gbxCodigo: TGroupBox
      Left = 6
      Top = 6
      Width = 69
      Height = 36
      Caption = ' C'#211'DIGO '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -9
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      inline fraConsultaEvento: TfraConsultaCodigoContabil
        Left = 2
        Top = 10
        Width = 65
        Height = 23
        HorzScrollBar.Range = 497
        HorzScrollBar.Visible = False
        VertScrollBar.Range = 23
        VertScrollBar.Visible = False
        AutoScroll = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        inherited sbnProcura: TSpeedButton
          Left = 80
        end
        inherited dtxDescricao: TtecDBText
          Left = 104
          Width = 9
        end
        inherited edfCodigo: TtecDBFindLookup
          Width = 64
          DataField = 'Codigo'
          DataSource = dtmcadastroeventos.dsrEventos
          Group = 'Evento'
          OnEnter = fraConsultaEventoedfCodigoEnter
          OnFound = nil
          Parameter = 'codigo'
          ActiveSetControls = True
          DenyInsert = True
          NoSetControls = <
            item
              Control = gbxCodigo
            end
            item
              Control = gbxClassificacao
            end
            item
              Control = gbxDescricao
            end
            item
              Control = gbxInativo
            end>
          SetControls = <
            item
              Control = rgpTipo
            end
            item
              Control = gbxVinculacao
            end
            item
              Control = gbxOpcoes
            end
            item
              Control = rgbTipoMovimentacao
            end
            item
              Control = gbxHistoricos
            end>
          LookupParameter = ' '
        end
      end
    end
  end
  object aclAbilitar: TActionList
    Left = 624
    Top = 8
    object Action1: TAction
      Caption = 'Action1'
    end
  end
end
