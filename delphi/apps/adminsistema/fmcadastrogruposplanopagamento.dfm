inherited frmCadastroGrupoPlanoPagamento: TfrmCadastroGrupoPlanoPagamento
  Left = 313
  Top = 209
  ActiveControl = flkCodigoGrupo
  Caption = 'Cadastro de grupos do plano de pagamento'
  ClientHeight = 100
  ClientWidth = 412
  Color = clBtnFace
  PixelsPerInch = 96
  TextHeight = 16
  inherited pnlBarra: TPanel
    Width = 412
    inherited bvlBotoesEd: TBevel
      Left = 201
    end
  end
  inherited pnlTopMenu: TPanel
    Width = 412
    TabOrder = 2
    inherited tblBarra: TToolBar
      Width = 183
      inherited sbnProcurar: TSpeedButton
        Visible = False
      end
    end
  end
  object pnlFundoJanela: TPanel
    Left = 0
    Top = 53
    Width = 412
    Height = 47
    Align = alClient
    BevelOuter = bvNone
    BorderWidth = 3
    TabOrder = 1
    object gbxFundoJanela: TGroupBox
      Left = 3
      Top = 3
      Width = 406
      Height = 41
      Align = alClient
      TabOrder = 0
      object sbnGrupo: TSpeedButton
        Left = 116
        Top = 9
        Width = 23
        Height = 23
        Flat = True
        Glyph.Data = {
          
          DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
        NumGlyphs = 2
        OnClick = sbnGrupoClick
      end
      object lblCodigoGrupo: TLabel
        Left = 10
        Top = 13
        Width = 40
        Height = 16
        Caption = 'C'#243'digo'
      end
      object flkCodigoGrupo: TtecDBFindLookup
        Left = 54
        Top = 9
        Width = 60
        Height = 24
        CharCase = ecUpperCase
        DataField = 'grupo'
        DataSource = dtmCadastroPlanosPagamentos.dsrPlanosGrupos
        MaxLength = 4
        Maximo = 0
        Minimo = 37353
        Adicional = 0
        Opcional = True
        OnMessage = flkCodigoGrupoMessage
        TabOrder = 0
        PermitirZero = False
        PermitirNulo = False
        Operacao = opATRIBUICAO
        LookupField = 'codigo'
        LookupSource = dtmCadastroPlanosPagamentos.dsrProcuraGrupoPlano
        Parameter = ' '
        ActiveSetControls = False
        DenyInsert = False
        NoSetControls = <>
        SetControls = <>
        LookupParameter = 'codigo'
      end
      object dtxDescricaoGrupo: TtecDBText
        Left = 140
        Top = 9
        Width = 255
        Height = 24
        TabStop = False
        Color = clBtnFace
        DataField = 'descricao'
        DataSource = dtmCadastroPlanosPagamentos.dsrProcuraGrupoPlano
        ReadOnly = True
        TabOrder = 1
      end
    end
  end
end
