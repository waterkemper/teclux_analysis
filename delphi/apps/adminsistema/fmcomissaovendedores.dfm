inherited frmComissaoVendedores: TfrmComissaoVendedores
  Left = 529
  Top = 232
  ActiveControl = fraConsultaFilialGrupoFilial
  Caption = 'Comiss'#227'o de Vendedores'
  ClientHeight = 428
  ClientWidth = 800
  FormStyle = fsMDIChild
  Visible = True
  PixelsPerInch = 96
  TextHeight = 16
  inherited pnlBarra: TToolBar
    Width = 800
    inherited sbnAjuda: TSpeedButton
      Action = actArquivo
    end
  end
  inherited pnlFundoJanela: TPanel
    Top = 40
    Width = 800
    Height = 388
    object gbxVendedor: TGroupBox
      Left = 404
      Top = 14
      Width = 389
      Height = 53
      Caption = '&Vendedor'
      TabOrder = 1
      object sbnProcurarVendedor: TSpeedButton
        Left = 71
        Top = 25
        Width = 23
        Height = 22
        Hint = 'Procurar Vendedor'
        Flat = True
        Glyph.Data = {
          
          DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
        NumGlyphs = 2
        OnClick = sbnProcurarVendedorClick
      end
      object edfVendedor: TtecDbEditFind
        Left = 8
        Top = 23
        Width = 60
        Height = 24
        Alignment = taLeftJustify
        DataField = 'codigo'
        DataSource = dtmComissaoVendedores.dsrPesquisarVendedores
        Group = 'Vendedor'
        MaxLength = 10
        Maximo = 0
        Minimo = 37353
        Adicional = 0
        Opcional = True
        TabOrder = 0
        PermitirZero = False
        PermitirNulo = False
        Operacao = opATRIBUICAO
        Parameter = 'codigo'
        ActiveSetControls = False
        DenyInsert = True
        NoSetControls = <>
        SetControls = <>
      end
      object dtxVendedor: TtecDBText
        Left = 96
        Top = 24
        Width = 281
        Height = 23
        TabStop = False
        BorderStyle = bsNone
        Color = clBtnFace
        DataField = 'nome'
        DataSource = dtmComissaoVendedores.dsrPesquisarVendedores
        ReadOnly = True
        TabOrder = 1
        Alignment = taLeftJustify
      end
    end
    object gbxAgentes: TGroupBox
      Left = 3
      Top = 80
      Width = 263
      Height = 139
      Caption = '&Agentes'
      TabOrder = 2
      object sbnMarcarAgentes: TSpeedButton
        Left = 232
        Top = 18
        Width = 22
        Height = 22
        Hint = 'Marcar Agentes'
        Flat = True
        Glyph.Data = {
          
          DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
        NumGlyphs = 2
        OnClick = sbnMarcarAgentesClick
      end
      object sbnDesmarcarAgentes: TSpeedButton
        Left = 232
        Top = 42
        Width = 22
        Height = 22
        Hint = 'Desmarcar Agentes'
        Flat = True
        Glyph.Data = {
          
          DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
        NumGlyphs = 2
        OnClick = sbnDesmarcarAgentesClick
      end
      object clbAgentes: TCheckListBox
        Left = 8
        Top = 17
        Width = 220
        Height = 115
        ItemHeight = 15
        Style = lbOwnerDrawVariable
        TabOrder = 0
      end
    end
    object gbxConceitos: TGroupBox
      Left = 275
      Top = 80
      Width = 263
      Height = 139
      Caption = '&Conceitos'
      TabOrder = 3
      object sbnMarcarConceitos: TSpeedButton
        Left = 233
        Top = 18
        Width = 22
        Height = 22
        Hint = 'Marcar Conceitos'
        Flat = True
        Glyph.Data = {
          
          DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
        NumGlyphs = 2
        OnClick = sbnMarcarConceitosClick
      end
      object sbnDesmarcarConceitos: TSpeedButton
        Left = 233
        Top = 42
        Width = 22
        Height = 22
        Hint = 'Desmarcar Conceitos'
        Flat = True
        Glyph.Data = {
          
          DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
        NumGlyphs = 2
        OnClick = sbnDesmarcarConceitosClick
      end
      object clbConceitos: TCheckListBox
        Left = 8
        Top = 17
        Width = 220
        Height = 115
        ItemHeight = 15
        Style = lbOwnerDrawVariable
        TabOrder = 0
      end
    end
    object gbxPeriodoComissao: TGroupBox
      Left = 543
      Top = 80
      Width = 250
      Height = 65
      Caption = '&Per'#237'odo Comiss'#227'o'
      TabOrder = 4
      object lblComissaoInicio: TLabel
        Left = 5
        Top = 30
        Width = 30
        Height = 16
        Caption = 'In'#237'cio'
      end
      object lblComissaoFim: TLabel
        Left = 115
        Top = 30
        Width = 22
        Height = 16
        Caption = 'Fim'
      end
      object edtComissaoInicio: TEditData
        Left = 37
        Top = 26
        Width = 75
        Height = 24
        MaxLength = 10
        TabOrder = 0
        Minimo = 37353
        Maximo = 0
      end
      object edtComissaoFim: TEditData
        Left = 139
        Top = 26
        Width = 75
        Height = 24
        MaxLength = 10
        TabOrder = 1
        OnEnter = edtComissaoFimEnter
        Minimo = 37353
        Maximo = 0
      end
    end
    object gbxEventos: TGroupBox
      Left = 3
      Top = 219
      Width = 536
      Height = 110
      Caption = 'Eve&ntos'
      TabOrder = 5
      object sbnMarcarEventos: TSpeedButton
        Left = 508
        Top = 21
        Width = 22
        Height = 22
        Hint = 'Marcar Eventos'
        Flat = True
        Glyph.Data = {
          
          DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
        NumGlyphs = 2
        OnClick = sbnMarcarEventosClick
      end
      object sbnDesmarcarEventos: TSpeedButton
        Left = 508
        Top = 45
        Width = 22
        Height = 22
        Hint = 'Desmarcar Eventos'
        Flat = True
        Glyph.Data = {
          
          DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
        NumGlyphs = 2
        OnClick = sbnDesmarcarEventosClick
      end
      object clbEventos: TCheckListBox
        Left = 8
        Top = 21
        Width = 497
        Height = 84
        ItemHeight = 15
        Items.Strings = (
          'VENDAS'
          'ESTORNOS DE VENDA'
          'ADICIONAL PRODUTOS E SERVI'#199'OS'
          'ESTORNOS ADICIONAL PRODUTOS E SERVI'#199'OS'
          'ABATIMENTO CREDITO TROCA')
        Style = lbOwnerDrawVariable
        TabOrder = 0
      end
    end
    object rgpResumo: TRadioGroup
      Left = 543
      Top = 147
      Width = 122
      Height = 94
      Caption = 'Resumo'
      ItemIndex = 0
      Items.Strings = (
        'Com R&esumo'
        '&Sem Resumo'
        'S'#243' Res&umo')
      TabOrder = 6
    end
    inline fraConsultaFilialGrupoFilial: TfraConsultaFilialGrupoFilial
      Left = 5
      Top = 8
      Width = 394
      Height = 58
      Color = clBtnFace
      ParentColor = False
      TabOrder = 0
      inherited pgcFiliaisGruposFiliais: TPageControl
        Width = 394
        inherited tstFilial: TTabSheet
          inherited fraConsultaFilial: TfraConsultaCodigo
            HorzScrollBar.Range = 383
            Font.Pitch = fpVariable
            ParentColor = False
            inherited dtxDescricao: TtecDBText
              Width = 295
            end
            inherited edfCodigo: TtecDBFindLookup
              Parameter = 'codigo'
              ActiveSetControls = False
              DenyInsert = True
              NoSetControls = <>
              SetControls = <>
            end
          end
        end
        inherited tstGrupodeFilial: TTabSheet
          inherited fraConsultaGrupoFilial: TfraConsultaCodigo
            HorzScrollBar.Range = 382
            Font.Pitch = fpVariable
            inherited dtxDescricao: TtecDBText
              Width = 295
            end
            inherited edfCodigo: TtecDBFindLookup
              Parameter = 'codigo'
              ActiveSetControls = False
              DenyInsert = True
              NoSetControls = <>
              SetControls = <>
            end
          end
        end
      end
    end
    object ckbExibirProdutos: TCheckBox
      Left = 544
      Top = 248
      Width = 129
      Height = 31
      Caption = 'Exibir Produtos'
      TabOrder = 7
    end
    object ckbIncluirDevolucoesSemContrato: TCheckBox
      Left = 544
      Top = 272
      Width = 241
      Height = 31
      Caption = 'Incluir Devolu'#231#245'es sem Contrato'
      TabOrder = 8
    end
    object ckbConsiderarTrocasComoDevolucoes: TCheckBox
      Left = 544
      Top = 296
      Width = 241
      Height = 31
      Caption = 'Considerar Trocas como Devolu'#231#245'es'
      TabOrder = 9
    end
  end
  object aclFolha: TActionList
    Left = 592
    Top = 8
    object actArquivo: TAction
    end
  end
  object ecvValida: TtecEditionControlValidation
    EditionControl = <
      item
        Control = edtComissaoFim
      end
      item
        Control = edtComissaoInicio
      end>
    Left = 664
    Top = 8
  end
end
