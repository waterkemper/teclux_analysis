inherited frmrelatoriocaixa: Tfrmrelatoriocaixa
  Left = 392
  Top = 151
  ActiveControl = edtPeriodoInicial
  Caption = 'Relat'#243'rio de Caixa'
  ClientHeight = 535
  ClientWidth = 1030
  FormStyle = fsMDIChild
  Visible = True
  PixelsPerInch = 96
  TextHeight = 16
  inherited pnlBarra: TToolBar
    Width = 1030
  end
  inherited pnlFundoJanela: TPanel
    Top = 40
    Width = 1030
    Height = 495
    Align = alClient
    object gbxRelatorioCaixa: TGroupBox
      Left = 3
      Top = 3
      Width = 1024
      Height = 489
      Align = alClient
      TabOrder = 0
      object gbxPeriodo: TGroupBox
        Left = 478
        Top = 10
        Width = 267
        Height = 55
        Caption = 'Per'#237'odo de Fechamento'
        TabOrder = 4
        object lblPeriodoInicial: TLabel
          Left = 13
          Top = 26
          Width = 33
          Height = 16
          Caption = 'Inicial'
        end
        object lblPeriodoFinal: TLabel
          Left = 146
          Top = 26
          Width = 28
          Height = 16
          Caption = 'Final'
        end
        object edtPeriodoInicial: TEditData
          Left = 51
          Top = 22
          Width = 80
          Height = 24
          MaxLength = 10
          TabOrder = 0
          Minimo = 37353
          Maximo = 0
        end
        object edtPeriodoFinal: TEditData
          Left = 179
          Top = 22
          Width = 80
          Height = 24
          MaxLength = 10
          TabOrder = 1
          Minimo = 37353
          Maximo = 0
        end
      end
      object gbxFiliais: TGroupBox
        Left = 6
        Top = 10
        Width = 227
        Height = 119
        Caption = '&Filiais'
        TabOrder = 0
        object sbnMarcarFiliais: TSpeedButton
          Left = 200
          Top = 18
          Width = 22
          Height = 22
          Hint = 'Marcar Filiais'
          Flat = True
          Glyph.Data = {
            
            DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
          NumGlyphs = 2
          OnClick = sbnMarcarFiliaisClick
        end
        object sbnDesmarcarFiliais: TSpeedButton
          Left = 200
          Top = 42
          Width = 22
          Height = 22
          Hint = 'Desmarcar Filiais'
          Flat = True
          Glyph.Data = {
            
            DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
          NumGlyphs = 2
          OnClick = sbnDesmarcarFiliaisClick
        end
        object clbFiliais: TCheckListBox
          Left = 6
          Top = 15
          Width = 190
          Height = 97
          OnClickCheck = clbFiliaisClickCheck
          ItemHeight = 15
          Style = lbOwnerDrawVariable
          TabOrder = 0
        end
      end
      object gbxGruposFiliais: TGroupBox
        Left = 6
        Top = 129
        Width = 227
        Height = 118
        Caption = '&Grupos de Filiais'
        TabOrder = 1
        object sbnMarcarGruposFiliais: TSpeedButton
          Left = 200
          Top = 18
          Width = 22
          Height = 22
          Hint = 'Marcar Grupos de Filiais'
          Flat = True
          Glyph.Data = {
            
            DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
          NumGlyphs = 2
          OnClick = sbnMarcarGruposFiliaisClick
        end
        object sbnDesmarcarGruposFiliais: TSpeedButton
          Left = 200
          Top = 42
          Width = 22
          Height = 22
          Hint = 'Desmarcar Grupos de Filiais'
          Flat = True
          Glyph.Data = {
            
            DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
          NumGlyphs = 2
          OnClick = sbnDesmarcarGruposFiliaisClick
        end
        object clbGrupodeFiliais: TCheckListBox
          Left = 6
          Top = 15
          Width = 190
          Height = 96
          OnClickCheck = clbGrupodeFiliaisClickCheck
          ItemHeight = 15
          Style = lbOwnerDrawVariable
          TabOrder = 0
        end
      end
      object gbxAutenticacoes: TGroupBox
        Left = 240
        Top = 10
        Width = 233
        Height = 236
        Caption = 'Autentica'#231#245'es'
        TabOrder = 2
        object sbnMarcarAutenticacoes: TSpeedButton
          Left = 204
          Top = 14
          Width = 22
          Height = 22
          Hint = 'Marcar Autentica'#231#245'es'
          Flat = True
          Glyph.Data = {
            
            DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
          NumGlyphs = 2
          OnClick = sbnMarcarAutenticacoesClick
        end
        object sbnDesmarcarAutenticacoes: TSpeedButton
          Left = 204
          Top = 38
          Width = 22
          Height = 22
          Hint = 'Desmarcar Autentica'#231#245'es'
          Flat = True
          Glyph.Data = {
            
            DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
          NumGlyphs = 2
          OnClick = sbnDesmarcarAutenticacoesClick
        end
        object clbAutenticacoes: TCheckListBox
          Left = 5
          Top = 15
          Width = 196
          Height = 214
          OnClickCheck = clbFiliaisClickCheck
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -10
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ItemHeight = 15
          ParentFont = False
          Sorted = True
          Style = lbOwnerDrawVariable
          TabOrder = 0
        end
      end
      object gbxAgrupamento: TGroupBox
        Left = 478
        Top = 459
        Width = 323
        Height = 23
        TabOrder = 6
        object ckbAgruparTipoAutenticacao: TCheckBox
          Left = 7
          Top = 3
          Width = 210
          Height = 15
          Caption = 'Agrupar por tipo de autenticacao'
          TabOrder = 0
        end
      end
      object gbxCaixas: TGroupBox
        Left = 6
        Top = 248
        Width = 467
        Height = 235
        Caption = ' CAIXAS'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 3
        inline fraSelecaoAleatoriaFuncionarios: TfraSelecaoAleatoria
          Left = 2
          Top = 16
          Width = 463
          Height = 217
          HorzScrollBar.Range = 26
          Align = alClient
          AutoScroll = False
          TabOrder = 0
          inherited Splitter1: TSplitter
            Left = 432
            Height = 202
          end
          inherited dbgSelecaoAleatoria: TtecDBGrid
            Width = 432
            Height = 202
            Font.Height = -12
            Font.Name = 'Helvetica'
            Options = [dgEditing, dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgCancelOnExit]
            OnDblClick = fraSelecaoAleatoriaFuncionariosdbgSelecaoAleatoriaDblClick
            OnKeyDown = fraSelecaoAleatoriaFuncionariosdbgSelecaoAleatoriaKeyDown
            DefaultRowHeight = 19
          end
          inherited pnlLabels: TPanel
            Left = 435
            Height = 202
            inherited sbnProcura: TSpeedButton
              Hint = 'CTRL + F9 - Procura funcion'#225'rio'
              OnClick = fraSelecaoAleatoriaFuncionariossbnProcuraClick
            end
            inherited sbnExcluir: TSpeedButton
              Hint = 'Excluir funcion'#225'rio'
            end
            inherited sbnExcluirTodos: TSpeedButton
              Hint = 'Excluir todos os funcion'#225'rios'
            end
            inherited sbnIncluirItem: TSpeedButton
              Hint = 'Incluir funcion'#225'rio'
            end
          end
          inherited pnlAbaixo: TPanel
            Top = 202
            Width = 463
            Font.Pitch = fpVariable
            inherited lblOrientacao: TLabel
              Font.Pitch = fpVariable
            end
            inherited pnlAbaixoDireita: TPanel
              Left = 432
              Font.Pitch = fpVariable
            end
          end
          inherited qrySelecaoAleatoria: TtecQuery
            AfterOpen = fraSelecaoAleatoriaFuncionariosqrySelecaoAleatoriaAfterOpen
            Sql.Strings = (
              'Select u.codigo,'
              '            u.nome'
              'From  usuarios u'
              'WHERE false')
          end
        end
      end
      object gbxEvento: TGroupBox
        Left = 478
        Top = 72
        Width = 539
        Height = 385
        Caption = ' EVENTOS'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 5
        inline fraSelecaoAleatoriaEventos: TfraSelecaoAleatoria
          Left = 2
          Top = 16
          Width = 535
          Height = 367
          HorzScrollBar.Range = 26
          Align = alClient
          AutoScroll = False
          TabOrder = 0
          inherited Splitter1: TSplitter
            Left = 504
            Height = 352
          end
          inherited dbgSelecaoAleatoria: TtecDBGrid
            Width = 504
            Height = 352
            Font.Height = -12
            Font.Name = 'Helvetica'
            Options = [dgEditing, dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgCancelOnExit]
            OnDblClick = fraSelecaoAleatoriaEventosdbgSelecaoAleatoriaDblClick
            OnKeyDown = fraSelecaoAleatoriaEventosdbgSelecaoAleatoriaKeyDown
            DefaultRowHeight = 19
          end
          inherited pnlLabels: TPanel
            Left = 507
            Height = 352
            inherited sbnProcura: TSpeedButton
              Hint = 'CTRL + F9 - Procura funcion'#225'rio'
              OnClick = fraSelecaoAleatoriaEventossbnProcuraClick
            end
            inherited sbnExcluir: TSpeedButton
              Hint = 'Excluir funcion'#225'rio'
            end
            inherited sbnExcluirTodos: TSpeedButton
              Hint = 'Excluir todos os funcion'#225'rios'
            end
            inherited sbnIncluirItem: TSpeedButton
              Hint = 'Incluir funcion'#225'rio'
            end
          end
          inherited pnlAbaixo: TPanel
            Top = 352
            Width = 535
            Font.Pitch = fpVariable
            inherited lblOrientacao: TLabel
              Font.Pitch = fpVariable
            end
            inherited pnlAbaixoDireita: TPanel
              Left = 504
              Font.Pitch = fpVariable
            end
          end
          inherited qrySelecaoAleatoria: TtecQuery
            AfterOpen = fraSelecaoAleatoriaEventosqrySelecaoAleatoriaAfterOpen
            Sql.Strings = (
              'Select u.codigo,'
              '            u.nome'
              'From  usuarios u'
              'WHERE false')
          end
        end
      end
    end
  end
end
