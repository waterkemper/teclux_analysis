inherited frmSPC: TfrmSPC
  Left = 633
  Top = 155
  ActiveControl = pgcGeracaoSPC
  Caption = 'Registro no SPC'
  ClientHeight = 602
  ClientWidth = 806
  FormStyle = fsMDIChild
  Visible = True
  PixelsPerInch = 96
  TextHeight = 16
  inherited pnlBarra: TToolBar
    Width = 806
    inherited bvlBotoes: TBevel
      Width = 4
    end
    object sbnGerar: TSpeedButton
      Left = 80
      Top = 2
      Width = 75
      Height = 39
      Hint = 'Gerar consulta de SPC'
      Caption = 'Gerar F6'
      Flat = True
      Glyph.Data = {
        
        DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
      Layout = blGlyphTop
      NumGlyphs = 2
      Spacing = 0
      OnClick = sbnGerarClick
    end
    object sbnConfirma: TSpeedButton
      Left = 155
      Top = 2
      Width = 75
      Height = 39
      Hint = 'Confirma Retirada do SPC'
      Caption = 'Confirma F5'
      Enabled = False
      Flat = True
      Glyph.Data = {
        
        7F7F7F7F7F7F7F7F7F7F7F7F7F7F7F7F7F7F7F7FC3C3C3C3C3C3}
      Layout = blGlyphTop
      NumGlyphs = 2
      Spacing = 0
      OnClick = sbnConfirmaClick
    end
  end
  inherited pnlFundoJanela: TPanel
    Top = 41
    Width = 806
    Height = 561
    object pgcGeracaoSPC: TPageControl
      Left = 3
      Top = 86
      Width = 796
      Height = 453
      ActivePage = tstGeracao
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      OnChange = pgcGeracaoSPCChange
      object tstGeracao: TTabSheet
        Caption = ' Gerar '
        object pnlFilialCentralizadora: TPanel
          Left = 0
          Top = 113
          Width = 788
          Height = 41
          Align = alTop
          BevelOuter = bvNone
          TabOrder = 1
          object ckbReenviarRejeitados: TCheckBox
            Left = 8
            Top = -1
            Width = 139
            Height = 23
            Caption = 'Reenviar Rejeitados'
            Checked = True
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            ParentFont = False
            State = cbChecked
            TabOrder = 0
            Visible = False
          end
        end
        object pnlClientes: TPanel
          Left = 0
          Top = 154
          Width = 788
          Height = 43
          Align = alTop
          BevelOuter = bvNone
          TabOrder = 2
          object gbxClientes: TGroupBox
            Left = 8
            Top = 0
            Width = 409
            Height = 36
            Caption = 'CLIENTE'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -9
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            ParentFont = False
            TabOrder = 0
            object sbnConsultaCliente: TSpeedButton
              Left = 77
              Top = 10
              Width = 23
              Height = 23
              Hint = 'Procurar Clientes'
              Flat = True
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -12
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              Glyph.Data = {
                
                DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
              NumGlyphs = 2
              ParentFont = False
              OnClick = sbnConsultaClienteClick
            end
            object dtxClientes: TtecDBText
              Left = 101
              Top = 10
              Width = 305
              Height = 23
              TabStop = False
              Color = clBtnFace
              DataField = 'nome'
              DataSource = dtmspc.dsrProcuraClientes
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -12
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              ParentFont = False
              ReadOnly = True
              TabOrder = 1
              Alignment = taLeftJustify
            end
            object edfCliente: TtecDbEditFind
              Left = 2
              Top = 10
              Width = 73
              Height = 23
              Alignment = taLeftJustify
              DataField = 'codigo'
              DataSource = dtmspc.dsrProcuraClientes
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -12
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              Group = 'ClienteInicial'
              MaxLength = 9
              Maximo = 0
              Minimo = 37353
              Adicional = 0
              Opcional = True
              OnKeyDown = edfClienteKeyDown
              ParentFont = False
              TabOrder = 0
              PermitirZero = False
              PermitirNulo = False
              Operacao = opATRIBUICAO
              Parameter = 'codigo'
              ActiveSetControls = False
              DenyInsert = True
              NoSetControls = <>
              SetControls = <>
            end
          end
          object gbxSemAviso: TGroupBox
            Left = 424
            Top = 0
            Width = 80
            Height = 37
            Caption = 'SEM AVISO AT'#201
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -9
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            ParentFont = False
            TabOrder = 1
            object edtSemAviso: TEditData
              Left = 2
              Top = 10
              Width = 75
              Height = 23
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -12
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              ParentFont = False
              TabOrder = 0
              OnKeyDown = edtSemAvisoKeyDown
              Minimo = 37353
              Maximo = 0
            end
          end
        end
        object pnlConsulta: TPanel
          Left = 0
          Top = 197
          Width = 788
          Height = 78
          Align = alTop
          BevelOuter = bvNone
          TabOrder = 3
          object gbxPeriodo: TGroupBox
            Left = 332
            Top = 0
            Width = 173
            Height = 36
            Caption = '&INTERVALO'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -9
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            ParentFont = False
            TabOrder = 3
            object lblAIntervalor: TLabel
              Left = 83
              Top = 13
              Width = 7
              Height = 15
              Caption = 'a'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -12
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              ParentFont = False
            end
            object edtdataInicial: TEditData
              Left = 2
              Top = 10
              Width = 75
              Height = 23
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -12
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              MaxLength = 10
              ParentFont = False
              TabOrder = 0
              Minimo = 37353
              Maximo = 0
            end
            object edtdataFinal: TEditData
              Left = 95
              Top = 10
              Width = 75
              Height = 23
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -12
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              MaxLength = 10
              ParentFont = False
              TabOrder = 1
              OnKeyDown = edtdataFinalKeyDown
              Minimo = 37353
              Maximo = 0
            end
          end
          object gbxRegistradoPor: TGroupBox
            Left = 8
            Top = 0
            Width = 100
            Height = 73
            Caption = 'REGISTRADO'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -9
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            ParentFont = False
            TabOrder = 0
            object ckbRegistradoSPC: TCheckBox
              Left = 8
              Top = 16
              Width = 80
              Height = 20
              Caption = 'SPC'
              Checked = True
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -12
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              ParentFont = False
              State = cbChecked
              TabOrder = 0
              OnClick = ckbRegistradoSPCClick
            end
            object ckbRegistradoEmpresa: TCheckBox
              Left = 8
              Top = 40
              Width = 80
              Height = 20
              Caption = 'Empresa'
              Checked = True
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -12
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              ParentFont = False
              State = cbChecked
              TabOrder = 1
              OnClick = ckbRegistradoEmpresaClick
            end
          end
          object gbxTipodeData: TGroupBox
            Left = 116
            Top = 0
            Width = 100
            Height = 73
            Caption = 'TIPOS DE DATAS'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -9
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            ParentFont = False
            TabOrder = 1
            object ckbTipoDataInclusao: TCheckBox
              Left = 8
              Top = 16
              Width = 80
              Height = 20
              Caption = 'Inclus'#227'o'
              Checked = True
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -12
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              ParentFont = False
              State = cbChecked
              TabOrder = 0
              OnClick = ckbTipoDataInclusaoClick
            end
            object ckbTipoDataExclusao: TCheckBox
              Left = 8
              Top = 40
              Width = 80
              Height = 20
              Caption = 'Exclus'#227'o'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -12
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              ParentFont = False
              TabOrder = 1
              OnClick = ckbTipoDataExclusaoClick
            end
          end
          object gbxSituacaoAtual: TGroupBox
            Left = 225
            Top = 0
            Width = 100
            Height = 73
            Caption = 'SITUA'#199#195'O ATUAL'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -9
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            ParentFont = False
            TabOrder = 2
            object ckbExcluidos: TCheckBox
              Left = 8
              Top = 40
              Width = 80
              Height = 20
              Caption = 'Exclu'#237'dos'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -12
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              ParentFont = False
              TabOrder = 0
              OnClick = ckbExcluidosClick
            end
            object ckbIncluidos: TCheckBox
              Left = 8
              Top = 16
              Width = 80
              Height = 20
              Caption = 'Inclu'#237'dos'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -12
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              ParentFont = False
              TabOrder = 1
              OnClick = ckbIncluidosClick
            end
          end
        end
        object pnlGerarRemessa: TPanel
          Left = 0
          Top = 275
          Width = 788
          Height = 61
          Align = alTop
          BevelOuter = bvNone
          TabOrder = 4
          object ckbReenviarUltimaRemessa: TCheckBox
            Left = 250
            Top = 37
            Width = 255
            Height = 23
            Caption = 'Reenviar '#250'ltima remessa enviada ao SPC'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            ParentFont = False
            TabOrder = 0
            OnClick = ckbReenviarUltimaRemessaClick
          end
          object gbxArquivoRemessa: TGroupBox
            Left = 8
            Top = 0
            Width = 495
            Height = 36
            Caption = 'NOME DO ARQUIVO DE REMESSA'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -9
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            ParentFont = False
            TabOrder = 1
            object dtxArquivoRemessa: TtecDBText
              Left = 2
              Top = 10
              Width = 490
              Height = 22
              TabStop = False
              Color = clBtnFace
              DataField = 'arquivo'
              DataSource = dtmspc.dsrRemessa
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -11
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              ParentFont = False
              ReadOnly = True
              TabOrder = 0
              Alignment = taLeftJustify
            end
          end
        end
        object pnlProcessarRetorno: TPanel
          Left = 0
          Top = 336
          Width = 788
          Height = 82
          Align = alTop
          BevelOuter = bvNone
          TabOrder = 5
          object gbxRetornoaProcessar: TGroupBox
            Left = 8
            Top = 8
            Width = 777
            Height = 74
            Caption = 'ARQUIVOS DE RETORNO A PROCESSAR'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -9
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            ParentFont = False
            TabOrder = 0
            object dbgProcessarRetorno: TtecDBGrid
              Left = 3
              Top = 12
              Width = 772
              Height = 60
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -12
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              Options = [dgRowLines, dgTabs, dgRowSelect]
              ParentFont = False
              TabOrder = 0
              TitleFont.Charset = DEFAULT_CHARSET
              TitleFont.Color = clBlack
              TitleFont.Height = -9
              TitleFont.Name = 'helvetica'
              TitleFont.Pitch = fpVariable
              TitleFont.Style = []
              RowWrap = 60
              Large = False
              DoubleRowColor = False
              TitleMinHeight = 50
              CellHeights = 100
              StrippedColor = 16054260
              CanDelete = False
              CanNotInsertFromGrid = False
              PostOnEnter = False
              PostOnSetUpDown = False
              DenySort = False
              DefaultRowHeight = 19
              ExibirNumerodaLinha = False
              NaoAtribuirDadosaTabelaaoDigitar = False
              ShowWhenFieldInvisible = False
              CanClickWhenEditing = False
            end
          end
        end
        object pnlFiliais: TPanel
          Left = 0
          Top = 0
          Width = 788
          Height = 113
          Align = alTop
          BevelOuter = bvNone
          TabOrder = 0
          object gbxFiliais: TGroupBox
            Left = 8
            Top = 0
            Width = 401
            Height = 105
            Caption = '&FILIAIS'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -9
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            ParentFont = False
            TabOrder = 0
            object sbnMarcarFiliais: TSpeedButton
              Left = 375
              Top = 14
              Width = 22
              Height = 22
              Hint = 'Marcar Filiais'
              Flat = True
              Glyph.Data = {
                
                DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
              NumGlyphs = 2
            end
            object sbnDesmarcarFiliais: TSpeedButton
              Left = 375
              Top = 38
              Width = 22
              Height = 22
              Hint = 'Desmarcar Filiais'
              Flat = True
              Glyph.Data = {
                
                DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
              NumGlyphs = 2
            end
            object clbFiliais: TCheckListBox
              Left = 3
              Top = 12
              Width = 368
              Height = 89
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -12
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              ItemHeight = 15
              ParentFont = False
              Style = lbOwnerDrawVariable
              TabOrder = 0
            end
          end
          object gbxGrupodeFiliais: TGroupBox
            Left = 416
            Top = 0
            Width = 369
            Height = 105
            Caption = 'GRU&PO DE FILIAIS'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -9
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            ParentFont = False
            TabOrder = 1
            object sbnmarcarGrupodeFiliais: TSpeedButton
              Left = 343
              Top = 14
              Width = 22
              Height = 22
              Hint = 'Marcar Grupo de Filiais'
              Flat = True
              Glyph.Data = {
                
                DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
              NumGlyphs = 2
            end
            object sbnDesmarcarGrupodeFiliais: TSpeedButton
              Left = 343
              Top = 38
              Width = 22
              Height = 22
              Hint = 'Desmarcar Grupo de Filiais'
              Flat = True
              Glyph.Data = {
                
                DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
              NumGlyphs = 2
            end
            object clbGrupodeFiliais: TCheckListBox
              Left = 3
              Top = 13
              Width = 338
              Height = 89
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -12
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              ItemHeight = 15
              ParentFont = False
              Style = lbOwnerDrawVariable
              TabOrder = 0
            end
          end
        end
      end
      object tstGerenciarSPC: TTabSheet
        Caption = ' Gerenciar '
        ImageIndex = 1
        object dbgSPC: TtecDBGrid
          Left = 0
          Top = 0
          Width = 357
          Height = 163
          DataSource = dtmspc.dsrSPC
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          Options = [dgTitles, dgIndicator, dgColLines, dgRowLines, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
          ParentFont = False
          ReadOnly = True
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clBlack
          TitleFont.Height = -13
          TitleFont.Name = 'helvetica'
          TitleFont.Pitch = fpVariable
          TitleFont.Style = []
          OnDrawColumnCell = dbgSPCDrawColumnCell
          OnDblClick = dbgSPCDblClick
          OnKeyDown = dbgSPCKeyDown
          RowWrap = 60
          Large = False
          DoubleRowColor = True
          TitleMinHeight = 50
          CellHeights = 100
          StrippedColor = 16054260
          CanDelete = False
          CanNotInsertFromGrid = False
          PostOnEnter = False
          PostOnSetUpDown = False
          DenySort = False
          DefaultRowHeight = 18
          ExibirNumerodaLinha = False
          NaoAtribuirDadosaTabelaaoDigitar = False
          ShowWhenFieldInvisible = False
          CanClickWhenEditing = False
          Columns = <
            item
              Expanded = False
              FieldName = 'numero'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -12
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              Title.Alignment = taCenter
              Title.Caption = 'SPC'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlack
              Title.Font.Height = -9
              Title.Font.Name = 'helvetica'
              Title.Font.Pitch = fpVariable
              Title.Font.Style = []
              Width = 60
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'nomecliente'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -12
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              Title.Alignment = taCenter
              Title.Caption = 'NOME DO CLIENTE'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlack
              Title.Font.Height = -9
              Title.Font.Name = 'helvetica'
              Title.Font.Pitch = fpVariable
              Title.Font.Style = []
              Width = 220
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'marcar'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -12
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              Title.Alignment = taCenter
              Title.Caption = 'OK'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlack
              Title.Font.Height = -9
              Title.Font.Name = 'helvetica'
              Title.Font.Pitch = fpVariable
              Title.Font.Style = []
              Width = 30
              Visible = True
            end>
        end
        object dbgParcelasSPC: TtecDBGrid
          Left = 355
          Top = 0
          Width = 432
          Height = 163
          DataSource = dtmspc.dsrParcelasSPC
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
          ParentFont = False
          ReadOnly = True
          TabOrder = 1
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clBlack
          TitleFont.Height = -13
          TitleFont.Name = 'helvetica'
          TitleFont.Pitch = fpVariable
          TitleFont.Style = []
          RowWrap = 60
          Large = False
          DoubleRowColor = True
          TitleMinHeight = 50
          CellHeights = 100
          StrippedColor = 16054260
          CanDelete = False
          CanNotInsertFromGrid = False
          PostOnEnter = False
          PostOnSetUpDown = False
          DenySort = False
          DefaultRowHeight = 18
          ExibirNumerodaLinha = False
          NaoAtribuirDadosaTabelaaoDigitar = False
          ShowWhenFieldInvisible = False
          CanClickWhenEditing = False
          Columns = <
            item
              Expanded = False
              FieldName = 'contratoparcela'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -12
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              Title.Alignment = taCenter
              Title.Caption = 'CONTRATO'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlack
              Title.Font.Height = -9
              Title.Font.Name = 'helvetica'
              Title.Font.Pitch = fpVariable
              Title.Font.Style = []
              Width = 85
              Visible = True
            end
            item
              Alignment = taCenter
              Expanded = False
              FieldName = 'parcelaorigem'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -12
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              Title.Alignment = taCenter
              Title.Caption = 'PARCELA'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlack
              Title.Font.Height = -9
              Title.Font.Name = 'helvetica'
              Title.Font.Pitch = fpVariable
              Title.Font.Style = []
              Width = 45
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'datavencto'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -12
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              Title.Alignment = taCenter
              Title.Caption = 'VENCTO'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlack
              Title.Font.Height = -9
              Title.Font.Name = 'helvetica'
              Title.Font.Pitch = fpVariable
              Title.Font.Style = []
              Width = 65
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'valorvencto'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -12
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              Title.Alignment = taCenter
              Title.Caption = 'VALOR'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlack
              Title.Font.Height = -9
              Title.Font.Name = 'helvetica'
              Title.Font.Pitch = fpVariable
              Title.Font.Style = []
              Width = 80
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'nomecausa'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -12
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              Title.Alignment = taCenter
              Title.Caption = 'RAZ'#195'O OBSOLESC'#202'NCIA'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlack
              Title.Font.Height = -9
              Title.Font.Name = 'helvetica'
              Title.Font.Pitch = fpVariable
              Title.Font.Style = []
              Width = 120
              Visible = True
            end>
        end
        object dbgSPCRemessa: TtecDBGrid
          Left = 0
          Top = 227
          Width = 233
          Height = 142
          DataSource = dtmspc.dsrSPCRemessa
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
          ParentFont = False
          ReadOnly = True
          TabOrder = 3
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clBlack
          TitleFont.Height = -13
          TitleFont.Name = 'helvetica'
          TitleFont.Pitch = fpVariable
          TitleFont.Style = []
          RowWrap = 60
          Large = False
          DoubleRowColor = True
          TitleMinHeight = 50
          CellHeights = 100
          StrippedColor = 16054260
          CanDelete = False
          CanNotInsertFromGrid = False
          PostOnEnter = False
          PostOnSetUpDown = False
          DenySort = False
          DefaultRowHeight = 18
          ExibirNumerodaLinha = False
          NaoAtribuirDadosaTabelaaoDigitar = False
          ShowWhenFieldInvisible = False
          CanClickWhenEditing = False
          Columns = <
            item
              Expanded = False
              FieldName = 'remessa'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -12
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              Title.Alignment = taCenter
              Title.Caption = 'REMESSA'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlack
              Title.Font.Height = -9
              Title.Font.Name = 'helvetica'
              Title.Font.Pitch = fpVariable
              Title.Font.Style = []
              Width = 60
              Visible = True
            end
            item
              Alignment = taCenter
              Expanded = False
              FieldName = 'operacao'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -12
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              Title.Alignment = taCenter
              Title.Caption = 'OP.'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlack
              Title.Font.Height = -9
              Title.Font.Name = 'helvetica'
              Title.Font.Pitch = fpVariable
              Title.Font.Style = []
              Width = 25
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'dataenvio'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -12
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              Title.Alignment = taCenter
              Title.Caption = 'DATA ENVIO'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlack
              Title.Font.Height = -9
              Title.Font.Name = 'helvetica'
              Title.Font.Pitch = fpVariable
              Title.Font.Style = []
              Width = 105
              Visible = True
            end>
        end
        object dtxBloqueada: TtecDBText
          Left = 160
          Top = 376
          Width = 121
          Height = 24
          TabStop = False
          Color = clBtnFace
          DataField = 'NBloqueada'
          DataSource = dtmspc.dsrSPC
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clRed
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = [fsBold]
          ParentFont = False
          ReadOnly = True
          TabOrder = 4
          Alignment = taLeftJustify
        end
        object dtxSituacao: TtecDBText
          Left = 25
          Top = 392
          Width = 423
          Height = 24
          TabStop = False
          Color = clBtnFace
          DataField = 'Situacao'
          DataSource = dtmspc.dsrSPCRemessaDados
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clRed
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 5
          Alignment = taLeftJustify
        end
        object pgcInformacoes: TPageControl
          Left = 239
          Top = 166
          Width = 549
          Height = 204
          ActivePage = tstEndereco
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 6
          object tstDadosSPC: TTabSheet
            Caption = 'Dados SPC'
            ImageIndex = 3
            object gbxContratos: TGroupBox
              Left = 0
              Top = 0
              Width = 541
              Height = 177
              Align = alClient
              TabOrder = 0
              object gbxRetorno: TGroupBox
                Left = 9
                Top = 127
                Width = 476
                Height = 36
                Caption = 'MENSAGEM DE RETORNO DO SPC'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Height = -9
                Font.Name = 'helvetica'
                Font.Pitch = fpVariable
                Font.Style = []
                ParentFont = False
                TabOrder = 0
                object dtxMsgRetorno: TtecDBText
                  Left = 2
                  Top = 10
                  Width = 471
                  Height = 20
                  TabStop = False
                  Color = clBtnFace
                  Font.Charset = DEFAULT_CHARSET
                  Font.Color = clRed
                  Font.Height = -12
                  Font.Name = 'helvetica'
                  Font.Pitch = fpVariable
                  Font.Style = []
                  ParentFont = False
                  ReadOnly = True
                  TabOrder = 0
                  Alignment = taLeftJustify
                end
              end
              object gbxRegistro: TGroupBox
                Left = 8
                Top = 75
                Width = 241
                Height = 50
                Caption = 'REGISTRO ou RETIRADA'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Height = -9
                Font.Name = 'helvetica'
                Font.Pitch = fpVariable
                Font.Style = []
                ParentFont = False
                TabOrder = 1
                object gbxDataEmpresa: TGroupBox
                  Left = 7
                  Top = 12
                  Width = 110
                  Height = 36
                  Caption = 'NA EMPRESA'
                  TabOrder = 0
                  object dtxDataEmpresa: TtecDBText
                    Left = 2
                    Top = 10
                    Width = 105
                    Height = 20
                    TabStop = False
                    Color = clBtnFace
                    DataField = 'dataempresa'
                    DataSource = dtmspc.dsrSPCRemessaDados
                    Font.Charset = DEFAULT_CHARSET
                    Font.Color = clBlack
                    Font.Height = -12
                    Font.Name = 'helvetica'
                    Font.Pitch = fpVariable
                    Font.Style = []
                    ParentFont = False
                    ReadOnly = True
                    TabOrder = 0
                    Alignment = taLeftJustify
                  end
                end
                object gbxDataSPC: TGroupBox
                  Left = 124
                  Top = 12
                  Width = 110
                  Height = 36
                  Caption = 'NO SPC'
                  TabOrder = 1
                  object dtxDataSPC: TtecDBText
                    Left = 2
                    Top = 10
                    Width = 105
                    Height = 20
                    TabStop = False
                    Color = clBtnFace
                    DataField = 'dataspc'
                    DataSource = dtmspc.dsrSPCRemessaDados
                    Font.Charset = DEFAULT_CHARSET
                    Font.Color = clBlack
                    Font.Height = -12
                    Font.Name = 'helvetica'
                    Font.Pitch = fpVariable
                    Font.Style = []
                    ParentFont = False
                    ReadOnly = True
                    TabOrder = 0
                    Alignment = taLeftJustify
                  end
                end
              end
              object gbxCliente: TGroupBox
                Left = 7
                Top = 2
                Width = 393
                Height = 36
                Caption = 'CLIENTE'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Height = -9
                Font.Name = 'helvetica'
                Font.Pitch = fpVariable
                Font.Style = []
                ParentFont = False
                TabOrder = 2
                object dtxTipoCliente: TtecDBText
                  Left = 2
                  Top = 10
                  Width = 17
                  Height = 20
                  TabStop = False
                  Color = clBtnFace
                  DataField = 'tipocliente'
                  DataSource = dtmspc.dsrSPC
                  Font.Charset = DEFAULT_CHARSET
                  Font.Color = clBlack
                  Font.Height = -12
                  Font.Name = 'helvetica'
                  Font.Pitch = fpVariable
                  Font.Style = []
                  ParentFont = False
                  ReadOnly = True
                  TabOrder = 0
                  Alignment = taLeftJustify
                end
                object dtxcodigocliente: TtecDBText
                  Left = 18
                  Top = 10
                  Width = 65
                  Height = 20
                  TabStop = False
                  Color = clBtnFace
                  DataField = 'cliente'
                  DataSource = dtmspc.dsrSPC
                  Font.Charset = DEFAULT_CHARSET
                  Font.Color = clBlack
                  Font.Height = -12
                  Font.Name = 'helvetica'
                  Font.Pitch = fpVariable
                  Font.Style = []
                  ParentFont = False
                  ReadOnly = True
                  TabOrder = 1
                  Alignment = taLeftJustify
                end
                object dtxcliente: TtecDBText
                  Left = 83
                  Top = 10
                  Width = 307
                  Height = 20
                  TabStop = False
                  Color = clBtnFace
                  DataField = 'nomecliente'
                  DataSource = dtmspc.dsrSPC
                  Font.Charset = DEFAULT_CHARSET
                  Font.Color = clBlack
                  Font.Height = -12
                  Font.Name = 'helvetica'
                  Font.Pitch = fpVariable
                  Font.Style = []
                  ParentFont = False
                  ReadOnly = True
                  TabOrder = 2
                  Alignment = taLeftJustify
                end
              end
              object gbxAvalista: TGroupBox
                Left = 7
                Top = 38
                Width = 393
                Height = 36
                Caption = 'AVALISTA'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Height = -9
                Font.Name = 'helvetica'
                Font.Pitch = fpVariable
                Font.Style = []
                ParentFont = False
                TabOrder = 3
                object dtxcodigoavalista: TtecDBText
                  Left = 2
                  Top = 10
                  Width = 81
                  Height = 20
                  TabStop = False
                  Color = clBtnFace
                  DataField = 'avalista'
                  DataSource = dtmspc.dsrSPC
                  Font.Charset = DEFAULT_CHARSET
                  Font.Color = clBlack
                  Font.Height = -12
                  Font.Name = 'helvetica'
                  Font.Pitch = fpVariable
                  Font.Style = []
                  ParentFont = False
                  ReadOnly = True
                  TabOrder = 0
                  Alignment = taLeftJustify
                end
                object dtxAvalista: TtecDBText
                  Left = 83
                  Top = 10
                  Width = 307
                  Height = 20
                  TabStop = False
                  Color = clBtnFace
                  DataField = 'nomeavalista'
                  DataSource = dtmspc.dsrSPC
                  Font.Charset = DEFAULT_CHARSET
                  Font.Color = clBlack
                  Font.Height = -12
                  Font.Name = 'helvetica'
                  Font.Pitch = fpVariable
                  Font.Style = []
                  ParentFont = False
                  ReadOnly = True
                  TabOrder = 1
                  Alignment = taLeftJustify
                end
              end
              object gbxValorDebito: TGroupBox
                Left = 360
                Top = 75
                Width = 126
                Height = 50
                Caption = 'VALOR DO D'#201'BITO'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Height = -9
                Font.Name = 'helvetica'
                Font.Pitch = fpVariable
                Font.Style = []
                ParentFont = False
                TabOrder = 4
                object dtxTotalemAberto: TtecDBText
                  Left = 2
                  Top = 16
                  Width = 122
                  Height = 24
                  TabStop = False
                  Color = clBtnFace
                  DataField = 'valordebito'
                  DataSource = dtmspc.dsrSPCRemessaDados
                  Font.Charset = DEFAULT_CHARSET
                  Font.Color = clBlack
                  Font.Height = -16
                  Font.Name = 'helvetica'
                  Font.Pitch = fpVariable
                  Font.Style = [fsBold]
                  ParentFont = False
                  ReadOnly = True
                  TabOrder = 0
                  Alignment = taLeftJustify
                end
              end
              object gbxDataCompra: TGroupBox
                Left = 404
                Top = 2
                Width = 80
                Height = 36
                Caption = 'DATA COMPRA'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Height = -9
                Font.Name = 'helvetica'
                Font.Pitch = fpVariable
                Font.Style = []
                ParentFont = False
                TabOrder = 5
                object dtxDataCompra: TtecDBText
                  Left = 2
                  Top = 10
                  Width = 75
                  Height = 20
                  TabStop = False
                  Color = clBtnFace
                  DataField = 'datacompra'
                  DataSource = dtmspc.dsrSPCRemessaDados
                  Font.Charset = DEFAULT_CHARSET
                  Font.Color = clBlack
                  Font.Height = -12
                  Font.Name = 'helvetica'
                  Font.Pitch = fpVariable
                  Font.Style = []
                  ParentFont = False
                  ReadOnly = True
                  TabOrder = 0
                  Alignment = taLeftJustify
                end
              end
              object gbxDataVencto: TGroupBox
                Left = 405
                Top = 38
                Width = 80
                Height = 36
                Caption = 'VENCIMENTO'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Height = -9
                Font.Name = 'helvetica'
                Font.Pitch = fpVariable
                Font.Style = []
                ParentFont = False
                TabOrder = 6
                object dtxDataVencto: TtecDBText
                  Left = 2
                  Top = 10
                  Width = 75
                  Height = 20
                  TabStop = False
                  Color = clBtnFace
                  DataField = 'datavencto'
                  DataSource = dtmspc.dsrSPCRemessaDados
                  Font.Charset = DEFAULT_CHARSET
                  Font.Color = clBlack
                  Font.Height = -12
                  Font.Name = 'helvetica'
                  Font.Pitch = fpVariable
                  Font.Style = []
                  ParentFont = False
                  ReadOnly = True
                  TabOrder = 0
                  Alignment = taLeftJustify
                end
              end
            end
          end
          object tstDadosPessoais: TTabSheet
            Caption = 'Dados Pessoais'
            ImageIndex = 1
            object gbxFiliacao: TGroupBox
              Left = 0
              Top = 0
              Width = 279
              Height = 86
              Caption = 'FILIA'#199#195'O'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -9
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              ParentFont = False
              TabOrder = 0
              object gbxNomeMae: TGroupBox
                Left = 6
                Top = 10
                Width = 267
                Height = 36
                Caption = 'M'#195'E'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Height = -9
                Font.Name = 'helvetica'
                Font.Pitch = fpVariable
                Font.Style = []
                ParentFont = False
                TabOrder = 0
                object edtClientesMae: TDBEditTexto
                  Left = 2
                  Top = 10
                  Width = 264
                  Height = 23
                  Color = clBtnFace
                  DataField = 'mae'
                  DataSource = dtmspc.dsrSPCRemessaDados
                  Font.Charset = DEFAULT_CHARSET
                  Font.Color = clBlack
                  Font.Height = -12
                  Font.Name = 'helvetica'
                  Font.Pitch = fpVariable
                  Font.Style = []
                  MaxLength = 14
                  ParentFont = False
                  ReadOnly = True
                  TabOrder = 0
                  CharCasenoDefault = False
                  Alignment = taLeftJustify
                end
              end
              object gbxNomePai: TGroupBox
                Left = 6
                Top = 47
                Width = 267
                Height = 36
                Caption = 'PAI'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Height = -9
                Font.Name = 'helvetica'
                Font.Pitch = fpVariable
                Font.Style = []
                ParentFont = False
                TabOrder = 1
                object edtClientesPai: TDBEditTexto
                  Left = 2
                  Top = 10
                  Width = 264
                  Height = 23
                  Color = clBtnFace
                  DataField = 'pai'
                  DataSource = dtmspc.dsrSPCRemessaDados
                  Font.Charset = DEFAULT_CHARSET
                  Font.Color = clBlack
                  Font.Height = -12
                  Font.Name = 'helvetica'
                  Font.Pitch = fpVariable
                  Font.Style = []
                  MaxLength = 14
                  ParentFont = False
                  ReadOnly = True
                  TabOrder = 0
                  CharCasenoDefault = False
                  Alignment = taLeftJustify
                end
              end
            end
            object gbxNaturalidade: TGroupBox
              Left = 284
              Top = 130
              Width = 253
              Height = 36
              Caption = 'NATURALIDADE'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -9
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              ParentFont = False
              TabOrder = 1
              object dtxClientesNaturalEstado: TDBEditTexto
                Left = 222
                Top = 10
                Width = 28
                Height = 23
                TabStop = False
                DataField = 'naturalestado'
                DataSource = dtmspc.dsrSPCRemessaDados
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Height = -12
                Font.Name = 'helvetica'
                Font.Pitch = fpVariable
                Font.Style = []
                MaxLength = 24
                ParentFont = False
                ReadOnly = True
                TabOrder = 1
                CharCasenoDefault = False
                Alignment = taLeftJustify
              end
              object dtxNaturalCidadeCliente: TtecDBText
                Left = 2
                Top = 10
                Width = 220
                Height = 23
                TabStop = False
                Color = clBtnFace
                DataField = 'nomenaturalcidade'
                DataSource = dtmspc.dsrSPCRemessaDados
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Height = -12
                Font.Name = 'helvetica'
                Font.Pitch = fpVariable
                Font.Style = []
                ParentFont = False
                ReadOnly = True
                TabOrder = 0
                Alignment = taLeftJustify
              end
            end
            object rgpSexo: TtecDBRadioGroup
              Left = 284
              Top = 0
              Width = 87
              Height = 80
              Caption = 'SEXO'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -9
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              ParentFont = False
              TabOrder = 2
              TabStop = False
              DataField = 'sexo'
              DataSource = dtmspc.dsrSPCRemessaDados
              ReadOnly = True
              object rbnMasculino: TtecRadioButton
                Left = 5
                Top = 14
                Width = 80
                Height = 17
                Caption = 'Masculino'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Height = -12
                Font.Name = 'helvetica'
                Font.Pitch = fpVariable
                Font.Style = []
                ParentFont = False
                TabOrder = 0
                Value = 'M'
              end
              object rbnFeminino: TtecRadioButton
                Left = 5
                Top = 33
                Width = 80
                Height = 20
                Caption = 'Feminino'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Height = -12
                Font.Name = 'helvetica'
                Font.Pitch = fpVariable
                Font.Style = []
                ParentFont = False
                TabOrder = 1
                Value = 'F'
              end
              object rbnEmpresa: TtecRadioButton
                Left = 5
                Top = 55
                Width = 80
                Height = 17
                Caption = 'Empresa'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Height = -12
                Font.Name = 'helvetica'
                Font.Pitch = fpVariable
                Font.Style = []
                ParentFont = False
                TabOrder = 2
                Value = 'E'
              end
            end
            object rgpClientesCivil: TtecDBRadioGroup
              Left = 376
              Top = 0
              Width = 161
              Height = 80
              Caption = 'ESTADO CIVIL'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -9
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              ParentFont = False
              TabOrder = 3
              TabStop = False
              DataField = 'civil'
              DataSource = dtmspc.dsrSPCRemessaDados
              ReadOnly = True
              object rbnCasado: TtecRadioButton
                Left = 8
                Top = 14
                Width = 68
                Height = 17
                Caption = 'Casado'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Height = -12
                Font.Name = 'helvetica'
                Font.Pitch = fpVariable
                Font.Style = []
                ParentFont = False
                TabOrder = 0
                Value = 'C'
              end
              object rbnSolteiro: TtecRadioButton
                Left = 91
                Top = 14
                Width = 65
                Height = 17
                Caption = 'Solteiro'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Height = -12
                Font.Name = 'helvetica'
                Font.Pitch = fpVariable
                Font.Style = []
                ParentFont = False
                TabOrder = 1
                Value = 'S'
              end
              object rbnDivorciado: TtecRadioButton
                Left = 8
                Top = 33
                Width = 82
                Height = 17
                Caption = 'Divorciado'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Height = -12
                Font.Name = 'helvetica'
                Font.Pitch = fpVariable
                Font.Style = []
                ParentFont = False
                TabOrder = 2
                Value = 'D'
              end
              object rbnViuvo: TtecRadioButton
                Left = 91
                Top = 33
                Width = 65
                Height = 17
                Caption = 'Vi'#250'vo'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Height = -12
                Font.Name = 'helvetica'
                Font.Pitch = fpVariable
                Font.Style = []
                ParentFont = False
                TabOrder = 3
                Value = 'V'
              end
              object rbnOutros: TtecRadioButton
                Left = 8
                Top = 55
                Width = 65
                Height = 17
                Caption = 'Outros'
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Height = -12
                Font.Name = 'helvetica'
                Font.Pitch = fpVariable
                Font.Style = []
                ParentFont = False
                TabOrder = 4
                Value = 'O'
              end
            end
            object gbxConjuge: TGroupBox
              Left = 368
              Top = 87
              Width = 168
              Height = 36
              Caption = 'C'#212'NJUGE'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -9
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              ParentFont = False
              TabOrder = 4
              object edtNomeConjuge: TDBEditTexto
                Left = 6
                Top = 10
                Width = 158
                Height = 23
                Color = clBtnFace
                DataField = 'conjuge'
                DataSource = dtmspc.dsrSPCRemessaDados
                MaxLength = 14
                ReadOnly = True
                TabOrder = 0
                CharCasenoDefault = False
                Alignment = taLeftJustify
              end
            end
            object gbxIdentificacao: TGroupBox
              Left = 149
              Top = 88
              Width = 131
              Height = 85
              Caption = 'IDENTIDADE'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -9
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              ParentFont = False
              TabOrder = 5
              object gbxDocumento: TGroupBox
                Left = 7
                Top = 10
                Width = 117
                Height = 36
                Caption = 'DOCUMENTO'
                TabOrder = 0
                object DBEditDocumento1: TDBEditDocumento
                  Left = 2
                  Top = 10
                  Width = 112
                  Height = 23
                  Color = clBtnFace
                  DataField = 'documento'
                  DataSource = dtmspc.dsrSPCRemessaDados
                  MaxLength = 16
                  ReadOnly = True
                  TabOrder = 0
                  CharCasenoDefault = False
                  Alignment = taLeftJustify
                  Tipo = Identidade
                end
              end
              object gbxOrgao: TGroupBox
                Left = 7
                Top = 46
                Width = 80
                Height = 36
                Caption = #211'RG'#195'O'
                TabOrder = 1
              end
              object gbxUFDocto: TGroupBox
                Left = 92
                Top = 46
                Width = 33
                Height = 36
                Caption = 'UF'
                TabOrder = 2
                object DBEditTexto1: TDBEditTexto
                  Left = 2
                  Top = 10
                  Width = 28
                  Height = 23
                  Color = clBtnFace
                  DataField = 'idestado'
                  DataSource = dtmspc.dsrSPCRemessaDados
                  Font.Charset = DEFAULT_CHARSET
                  Font.Color = clBlack
                  Font.Height = -12
                  Font.Name = 'helvetica'
                  Font.Pitch = fpVariable
                  Font.Style = []
                  MaxLength = 12
                  ParentFont = False
                  ReadOnly = True
                  TabOrder = 0
                  CharCasenoDefault = False
                  Alignment = taLeftJustify
                end
              end
            end
            object gbxDataNascto: TGroupBox
              Left = 284
              Top = 87
              Width = 80
              Height = 36
              Caption = 'DATA NASCTO'
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -9
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              ParentFont = False
              TabOrder = 6
              object edtClientesNascto: TDBEditData
                Left = 2
                Top = 10
                Width = 75
                Height = 23
                Alignment = taLeftJustify
                Color = clBtnFace
                DataField = 'nascto'
                DataSource = dtmspc.dsrSPCRemessaDados
                Font.Charset = DEFAULT_CHARSET
                Font.Color = clBlack
                Font.Height = -12
                Font.Name = 'helvetica'
                Font.Pitch = fpVariable
                Font.Style = []
                Maximo = 0
                Minimo = 37353
                Adicional = 0
                ParentFont = False
                ReadOnly = True
                TabOrder = 0
                Opcional = True
              end
            end
            inline fracgcoucpf1: Tfracgcoucpf
              Left = 3
              Top = 90
              Width = 138
              Height = 83
              HorzScrollBar.Visible = False
              VertScrollBar.Visible = False
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              ParentFont = False
              TabOrder = 7
              inherited gbxCPGouCGC: TGroupBox
                Width = 138
                Height = 83
                inherited rgbTipoPessoa: TtecDBRadioGroup
                  Width = 121
                  DataField = 'pessoatipo'
                  DataSource = dtmspc.dsrSPCRemessaDados
                  ReadOnly = True
                  OnChange = fracgcoucpf1rgbTipoPessoaChange
                  inherited rbnFisica: TtecRadioButton
                    Left = 6
                    Top = 9
                  end
                  inherited rbnJuridica: TtecRadioButton
                    Left = 61
                    Top = 9
                  end
                end
                inherited gbxCPF_CNPJ: TGroupBox
                  Left = 3
                  Top = 44
                  Height = 36
                  inherited edtCPFCNPJ: TDBEditCPFCNPJ
                    Top = 10
                    DataField = 'pessoanumero'
                    DataSource = dtmspc.dsrSPCRemessaDados
                    ReadOnly = True
                  end
                end
              end
            end
          end
          object tstEndereco: TTabSheet
            Caption = 'Endere'#231'o'
            ImageIndex = 2
            inline fraEndereco1: TfraEndereco
              Left = 8
              Top = 5
              Width = 433
              Height = 153
              HorzScrollBar.Visible = False
              VertScrollBar.Visible = False
              Font.Charset = DEFAULT_CHARSET
              Font.Color = clBlack
              Font.Height = -13
              Font.Name = 'helvetica'
              Font.Pitch = fpVariable
              Font.Style = []
              ParentFont = False
              TabOrder = 0
              inherited gbxEndereco: TGroupBox
                Width = 433
                Height = 153
                inherited pnlNumero: TPanel
                  Width = 429
                  inherited edtNumeroEndereco: TDBEditTexto
                    Left = 0
                    DataField = 'numero_rua'
                    DataSource = dtmspc.dsrSPCRemessaDados
                  end
                  inherited edtComplemento: TDBEditTexto
                    Left = 79
                    Width = 345
                    DataField = 'complemento'
                    DataSource = dtmspc.dsrSPCRemessaDados
                  end
                end
                inherited pnlBairro: TPanel
                  Width = 429
                  inherited edtBairro: TtecDoubleDBEdit
                    Left = 0
                    Width = 424
                    LookupField = 'nomebairro'
                    LookupSource = dtmspc.dsrSPCRemessaDados
                    DataField = 'nomebairro'
                    DataSource = dtmspc.dsrSPCRemessaDados
                    MaxLength = 9
                  end
                end
                inherited pnlCidade: TPanel
                  Width = 429
                  Height = 26
                  inherited edtCEP: TDBEditCep
                    Left = -1
                    Top = 0
                    DataField = 'cep'
                    DataSource = dtmspc.dsrSPCRemessaDados
                    MaxLength = 6
                  end
                  inherited edtCidade: TtecDoubleDBEdit
                    Left = 78
                    Top = 0
                    Width = 316
                    LookupField = 'nomecidade'
                    LookupSource = dtmspc.dsrSPCRemessaDados
                    DataField = 'nomecidade'
                    DataSource = dtmspc.dsrSPCRemessaDados
                    MaxLength = 9
                  end
                  inherited edtEstado: TDBEditTexto
                    Left = 393
                    Top = 0
                    DataField = 'estado'
                    DataSource = dtmspc.dsrSPCRemessaDados
                    MaxLength = 9
                  end
                end
                inherited pnlRua: TPanel
                  Width = 429
                  inherited mmoRua: TtecDBMemo
                    Width = 424
                    DataField = 'rua'
                    DataSource = dtmspc.dsrSPCRemessaDados
                    Font.Height = -12
                  end
                end
                inherited pnllblCidade: TPanel
                  Width = 429
                end
                inherited pnllblBairro: TPanel
                  Width = 429
                end
                inherited pnllblNumero: TPanel
                  Width = 429
                end
              end
            end
          end
        end
        object rgpClienteAvalista: TtecDBRadioGroup
          Left = 0
          Top = 165
          Width = 88
          Height = 56
          Caption = 'EXIBIR DADOS'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 2
          TabStop = False
          DataField = 'tipo'
          DataSource = dtmspc.dsrSPCRemessa
          object rbnCliente: TtecRadioButton
            Left = 10
            Top = 14
            Width = 65
            Height = 17
            Caption = 'Cliente'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            ParentFont = False
            TabOrder = 0
            Value = 'C'
          end
          object rbnAvalista: TtecRadioButton
            Left = 10
            Top = 33
            Width = 65
            Height = 17
            Caption = 'Avalista'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            ParentFont = False
            TabOrder = 1
            Value = 'A'
          end
        end
        object gbxNrTotal: TGroupBox
          Left = 96
          Top = 166
          Width = 65
          Height = 36
          Caption = 'TOTAL'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 7
          object lblSPCTotal: TLabel
            Left = 2
            Top = 10
            Width = 60
            Height = 23
            Alignment = taRightJustify
            AutoSize = False
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            ParentFont = False
          end
        end
        object gbxMarcadas: TGroupBox
          Left = 168
          Top = 166
          Width = 65
          Height = 36
          Caption = 'MARCADAS'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 8
          object lblSPCMarcados: TLabel
            Left = 2
            Top = 10
            Width = 60
            Height = 23
            Alignment = taRightJustify
            AutoSize = False
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            ParentFont = False
          end
        end
      end
    end
    object pnlOperaco: TPanel
      Left = 3
      Top = 3
      Width = 800
      Height = 78
      Align = alTop
      BevelOuter = bvNone
      TabOrder = 1
      object lblOperacao: TLabel
        Left = 16
        Top = 2
        Width = 57
        Height = 12
        Caption = ' OPERA'#199#195'O '
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
      end
      object rgpOperacao: TRadioGroup
        Left = 2
        Top = 10
        Width = 784
        Height = 64
        Columns = 4
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ItemIndex = 0
        Items.Strings = (
          'Inclus'#227'o no SPC'
          'Exclus'#227'o do SPC'
          'For'#231'ar exclus'#227'o do SPC'
          'Cancelar exclus'#227'o for'#231'ada   '
          'Gerar arquivo de remessa'
          'Processar arquivo de retorno'
          'Consulta')
        ParentFont = False
        TabOrder = 0
        OnClick = rgpOperacaoClick
      end
    end
  end
end
