...
uses
  ...
  //Projeto
  ZPgSqlQuery, cpquery, Grids, DBGrids, cpdbgrid, cpdocumento,
  ...
  frconsultacontabil, frconsultacodigocontabil, ToolWin, cpdbedittext,
  cpdbmemo;
...
type
  TfrmPlanoContas = class(TfrmCadastroPadrao)
    ...
    rgpNatureza: TtecDBRadioGroup;
    rbnCONTASDOATIVO: TtecRadioButton;
    rbnCONTASDOPASSIVO: TtecRadioButton;
    rbnPATRIMONIOLiQUIDO: TtecRadioButton;
    rbnCONTASDERECEITA: TtecRadioButton;
    rbnCONTASDECOMPENSACAO: TtecRadioButton;
    rbnOUTRAS: TtecRadioButton;
    ckbDemonstrativoSPED: TDBCheckBox;
    stxContasInativas: TStaticText;
    pnlPlanoContas: TPanel;
    pnlTipoPlanoReferencial: TPanel;
    pnlPlanoContasReferencial: TPanel;
    gbxVinculacaoReferencial: TGroupBox;
    ...
    procedure rgpNaturezaChange(Sender: TObject);
    procedure rgpNaturezaClickItem(Sender: TObject);
    procedure rgpNatureza_Click(Sender: TObject);
    procedure fraConsultaClassificacaoReferencialedfCodigoExit(
      Sender: TObject);
    procedure edtDescricaoExit(Sender: TObject);
    procedure fraConsultaClassificacaoReferencialedfCodigoKeyDown(
      Sender: TObject; var Key: Word; Shift: TShiftState);
  private
    { Private declarations }
     procedure AtribuirDadosContaReferencial(Found: Boolean);
  protected
...
begin
   ...
   pnlTipoPlanoReferencial.caption := dtmPlanoContas.TipoPlanoReferencial_('descricao');
   ...
   fraConsultaClassificacaoReferencial.OnFound := AtribuirDadosContaReferencial;
...
begin
 inherited;
 {
 if fraConsultaClassificacaoReferencial.edfCodigo.Focused then
   if TeclaEnterOuReturn(key) then
     If dtmPlanoContas.Inserindo then
        InternoGravar;
        }
end;
...
procedure TfrmPlanoContas.rgpNaturezaChange(Sender: TObject);
begin
  inherited;
...
procedure TfrmPlanoContas.rgpNaturezaClickItem(Sender: TObject);
begin
  inherited;
  if dtmPlanoContas.qryPlanoContasnatureza.AsString<>'' then
  ;
...
procedure TfrmPlanoContas.fraConsultaClassificacaoReferencialedfCodigoExit(
  Sender: TObject);
begin
  inherited;
  fraConsultaClassificacaoReferencial.edfCodigoExit(Sender);
...
procedure TfrmPlanoContas.AtribuirDadosContaReferencial(Found: Boolean);
begin
  with dtmPlanoContas do
  begin
    qryPlanoContasclassificacaoreferencial.asstring := fraConsultaClassificacaoReferencial.qryProcuraPlanoReferencialclassificacao.AsString;
    qryPlanoContastiporeferencial.AsString := fraConsultaClassificacaoReferencial.qryProcuraPlanoReferencialtipo.AsString;
  end;
end;

procedure TfrmPlanoContas.fraConsultaClassificacaoReferencialedfCodigoKeyDown(
  Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
  fraConsultaClassificacaoReferencial.edfCodigoKeyDown(Sender, Key,
  Shift);
