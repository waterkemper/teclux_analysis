...
uses
  ...
  //Projeto
  dmcadastropedidos, fmcadastroprevisoespedidos, fmcadastroprodutospedidos_,
  ...
  fmFormacaoPrecoVenda, AdvObj, BaseGrid, AdvGrid, DBAdvGrid, PCheck,
  PDBCheck;
//   Windows, Messages;
...
type
  TfrmCadastroPedidos = class(TfrmCadastroPadrao)
    ...
    dtxValorProdutos: TDBEditNumero;
    dtxTotalIPI: TDBEditNumero;
    ...
    tstNotasPendentesdeEntrada: TTabSheet;
    dbgConsultaProdutosEntradaSefaz: TtecDBGrid;
    Shape7: TShape;
    Label3: TLabel;
    ckbRecebido: TPDBCheck;
    ...
    procedure pgcPedidosChange(Sender: TObject);
  ...
  private
    ...
    ffrmcadastroprodutospedidos_: Tfrmcadastroprodutospedidos_;
//    FTfrmCadastroPedidosDestroy: TNotifyEvent;
...
//    dtmCadastroPedidos: TdtmCadastroPedidos;
  ...
  public
    ...
    property frmcadastroprodutospedidos_: Tfrmcadastroprodutospedidos_ read ffrmcadastroprodutospedidos_ write ffrmcadastroprodutospedidos_;


    procedure Criar_frmCadastroProdutosPedidos_;
//    property TfrmCadastroPedidosDestroy: TNotifyEvent read FTfrmCadastroPedidosDestroy write FTfrmCadastroPedidosDestroy;
...
begin
  frmCadastroPrevisoesPedidos := TfrmCadastroPrevisoesPedidos.Create(self);
...
begin
  ...
  begin
    if dtmCadastroPedidos.qryPedidosfilialfatura.asinteger > 0  then
    begin
...
//        freeandnil(ffrmCadastroProdutosPedidos_);
    ...
    begin
      MensagemAviso('Filial da fatura não informada.');
      pgcPedidos.activepage := tstPedidos;
      if flkFilialFatura.CanFocus then
...
begin
  inherited;
  dbgProdutosPedidos.ReadOnly  := dtmCadastroPedidos.ProdutosPedidoReadOnly;
...
//  edtFreteNota.Enabled := (flkFornecedorTransporte.Text <> '');
  sbnVisualizarGrade.Visible :=  false; {dtmCadastroPedidos.UtilizandoGrade;} 
  ...
  {
  if dtmCadastroPedidos.qryPedidosatualizarprecovenda.asBoolean then
    dbckbAtualizarPrecoVenda.color := clBtnHighlight
  else
    dbckbAtualizarPrecoVenda.color := clBtnFace;
  }

  sbnIncluirProduto.Enabled := not dtmCadastroPedidos.qryPedidosentrega.readonly;

  sbnAtendimento.Enabled := (dtmCadastroPedidos.qryPedidosfornecedor.asInteger <> 0);
...
//dbgProdutosPedidos.ColumnbyName('precosugerido                                                                ').Visible := ParSistema.LocalCalculoPreco in [lcpPEDIDO, lcpAMBOS];
//  sbnRecalcularPrecoSugerido.Visible    := ParSistema.LocalCalculoPreco in [lcpPEDIDO, lcpAMBOS];
  sbnCustoePrecoVenda.Visible    := ParSistema.LocalCalculoPreco in [lcpPEDIDO, lcpAMBOS];

  dbgProdutosPedidos.ColumnByName('ValorGrade1').Visible := ParSistema.UsarGradesProdutos;
  dbgProdutosPedidos.ColumnByName('ValorGrade2').Visible := ParSistema.UsarGradesProdutos;

  if dbgProdutosPedidos.ColumnByName('quantidade').Width < ParSistema.TamanhoMascaraQuantidade then
    dbgProdutosPedidos.ColumnByName('quantidade').Width := ParSistema.TamanhoMascaraQuantidade;

  if dbgProdutosPedidos.ColumnByName('TotalRecebido').Width < ParSistema.TamanhoMascaraQuantidade then
    dbgProdutosPedidos.ColumnByName('TotalRecebido').Width := ParSistema.TamanhoMascaraQuantidade;
  ...
  if not ParSistema.UsarGradesProdutos then
    dbgProdutosSimilares.ColumnByName('descricao').Width := dbgProdutosSimilares.ColumnByName('descricao').Width +
                                             dbgProdutosSimilares.ColumnByName('ValorGrade1').Width+
                                             dbgProdutosSimilares.ColumnByName('ValorGrade2').Width;

  with dtmCadastroPedidos do
  begin
    AcertarLarguraGradeCasasDecimais(dbgProdutosPedidos, 10);
...
begin
  ...
  if dbgProdutosPedidos.SelectedField = dtmCadastroPedidos.qryProdutosPedidospermitiratualizardigitado then
    dtmCadastroPedidos.PermitirAtualizarDigitado := not dtmCadastroPedidos.PermitirAtualizarDigitado
  else
  begin

//    if not dtmCadastroPedidos.ProdutosPedidoReadOnly then
    begin
      if dtmCadastroPedidos.qryProdutosPedidos.IsEmpty then
      begin
...
begin
  if Shift = [ssCtrl] then
    case Key of
      TeclaEditarRegistro : if sbnIncluirProduto.Enabled {and not dtmCadastroPedidos.ProdutosPedidoReadOnly} then
                               AcionaCadastroProdutosPedidos(True);
      TeclaInserirRegistro: if sbnIncluirProduto.Enabled {and not dtmCadastroPedidos.ProdutosPedidoReadOnly }then
                               AcionaCadastroProdutosPedidos(False);
    end;
end;
...
begin
   ...
   {
   if assigned(TfrmCadastroPedidosDestroy) then
     TfrmCadastroPedidosDestroy(self);
     }
...
begin
  ...
  frmEnviarEmail :=
      TfrmEnviarEmail.Create(frmEnviarEmail,
                             ...
                             Contato,
                             'Pedido de Compra',
                             'Pedido ' +dtmCadastroPedidos.qryPedidosnumero.asString,
                             PedidosEmail
                             );
  try
...
begin
{
  ...
  dbgProdutosPedidos.ColumnByName('ValorGrade1').Title.Caption := dtmCadastroPedidos.LinhadaGrade;
  dbgProdutosPedidos.ColumnByName('ValorGrade2').Title.Caption := dtmCadastroPedidos.colunadaGrade;
end;
...
begin
  ...
  begin
    dtmCadastroPedidos.CalcularCusto;
    dtmCadastroPedidos.CalcularPrecoVenda;
//    dtmCadastroPedidos.IncluirPrecosNaoCadastrados;
...
begin
  Result := False;
    frmCadastroPedidosGrade:= TfrmCadastroPedidosGrade.Create(self);
...
begin
  ...
  begin
    ...
    if qryProdutosPedidosSituacao.AsString = 'A' then
    begin
      TDBGrid(Sender).Canvas.Brush.Color := clWhite;
      TDBGrid(Sender).Canvas.Font.Color  := clBlack;
    end
    else
    if qryProdutosPedidosSituacao.AsString = 'P' then
    begin
       TDBGrid(Sender).Canvas.Brush.Color := clAqua;
       TDBGrid(Sender).Canvas.Font.Color  := clBlack;
    end
    else
    if qryProdutosPedidosSituacao.AsString = 'L' then
    begin
      if qryProdutosPedidosquantidade.asCurrency <
         qryProdutosPedidostotalrecebido.asCurrency then
      begin
//        TDBGrid(Sender).Canvas.Brush.Color := $00CACAFF;
//        TDBGrid(Sender).Canvas.Font.Color  := clGreen;
    ...
    if qryProdutosPedidosSituacao.AsString = 'C' then
    begin
       TDBGrid(Sender).Canvas.Brush.Color := clYellow;
       TDBGrid(Sender).Canvas.Font.Color  := clBlack;
    end
    else
    if qryProdutosPedidosSituacao.AsString = 'Q' then
    begin
    ...
    if (column.FieldName =
       dtmCadastroPedidos.qryProdutosPedidosprecodigitado.FieldName) then
     if qryProdutosPedidosprecodigitadoalterado.AsBoolean then
       TDBGrid(Sender).Canvas.Font.Color:= clRed;
    ...
    begin
     ...
     if (Column.FieldName =
         dtmCadastroPedidos.qryProdutosPedidospercAtual_x_Venda.FieldName) then
       if truncar(dtmCadastroPedidos.qryProdutosPedidospercAtual_x_Venda.AsFloat,2) <> 0 then
         TDBGrid(Sender).Canvas.Font.Color:= clRed;

     if (Column.FieldName =
         dtmCadastroPedidos.qryProdutosPedidospercVenda_x_Digitado.FieldName) then
       if truncar(dtmCadastroPedidos.qryProdutosPedidospercVenda_x_Digitado.AsFloat,2) <> 0 then
         TDBGrid(Sender).Canvas.Font.Color:= clRed;
    end;

     if (Column.FieldName = dtmCadastroPedidos.qryProdutosPedidosnumero.FieldName) then
     begin
       if TDBGrid(Sender).Canvas.brush.Color = TColor(strtoint(parsistema.CorZebradoRelatorio)) then
         TDBGrid(Sender).Canvas.brush.Color := clWindow;
       if TDBGrid(Sender).Canvas.Font.style = [fsBold] then
         TDBGrid(Sender).Canvas.Font.style := [];

       if pgcPedidos.activepage = tstNotasPendentesdeEntrada then
         if pgcProdutos.activepage = tstProdutos then
           if dtmCadastroPedidos.qryProdutosPedidosproduto.asstring =
              dtmCadastroPedidos.qryConsultaProdutosEntradaSefazproduto.asString then
           begin
...
begin
  if not assigned(frmCopiarProdutosPedidos)  then
    frmCopiarProdutosPedidos := TfrmCopiarProdutosPedidos.Create(self);
...
(*
function AcionarTelaCadastroPedidos_(AOwner: TComponent): TfrmCadastroPedidos;
var
 i: integer;
begin


  TfrmPrincipalBasico(Application.MainForm).MostrarFormRegistrado(['Incluir'], 'TfrmCadastroPedidos', True);
  result := frmCadastroPedidos;
  frmCadastroPedidos.BringToFront;

  (*
//  Application.CreateForm(TfrmCadastroPedidos, frmCadastroPedidos);

  frmCadastroPedidos := TfrmCadastroPedidos.create(AOwner);

  with frmCadastroPedidos do
  begin
    edfPedido.datasource := frmCadastroPedidos.dtmCadastroPedidos.dsrPedidos;
    edtEmissao.datasource := frmCadastroPedidos.dtmCadastroPedidos.dsrPedidos;
    dtxConcluido.datasource := frmCadastroPedidos.dtmCadastroPedidos.dsrPedidos;
    dtxSituacao.datasource := frmCadastroPedidos.dtmCadastroPedidos.dsrPedidos;
    dtxCancelado.datasource := frmCadastroPedidos.dtmCadastroPedidos.dsrPedidos;
...
{
initialization
   AcionarTelaCadastroPedidos :=  AcionarTelaCadastroPedidos_;
   }

procedure TfrmCadastroPedidos.sbnAcionarTelaFormacaoPrecoVendaClick(
  Sender: TObject);
begin
  inherited;
  if not (dtmCadastroPedidos.qryPedidos.state in [dsedit, dsinsert]) then
     AcionarTelaFormacaoPrecoVenda(dtmCadastroPedidos.qryProdutosPedidospedido.asInteger,
                                 dtmCadastroPedidos.qryProdutosPedidosnumero.asInteger,
                                 dtmCadastroPedidos.qryProdutosPedidosproduto.asString,
                                 'Pedidos')
  else
    MensagemAviso('Para visualizar a formação do preço de venda grave o pedido');
end;

procedure TfrmCadastroPedidos.sbnCustoePrecoVendaClick(Sender: TObject);
var
  vAux_VisualizarDetalhesProdutosNota : Boolean;
begin
  inherited;
{  if Not dtmCadastroPedidos.SomenteLeitura then}
//  if (dtmCadastroPedidos.qryProdutosPedidossituacao.asString = 'P') or
//     (dtmCadastroPedidos.qryProdutosPedidossituacao.asString = 'A') then
  begin
    vAux_VisualizarDetalhesProdutosNota := dtmCadastroPedidos.VisualizarDetalhesProdutosNota;
    visulizandoCustoePrecoVenda := not visulizandoCustoePrecoVenda;
    if visulizandoCustoePrecoVenda then
    begin
      if vAux_VisualizarDetalhesProdutosNota then
        dtmCadastroPedidos.VisualizarDetalhesProdutosNota := false;

      if dtmCadastroPedidos.CalcularCusto then
        dtmCadastroPedidos.CalcularPrecoVenda;

      if vAux_VisualizarDetalhesProdutosNota then
        dtmCadastroPedidos.VisualizarDetalhesProdutosNota := true;
    end;
    VisualizarPaineisCusto;
  end;
end;

procedure TfrmCadastroPedidos.VisualizarPaineisCusto;
begin
  lblAlterados.Visible := not lblAlterados.Visible;
  shAlterados.Visible := lblAlterados.Visible;
  
//  dbgProdutosPedidos.Columnbyname('precounitariopedido').Visible :=
//    not dbgProdutosPedidos.Columnbyname('precounitariopedido').Visible;

//  dbgProdutosPedidos.Columnbyname('quantidade').Visible :=
//    not dbgProdutosPedidos.Columnbyname('quantidade').Visible;

// ? se retirar a especificação do tamanho abaixo o tamanho da coluna fica estranhamente expandido
//  dbgProdutosPedidos.Columnbyname('unidadenf').width := 30;

//  dbgProdutosPedidos.Columnbyname('produtototal').Visible :=
//    not dbgProdutosPedidos.Columnbyname('produtototal').Visible;

//  dbgProdutosPedidos.Columnbyname('aliquotaicms').Visible :=
//    not dbgProdutosPedidos.Columnbyname('aliquotaicms').Visible;

//  dbgProdutosPedidos.Columnbyname('aliquotaipi').Visible :=
//    not dbgProdutosPedidos.Columnbyname('aliquotaipi').Visible;

//  dbgProdutosPedidos.Columnbyname('valoripi').Visible :=
//    not dbgProdutosPedidos.Columnbyname('valoripi').Visible;

  dbgProdutosPedidos.Columnbyname('custoparavenda').Visible :=
    not dbgProdutosPedidos.Columnbyname('custoparavenda').Visible;

  dbgProdutosPedidos.Columnbyname('precoatual').Visible :=
    not dbgProdutosPedidos.Columnbyname('precoatual').Visible;

  dbgProdutosPedidos.Columnbyname('percAtual_x_Venda').Visible :=
    not dbgProdutosPedidos.Columnbyname('percAtual_x_Venda').Visible;

  dbgProdutosPedidos.Columnbyname('percAtual_x_Venda').width := 50;

  dbgProdutosPedidos.Columnbyname('precovenda').Visible :=
    not dbgProdutosPedidos.Columnbyname('precovenda').Visible;

  dbgProdutosPedidos.Columnbyname('percVenda_x_Digitado').Visible :=
    not dbgProdutosPedidos.Columnbyname('percVenda_x_Digitado').Visible;

  dbgProdutosPedidos.Columnbyname('percVenda_x_Digitado').width := 50;

  dbgProdutosPedidos.Columnbyname('precodigitado').Visible :=
    not dbgProdutosPedidos.Columnbyname('precodigitado').Visible;

  dbgProdutosPedidos.Columnbyname('permitiratualizardigitado').Visible :=
    not dbgProdutosPedidos.Columnbyname('permitiratualizardigitado').Visible;

  dbgProdutosPedidos.Columnbyname('codigofiscal').Visible :=
    not dbgProdutosPedidos.Columnbyname('codigofiscal').Visible;

//  dbgProdutosPedidos.Columnbyname('valoricms').Visible :=
//    not dbgProdutosPedidos.Columnbyname('valoricms').Visible;

  dbgProdutosPedidos.Columnbyname('icmsoutras').Visible :=
    not dbgProdutosPedidos.Columnbyname('icmsoutras').Visible;

  dbgProdutosPedidos.Columnbyname('icmsisentas').Visible :=
    not dbgProdutosPedidos.Columnbyname('icmsisentas').Visible;

//  dbgProdutosPedidos.Columnbyname('codigopreco').Visible :=
//    not dbgProdutosPedidos.Columnbyname('codigopreco').Visible;

//  dbgProdutosPedidos.Columnbyname('descricaoprecoproduto').Visible :=
//    not dbgProdutosPedidos.Columnbyname('descricaoprecoproduto').Visible;

end;

procedure TfrmCadastroPedidos.ckbMarcarTodosClick(Sender: TObject);
begin
  inherited;
  dtmCadastroPedidos.MarcarDesmarcarAtualizarPrecodeVenda(ckbMarcarTodos.Checked);
end;

procedure TfrmCadastroPedidos.pgcPedidosChange(Sender: TObject);
begin
  inherited;
  if pgcPedidos.ActivePage = tstNotasPendentesdeEntrada then
    dtmCadastroPedidos.ConsultaProdutosEntradaSefaz;
end;

procedure TfrmCadastroPedidos.Criar_frmCadastroProdutosPedidos_;
begin
  if not assigned(frmCadastroProdutosPedidos_)  then
  begin
    frmCadastroProdutosPedidos_ := TfrmCadastroProdutosPedidos_.Create(self);
...
procedure TfrmCadastroPedidos.sbnAtendimentoClick(Sender: TObject);
begin
  inherited;
  if dtmCadastroPedidos.qryPedidosfornecedor.asinteger <> 0 then
    TfrmPrincipalBasico(Application.MainForm).MostrarFormRegistrado(['IncluirAtendimento',
                                                                       dtmCadastroPedidos.qryPedidosfornecedor.asinteger,
                                                                       'F',
                                                                       'V'   //cobrança
