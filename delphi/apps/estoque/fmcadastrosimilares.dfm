inherited frmCadastroSimilares: TfrmCadastroSimilares
  Left = 943
  Top = 240
  Caption = 'Cadastro de Produtos Similares'
  ClientHeight = 138
  ClientWidth = 442
  PixelsPerInch = 96
  TextHeight = 16
  inherited pnlBarra: TPanel
    Left = 200
    Top = 50
    Width = 20
    Height = 20
    inherited bvlBotoesEd: TBevel
      Left = 201
    end
  end
  inherited pnlTopMenu: TPanel
    Width = 442
    TabOrder = 2
    inherited tblBarra: TToolBar
      Width = 217
      inherited sbnProcurar: TSpeedButton
        Visible = False
      end
    end
  end
  object pnlFundoJanela: TPanel
    Left = 0
    Top = 45
    Width = 442
    Height = 93
    Align = alClient
    BevelOuter = bvNone
    BorderWidth = 3
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -12
    Font.Name = 'helvetica'
    Font.Pitch = fpVariable
    Font.Style = []
    ParentFont = False
    TabOrder = 1
    object gbxCodProduto: TGroupBox
      Left = 6
      Top = 10
      Width = 125
      Height = 36
      Caption = 'PRODUTO'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -9
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      object dtxCaracteristica: TtecDBText
        Left = 2
        Top = 11
        Width = 120
        Height = 23
        TabStop = False
        Color = clBtnFace
        DataField = 'codigovisual'
        DataSource = dtmCadastroProdutos.dsrCaracteristicas
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        ReadOnly = True
        TabOrder = 0
        Alignment = taLeftJustify
      end
    end
    object gbxSimilar: TGroupBox
      Left = 6
      Top = 56
      Width = 428
      Height = 36
      Caption = 'SIMILAR'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -9
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      object sbnProcurarSimilar: TSpeedButton
        Left = 122
        Top = 10
        Width = 23
        Height = 23
        Hint = 'Procurar Similar'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        Glyph.Data = {
          
          C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3C3}
        NumGlyphs = 2
        ParentFont = False
        OnClick = sbnProcurarSimilarClick
      end
      object flkSimilar: TtecDBFindLookup
        Left = 2
        Top = 10
        Width = 120
        Height = 23
        Alignment = taLeftJustify
        DataaFieldInterno = 'similar'
        DataaFieldVisual = 'similarvisual'
        DataField = 'similarvisual'
        DataSource = dtmCadastroProdutos.dsrSimilares
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        MaxLength = 16
        Maximo = 0
        Minimo = 37353
        Adicional = 0
        Opcional = True
        OnMessage = flkSimilarMessage
        ParentFont = False
        TabOrder = 0
        PermitirZero = False
        PermitirNulo = False
        Operacao = opATRIBUICAO
        LookupaFieldinterno = 'codigo'
        LookupaFieldVisual = 'codigovisual'
        LookupField = 'codigovisual'
        LookupSource = dtmCadastroProdutos.dsrProcuraSimilares
        LookupQueryParameter = 'codigovisual'
        Parameter = ' '
        ActiveSetControls = False
        DenyInsert = True
        NoSetControls = <>
        SetControls = <>
        LookupParameter = 'codigovisual'
      end
      object dtxSimilar: TtecDBText
        Left = 145
        Top = 10
        Width = 280
        Height = 23
        TabStop = False
        Color = clBtnFace
        DataField = 'descricao'
        DataSource = dtmCadastroProdutos.dsrProcuraSimilares
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        ReadOnly = True
        TabOrder = 1
        Alignment = taLeftJustify
      end
    end
  end
end
