...
type
  ...
  TfrmLancamentosManuais = class(TfrmCadastroPadrao)
    ...
    pnlCenter: TPanel;
    GroupBoxQtadeLotes: TGroupBox;
    dbgExisteEstoqueLotes: TtecDBGrid;
    Splitter1: TSplitter;
    Splitter2: TSplitter;
    dtxprodutovisual: TDBEditTexto;
    dtxnrlote: TDBEditTexto;
...
uses dmlancamentosmanuais, cpquery;
...
begin
  for a:= ComponentCount - 1 downto 0 do begin
    ...
    if (Components[a] is TfraConsultaProduto) OR
       (Components[a] is TtecFindCustom)  or
...
begin
  Result := dtmLancamentosManuais.qryMovimentos.CheckRequiredFields;
