inherited frmRelatorioCurvaABC: TfrmRelatorioCurvaABC
  Left = 226
  Top = 186
  ActiveControl = edtDataInicial
  Caption = 'Curva ABC'
  ClientHeight = 473
  ClientWidth = 760
  FormStyle = fsMDIChild
  Visible = True
  PixelsPerInch = 96
  TextHeight = 16
  inherited pnlBarra: TToolBar
    Width = 760
    inherited sbnAjuda: TSpeedButton
      Hint = ''
      OnClick = nil
    end
    object sbnSalvar: TSpeedButton
      Left = 79
      Top = 4
      Width = 75
      Height = 35
      Hint = 'Grava a Altera'#231#227'o ou Inclus'#227'o na Tabela'
      AllowAllUp = True
      Anchors = [akTop, akRight]
      Constraints.MaxHeight = 35
      Constraints.MinHeight = 35
      Caption = 'Gravar F5'
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -12
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      Glyph.Data = {
        
        7F7F7F7F7F7F7F7F7F7F7F7F7F7F7F7F7F7F7F7FC3C3C3C3C3C3}
      Layout = blGlyphTop
      NumGlyphs = 2
      ParentFont = False
      Spacing = 0
      OnClick = sbnSalvarClick
    end
    object sbnGerar: TSpeedButton
      Left = 154
      Top = 2
      Width = 75
      Height = 39
      Hint = 'Gerar Consulta de compras'
      Caption = 'Gerar F6'
      Flat = True
      Glyph.Data = {
        
        DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
      Layout = blGlyphTop
      NumGlyphs = 2
      Spacing = 0
      OnClick = sbnGerarClick
    end
  end
  inherited pnlFundoJanela: TPanel
    Top = 40
    Width = 760
    Height = 433
    Align = alClient
    object pgcParametros: TPageControl
      Left = 3
      Top = 3
      Width = 754
      Height = 427
      ActivePage = tstParametros
      Align = alClient
      TabOrder = 0
      object tstParametros: TTabSheet
        Caption = 'Par'#226'metros'
        object gbxPercentuaisABC: TGroupBox
          Left = 0
          Top = 88
          Width = 249
          Height = 54
          Caption = 'Percentuais Curva ABC'
          TabOrder = 4
          object lblpercentualA: TLabel
            Left = 8
            Top = 24
            Width = 9
            Height = 16
            Caption = 'A'
          end
          object lblpercentualB: TLabel
            Left = 88
            Top = 24
            Width = 9
            Height = 16
            Caption = 'B'
          end
          object lblpercentualC: TLabel
            Left = 168
            Top = 24
            Width = 9
            Height = 16
            Caption = 'C'
          end
          object edtpercentualA: TEditNumero
            Left = 24
            Top = 20
            Width = 57
            Height = 24
            MaxLength = -1
            TabOrder = 1
            Text = '20'
            Mascara = True
            Alignment = taLeftJustify
            TipoMascara = tmGERAL
            NrDecimal = 0
            Decimais = False
            Negativo = False
            Tamanho = 5
          end
          object edtpercentualB: TEditNumero
            Left = 104
            Top = 20
            Width = 57
            Height = 24
            MaxLength = -1
            TabOrder = 0
            Text = '30'
            Mascara = True
            Alignment = taLeftJustify
            TipoMascara = tmGERAL
            NrDecimal = 0
            Decimais = False
            Negativo = False
            Tamanho = 5
          end
          object edtpercentualC: TEditNumero
            Left = 184
            Top = 20
            Width = 57
            Height = 24
            MaxLength = -1
            TabOrder = 2
            Text = '50'
            Mascara = True
            Alignment = taLeftJustify
            TipoMascara = tmGERAL
            NrDecimal = 0
            Decimais = False
            Negativo = False
            Tamanho = 5
          end
        end
        object gbxProdutos: TGroupBox
          Left = 496
          Top = 0
          Width = 249
          Height = 141
          Caption = 'Produtos'
          TabOrder = 3
          object ckbProdutoEmLinha: TCheckBox
            Left = 8
            Top = 18
            Width = 97
            Height = 20
            Caption = '&Em Linha'
            Checked = True
            State = cbChecked
            TabOrder = 0
            OnClick = ckbProdutoEmLinhaClick
          end
          object ckbProdutoForadeLinha: TCheckBox
            Left = 126
            Top = 18
            Width = 113
            Height = 20
            Caption = '&Fora de Linha'
            TabOrder = 1
            OnClick = ckbProdutoForadeLinhaClick
          end
          object ckbProdutoComMontagem: TCheckBox
            Left = 8
            Top = 78
            Width = 113
            Height = 20
            Caption = '&Com Montagem'
            Checked = True
            State = cbChecked
            TabOrder = 4
            OnClick = ckbProdutoComMontagemClick
          end
          object ckbProdutoSemMontagem: TCheckBox
            Left = 126
            Top = 78
            Width = 113
            Height = 20
            Caption = '&Sem Montagem'
            Checked = True
            State = cbChecked
            TabOrder = 5
            OnClick = ckbProdutoSemMontagemClick
          end
          object ckbProdutoNaoBrinde: TCheckBox
            Left = 126
            Top = 108
            Width = 89
            Height = 20
            Caption = '&N'#227'o Brinde'
            Checked = True
            State = cbChecked
            TabOrder = 7
            OnClick = ckbProdutoNaoBrindeClick
          end
          object ckbProdutoBrinde: TCheckBox
            Left = 8
            Top = 108
            Width = 89
            Height = 20
            Caption = '&Brinde'
            Checked = True
            State = cbChecked
            TabOrder = 6
            OnClick = ckbProdutoBrindeClick
          end
          object ckbProdutoAtivo: TCheckBox
            Left = 8
            Top = 48
            Width = 97
            Height = 20
            Caption = '&Ativo'
            Checked = True
            State = cbChecked
            TabOrder = 2
            OnClick = ckbProdutoAtivoClick
          end
          object ckbProdutoInativo: TCheckBox
            Left = 126
            Top = 48
            Width = 113
            Height = 20
            Caption = '&Inativo'
            TabOrder = 3
            OnClick = ckbProdutoInativoClick
          end
        end
        object gbxPeriodo: TGroupBox
          Left = 0
          Top = 0
          Width = 140
          Height = 78
          Caption = 'Per'#237'odo'
          TabOrder = 1
          object lblDataInicial: TLabel
            Left = 8
            Top = 22
            Width = 33
            Height = 16
            Alignment = taRightJustify
            Caption = 'Inicial'
          end
          object lblDataFinal: TLabel
            Left = 13
            Top = 49
            Width = 28
            Height = 16
            Alignment = taRightJustify
            Caption = 'Final'
          end
          object edtDataInicial: TEditData
            Left = 49
            Top = 18
            Width = 80
            Height = 24
            MaxLength = 10
            TabOrder = 0
            Minimo = 37353
            Maximo = 0
          end
          object edtDataFinal: TEditData
            Left = 49
            Top = 45
            Width = 80
            Height = 24
            MaxLength = 10
            TabOrder = 1
            Minimo = 37353
            Maximo = 0
          end
        end
        object rbnAnalisar: TRadioGroup
          Left = 144
          Top = 0
          Width = 105
          Height = 77
          Caption = 'Analisar'
          ItemIndex = 0
          Items.Strings = (
            'Vendas'
            'Compras')
          TabOrder = 0
          Visible = False
        end
        object rbnCurvaABC: TRadioGroup
          Left = 264
          Top = 0
          Width = 225
          Height = 141
          Caption = 'Curva ABC'
          ItemIndex = 0
          Items.Strings = (
            'Lucro por Produto'
            'Quantidade'
            'Faturamento'
            'Fornecedor'
            'Lucros por Grupo/Classe/Marca'
            'Lucros por Marca')
          TabOrder = 2
        end
        object gbxQuantidade: TGroupBox
          Left = 123
          Top = 147
          Width = 67
          Height = 69
          Caption = 'Qtde'
          TabOrder = 6
          object edtQuantidade: TEditNumero
            Left = 8
            Top = 28
            Width = 49
            Height = 24
            MaxLength = -1
            TabOrder = 0
            Text = '100'
            Mascara = False
            Alignment = taLeftJustify
            TipoMascara = tmGERAL
            NrDecimal = 0
            Decimais = False
            Negativo = False
            Tamanho = 5
          end
        end
        object rbnAnalizarProdutos: TRadioGroup
          Left = 0
          Top = 147
          Width = 121
          Height = 69
          Caption = 'Analisar'
          ItemIndex = 0
          Items.Strings = (
            '&Todos'
            '&Especificados')
          TabOrder = 5
        end
        inline fraListaFiliais1: TfraListaFiliais
          Left = 1
          Top = 229
          Width = 326
          Height = 146
          TabOrder = 8
          inherited gbxFiliais: TGroupBox
            Caption = '&Filiais de Venda'
          end
        end
        inline fraListaGruposFiliais1: TfraListaGruposFiliais
          Left = 335
          Top = 229
          Width = 326
          Height = 146
          TabOrder = 7
          inherited gbxGrupodeFiliais: TGroupBox
            Caption = 'Gru&po de Filiais de Venda'
          end
        end
        inline fraConsultaIPPGCM: TfraConsultaIPPGCM
          Left = 196
          Top = 153
          Width = 542
          Height = 58
          VertScrollBar.Range = 57
          AutoScroll = False
          TabOrder = 9
          inherited pgcProdutos: TtecPageControl
            ActivePage = fraConsultaIPPGCM.tstMarcas
            inherited tstItemProduto: TTabSheet
              inherited fraConsultaItemProduto: TfraConsultaCodigo
                Font.Pitch = fpVariable
                inherited edfCodigo: TtecDBFindLookup
                  Parameter = ' '
                  ActiveSetControls = True
                  DenyInsert = True
                  NoSetControls = <>
                  SetControls = <>
                end
              end
            end
            inherited tstProdutos: TTabSheet
              inherited fraConsultaProduto: TfraConsultaCodigo
                Font.Pitch = fpVariable
                inherited edfCodigo: TtecDBFindLookup
                  Parameter = ' '
                  ActiveSetControls = False
                  DenyInsert = True
                  NoSetControls = <>
                  SetControls = <>
                end
              end
            end
            inherited tstGrupodeProdutos: TTabSheet
              inherited fraConsultaGrupo: TfraConsultaCodigo
                Font.Pitch = fpVariable
                inherited edfCodigo: TtecDBFindLookup
                  Parameter = 'codigo'
                  ActiveSetControls = False
                  DenyInsert = True
                  NoSetControls = <>
                  SetControls = <>
                end
              end
            end
            inherited tstClasses: TTabSheet
              inherited fraConsultaClasse: TfraConsultaCodigo
                Font.Pitch = fpVariable
                inherited edfCodigo: TtecDBFindLookup
                  Parameter = 'codigo'
                  ActiveSetControls = False
                  DenyInsert = True
                  NoSetControls = <>
                  SetControls = <>
                end
              end
            end
            inherited tstMarcas: TTabSheet
              inherited fraConsultaMarcas: TfraConsultaCodigo
                Font.Pitch = fpVariable
                inherited edfCodigo: TtecDBFindLookup
                  Parameter = 'codigo'
                  ActiveSetControls = False
                  DenyInsert = True
                  NoSetControls = <>
                  SetControls = <>
                end
              end
            end
          end
        end
      end
      object tstDados: TTabSheet
        Caption = 'Dados'
        ImageIndex = 1
        object dbgCurvaABC: TtecDBGrid
          Left = 0
          Top = 0
          Width = 746
          Height = 396
          Align = alClient
          DataSource = dtmRelatorioCurvaABC.dsrVendas
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
          ParentFont = False
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clBlack
          TitleFont.Height = -13
          TitleFont.Name = 'helvetica'
          TitleFont.Pitch = fpVariable
          TitleFont.Style = []
          RowWrap = 60
          Large = False
          DoubleRowColor = True
          TitleMinHeight = 100
          CellHeights = 100
          StrippedColor = 16054260
          CanDelete = False
          CanNotInsertFromGrid = False
          PostOnEnter = False
          PostOnSetUpDown = False
          DenySort = False
          DefaultRowHeight = 18
          ExibirNumerodaLinha = False
          NaoAtribuirDadosaTabelaaoDigitar = False
          ShowWhenFieldInvisible = False
          CanClickWhenEditing = False
        end
      end
    end
  end
  object ecvValida: TtecEditionControlValidation
    EditionControl = <
      item
        Control = edtDataFinal
      end
      item
        Control = edtDataInicial
      end
      item
        Control = edtQuantidade
      end
      item
        Control = edtpercentualA
      end
      item
        Control = edtpercentualB
      end
      item
        Control = edtpercentualC
      end>
    Left = 560
    Top = 8
  end
  object svdExportarExcel: TSaveDialog
    DefaultExt = 'xls'
    Filter = '*.xls'
    Title = 'Exportar Relat'#243'rio Curva ABC para Excel'
    Left = 592
    Top = 8
  end
end
