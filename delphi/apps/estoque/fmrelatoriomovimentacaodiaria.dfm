inherited frmRelatorioMovimentacaoDiaria: TfrmRelatorioMovimentacaoDiaria
  Left = 291
  Top = 206
  ActiveControl = edtDataInicial
  Caption = 'Relat'#243'rio de Movimenta'#231#227'o Di'#225'ria'
  ClientHeight = 479
  ClientWidth = 959
  FormStyle = fsMDIChild
  Visible = True
  PixelsPerInch = 96
  TextHeight = 16
  inherited pnlBarra: TToolBar
    Width = 959
  end
  inherited pnlFundoJanela: TPanel
    Top = 42
    Width = 959
    Height = 437
    object gbxMovimentacaoDiaria: TGroupBox
      Left = 3
      Top = 3
      Width = 953
      Height = 431
      Align = alClient
      TabOrder = 0
      object gbxPeriodo: TGroupBox
        Left = 6
        Top = 250
        Width = 172
        Height = 39
        Caption = 'PER'#205'ODO DO LAN'#199'AMENTO'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        object lblA: TLabel
          Left = 81
          Top = 21
          Width = 7
          Height = 15
          Caption = 'a'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
        end
        object edtDataInicial: TEditData
          Left = 2
          Top = 14
          Width = 75
          Height = 23
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = 10
          ParentFont = False
          TabOrder = 0
          Minimo = 37353
          Maximo = 0
        end
        object edtDataFinal: TEditData
          Left = 92
          Top = 14
          Width = 75
          Height = 23
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = 10
          ParentFont = False
          TabOrder = 1
          Minimo = 37353
          Maximo = 0
        end
      end
      object pgcFilialGrupoFilial: TtecPageControl
        Left = 6
        Top = 16
        Width = 202
        Height = 230
        ActivePage = tstGrupoFilial
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 5
        TabWidth = 90
        object tstFilial: TTabSheet
          Caption = '  Filia&l'
          object sbnMarcarFiliais: TSpeedButton
            Left = 168
            Top = 2
            Width = 22
            Height = 22
            Hint = 'Marcar Filiais'
            Flat = True
            Glyph.Data = {
              
              DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
            NumGlyphs = 2
            OnClick = sbnMarcarFiliaisClick
          end
          object sbnDesmarcarFiliais: TSpeedButton
            Left = 168
            Top = 26
            Width = 22
            Height = 22
            Hint = 'Desmarcar Filiais'
            Flat = True
            Glyph.Data = {
              
              DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
            NumGlyphs = 2
            OnClick = sbnDesmarcarFiliaisClick
          end
          object clbFiliais: TCheckListBox
            Left = 0
            Top = 0
            Width = 166
            Height = 200
            Align = alLeft
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            ItemHeight = 15
            ParentFont = False
            TabOrder = 0
          end
        end
        object tstGrupoFilial: TTabSheet
          Caption = '  &Grupo Filial'
          ImageIndex = 1
          object sbnmarcarGrupodeFiliais: TSpeedButton
            Left = 168
            Top = 2
            Width = 22
            Height = 22
            Hint = 'Marcar Grupo de Filiais'
            Flat = True
            Glyph.Data = {
              
              DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
            NumGlyphs = 2
            OnClick = sbnmarcarGrupodeFiliaisClick
          end
          object sbnDesmarcarGrupodeFiliais: TSpeedButton
            Left = 168
            Top = 26
            Width = 22
            Height = 22
            Hint = 'Desmarcar Grupo de Filiais'
            Flat = True
            Glyph.Data = {
              
              DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
            NumGlyphs = 2
            OnClick = sbnDesmarcarGrupodeFiliaisClick
          end
          object clbGrupodeFiliais: TCheckListBox
            Left = 0
            Top = 0
            Width = 161
            Height = 200
            Align = alLeft
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            ItemHeight = 15
            ParentFont = False
            TabOrder = 0
          end
        end
      end
      object rgpResumo: TRadioGroup
        Left = 447
        Top = 256
        Width = 120
        Height = 88
        Caption = 'Resumo'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ItemIndex = 2
        Items.Strings = (
          '&Com resumo'
          '&Sem resumo'
          'S'#243' &resumo')
        ParentFont = False
        TabOrder = 2
      end
      object gbxAgrupamento: TGroupBox
        Left = 735
        Top = 257
        Width = 131
        Height = 138
        Caption = 'AGRUPAMENTO'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 4
        object ckbAgruparGrupoFilial: TCheckBox
          Left = 6
          Top = 15
          Width = 113
          Height = 20
          Caption = 'Grupo de Filiais'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 0
        end
        object ckbAgruparFilial: TCheckBox
          Left = 6
          Top = 35
          Width = 113
          Height = 21
          Caption = 'Filiais'
          Checked = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          State = cbChecked
          TabOrder = 1
        end
        object ckbAgruparProduto: TCheckBox
          Left = 6
          Top = 115
          Width = 113
          Height = 20
          Caption = 'Produto'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 2
        end
        object ckbAgruparMovimento: TCheckBox
          Left = 6
          Top = 95
          Width = 113
          Height = 20
          Caption = 'Movimento'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 3
        end
        object ckbAgruparData: TCheckBox
          Left = 6
          Top = 55
          Width = 113
          Height = 20
          Caption = 'Data'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 4
        end
        object ckbAgruparbGrupoProduto: TCheckBox
          Left = 6
          Top = 75
          Width = 113
          Height = 20
          Caption = 'Grupos estoque'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 5
        end
      end
      object ckbSaldoInicial: TCheckBox
        Left = 8
        Top = 349
        Width = 123
        Height = 21
        Caption = 'Com saldo inicial'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 6
      end
      object rgbOrdenacao: TRadioGroup
        Left = 576
        Top = 256
        Width = 151
        Height = 113
        Caption = 'Ordena'#231#227'o'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ItemIndex = 2
        Items.Strings = (
          'C'#243'digo Produto'
          'Descri'#231#227'o Produto'
          'Movimento'
          'Localiza'#231#227'o')
        ParentFont = False
        TabOrder = 3
      end
      object gbxMovimentos: TGroupBox
        Left = 214
        Top = 12
        Width = 331
        Height = 231
        Caption = 'TIPOS DE MOVIMENTO'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 7
        object Bevel1: TBevel
          Left = 2
          Top = 14
          Width = 327
          Height = 6
          Align = alTop
          Shape = bsSpacer
        end
        object clbMovimentos: TCheckListBox
          Left = 2
          Top = 20
          Width = 298
          Height = 209
          Align = alClient
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ItemHeight = 15
          ParentFont = False
          TabOrder = 0
        end
        object pnldireito: TPanel
          Left = 300
          Top = 20
          Width = 29
          Height = 209
          Align = alRight
          BevelOuter = bvNone
          TabOrder = 1
          object sbnDesmarcarMovimentos: TSpeedButton
            Left = 4
            Top = 30
            Width = 22
            Height = 22
            Hint = 'Desmarcar Movimentos'
            Flat = True
            Glyph.Data = {
              
              DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
            NumGlyphs = 2
            OnClick = sbnDesmarcarMovimentosClick
          end
          object sbnMarcarMovimentos: TSpeedButton
            Left = 4
            Top = 6
            Width = 22
            Height = 22
            Hint = 'Marcar Movimentos'
            Flat = True
            Glyph.Data = {
              
              DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
            NumGlyphs = 2
            OnClick = sbnMarcarMovimentosClick
          end
        end
      end
      inline fraListaGrupoProdutos: TfraListaGrupoProdutos
        Left = 550
        Top = 13
        Width = 396
        Height = 230
        TabOrder = 8
        inherited gbx: TGroupBox
          Height = 230
          Caption = '&GRUPO DE PRODUTOS'
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          ParentFont = False
          inherited Bevel1: TBevel
            Top = 14
          end
          inherited clbgrupoprodutos: TCheckListBox
            Top = 20
            Height = 208
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            ParentFont = False
          end
          inherited pnldireito: TPanel
            Top = 20
            Height = 208
          end
        end
      end
      object ckbNomedoPreco: TCheckBox
        Left = 8
        Top = 373
        Width = 123
        Height = 21
        Caption = 'Nome do pre'#231'o'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 9
      end
      object rgpTipodeMovimentacao: TRadioGroup
        Left = 215
        Top = 256
        Width = 226
        Height = 88
        Caption = 'Tipos de Movimenta'#231#245'es'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ItemIndex = 0
        Items.Strings = (
          'Todas as Movimenta'#231#245'es'
          'Somente Movimenta'#231#245'es Avulsas'
          'Sem Movimenta'#231#245'es Avulsas')
        ParentFont = False
        TabOrder = 1
      end
      object gbxPeriodoProcessamento: TGroupBox
        Left = 6
        Top = 297
        Width = 172
        Height = 39
        Caption = 'PER'#205'ODO DO PROCESSAMENTO'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 10
        object Label1: TLabel
          Left = 81
          Top = 19
          Width = 7
          Height = 15
          Caption = 'a'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
        end
        object edtDataInicialProcessamento: TEditData
          Left = 2
          Top = 14
          Width = 75
          Height = 23
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = 10
          ParentFont = False
          TabOrder = 0
          Minimo = 37353
          Maximo = 0
        end
        object edtDataFinalProcessamento: TEditData
          Left = 92
          Top = 14
          Width = 75
          Height = 23
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = 10
          ParentFont = False
          TabOrder = 1
          Minimo = 37353
          Maximo = 0
        end
      end
    end
  end
  object ecvValida: TtecEditionControlValidation
    EditionControl = <
      item
        Control = edtDataInicial
      end>
    Left = 191
    Top = 5
  end
end
