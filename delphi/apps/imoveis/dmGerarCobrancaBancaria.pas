unit dmGerarCobrancaBancaria;

interface

uses
  SysUtils, Classes, dmbasico, dmtecsoft, DB, cpdatasource, ZQuery, Variants,
  ZPgSqlQuery, cpquery, Forms,
  ACBrBoleto, Biblio, ctconstantes, fr_dset, fr_dbset,
  fr_class, fmPreviewPadrao, ZTransact, DateUtils, clparametrossistema,
  ACBrBase, dialogs, ACBrBoletoConversao;

type
  TdtmGerarCobrancaBancaria = class(TdtmBasico)
    ...
    qryContratosParcelasNrParcela: TIntegerField;
    ...
    qryDadosRetornoTecVIM: TBooleanField;
    ...
    qryContratosParcelasTaxaJuros: TFloatField;
    qryContasEspecieDocto: TStringField;
    qryContasCodigoProtesto: TStringField;
    qryContasNrDiasProtesto: TIntegerField;
    qryContratosParcelasDataMoraJuros: TDateField;
    qryContratosParcelasValorDesconto: TCurrencyField;
    qryContratosParcelasValorMoraJuros: TCurrencyField;
    qryContratosParcelasPercentualMulta: TFloatField;
    qryContratosParcelasNossoNumero: TLargeintField;
    qryContasconvenio: TLargeintField;
    
    vACBrBoleto: TACBrBoleto;
    qryRetornoACBr: TtecQuery;
    dsrRetornoACBr: TtecDataSource;
    qryRetornoACBrContrato: TIntegerField;
    qryRetornoACBrEmpreendimento: TIntegerField;
    qryRetornoACBrNomeEmpreendimento: TStringField;
    qryRetornoACBrCliente: TIntegerField;
    qryRetornoACBrNomeCliente: TStringField;
    qryRetornoACBrTipoParcela: TStringField;
    qryRetornoACBrParcela: TIntegerField;
    qryRetornoACBrOrigemParcela: TStringField;
    qryRetornoACBrBanco: TIntegerField;
    qryRetornoACBrCodMov: TIntegerField;
    qryRetornoACBrDesccricaoMov: TStringField;
    qryRetornoACBrDataVencto: TDateField;
    qryRetornoACBrValorVencto: TFloatField;
    qryRetornoACBrDataPagto: TDateField;
    qryRetornoACBrValorPagto: TFloatField;
    qryRetornoACBrDataCreditado: TDateField;
    qryRetornoACBrValorCreditado: TFloatField;
    qryRetornoACBrMoeda: TIntegerField;
    qryRetornoACBrAcrescimo: TFloatField;
    qryRetornoACBrDescontos: TFloatField;
    qryRetornoACBrIOF: TFloatField;
    qryRetornoACBrAbatimentos: TFloatField;
    qryRetornoACBrTarifa: TFloatField;
    qryRetornoACBrErro: TBooleanField;
    qryRetornoACBrOutras: TBooleanField;
    qryRetornoACBrtecLUX: TBooleanField;
    qryRetornoACBrSite: TBooleanField;
    qryRetornoACBrtecVIM: TBooleanField;
    qryRetornoACBrCobCaixa: TBooleanField;
    qryRetornoACBrDescErro: TStringField;
    qryRetornoACBrSituacao: TStringField;
    qryRetornoACBrObservacoes: TStringField;
    qryDadosRetornoObservacoes: TStringField;
    qryContasEmailNoArquivoBanco: TBooleanField;
    qryProcessarRecebimentosLancamentos: TBooleanField;
    procedure frpRelatorioRetornoBeforePrint(Memo: TStringList;
      View: TfrView);
    procedure ZmonitorMonitorEvent(Sql, Result: String);
    procedure qryContratosParcelasCalcFields(DataSet: TDataSet);
  private
    FDataFinal: String;
    FDataInicial: String;
    FNomeEmp: String;
  ...
  protected
    ...
    function GerarLancamentosContabeis: boolean;
  ...
  public
    ...
    procedure ProcessarArquivoRetornoACBr(Arquivo: String);
    procedure ProcessarArquivoRetornoCEF (Arquivo: String);
...
var
  dtmGerarCobrancaBancaria: TdtmGerarCobrancaBancaria;
  Arq: File;
  GABuffer: ^T_GABuffer;
  NomeArquivoCliente,
  NomeArquivoTitulos,
...
{ TdtmGerarCobrancaBancaria }

function TdtmGerarCobrancaBancaria.ConsultarClientes(Empreendimento,DataInicial, DataFinal: String): Boolean;
begin
...
function TdtmGerarCobrancaBancaria.ConverterTipoParcela(Tipo: Integer): String;
begin
   Result:= CHR(Tipo + $40);        {Retorna 'A' para 1, 'B' para 2, etc}
end;

function TdtmGerarCobrancaBancaria.ConverterTipoParcela(Tipo: Char): Integer;
begin
  Result:= Ord(Tipo) - $40; {Retorna 1 para 'A', 2 para 'B', etc}
end;


function TdtmGerarCobrancaBancaria.ExisteMarcados: Boolean;
begin
  Result:= False;
...
function TdtmGerarCobrancaBancaria.GeraNro(Numero: Currency; Size: Integer): String;
Var Campo: String;
Begin
...
procedure TdtmGerarCobrancaBancaria.GerarArquivosRemessa(Diretorio, Empreendimento: String);
var Pos: TBookmark;
begin
...
function TdtmGerarCobrancaBancaria.GeraStr(Campo: String; Size: Integer): String;
var Pos: Integer;
begin
...
function TdtmGerarCobrancaBancaria.GeraVal(Numero: Currency; Size: integer): String;
Var Campo: String;
Begin
...
procedure TdtmGerarCobrancaBancaria.GravarArquivoEspelho;
var
  Arq: TextFile;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        qryContratosParcelas.Locate('Contrato;TipoParcela;NrParcela',varArrayof([qryParcelascontrato.AsVariant,
...
procedure TdtmGerarCobrancaBancaria.GravarArquivoParcelas(var Arq);
...
procedure TdtmGerarCobrancaBancaria.GravarDadosCliente(var Arq);
Var Arquivo:  File Absolute Arq;
...
begin
  ...
  begin
    ...
    {if qryContratosParcelasComplemento.AsString <> ''then
      if qryContratosParcelasNrParcela.AsInteger > 0
        then Endereco:= Rua + ', ' + qryContratosParcelasNrParcela.AsString +
...
function TdtmGerarCobrancaBancaria.GravarParcelas: Boolean;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if qryDadosRetornoTecVIM.AsBoolean then
        begin
    ...
    begin
      ...
      Perpetrar([qryParcelasRetorno], AguardareTerminarDentroDm);
      result := GerarLancamentosContabeis;
    end;
  ...
  if result then
    result := Perpetrar([qryParcelasRetorno, qryMovtosBancos, qryMovtosbancosEventos])
  else
    Rollback([qryParcelasRetorno, qryMovtosBancos, qryMovtosbancosEventos])
end;

procedure TdtmGerarCobrancaBancaria.ImprimirRelatorio;
Var
  Relatorio: TfrReport;
...
procedure TdtmGerarCobrancaBancaria.MarcarSelecionados(Marcando,Todos: Boolean);
begin
...
procedure TdtmGerarCobrancaBancaria.MontarTabelaMovimentos;
begin
   with GABuffer^ do begin
...
procedure TdtmGerarCobrancaBancaria.MontarTabelasMotivosOcorrencia;
begin
  With GABuffer^ do
  Begin
...
procedure TdtmGerarCobrancaBancaria.MoveCrLf(Campo: String; var PraOnde; Size: Integer);
begin
  Move(CrLf, PraOnde, Size);
end;

procedure TdtmGerarCobrancaBancaria.MoveNro(Numero: Currency; var PraOnde;Size: Integer);
Var Campo: String;
Begin
  Campo := FloatToStr(Numero);
  While Length(Campo) < Size do
...
procedure TdtmGerarCobrancaBancaria.MoveInt(Numero: LargeInt; var PraOnde;Size: Integer);
Var Campo: String;
Begin
...
procedure TdtmGerarCobrancaBancaria.MoveStr(Campo: String; var PraOnde; Size: Integer);
var Pos: Integer;
begin
...
procedure TdtmGerarCobrancaBancaria.MoveStrDir(Campo: String; var PraOnde; Size: Integer);
var Pos: Integer;
begin
  Campo := RetiraAcento(Campo);
  Campo := ANSIUpperCase(Campo);
...
procedure TdtmGerarCobrancaBancaria.MoveVal(Numero: Currency; var PraOnde; Size: Integer);
Var Campo: String;
Begin
...
function TdtmGerarCobrancaBancaria.ParametrosContratos: String;
...
procedure TdtmGerarCobrancaBancaria.ProcessarArquivoRetornoACBr(Arquivo: string);
var i:            Integer;
    vComentario:  String;
    vQueParcelas: String;
...
begin                                                  {================= ATENÇÃO =================}
                                                       {Esta rotina só foi testada com o Banco ITAÚ}

   with vACBrBoleto do begin
      NomeArqRetorno:= ExtractFileName(Arquivo);
      DirArqRetorno := ExtractFilePath(Arquivo);
      DirArqRetorno := Copy(DirArqRetorno,1,Length(DirArqRetorno)-1);           {Retira a "\" final}
      ...
      with Cedente do begin
         ...
         then CNPJCPF:= SomenteNumero(qryContasCNPJAgenciaReferencia.AsString)
         else CNPJCPF:= SomenteNumero(qryFiliaisCNPJ.                AsString);
      end;
      LerRetorno;

      vQueParcelas:= '';
      for i:= 0 to ListadeBoletos.Count - 1 do with vACBrBoleto.ListaDeBoletos.Objects[i] do begin
          case Banco.TipoCobranca of
             cobItau: if (FLVal(Copy(SeuNumero,1,6)) > 0) and
                         (FLVal(Copy(SeuNumero,7,2)) > 0) and
                         (FLVal(Copy(SeuNumero,9,3)) > 0)
                      then vQueParcelas:= vQueParcelas + '(pi.Contrato = ' + FStr(FLVal(Copy(SeuNumero,1,6)),$00)  +
                                                     ' AND pi.Tipo = '''   +  Chr(FLVal(Copy(SeuNumero,7,2)))      +
                                                   ''' AND pi.Numero = '   + FStr(FLVal(Copy(SeuNumero,9,3)),$00)  + ') OR ';
          end;
      end;
   end;

   if vQueParcelas = '' then begin
      MensagemAviso(ctNaoFoiSelecionadoParcelas);
      Exit;
   end;
   Delete(vQueParcelas, Length(vQueParcelas)-3, 4);                       {Retirando o " OR " final}
   vQueParcelas:= 'AND (' + vQueParcelas + ')';

   qryRetornoACBr.MacroByName('vQueParcelas').AsString:= vQueParcelas;
   qryRetornoACBr.Close;
   qryRetornoACBr.Open;
   ...
   if qryRetornoACBr.IsEmpty then begin
      MensagemAviso(ctNaoFoiSelecionadoParcelas);
      Exit;
   end;
...
//  vTotalValorRecebidoViaSite:= 0;
 ...
 //neste caso poderia perpetrar duas vezes incluindo duas parcelas, por isso a quitação é feita em duas partes.
   ...
   for i:= 0 to vACBrBoleto.ListadeBoletos.Count-1 do with vACBrBoleto.ListadeBoletos[i] do begin
       if (FLVal(Copy(SeuNumero,1,6)) > 0) and
          (FLVal(Copy(SeuNumero,7,2)) > 0) and
          (FLVal(Copy(SeuNumero,9,3)) > 0) then begin

           vContrato:= FStr(FLVal(Copy(SeuNumero,1,6)),$00);
           vTipo    := Chr (FLVal(Copy(SeuNumero,7,2)));
           vNumero  := FStr(FLVal(Copy(SeuNumero,9,3)),$00);

           if not qryRetornoACBr.Locate('Contrato;TipoParcela,Parcela', VarArrayOf([vContrato,vTipo,vNumero]), [])
           then MensagemAviso(format('A parcela "%s" não foi encontrada', [vContrato + '/' + vTipo + '-' + vNumero]))
           else begin

              qryRetornoACBr.Edit;
              if qryRetornoACBrObservacoes.AsString = '' then begin
                 if OcorrenciaOriginal.Descricao <> '' then begin
                    qryRetornoACBrObservacoes.AsString := OcorrenciaOriginal.Descricao;
                    qryRetornoACBrCodMov.     AsInteger:= FLVal(Copy(OcorrenciaOriginal.Descricao,1,2));
                 end;

                              {COMMENT ON COLUMN "public"."vendasimoveis"."situacao"
                               IS '[R]eservado, [V]endido, [C]ancelado, [Q]uitado, [T]ransferido para,
                                   trans[F]erido de, Rene[G]ociado para Renegocia[D]o de Re[S]cindido';}

                 case qryRetornoACBrSituacao.AsString[1] of
                    'R':      vSituacao:= 'reservado';
                    'V':      vSituacao:= 'vendido';
                    'C':      vSituacao:= 'cancelado';
                    'Q':      vSituacao:= 'quitado';
                    'T', 'F': vSituacao:= 'transferido';
                    'G', 'D': vSituacao:= 'renegociado';
                    'S':      vSituacao:= 'rescindido';
                 end;

                 if  (qryRetornoACBrSituacao.AsString[1] IN ['R', 'C', 'Q', 'T', 'G', 'S'])
                 then if qryRetornoACBrObservacoes.AsString <> ''
                      then qryRetornoACBrObservacoes.AsString:= qryRetornoACBrObservacoes.AsString +
                                                                ', contrato ' + vSituacao
                      else qryRetornoACBrObservacoes.AsString := 'Contrato '  + vSituacao;

                 if not qryRetornoACBrDataPagto.IsNull
                 then if qryRetornoACBrObservacoes.AsString <> ''
                      then qryRetornoACBrObservacoes.AsString:= qryRetornoACBrObservacoes.AsString +
                                                                ', a parcela já está paga'
                      else qryRetornoACBrObservacoes.AsString:= 'A parcela já está paga';

                 if (qryRetornoACBrCodMov.   AsInteger  = 6) and
                    (qryRetornoACBrDataPagto.IsNull)         and (ValorRecebido > 0) then
                 begin
                    qryRetornoACBrDataPagto.     AsDateTime:= DataOcorrencia;
                    qryRetornoACBrValorPagto.    AsCurrency:= ValorRecebido;
                    qryRetornoACBrDataCreditado. AsDateTime:= DataOcorrencia;
                    qryRetornoACBrValorCreditado.AsCurrency:= ValorRecebido;
                 end
                 else
                 begin
                    qryRetornoACBrDataCreditado. Clear;
                    qryRetornoACBrValorCreditado.Clear;
                    if qryRetornoACBrDataPagto.IsNull then begin
                       qryRetornoACBrDataPagto. Clear;
                       qryRetornoACBrValorPagto.Clear;
                    end;
                 end;
                 qryRetornoACBrDataVencto. AsDateTime:= Vencimento;
                 qryRetornoACBrValorVencto.AsCurrency:= ValorDocumento;
              end;
           end;

           if qryRetornoACBrObservacoes.AsString = '' then begin
              if DescricaoMotivoRejeicaoComando.Count > 0
              then qryRetornoACBrObservacoes.AsString:= DescricaoMotivoRejeicaoComando[0]
              else if OcorrenciaOriginal.Descricao <> ''
                   then qryRetornoACBrObservacoes.AsString:= OcorrenciaOriginal.Descricao
                   else qryRetornoACBrObservacoes.AsString:= 'OCORRÊNCIA NÃO ENCONTRADA';
           end;

           {----------------------------------------------------------- Equivalente a Segmento T da CEF}
           qryDadosRetorno.Append;
           qryDadosRetornoContrato.          AsInteger := qryRetornoACBrContrato.          AsInteger;
           qryDadosRetornoEmpreendimento.    AsInteger := qryRetornoACBrEmpreendimento.    AsInteger;
           qryDadosRetornoNomeEmpreendimento.AsString  := qryRetornoACBrNomeEmpreendimento.AsString;
           qryDadosRetornoCliente.           AsInteger := qryRetornoACBrCliente.           AsInteger;
           qryDadosRetornoNomeCliente.       AsString  := qryRetornoACBrNomeCliente.       AsString;
           qryDadosRetornoTipoParcela.       AsString  := qryRetornoACBrTipoParcela.       AsString;
           qryDadosRetornoParcela.           AsInteger := qryRetornoACBrParcela.           AsInteger;
           qryDadosRetornoOrigemParcela.     AsString  := qryRetornoACBrOrigemParcela.     AsString;
           qryDadosRetornoBanco.             AsInteger := qryRetornoACBrBanco.             AsInteger;
           qryDadosRetornoDataVencto.        AsDateTime:= qryRetornoACBrDataVencto.        AsDateTime;
           qryDadosRetornoValorVencto.       AsCurrency:= qryRetornoACBrValorVencto.       AsCurrency;
           qryDadosRetornoMoeda.             AsString  := qryRetornoACBrMoeda.             AsString;
           qryDadosRetornoTecLUX.            AsBoolean := True;
           qryDadosRetornoSite.              AsBoolean := False;
           qryDadosRetornoTecVIM.            AsBoolean := False;
           qryDadosRetornoCobCaixa.          AsBoolean := False;

           {----------------------------------------------------------- Equivalente a Segmento U da CEF}
           qryDadosRetornoCodMov.            AsInteger := qryRetornoACBrCodMov.            AsInteger;
           qryDadosRetornoDesccricaoMov.     AsString  := qryRetornoACBrDesccricaoMov.     AsString;
           qryDadosRetornoDataPagto.         AsVariant := qryRetornoACBrDataPagto.         AsVariant;
           qryDadosRetornoValorPagto.        AsVariant := qryRetornoACBrValorPagto.        AsVariant;
           qryDadosRetornoDataCreditado.     AsVariant := qryRetornoACBrDataCreditado.     AsVariant;
           qryDadosRetornoValorCreditado.    AsVariant := qryRetornoACBrValorCreditado.    AsVariant;
           qryDadosRetornoAcrescimo.         AsCurrency:= qryRetornoACBrAcrescimo.         AsCurrency;
           qryDadosRetornoIOF.               AsCurrency:= qryRetornoACBrIOF.               AsCurrency;
           qryDadosRetornoDescontos.         AsCurrency:= qryRetornoACBrDescontos.         AsCurrency;
           qryDadosRetornoAbatimentos.       AsCurrency:= qryRetornoACBrAbatimentos.       AsCurrency;
           qryDadosRetornoObservacoes.       AsString  := qryRetornoACBrObservacoes.       AsString;

           qryDadosRetorno.Post;
       end;
   end;
end; {ProcessarArquivoRetornoACBr}



procedure TdtmGerarCobrancaBancaria.ProcessarArquivoRetornoCEF(Arquivo: String);
var Arq: TextFile;
    Linha : String;
    Segmento: Char;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          qryDadosRetornoTecLUX.AsBoolean:= False;
          qryDadosRetornoSite.AsBoolean:= False;
          qryDadosRetornoTecVIM.AsBoolean:= True;
          qryDadosRetornoCobCaixa.AsBoolean:= False;
          qryDadosRetorno.Post;
        end;
      end
...
procedure TdtmGerarCobrancaBancaria.RefazConsultaParcelas(Empreendimento,Cliente, Parcelas: Integer; TipoParcela, DataVencto: Variant);
begin
...
function TdtmGerarCobrancaBancaria.TiraPonto(vValStr: String): String;
Var I: Integer;
begin
  Result := '';
...
function TdtmGerarCobrancaBancaria.TratarData(Valor: String): TDateTime;
var Data: TDateTime;
begin
...
function TdtmGerarCobrancaBancaria.TratarValor(Valor: String): Currency;
Var Numero: Integer;
begin
...
procedure TdtmGerarCobrancaBancaria.frpRelatorioRetornoBeforePrint(
...
procedure TdtmGerarCobrancaBancaria.ZMonitorMonitorEvent(Sql, Result: String);
var
 Listar : TStringList;
 arquivo: String;
begin
...
procedure TdtmGerarCobrancaBancaria.GerarMovimentacaoBancaria;
...
//      qryMovtosBancosdocumento.AsString     := RecebimentosInt.NroCheque;
    ...
    Perpetrar([qryMovtosBancos, qryMovtosbancosEventos], AguardareTerminarDentroDm);
  end;
...
function TdtmGerarCobrancaBancaria.DescricaoParcelas: String;
...
function TdtmGerarCobrancaBancaria.GerarLancamentosContabeis: boolean;
begin
...
//        qryProcessarRecebimentos.ExecSql;
    ...
    RefazConsultaPorNome(qryProcessarRecebimentos,['Filial','Recibo'],
                                                  [FilialBase,qryParcelasRetornorecibopagto.AsInteger]);
  except
    on E: Exception do
    begin
      result := false;
...
function TdtmGerarCobrancaBancaria.GetNumeroRecibo: Integer;
begin
  spcRecibos.Open;
...
function TdtmGerarCobrancaBancaria.GetNumeroOrigem: Int64;
...
procedure TdtmGerarCobrancaBancaria.VerificacoesContrato(Contrato: String);
begin
...
constructor TdtmGerarCobrancaBancaria.Create(AOwner: TComponent);
begin
  inherited;
...
procedure TdtmGerarCobrancaBancaria.VerificarEmpreendimento(Emp: String);
begin
...
procedure TdtmGerarCobrancaBancaria.GerarDadosRelatorioEmpreendimento;
begin
...
function TdtmGerarCobrancaBancaria.ExisteEnderecoTamanhoMaior40: Boolean;
begin
  Result:= False;
  GuardarRegistroAtual(qryContratosParcelas, True);
  qryContratosParcelas.First;
...
function TdtmGerarCobrancaBancaria.DiminuiuCampo: Boolean;
var Rua: String;
begin
   Rua:= qryContratosParcelasEndereco.AsString;
   if Length(Rua) > 40 then begin
      Rua:= Trocar(Rua, '  ',            ' ');
      Rua:= Trocar(Rua, 'APTO',          'AP');
      Rua:= Trocar(Rua, 'RUA ',          'R ');
      Rua:= Trocar(Rua, 'AVENIDA ',      'AV ');
      ...
      Rua:= Trocar(Rua, 'PROFESSOR',     'PROF');
      Rua:= Trocar(Rua, 'RODOVIA',       'ROD');
      Rua:= Trocar(Rua, 'SENADOR',       'SEN');
      Rua:= Trocar(Rua, 'SERVIDAO',      'SERV');
      Rua:= Trocar(Rua, 'VEREADOR',      'VER');

      qryContratosParcelas.Edit;
      qryContratosParcelasEndereco. AsString:= Rua;
      qryContratosParcelasCampoMaior.AsBoolean:= Length(Rua) <= 40;
...
procedure TdtmGerarCobrancaBancaria.GerarArquivoACBrCNAB(c240ou400: TACBrLayoutRemessa;
                                                         Diretorio: string);
...
begin
   ...
   NomeArquivoCliente:= Diretorio + 'C' + FormatDateTime('DDMMYY',DataServidor) + '.TXT';
   NomeArquivoEspelho:= Diretorio + 'E' + FormatDateTime('DDMMYY',DataServidor) + '.TXT';
   NomeArquivoTitulos:=             'T' + FormatDateTime('DDMMYY',DataServidor) + '.TXT';

   try
       ACBrBoleto1:= TACBrBoleto.Create(Self);
       qryContratosParcelas.First;
       while not qryContratosParcelas.Eof do begin
          if qryContratosParcelasSelecionado.AsBoolean then begin
             TotalVencimentos := TotalVencimentos + qryContratosParcelasVenctoParcela.AsCurrency;
             ...
             with Titulo do begin
                ...
                with ACBrBoleto.Cedente do begin
                   ...
                   else Nome:= qryFiliaisRazao.     AsString;
                ...
                then DataProtesto:= SomarDia(qryParcelasDataVencto.AsDateTime,
                                             qryContasNrDiasProtesto.AsInteger, 'N')
                else DataProtesto:= 0;                                              // Não protestar

                Aceite           := atNao;
                DataProcessamento:= DataServidor;
                Vencimento       := qryContratosParcelasVenctoParcela.AsDateTime;
                DataDocumento    := DataServidor;
                                                                                 {CCCCCC + TT + PPP}
                NumeroDocumento:= FStr(qryContratosParcelasContrato.       AsInteger,   $61) +
                                  FStr(Ord(qryContratosParcelasTipoParcela.AsString[1]),$21) +
                                  FStr(qryContratosParcelasNrParcela.      AsInteger,   $31);
                SeuNumero      := NumeroDocumento;

                ValorDocumento := qryContratosParcelasValorParcela. AsFloat;
//              DataDesconto   := qryContratosParcelasVenctoParcela.AsDateTime;
//              ValorDesconto  := qryContratosParcelasValorDesconto.AsCurrency;
                DataMoraJuros  := qryContratosParcelasDataMoraJuros.AsDateTime;
                ValorMoraJuros := qryContratosParcelasValorMoraJuros.AsCurrency;

//------------------------------------------------------------- Colocado como comentário a pedido do
//                                                              Márcio Silveira em 04/OUT/2016
//              PercentualMulta:= qryContratosParcelasPercentualMulta.AsCurrency;

                if Titulo.ACBrBoleto.Banco.TipoCobranca = cobBancoDoBrasil
                then NossoNumero:= Copy(qryContratosParcelasNossoNumero.AsString,1,10)
                else NossoNumero:= qryContratosParcelasNossoNumero.     AsString;

                StrGeral:= NossoNumero;

                StrGeral:= NumeroDocumento;
                ...
                with Sacado do begin
                   if qryContratosParcelasPessoaTipo.AsString = 'F'
                   then Pessoa := pFisica
                   else Pessoa := pJuridica;
                   CNPJCPF    := qryContratosParcelaspessoanumero. AsString;
                   NomeSacado := qryContratosParcelasRazao.        AsString;
                   Logradouro := qryContratosParcelasEndereco.     AsString;
//                 Numero     := qryParcelasEndNumero.             AsString;     Já está no Endereço
                   Complemento:= qryContratosParcelasComplemento.  AsString;
                   Bairro     := qryContratosParcelasNomeBairro.   AsString;
                   Cidade     := qryContratosParcelasNomeCidade.   AsString;
                   UF         := qryContratosParcelasEstado.       AsString;
                   CEP        := FStr(qryContratosParcelasCEP.AsInteger,$81);

                   if qryContasEmailNoArquivoBanco.AsBoolean
                   then Email:= qryContratosParcelasEmail.AsString
                   else Email:= '';
                end;
       ...
       ACBrBoleto1.NomeArqRemessa:= NomeArquivoTitulos; // + '.REM';
       ACBrBoleto1.DirArqRemessa := Diretorio;
       ACBrBoleto1.GerarRemessa(qryContasSequencial.AsInteger + 1);
       Gerou := True;
   ...
   if Gerou then begin
      ...
      MensagemAviso(format(ctARQUIVOGERADOSUCESSO,[Diretorio + NomeArquivoTitulos]));
   end
   else begin
...
procedure TdtmGerarCobrancaBancaria.qryContratosParcelasCalcFields(DataSet: TDataSet);
begin
   inherited;
   if (qryContratosParcelasTaxaJuros.AsCurrency > 0) and
      (qryContasNrDiasProtesto.      AsInteger  > 0) then begin

      qryContratosParcelasDataMoraJuros.AsDateTime:= SomarDia(qryContratosParcelasVenctoParcela.AsDateTime,
                                                              qryContasNrDiasProtesto.AsInteger, 'N');
      qryContratosParcelasValorMoraJuros.AsCurrency := qryContratosParcelasValorParcela.AsCurrency *
                                                      (0.01 * qryContratosParcelasTaxaJuros.Ascurrency) / 30.0;
    end
    else begin
       qryContratosParcelasDataMoraJuros.Clear;
       qryContratosParcelasValorMoraJuros.AsCurrency:= 0.00;
  end;
end;
