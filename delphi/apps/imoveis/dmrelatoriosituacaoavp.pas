...
type
  Tdtmrelatoriosituacaoavp = class(TdtmBasico)
    ...
    qryRelatoriosituacaoavpsaldoinicialcp: TFloatField;
    qryRelatoriosituacaoavpsaldoiniciallp: TFloatField;
    qryRelatoriosituacaoavpmaiscp: TFloatField;
    qryRelatoriosituacaoavpmaislp: TFloatField;
    qryRelatoriosituacaoavpmenoscp: TFloatField;
    qryRelatoriosituacaoavpmenoslp: TFloatField;
    qryRelatoriosituacaoavpsaldofinalcp: TFloatField;
    qryRelatoriosituacaoavpsaldofinallp: TFloatField;
    qryRelatoriosituacaoavpvalorcp: TFloatField;
    qryRelatoriosituacaoavpvalorlp: TFloatField;
...
procedure Tdtmrelatoriosituacaoavp.qryRelatoriosituacaoavpBeforeOpen(
  DataSet: TDataSet);
var
 vListaSituacao, vDescricaoListaSituacao: String;
begin
  inherited;
  qryRelatoriosituacaoavp.ParamByName('DataInicial').AsDateTime:= DataInicial;
  qryRelatoriosituacaoavp.ParamByName('DataFinal').AsDateTime  := DataFinal;

  qryRelatoriosituacaoavp.ParamByName('DataSituacaoEm').AsDateTime  := DataSituacaoEm;
{  qryRelatoriosituacaoavp.ParamByName('exercicio').AsInteger  := YearOf(FDataSituacaoEm);}
  ...
  if Transferidos then vListaSituacao := vListaSituacao + '''F'', ''T'', ';
  if Renegociados then vListaSituacao := vListaSituacao + '''D'', ''G'', ';

  if vListaSituacao <> '' then
    delete(vListaSituacao, length(vListaSituacao)-1, 2);

  vDescricaoListaSituacao := '';
  if vListaSituacao <> '' then
  begin
    qryRelatoriosituacaoavp.MacroByName('ListaSituacao').AsString:= 'and vi.situacao in ('+vListaSituacao+')';
    if Vendidos          then vDescricaoListaSituacao:= vDescricaoListaSituacao + ' VENDIDOS,';
    if Rescindidos       then vDescricaoListaSituacao:= vDescricaoListaSituacao + ' RESCINDIDOS,';
    if Transferidos {De} then vDescricaoListaSituacao:= vDescricaoListaSituacao + ' TRANSFERIDOS,';
    if Renegociados {De} then vDescricaoListaSituacao:= vDescricaoListaSituacao + ' RENEGOCIADOS,';
    Delete(vDescricaoListaSituacao,Length(vDescricaoListaSituacao),1);
  end
  else
  begin
    qryRelatoriosituacaoavp.MacroByName('ListaSituacao').AsString:=
       'and vi.situacao in ('+'''V'',''S'',''F'',''T'',''D'',''G'''+')';
    vDescricaoListaSituacao := 'VENDIDOS, RESCINDIDOS, TRANSFERIDOS, RENEGOCIADOS';
  end;

  fParametroCabecalho := fParametroCabecalho + ' Situações: '+vDescricaoListaSituacao;
...
begin
  ...
  fOrdenacaoI := value;
  case value of
  0: fordenacao := 'order by /*situacao,*/ TO_ASCII(nomeempreendimento, ''LATIN1''), contrato ';
  1: fordenacao := 'order by /*situacao,*/ contrato';
  2: fordenacao := 'order by /*situacao,*/ TO_ASCII(nomecliente, ''LATIN1''), cliente, contrato';
  end;
end;
...
begin
  inherited;
  if VarName = 'DATASITUACAO' then
    Value := FormatDateTime('DD/MM/YYYY',DataSituacaoEm)
  else
  if VarName = 'TITULO' then
  begin
    Value := 'VALORES A RECEBER DE CURTO E LONGO PRAZO EM '+FormatDateTime('DD/MM/YYYY',DataSituacaoEm) +' E SITUAÇÃO DO AVP ENTRE '+FormatDateTime('DD/MM/YYYY',DataInicial)+
