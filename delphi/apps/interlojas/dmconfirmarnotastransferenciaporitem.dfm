inherited dtmConfirmarNotasTransferenciaPorItem: TdtmConfirmarNotasTransferenciaPorItem
  OldCreateOrder = False
  Left = 442
  Top = 214
  Height = 533
  Width = 694
  object qryProcurarUsuarios: TtecQuery
    Tag = -1
    ...
    object qryProcurarUsuarioscodigo: TIntegerField
      FieldName = 'codigo'
      Required = True
    end
    object qryProcurarUsuariosnome: TStringField
      FieldName = 'nome'
      Size = 40
    end
  end
  object dsrProcurarUsuarios: TtecDataSource
    DataSet = qryProcurarUsuarios
    Left = 265
    Top = 20
  end
  ...
  object qryPedidosFiliais: TtecQuery
    ...
    Sql.Strings = (
      'Select     pf.codigo,'
        ...
        '               cast(to_char(pf.data,'#39'DD/MM/YYYY HH24:MI'#39') as var' +
        'char) as data,'
      '               pf.requisitada,'
      '               pf.produto,'
      '               pf.qtdepedida,'
      ...
      '               p.descricao,'
        ...
        '               (select gr.descricao from grades gr where gr.codi' +
        'go = c.grade1) as linha,'
      
        '               (select gr.descricao from grades gr where gr.codi' +
        'go = c.grade2) as coluna, '
      '               p.valorgrade1,'
      '               p.valorgrade2,'
      '               false as selecionar,'
      '               0.00 as semvalor'
        ...
        ' = n.dadofiscal),  produtos p,  filiais f,  caracteristicas c'
      'Where     '
      '/*           Periodo               */               '
      '       and  (pf.requisitante =:requisitante)'
      '       and  (pf.situacao = '#39'F'#39')'
      '       and  (pf.produto = p.codigo)    '
      '       and  (p.caracteristica = c.codigo)'
      '/*           Usuario         */       '
      '       and  (pf.requisitada = f.codigo)'
      '               ')
    RequestLive = True
    Left = 80
    Top = 80
    ...
    object qryPedidosFiliaisnome: TStringField
      FieldName = 'nome'
      Size = 40
    end
    ...
    object qryPedidosFiliaisselecionar: TBooleanField
      FieldName = 'selecionar'
    end
    object qryPedidosFiliaissemvalor: TFloatField
      FieldName = 'semvalor'
    end
    object qryPedidosFiliaisdata: TStringField
      FieldName = 'data'
    end
  ...
  object qryNotaEntregue: TtecQuery
    Tag = -1
    Database = dtmTecSoft.dbaTecSoft
    Transaction = dtmTecSoft.tstTecSoft
    CachedUpdates = False
    ...
    Macros = <>
    Sql.Strings = (
      'Select (produtosdf.qtde - produtospf.qtde) as quantidade'
      'From (Select count(pd.produto) as qtde'
      '          From produtosdadosfiscais pd'
      '          Where (pd.dadofiscal =:par)) as produtosdf, '
      '         (Select count(pf.produto) as qtde'
      '          From pedidosfiliais pf'
      '          Where (pf.dadofiscal =:par)) as produtospf')
    RequestLive = False
    Left = 265
    Top = 143
    ParamData = <
    ...
    object qryNotaEntreguequantidade: TFloatField
  ...
  object qryProdutosDadosFiscais: TtecQuery
    ...
    Sql.Strings = (
      'select pd.*,'
        ...
        're p.codigo = pd.produto and p.caracteristica = c.codigo) as com' +
        'posto'
      ''
      'from   produtosdadosfiscais pd'
