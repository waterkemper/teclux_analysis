unit dmconfirmarnotastransferenciaporitem;

interface

uses
  SysUtils, Classes, Forms, Math, Variants, dmbasico, dmtecsoft, DB, ZQuery,
  ZPgSqlQuery, cpquery, cpdatasource, ctconstantes, biblio,
  clparametrossistema, ZTransact;

type
  TdtmConfirmarNotasTransferenciaPorItem = class(TdtmBasico)
    qryProcurarUsuarios: TtecQuery;
    qryProcurarUsuarioscodigo: TIntegerField;
    qryProcurarUsuariosnome: TStringField;
    dsrProcurarUsuarios: TtecDataSource;
    ...
    qryPedidosFiliaisselecionar: TBooleanField;
    ...
    qryNotaEntregue: TtecQuery;
    qryNotaEntreguequantidade: TFloatField;
  ...
  protected
    ...
    function  GetConsultaUsuarios: TtecQuery;
    function  GetPedidoSelecionado: Boolean;
  ...
  public
    ...
    procedure CadastrarNotaEntrada(DF: Integer);
    procedure ConfirmarTransferencia;
    constructor create(AOwner: TComponent); override;
    function  ExisteUsuarios(Campo, Codigo: String): Boolean;
    procedure FecharTabelas;
    function  GerarConsultaPedidosFiliais: Boolean;
    procedure Selecionar;
    procedure SelecionarRegistros;
    property  ConsultaUsuarios: TtecQuery read GetConsultaUsuarios;
    ...
    property  PedidoSelecionado: Boolean read GetPedidoSelecionado;
...
const
   WhereBase = 17;
   WherePeriodo  = WhereBase + 01;
   WhereUsuario  = WhereBase + 06;

implementation

{$R *.dfm}

{ TdtmConfirmarNotasTransferenciaPorItem }

procedure TdtmConfirmarNotasTransferenciaPorItem.AbrirTabelas;
begin
  Abre(ctLOJTabelaConsultaUsuarios);
end;

procedure TdtmConfirmarNotasTransferenciaPorItem.BloquearEstoque;
var
  Pos: TBookmark;
  WhereSQL: String;
begin
  ...
  try
    if qryProdutosNotasPag.RecordCount = 0 then
      ...
      while Not qryProdutosNotasPag.Eof do begin
        WhereSQL := WhereSQL +
        '((e.produto = ' + qryProdutosNotasPagproduto.AsString + ')and' +
        '(e.filial = '   + qryProdutosNotasPagfilial.AsString  + '))or';
...
procedure TdtmConfirmarNotasTransferenciaPorItem.CadastrarNotaEntrada(DF: Integer);
begin
  ReFazConsulta(qryNotaEntregue,[0],[DF],false);
  if (qryNotaEntreguequantidade.AsCurrency = 0) then begin

    ReFazConsulta(qryNaturezasPadrao,[],[]);

    ReFazConsulta(qryDadosFiscais,[0],[DF]);
    ReFazConsulta(qryProdutosDadosFiscais,[0],[DF]);
//    ReFazConsulta(qryCalculosDadosFiscais,[0],[DF]);
    ...
    BloquearEstoque;
    IncluirMovimentos;
    Perpetrar([qryNotasPag, qryMovimentos, qryProdutosNotasPag]);
  end
end;

procedure TdtmConfirmarNotasTransferenciaPorItem.ConfirmarTransferencia;
var
  Pos: TBookmark;
  DF : Integer;
  Lista: String;
begin
  if MensagemConfirmacao(ctCONFIRMARPRODUTOSTRANSFERENCIA) = smbOk then begin
    Pos:= qryPedidosFiliais.GetBookmark;
    qryPedidosFiliais.DisableControls;
    try
      Lista:= '';
      qryPedidosFiliais.First;
      while not qryPedidosFiliais.Eof do begin
        if qryPedidosFiliaisselecionar.AsBoolean then
          Lista:= Lista + qryPedidosFiliaiscodigo.AsString + ',';
    ...
    if (Lista <> '') then begin
      ...
      qryAtualizarPedidosFiliais.Sql[06]:= '(pf.codigo in (' + Lista + '))';
      qryAtualizarPedidosFiliais.Open;
      if (qryAtualizarPedidosFiliais.RecordCount > 0) then begin
        qryAtualizarPedidosFiliais.First;
        while not qryAtualizarPedidosFiliais.Eof do begin
          qryAtualizarPedidosFiliais.Edit;
          qryAtualizarPedidosFiliaissituacao.AsString:= 'D';
          qryAtualizarPedidosFiliais.Post;
          qryAtualizarPedidosFiliais.Next;
        end;
      end;
      Perpetrar([qryAtualizarPedidosFiliais]);
    end;

    qryAtualizarPedidosFiliais.First;
    DF := qryAtualizarPedidosFiliaisdadofiscal.AsInteger;
    while not qryAtualizarPedidosFiliais.Eof do begin
      if (DF <> qryAtualizarPedidosFiliaisdadofiscal.AsInteger) then
        CadastrarNotaEntrada(DF);
      DF := qryAtualizarPedidosFiliaisdadofiscal.AsInteger;
      qryAtualizarPedidosFiliais.Next;
    end;
    CadastrarNotaEntrada(DF);

    GerarConsultaPedidosFiliais;
  end;
end;

constructor TdtmConfirmarNotasTransferenciaPorItem.create(AOwner: TComponent);
begin
  inherited;
  qryProcurarUsuarios.Tag := ctTabelas;
  qryConsultaUsuarios.Tag := ctLOJTabelaConsultaUsuarios;
  qryProcurarUsuarios.Params[0].AsInteger := CodigoUsuario;
  qryPedidosFiliaisqtderecebida.DisplayFormat := ParSistema.MascaraQuantidadeGrade;
end;

function TdtmConfirmarNotasTransferenciaPorItem.ExisteUsuarios(Campo, Codigo: String): Boolean;
begin
  Result:= ExisteCodigo(qryConsultaUsuarios, campo, codigo);
end;

procedure TdtmConfirmarNotasTransferenciaPorItem.FecharTabelas;
begin
  Fecha(ctLOJTabelaConsultaUsuarios);
end;

function TdtmConfirmarNotasTransferenciaPorItem.GerarConsultaPedidosFiliais: Boolean;
begin
...
function TdtmConfirmarNotasTransferenciaPorItem.GetColunadaGrade: String;
begin
  result := PrimeiraLetraEmMaiuscula(qryPedidosFiliaislinha.AsString)
end;

function TdtmConfirmarNotasTransferenciaPorItem.GetConsultaUsuarios: TtecQuery;
begin
  Result:= qryConsultaUsuarios;
end;

function TdtmConfirmarNotasTransferenciaPorItem.GetLinhadaGrade: String;
begin
  result := PrimeiraLetraEmMaiuscula(qryPedidosFiliaiscoluna.AsString)
end;

function TdtmConfirmarNotasTransferenciaPorItem.GetPedidoSelecionado: Boolean;
begin
  Result:= qryPedidosFiliaisselecionar.AsBoolean;
end;

{
procedure TdtmConfirmarNotasTransferenciaPorItem.IncluirImpostos;
begin
  qryCalculosDadosFiscais.First;
  while not qryCalculosDadosFiscais.Eof do begin
    qryCalculosNotasPag.Insert;
    qryCalculosNotasPagcodigonota.AsInteger:= qryNotasPagcodigo.AsInteger;
    qryCalculosNotasPagnumero.AsInteger    := qryCalculosDadosFiscaisnumero.AsInteger;
    qryCalculosNotasPagtipo.AsString       := qryCalculosDadosFiscaistipo.AsString;
    qryCalculosNotasPagaliquota.AsFloat    := qryCalculosDadosFiscaisaliquota.AsFloat;
    qryCalculosNotasPagbase.AsCurrency     := qryCalculosDadosFiscaisbase.AsCurrency;
    qryCalculosNotasPagisentas.AsCurrency  := qryCalculosDadosFiscaisisentas.AsCurrency;
    qryCalculosNotasPagoutras.AsCurrency   := qryCalculosDadosFiscaisoutras.AsCurrency;
    qryCalculosNotasPagvalor.AsCurrency    := qryCalculosDadosFiscaisvalor.AsCurrency;
    qryCalculosNotasPagcodigofiscal.AsInteger := qryCalculosNotasPagcodigofiscal.AsInteger;
    qryCalculosNotasPagcodigonatureza.AsInteger := qryCalculosNotasPagcodigonatureza.AsInteger;
    qryCalculosNotasPag.Post;
    qryCalculosDadosFiscais.Next;
  end;
end;
}

procedure TdtmConfirmarNotasTransferenciaPorItem.IncluirMovimentos;
var
...
begin
  ...
  try
    ...
    begin
      qryProdutosNotasPag.First;
      while not qryProdutosNotasPag.Eof do begin
        if FiltrarComposto(qryProdutosNotasPagcomposto.AsBoolean ,SituacaoProduto) then
        begin
          QtdeMovto:= 0;
          MovExtra:= False;
          if qryEstoqueBloqueio.Locate('produto;filial', VarArrayOf([qryProdutosNotasPagproduto.AsString,
                                                                     qryProdutosNotasPagfilial.AsInteger]) ,[]) then
          begin
            while (QtdeMovto < qryProdutosNotasPagquantidade.AsCurrency) do
            begin
              NovoMovimento(qryProdutosNotasPagproduto.AsLargeInt, qryProdutosNotasPagfilial.AsInteger);
              qryMovimentosreferencia.AsString  := 'NF-' + IntToStr(qryNotasPagnumero.AsInteger) +
                                                   ' S-' + qryNotasPagserie.AsString + ' TRANSFERENCIA';
              ...
              begin
                if MovExtra then begin
                  ...
                  qryMovimentosvalor.AsCurrency:= RoundTo((qryMovimentosquantidade.AsCurrency *
                                                           qryProdutosNotasPagprecounitario.AsCurrency),-2);
                end
                else begin
                  qryMovimentosquantidade.AsCurrency   := (qryProdutosNotasPagquantidade.AsCurrency -
                                                          qryEstoqueBloqueiofuturo.AsCurrency);
              ...
              qryMovimentos.Post;
              MovExtra:= (QtdeMovto < qryProdutosNotasPagquantidade.AsCurrency);
            end;
          end;
        end;
        qryProdutosNotasPag.Next;
...
procedure TdtmConfirmarNotasTransferenciaPorItem.IncluirNota;
...
begin
  ...
  qryNotasPagcodigo.AsInteger:= spcNotasPagProximocodigo.AsInteger;
  spcNotasPagProximo.Close;

  qryNotasPagfornecedor.AsInteger       := qryDadosFiscaisfilial.AsInteger;
  qryNotasPagtipofornecedor.AsString    := 'L';
  qryNotasPagserie.AsString             := qryDadosFiscaisserie.AsString;
  qryNotasPagnumero.AsInteger           := qryDadosFiscaisnumeronota.AsInteger;
  qryNotasPagfilial.AsInteger           := qryDadosFiscaiscliente.AsInteger;
  qryNotasPagestado.AsString            := qryDadosFiscaisestadocfo.AsString;

  SelecionarCodigoNaturezaNotasPag(noTRANSFERENCIAMERCADORIASENTRADA);

  {
  if qryNotasPagestado.AsString = EstadoFilialBase then
     qryNotasPagcodigofiscal.AsInteger := (1000 + ctCFOTRANSFERENCIA)
  else if HeUnidadeFederacao(qryNotasPagestado.AsString) then
     qryNotasPagcodigofiscal.AsInteger := (2000 + ctCFOTRANSFERENCIA)
  else
     qryNotasPagcodigofiscal.AsInteger := (3000 + ctCFOTRANSFERENCIA);
  qryNotasPagnatureza.AsString          := qryDadosFiscaisnatureza.AsString;
  qryNotasPagcodigonatureza.AsInteger   := qryDadosFiscaiscodigonatureza.AsInteger;
  }

  qryNotasPagemissao.AsDateTime         := qryDadosFiscaisdata.AsDateTime;
  qryNotasPagdata.AsDateTime            := DataServidor;
  qryNotasPagcnpj.AsString              := qryFilialOrigemcnpj.AsString;
  qryNotasPaginscricaoestadual.AsString := qryFilialOrigeminscricaoestadual.AsString;
  qryNotasPagvalornota.AsCurrency       := qryDadosFiscaisvalortotal.AsCurrency;
  qryNotasPagbaseicms.AsCurrency        := qryDadosFiscaisbaseicms.AsCurrency;
  qryNotasPagvaloricms.AsCurrency       := qryDadosFiscaisvaloricms.AsCurrency;
...
procedure TdtmConfirmarNotasTransferenciaPorItem.IncluirProdutos;
begin
  ...
  while not qryProdutosDadosFiscais.Eof do begin
    ...
    qryProdutosNotasPagnumero.AsInteger         := qryProdutosDadosFiscaisnumero.AsInteger;
    qryProdutosNotasPagfilial.AsInteger         := FilialBase;
    qryProdutosNotasPagproduto.AsLargeInt       := qryProdutosDadosFiscaisproduto.AsLargeInt;
    qryProdutosNotasPagprecounitario.AsCurrency := qryProdutosDadosFiscaisprecovenda.AsCurrency;
    qryProdutosNotasPagquantidade.ascurrency    := qryProdutosDadosFiscaisquantidade.Ascurrency;


    if qryProdutosDadosFiscaisincidencia.AsString<>'' then
    begin
      qryProdutosNotasPagincidencia.AsString      := qryProdutosDadosFiscaisincidencia.AsString;
...
procedure TdtmConfirmarNotasTransferenciaPorItem.InicializarTabelas;
begin
...
{
  if qryCalculosNotasPag.Active then
    qryCalculosNotasPag.Close;
  qryCalculosNotasPag.Open;
  }
...
procedure TdtmConfirmarNotasTransferenciaPorItem.MontaIntervaloData;
const
  Data_1: String = '(cast(pf.data as date) = ';
  Data_2: String = '(cast(pf.data as date) between (';
begin
  if not DataEmBranco(FDataInicial) then begin
    if DataEmBranco(FDataFinal) then
         qryPedidosFiliais.Sql[WherePeriodo]:= Data_1 + '''' + FDataInicial + ''')'
    else qryPedidosFiliais.Sql[WherePeriodo]:= Data_2 + '''' + FDataInicial + ''') and (''' + FDataFinal + '''))';
  end
  else begin
    if not DataEmBranco(FDataFinal) then
         qryPedidosFiliais.Sql[WherePeriodo]:= Data_1 + '''' + FDataFinal + ''')'
    else qryPedidosFiliais.Sql[WherePeriodo]:= '';
  end;
end;

procedure TdtmConfirmarNotasTransferenciaPorItem.NovoMovimento(Produto: int64; Filial: Integer);
begin
...
procedure TdtmConfirmarNotasTransferenciaPorItem.Selecionar;
begin
  ReFazConsulta(qryProcurarUsuarios,[0],[qryConsultaUsuarioscodigo.AsInteger]);
end;

procedure TdtmConfirmarNotasTransferenciaPorItem.SelecionarRegistros;
begin
  MarcarRegistros(qryPedidosFiliais,
                  qryPedidosFiliaisselecionar,
                  qryPedidosFiliaissemvalor, False,False);
end;

procedure TdtmConfirmarNotasTransferenciaPorItem.SetDataFinal(const Value: String);
begin
...
procedure TdtmConfirmarNotasTransferenciaPorItem.SetDataInicial(const Value: String);
begin
...
procedure TdtmConfirmarNotasTransferenciaPorItem.SetUsuario(const Value: String);
begin
  qryPedidosFiliais.Sql[WhereUsuario]:=
  'and (c.grupo in (Select us.grupo From usuariossetores us Where (us.usuario = ' + Value + ')))';
end;

procedure TdtmConfirmarNotasTransferenciaPorItem.qryPedidosFiliaisAfterScroll(
  DataSet: TDataSet);
begin
  inherited;
...
procedure TdtmConfirmarNotasTransferenciaPorItem.ZMonitor1MonitorEvent(Sql,
  Result: String);
var
 Listar : TStringList;
begin
  ...
  if fileexists('c:\log100709.sql') then
    Listar.loadfromfile('c:\log100709.sql');
  Listar.add('');
  Listar.add(sql);
  Listar.add(result);
  listar.savetofile('c:\log100709.sql');
  listar.free;
end;

procedure TdtmConfirmarNotasTransferenciaPorItem.qryNotasPagNewRecord(
  DataSet: TDataSet);
begin
  inherited;
