...
uses
  ...
  cpdblookupcombobox,
  fmmensagemgrid, fmVisualizarImageURL, fmprincipalbasico, AdvCombo,
  ColCombo, AdvDBLookupComboBox, editlist, Lucombo, dblucomb, AdvListV,
  DBAdvLst, AdvObj, BaseGrid, AdvGrid, DBAdvGrid, AdvOfficeButtons;
...
type
  TfrmTransferenciaRequisicaoExposicao = class(TfrmCadastroPadrao)
    ...
    edtCodigoBarrasTransferencia: TEditTexto;
    ...
    sbnGerarLotes: TSpeedButton;
    gbxProdutosGruposFiliais: TGroupBox;
    dbgProdutosGrupos: TtecDBGrid;
    gbxProdutosFiliais: TGroupBox;
    dbgProdutosFiliais: TtecDBGrid;
    pnlBottom: TPanel;
    btnVisualizarDetalhes: TButton;
    gbxParametrosDetalhes: TGroupBox;
    Label4: TLabel;
    edtNumero_dias_de_Vendas: TEditNumero;
    Label6: TLabel;
    edtDiasProporcionalEstoque: TEditNumero;
    Label3: TLabel;
    edtDiasEstoque: TEditNumero;
    Splitter2: TSplitter;
    Splitter1: TSplitter;
    gbxPedidosFiliais: TGroupBox;
    GroupBox1: TGroupBox;
    gbxQtRequisitante: TGroupBox;
    sbnVisualizarFoto: TSpeedButton;
    sbnAtualizarRegistros: TSpeedButton;
    DBAdvGrid1: TDBAdvGrid;
    AdvOfficeCheckBoxMarcarDesmarcarLotes: TAdvOfficeCheckBox;
    Label5: TLabel;
    shCancelado: TShape;
    ckbUsarTabelaRedutora: TAdvOfficeCheckBox;
    ckbNaoConsiderarPedidosCompra: TAdvOfficeCheckBox;
    ckbPedidosEntreFiliais: TAdvOfficeCheckBox;
    ...
    procedure edtCodigoBarrasTransferenciaKeyDown(Sender: TObject;
    ...
    procedure sbnGerarLotesClick(Sender: TObject);
   // procedure edtCodigoBarrasTransferencia(Sender: TObject)
    procedure ExibirGradeDiferencas;
    procedure ExibirGradeDiferencas_(Sender: TObject);
    procedure btnVisualizarDetalhesClick(Sender: TObject);
    ...
    procedure DBAdvGrid1CheckBoxClick(Sender: TObject; ACol, ARow: Integer;
      State: Boolean);
    procedure AdvOfficeCheckBoxMarcarDesmarcarLotesClick(Sender: TObject);
...
begin
  ...
  sbnConfirma.Enabled := not sbnSalvar.Enabled and (rgbSituacao.ItemIndex = 0) and
                         (not vTabelaVazia) and (dtmTransferenciaRequisicaoExposicao.qryTotaisPedidosFiliaisqtderecebidamarcada.AsCurrency<>0);
...
//  gbxCodigoBarras.Visible := (not vTabelaVazia);
  ...
  sbnGerarLotes.Enabled := dtmTransferenciaRequisicaoExposicao.qryPedidosFiliaismarcarlotetransferencia.AsBoolean and (edfRequisitante.Text<>'');
...
//  DataSet:= dtmTransferenciaRequisicaoExposicao.TabelaRequisicao;
  ...
  edtCodigoBarrasTransferencia.Clear;
...
//  sbnConferir.visible := not parsistema.Permitir_Requisicoes_de_Produtos_Duplicadas;
//  sbnGerarLotes.visible := parsistema.Permitir_Requisicoes_de_Produtos_Duplicadas;

  dtmTransferenciaRequisicaoExposicao.ExibirGradeDiferencas := ExibirGradeDiferencas_;

  pnlBottom.visible := false;
  Splitter1.visible := false;
  sbnAtualizarRegistros.visible := false;

  dtmTransferenciaRequisicaoExposicao.ConferenciaProdutos_ := ConferirProdutos_;

  gbxCodigoBarras.Visible := parsistema.ExibirLocalizaporCodigoBarrasnasTransferencias;
...
begin
  ...
  if not (gdFocused in State) then begin
    if (dtmTransferenciaRequisicaoExposicao.QtdeRecebida > 0) then
    begin
    ...
    if dtmTransferenciaRequisicaoExposicao.qryPedidosFiliaiscodigoorigem.asinteger<>0 then
    begin
...
begin
  ...
  if Result then
  begin
    if dtmTransferenciaRequisicaoExposicao.CancelarRequisicao then
      if sbnAtualizarRegistros.visible then
        sbnAtualizarRegistrosClick(nil);
  end;
  dbgTransferenciaRequisicao.SetFocus;
...
function TfrmTransferenciaRequisicaoExposicao.TabelaDePesquisa: TZDataSet;
begin
...
procedure TfrmTransferenciaRequisicaoExposicao.edtCodigoBarrasTransferenciaKeyDown(
  Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
  if Key = VK_RETURN then
    if edtCodigoBarrasTransferencia.Text<>''then
    begin
      if gbxCodigoBarras.Caption = 'CÓDIGO DE BARRAS' then
        dtmTransferenciaRequisicaoExposicao.LocalizarProduto(edtCodigoBarrasTransferencia.Text,
                                                             pesCodigoBarras)
      else
        dtmTransferenciaRequisicaoExposicao.LocalizarProduto(edtCodigoBarrasTransferencia.Text,
                                                             pesproduto);
    end;
     edtCodigoBarrasTransferencia.SetFocus;
...
begin
  ...
  with dtmTransferenciaRequisicaoExposicao do
  begin
    if VerificarLoteTransferencia then
    begin

        LigarDesligarEventosPedidosFiliais(True);

        {aqui caso ocorra excessão - Gedovar}
        ConferirProdutos(qryPedidosFiliais, qryPedidosFiliaisproduto.fieldname,
         ...
         qryPedidosFiliaisqtderecebida.FieldName,
         {qryPedidosFiliaisqtdepedida.FieldName,}
         'TRF', qryPedidosFiliaislotetransferencia.AsString,
         qryPedidosFiliaisvalorgrade1.FieldName,
         qryPedidosFiliaisvalorgrade2.FieldName,
         qryPedidosFiliaislinha.FieldName,
         qryPedidosFiliaiscoluna.FieldName, 0, 0, nil,
         qryPedidosFiliaiscodigo.FieldName );

        LigarDesligarEventosPedidosFiliais(false);

        AbrirLotesPedidosFiliais;
        qryPedidosFiliaisAfterOpen(nil);
...
procedure TfrmTransferenciaRequisicaoExposicao.sbnGerarLotesClick(
  Sender: TObject);
begin
  inherited;
  dtmTransferenciaRequisicaoExposicao.GerarLoteTransferencia;
end;

procedure TfrmTransferenciaRequisicaoExposicao.ExibirGradeDiferencas;
begin
  frmMensagemGrid := TfrmMensagemGrid.Create(frmMensagemGrid);
  frmMensagemGrid.Caption := 'Mensagem de erro';
  frmMensagemGrid.lblTitulo.Caption := 'DIFERENÇAS NAS QUANTIDADES DOS PEDIDOS!';
  frmMensagemGrid.lblTExto.Caption := 'Verifique no quadro abaixo as diferenças nos pedidos. A consulta será reprocessada!.';
  frmMensagemGrid.dbgMensagem.DataSource := dtmTransferenciaRequisicaoExposicao.dsrPedidosFiliaisAtuais;
  frmMensagemGrid.ShowModal;
  frmMensagemGrid.Free;
  sbnGerar.Click;
end;

procedure TfrmTransferenciaRequisicaoExposicao.ExibirGradeDiferencas_(
  Sender: TObject);
begin
  ExibirGradeDiferencas
end;

procedure TfrmTransferenciaRequisicaoExposicao.btnVisualizarDetalhesClick(
  Sender: TObject);
begin
  inherited;
  ...
  begin
    btnVisualizarDetalhes.caption := 'Ocultar Detalhes';
    sbnAtualizarRegistros.visible := true;
  end
  else
  begin
    btnVisualizarDetalhes.caption := 'Visualizar Detalhes';
    sbnAtualizarRegistros.visible := false;
  end;

  dtmTransferenciaRequisicaoExposicao.AbrirDadosComprasFiliais;
end;

procedure TfrmTransferenciaRequisicaoExposicao.sbnVisualizarFotoClick(
  Sender: TObject);
begin
  inherited;
  AcionarTelaVisualizarImageURL(self, dtmTransferenciaRequisicaoExposicao.GetProduto);
end;

procedure TfrmTransferenciaRequisicaoExposicao.sbnAtualizarRegistrosClick(
  Sender: TObject);
begin
  inherited;
  ...
  try
    ...
    dbgTransferenciaRequisicao.SalvarPosicaoGrid;
    dbgProdutosGrupos.SalvarPosicaoGrid;
    dbgProdutosFiliais.SalvarPosicaoGrid;

    dtmTransferenciaRequisicaoExposicao.Reatualizar(dtmTransferenciaRequisicaoExposicao.qryPedidosFiliaisproduto.asString);

  finally

    dbgTransferenciaRequisicao.RestaurarPosicaoGrid;
    dbgProdutosGrupos.RestaurarPosicaoGrid;
    dbgProdutosFiliais.RestaurarPosicaoGrid;
...
procedure TfrmTransferenciaRequisicaoExposicao.DBAdvGrid1CheckBoxClick(
...
procedure TfrmTransferenciaRequisicaoExposicao.AdvOfficeCheckBoxMarcarDesmarcarLotesClick(
  Sender: TObject);
begin
  inherited;
  dtmTransferenciaRequisicaoExposicao.MarcarDesmarcarListadeLotes(AdvOfficeCheckBoxMarcarDesmarcarLotes.checked);

end;

procedure TfrmTransferenciaRequisicaoExposicao.ConferirProdutos_(
  Sender: TObject);
begin
