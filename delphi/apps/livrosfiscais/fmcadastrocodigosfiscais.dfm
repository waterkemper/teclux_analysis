inherited frmCadastroCodigosFiscais: TfrmCadastroCodigosFiscais
  Left = 578
  Top = 151
  Caption = 'Cadastro de C'#243'digos Fiscais de Opera'#231#227'o'
  ClientHeight = 423
  ClientWidth = 618
  FormStyle = fsMDIChild
  Visible = True
  PixelsPerInch = 96
  TextHeight = 16
  inherited pnlBarra: TPanel
    Top = 0
  end
  inherited pnlTopMenu: TPanel
    Width = 618
    inherited tblBarra: TToolBar
      Width = 393
    end
  end
  object pnlFundoJanela: TPanel
    Left = 0
    Top = 45
    Width = 618
    Height = 378
    Align = alClient
    BevelOuter = bvNone
    BorderWidth = 3
    TabOrder = 2
    object gbxFundoJanela: TGroupBox
      Left = 3
      Top = 3
      Width = 612
      Height = 238
      Align = alTop
      TabOrder = 0
      object gbxBases: TGroupBox
        Left = 158
        Top = 177
        Width = 184
        Height = 48
        Caption = 'BASE DE'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 5
        Visible = False
        object ckbBaseFaturamento: TDBCheckBox
          Left = 6
          Top = 11
          Width = 94
          Height = 16
          Caption = 'Faturamento'
          DataField = 'basefaturamento'
          DataSource = dtmCadastroCodigosFiscais.dsrCodigosFiscais
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 0
          ValueChecked = 'True'
          ValueUnchecked = 'False'
        end
        object ckbBasePIS: TDBCheckBox
          Left = 6
          Top = 30
          Width = 93
          Height = 14
          Caption = 'PIS'
          DataField = 'basepis'
          DataSource = dtmCadastroCodigosFiscais.dsrCodigosFiscais
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 1
          ValueChecked = 'True'
          ValueUnchecked = 'False'
        end
        object ckbBaseCOFINS: TDBCheckBox
          Left = 108
          Top = 11
          Width = 71
          Height = 16
          Caption = 'COFINS'
          DataField = 'basecofins'
          DataSource = dtmCadastroCodigosFiscais.dsrCodigosFiscais
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 2
          ValueChecked = 'True'
          ValueUnchecked = 'False'
        end
        object ckbBaseCSSL: TDBCheckBox
          Left = 108
          Top = 30
          Width = 71
          Height = 14
          Caption = 'CSSL'
          DataField = 'basecssl'
          DataSource = dtmCadastroCodigosFiscais.dsrCodigosFiscais
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 3
          ValueChecked = 'True'
          ValueUnchecked = 'False'
        end
      end
      object gbxValidades: TGroupBox
        Left = 189
        Top = 115
        Width = 176
        Height = 49
        Caption = 'VALIDADE'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 4
        object gbxValidoAte: TGroupBox
          Left = 90
          Top = 10
          Width = 80
          Height = 36
          Caption = 'AT'#201
          TabOrder = 1
          object edtValidoAte: TDBEditData
            Left = 2
            Top = 10
            Width = 75
            Height = 24
            Alignment = taCenter
            DataField = 'validoate'
            DataSource = dtmCadastroCodigosFiscais.dsrCodigosFiscais
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -13
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            MaxLength = -1
            Maximo = 37353
            Minimo = 37353
            Adicional = 0
            ParentFont = False
            TabOrder = 0
            Opcional = True
          end
        end
        object gbxValidoApos: TGroupBox
          Left = 6
          Top = 10
          Width = 80
          Height = 36
          Caption = 'AP'#211'S'
          TabOrder = 0
          object edtValidoApos: TDBEditData
            Left = 2
            Top = 10
            Width = 75
            Height = 24
            Alignment = taCenter
            DataField = 'validoapos'
            DataSource = dtmCadastroCodigosFiscais.dsrCodigosFiscais
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -13
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            MaxLength = -1
            Maximo = 0
            Minimo = 37353
            Adicional = 0
            ParentFont = False
            TabOrder = 0
            Opcional = True
          end
        end
      end
      object rgpTipo: TtecDBRadioGroup
        Left = 88
        Top = 115
        Width = 92
        Height = 49
        Caption = 'TIPO'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 3
        TabStop = False
        DataField = 'tipo'
        DataSource = dtmCadastroCodigosFiscais.dsrCodigosFiscais
        object rbnSintetico: TtecRadioButton
          Left = 10
          Top = 10
          Width = 75
          Height = 17
          Caption = 'Sint'#233'tico'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 0
          Value = 'S'
        end
        object rbnAnalitico: TtecRadioButton
          Left = 9
          Top = 28
          Width = 75
          Height = 17
          Caption = 'Anal'#237'tico'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 1
          Value = 'A'
        end
      end
      object ckbDestacarIPI: TDBCheckBox
        Left = 373
        Top = 149
        Width = 185
        Height = 18
        Caption = 'Destacar IPI na nota avulsa'
        DataField = 'destacaripi'
        DataSource = dtmCadastroCodigosFiscais.dsrCodigosFiscais
        TabOrder = 8
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object gbxCodigo: TGroupBox
        Left = 10
        Top = 6
        Width = 66
        Height = 36
        Caption = 'C.F.O.P.'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 0
      end
      object gbxDescricao: TGroupBox
        Left = 86
        Top = 6
        Width = 523
        Height = 36
        Caption = 'DESCRI'#199#195'O'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 1
        object edtDescricao: TDBEditTexto
          Left = 2
          Top = 10
          Width = 519
          Height = 24
          DataField = 'descricao'
          DataSource = dtmCadastroCodigosFiscais.dsrCodigosFiscais
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = -1
          ParentFont = False
          TabOrder = 0
          CharCasenoDefault = False
          Alignment = taLeftJustify
        end
      end
      object gbxDetalhes: TGroupBox
        Left = 87
        Top = 45
        Width = 522
        Height = 66
        Caption = 'DETALHAMENTO DA OPERA'#199#195'O'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 2
        object mmoDetalhes: TtecDBMemo
          Left = 2
          Top = 10
          Width = 519
          Height = 52
          DataField = 'detalhes'
          DataSource = dtmCadastroCodigosFiscais.dsrCodigosFiscais
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 0
        end
      end
      object ckbDevolucaoRetrono: TDBCheckBox
        Left = 374
        Top = 116
        Width = 187
        Height = 17
        Caption = 'Devolu'#231#227'o/Retorno'
        DataField = 'devolucaoretorno'
        DataSource = dtmCadastroCodigosFiscais.dsrCodigosFiscais
        TabOrder = 6
        ValueChecked = 'True'
        ValueUnchecked = 'False'
        OnClick = ckbDevolucaoRetronoClick
      end
      object ckbExigeNotaReferenciada: TDBCheckBox
        Left = 374
        Top = 132
        Width = 187
        Height = 17
        Caption = 'Exige nota referenciada'
        DataField = 'exigenotareferenciada'
        DataSource = dtmCadastroCodigosFiscais.dsrCodigosFiscais
        TabOrder = 7
        ValueChecked = 'True'
        ValueUnchecked = 'False'
      end
      object ckbvendasoucompras: TDBCheckBox
        Left = 374
        Top = 165
        Width = 187
        Height = 17
        Caption = 'Compras ou Vendas'
        DataField = 'vendasoucompras'
        DataSource = dtmCadastroCodigosFiscais.dsrCodigosFiscais
        ReadOnly = True
        TabOrder = 9
        ValueChecked = 'True'
        ValueUnchecked = 'False'
        OnClick = ckbDevolucaoRetronoClick
      end
      object ckbOutrasSaidas: TDBCheckBox
        Left = 374
        Top = 182
        Width = 187
        Height = 17
        Caption = 'Outras Sa'#237'das'
        DataField = 'outrassaidas'
        DataSource = dtmCadastroCodigosFiscais.dsrCodigosFiscais
        ReadOnly = True
        TabOrder = 10
        ValueChecked = 'True'
        ValueUnchecked = 'False'
        OnClick = ckbDevolucaoRetronoClick
      end
      object ckbUsoouConsumo: TDBCheckBox
        Left = 374
        Top = 215
        Width = 187
        Height = 17
        Caption = 'Uso ou Consumo'
        DataField = 'usoouconsumo'
        DataSource = dtmCadastroCodigosFiscais.dsrCodigosFiscais
        ReadOnly = True
        TabOrder = 12
        ValueChecked = 'True'
        ValueUnchecked = 'False'
        OnClick = ckbDevolucaoRetronoClick
      end
      object ckbSubstituicaoTributaria: TDBCheckBox
        Left = 374
        Top = 198
        Width = 187
        Height = 17
        Caption = 'Substitui'#231#227'o Tribut'#225'ria'
        DataField = 'st'
        DataSource = dtmCadastroCodigosFiscais.dsrCodigosFiscais
        ReadOnly = True
        TabOrder = 11
        ValueChecked = 'True'
        ValueUnchecked = 'False'
        OnClick = ckbDevolucaoRetronoClick
      end
    end
    object gbxContabilidade: TGroupBox
      Left = 3
      Top = 241
      Width = 612
      Height = 132
      Align = alTop
      Caption = 'CONTABILIDADE'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -9
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      object gbxDebitar: TGroupBox
        Left = 6
        Top = 11
        Width = 600
        Height = 36
        Caption = 'DEBITAR'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        object sbnConsultaDebitar: TSpeedButton
          Left = 68
          Top = 10
          Width = 23
          Height = 23
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          Glyph.Data = {
            
            DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
          NumGlyphs = 2
          ParentFont = False
          OnClick = sbnConsultaDebitarClick
        end
        object dtxDebitarDescricao: TtecDBText
          Left = 94
          Top = 10
          Width = 339
          Height = 24
          TabStop = False
          Color = clBtnFace
          DataField = 'descricao'
          DataSource = dtmCadastroCodigosFiscais.dsrProcuraDebitar
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 1
          Alignment = taLeftJustify
        end
        object flkDebitar: TtecDBFindLookup
          Left = 2
          Top = 10
          Width = 65
          Height = 24
          Alignment = taLeftJustify
          DataField = 'debitoconta'
          DataSource = dtmCadastroCodigosFiscais.dsrCodigosFiscais
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = 6
          Maximo = 0
          Minimo = 37353
          Adicional = 0
          Opcional = True
          ParentFont = False
          TabOrder = 0
          PermitirZero = False
          PermitirNulo = False
          Operacao = opATRIBUICAO
          LookupField = 'codigo'
          LookupSource = dtmCadastroCodigosFiscais.dsrProcuraDebitar
          LookupQueryParameter = 'codigo'
          Parameter = ' '
          ActiveSetControls = False
          DenyInsert = False
          NoSetControls = <>
          SetControls = <>
          LookupParameter = 'codigo'
        end
        object dtxDebitarClassificacao: TtecDBText
          Left = 434
          Top = 9
          Width = 163
          Height = 24
          TabStop = False
          Color = clBtnFace
          DataField = 'classificacao'
          DataSource = dtmCadastroCodigosFiscais.dsrProcuraDebitar
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 2
          Alignment = taLeftJustify
        end
      end
      object gbxCreditar: TGroupBox
        Left = 6
        Top = 50
        Width = 600
        Height = 36
        Caption = 'CREDITAR'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 1
        object sbnConsultaCreditar: TSpeedButton
          Left = 68
          Top = 10
          Width = 23
          Height = 25
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          Glyph.Data = {
            
            DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
          NumGlyphs = 2
          ParentFont = False
          OnClick = sbnConsultaCreditarClick
        end
        object dtxCreditarDescricao: TtecDBText
          Left = 94
          Top = 10
          Width = 339
          Height = 24
          TabStop = False
          Color = clBtnFace
          DataField = 'descricao'
          DataSource = dtmCadastroCodigosFiscais.dsrProcuraCreditar
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 0
          Alignment = taLeftJustify
        end
        object flkCreditar: TtecDBFindLookup
          Left = 2
          Top = 10
          Width = 65
          Height = 24
          Alignment = taLeftJustify
          DataField = 'creditoconta'
          DataSource = dtmCadastroCodigosFiscais.dsrCodigosFiscais
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = 6
          Maximo = 0
          Minimo = 37353
          Adicional = 0
          Opcional = True
          ParentFont = False
          TabOrder = 1
          PermitirZero = False
          PermitirNulo = False
          Operacao = opATRIBUICAO
          LookupField = 'codigo'
          LookupSource = dtmCadastroCodigosFiscais.dsrProcuraCreditar
          LookupQueryParameter = 'codigo'
          Parameter = ' '
          ActiveSetControls = False
          DenyInsert = True
          NoSetControls = <>
          SetControls = <>
          LookupParameter = 'codigo'
        end
        object dtxCreditarClassificacao: TtecDBText
          Left = 433
          Top = 10
          Width = 164
          Height = 24
          TabStop = False
          Color = clBtnFace
          DataField = 'classificacao'
          DataSource = dtmCadastroCodigosFiscais.dsrProcuraCreditar
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 2
          Alignment = taLeftJustify
        end
      end
      object gbxHistorico: TGroupBox
        Left = 6
        Top = 89
        Width = 599
        Height = 36
        Caption = 'HIST'#211'RICO'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 2
        object sbnConsultaHistorico: TSpeedButton
          Left = 55
          Top = 10
          Width = 23
          Height = 23
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          Glyph.Data = {
            
            DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
          NumGlyphs = 2
          ParentFont = False
          OnClick = sbnConsultaHistoricoClick
        end
        object dtxHistorico: TtecDBText
          Left = 80
          Top = 10
          Width = 516
          Height = 24
          TabStop = False
          Color = clBtnFace
          DataField = 'descricao'
          DataSource = dtmCadastroCodigosFiscais.dsrProcuraHistorico
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 0
          Alignment = taLeftJustify
        end
        object flkHistorico: TtecDBFindLookup
          Left = 2
          Top = 10
          Width = 50
          Height = 24
          Alignment = taLeftJustify
          DataField = 'historico'
          DataSource = dtmCadastroCodigosFiscais.dsrCodigosFiscais
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = 3
          Maximo = 0
          Minimo = 37353
          Adicional = 0
          Opcional = True
          ParentFont = False
          TabOrder = 1
          PermitirZero = False
          PermitirNulo = False
          Operacao = opATRIBUICAO
          LookupField = 'codigo'
          LookupSource = dtmCadastroCodigosFiscais.dsrProcuraHistorico
          LookupQueryParameter = 'codigo'
          Parameter = ' '
          ActiveSetControls = False
          DenyInsert = True
          NoSetControls = <>
          SetControls = <>
          LookupParameter = 'codigo'
        end
      end
    end
    object edfCodigoFiscal: TtecDbEditFind
      Left = 16
      Top = 18
      Width = 61
      Height = 24
      Alignment = taLeftJustify
      DataField = 'codigo'
      DataSource = dtmCadastroCodigosFiscais.dsrCodigosFiscais
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      Group = 'codigofiscal'
      MaxLength = 5
      Maximo = 0
      Minimo = 37353
      Adicional = 0
      Opcional = True
      ParentFont = False
      TabOrder = 2
      PermitirZero = False
      PermitirNulo = False
      Operacao = opATRIBUICAO
      Parameter = 'codigo'
      ActiveSetControls = True
      DenyInsert = False
      NoSetControls = <
        item
        end>
      SetControls = <>
    end
  end
end
