inherited frmLivroApuracaoPisCofins: TfrmLivroApuracaoPisCofins
  Left = 514
  Top = 149
  ActiveControl = edfFilial
  Caption = 'Livro de Apura'#231#227'o do PisCofins'
  ClientHeight = 429
  ClientWidth = 349
  FormStyle = fsMDIChild
  Visible = True
  PixelsPerInch = 96
  TextHeight = 16
  inherited pnlBarra: TToolBar
    Width = 349
  end
  inherited pnlFundoJanela: TPanel
    Top = 40
    Width = 349
    Height = 389
    Align = alClient
    object gbxLivroApuracaoPisCofins: TGroupBox
      Left = 3
      Top = 3
      Width = 343
      Height = 383
      Align = alClient
      TabOrder = 0
      object gbxImprimir: TGroupBox
        Left = 11
        Top = 285
        Width = 206
        Height = 92
        Caption = 'IMPRIMIR'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 5
        object ckbLivroApuracaoPisCofins: TCheckBox
          Left = 6
          Top = 11
          Width = 128
          Height = 25
          Caption = '&Livro de apura'#231#227'o'
          Checked = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          State = cbChecked
          TabOrder = 0
          OnClick = ckbLivroApuracaoPisCofinsClick
        end
        object ckbResumo: TCheckBox
          Left = 6
          Top = 29
          Width = 128
          Height = 25
          Caption = 'Resumo'
          Checked = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          State = cbChecked
          TabOrder = 1
          OnClick = ckbResumoClick
        end
        object ckbConferenciaCFOP: TCheckBox
          Left = 6
          Top = 61
          Width = 187
          Height = 25
          Caption = 'Visualiza'#231#227'o por CFOP/CST'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 2
          OnClick = ckbConferenciaCFOPClick
        end
      end
      object gbxFilial: TGroupBox
        Left = 10
        Top = 8
        Width = 326
        Height = 38
        Caption = 'FILIAL CONSOLIDADORA'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        object sbnProcurarFilial: TSpeedButton
          Left = 48
          Top = 10
          Width = 23
          Height = 22
          Hint = 'Procurar Filial'
          Flat = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          Glyph.Data = {
            
            DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
          NumGlyphs = 2
          ParentFont = False
          OnClick = sbnProcurarFilialClick
        end
        object edfFilial: TtecDbEditFind
          Left = 2
          Top = 10
          Width = 45
          Height = 24
          Alignment = taLeftJustify
          DataField = 'codigo'
          DataSource = dtmLivroApuracaoPisCofins.dsrProcuraFiliais
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          Group = 'Filial'
          MaxLength = 3
          Maximo = 0
          Minimo = 37353
          Adicional = 0
          Opcional = True
          ParentFont = False
          TabOrder = 0
          PermitirZero = False
          PermitirNulo = False
          Operacao = opATRIBUICAO
          Parameter = 'codigo'
          ActiveSetControls = False
          DenyInsert = True
          NoSetControls = <>
          SetControls = <>
        end
        object dtxFilial: TtecDBText
          Left = 73
          Top = 10
          Width = 250
          Height = 24
          TabStop = False
          Color = clBtnFace
          DataField = 'nome'
          DataSource = dtmLivroApuracaoPisCofins.dsrProcuraFiliais
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 1
          Alignment = taLeftJustify
        end
      end
      object gbxPeriodo: TGroupBox
        Left = 10
        Top = 50
        Width = 79
        Height = 47
        Caption = 'M'#202'S BASE'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 1
        object edtMesAno: TEditMesAno
          Left = 3
          Top = 16
          Width = 72
          Height = 24
          EditMask = '99/9999;1; '
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = 7
          ParentFont = False
          TabOrder = 0
          Text = '  /    '
        end
      end
      object gbxTiposdeNota: TGroupBox
        Left = 10
        Top = 128
        Width = 207
        Height = 73
        Caption = 'TIPOS DE NF DE SA'#205'DA'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 3
        object ckbNotasdeVenda: TCheckBox
          Left = 8
          Top = 16
          Width = 153
          Height = 17
          Caption = 'NOTAS DE VENDA'
          TabOrder = 0
        end
        object ckbNotasdeTransferencia_s: TCheckBox
          Left = 8
          Top = 32
          Width = 153
          Height = 17
          Caption = 'NOTAS DE TRANSFER'#202'NCIA'
          TabOrder = 1
        end
        object ckbNotasdeDevolucao_s: TCheckBox
          Left = 8
          Top = 48
          Width = 153
          Height = 17
          Caption = 'NOTAS DE DEVOLU'#199#195'O'
          TabOrder = 2
        end
      end
      object GroupBox1: TGroupBox
        Left = 10
        Top = 206
        Width = 207
        Height = 78
        Caption = ' TIPOS DE NOTAS FISCAIS DE ENTRADA'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 4
        object ckbNotasdeCompra: TCheckBox
          Left = 8
          Top = 16
          Width = 150
          Height = 17
          Caption = 'Notas de Compra'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 0
        end
        object ckbNotasdeTransferencia_e: TCheckBox
          Left = 8
          Top = 36
          Width = 150
          Height = 17
          Caption = 'Notas de Transfer'#234'ncia'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 1
        end
        object ckbNotasdeDevolucao_e: TCheckBox
          Left = 8
          Top = 56
          Width = 150
          Height = 17
          Caption = 'Notas de Devolu'#231#227'o'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'Helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 2
        end
      end
      object ckbSelecaoPelaDatadeEmissao: TCheckBox
        Left = 18
        Top = 104
        Width = 325
        Height = 17
        Caption = 'Selecionar por data de emiss'#227'o do documento'
        TabOrder = 2
      end
    end
  end
  object ecvValida: TtecEditionControlValidation
    EditionControl = <
      item
        Control = edfFilial
      end
      item
        Control = edtMesAno
      end>
    Left = 184
    Top = 8
  end
end
