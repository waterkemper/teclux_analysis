inherited frmrelatorioinventario: Tfrmrelatorioinventario
  Left = 640
  Top = 72
  ActiveControl = edfFilial
  Caption = 'Livro de Invent'#225'rio de Mercadorais'
  ClientHeight = 611
  ClientWidth = 627
  FormStyle = fsMDIChild
  Visible = True
  PixelsPerInch = 96
  TextHeight = 16
  inherited pnlBarra: TToolBar
    Width = 627
  end
  inherited pnlFundoJanela: TPanel
    Top = 39
    Width = 631
    Height = 575
    Align = alCustom
    object gbxParametros: TGroupBox
      Left = 3
      Top = 3
      Width = 625
      Height = 569
      Align = alClient
      TabOrder = 0
      object pgcFilialGrupoFilial: TtecPageControl
        Left = 8
        Top = 85
        Width = 465
        Height = 57
        ActivePage = tstFilial
        TabOrder = 4
        TabWidth = 90
        OnChange = pgcFilialGrupoFilialChange
        object tstFilial: TTabSheet
          Caption = '    &Filial'
          object sbnProcurarFilial: TSpeedButton
            Left = 63
            Top = 1
            Width = 23
            Height = 22
            Hint = 'Procurar Filial'
            Flat = True
            Glyph.Data = {
              
              DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
            NumGlyphs = 2
            OnClick = sbnProcurarFilialClick
          end
          object edfFilial: TtecDbEditFind
            Left = 2
            Top = 1
            Width = 60
            Height = 24
            Alignment = taLeftJustify
            DataField = 'codigo'
            DataSource = dtmrelatorioinventario.dsrProcuraFiliais
            Group = 'Filial'
            MaxLength = 3
            Maximo = 0
            Minimo = 37353
            Adicional = 0
            Opcional = True
            OnFound = edfFilialFound
            TabOrder = 0
            PermitirZero = False
            PermitirNulo = False
            Operacao = opATRIBUICAO
            Parameter = 'codigo'
            ActiveSetControls = False
            DenyInsert = True
            NoSetControls = <>
            SetControls = <>
          end
          object dtxFilial: TtecDBText
            Left = 86
            Top = 1
            Width = 370
            Height = 24
            TabStop = False
            Color = clBtnFace
            DataField = 'nome'
            DataSource = dtmrelatorioinventario.dsrProcuraFiliais
            ReadOnly = True
            TabOrder = 1
            Alignment = taLeftJustify
          end
        end
        object tstGrupoFilial: TTabSheet
          Caption = '    &Grupo Filial'
          ImageIndex = 1
          object sbnProcuraGrupoFilial: TSpeedButton
            Left = 63
            Top = 1
            Width = 23
            Height = 22
            Hint = 'Procurar Grupo Filial'
            Flat = True
            Glyph.Data = {
              
              DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
            NumGlyphs = 2
            OnClick = sbnProcuraGrupoFilialClick
          end
          object edfGrupoFilial: TtecDbEditFind
            Left = 2
            Top = 1
            Width = 60
            Height = 24
            Alignment = taLeftJustify
            DataField = 'codigo'
            DataSource = dtmrelatorioinventario.dsrProcuraGruposFiliais
            Group = 'GrupoFilial'
            MaxLength = 3
            Maximo = 0
            Minimo = 37353
            Adicional = 0
            Opcional = True
            OnFound = edfGrupoFilialFound
            TabOrder = 0
            PermitirZero = False
            PermitirNulo = False
            Operacao = opATRIBUICAO
            Parameter = 'codigo'
            ActiveSetControls = False
            DenyInsert = True
            NoSetControls = <>
            SetControls = <>
          end
          object dtxGrupoFilial: TtecDBText
            Left = 86
            Top = 1
            Width = 370
            Height = 24
            TabStop = False
            Color = clBtnFace
            DataField = 'descricao'
            DataSource = dtmrelatorioinventario.dsrProcuraGruposFiliais
            ReadOnly = True
            TabOrder = 1
            Alignment = taLeftJustify
          end
        end
      end
      object gbxData: TGroupBox
        Left = 161
        Top = 12
        Width = 75
        Height = 36
        Caption = 'NA DATA'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 1
        object edtData: TEditData
          Left = 2
          Top = 10
          Width = 70
          Height = 23
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = 10
          ParentFont = False
          TabOrder = 0
          Minimo = 37353
          Maximo = 0
        end
      end
      object rgpRelatorio: TRadioGroup
        Left = 6
        Top = 10
        Width = 148
        Height = 66
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ItemIndex = 0
        Items.Strings = (
          'Emiss'#227'o do Livro'
          'Termo de Abertura')
        ParentFont = False
        TabOrder = 0
        OnClick = rgpRelatorioClick
      end
      object rgpCusto: TRadioGroup
        Left = 10
        Top = 217
        Width = 132
        Height = 65
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ItemIndex = 0
        Items.Strings = (
          'M'#233'dio'
          #218'ltima Entrada')
        ParentFont = False
        TabOrder = 6
      end
      object rgpOrdenacao: TRadioGroup
        Left = 300
        Top = 217
        Width = 120
        Height = 60
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ItemIndex = 1
        Items.Strings = (
          'C'#243'digo'
          'Descri'#231#227'o')
        ParentFont = False
        TabOrder = 10
      end
      object rgpResumo: TRadioGroup
        Left = 300
        Top = 277
        Width = 119
        Height = 80
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ItemIndex = 1
        Items.Strings = (
          'Com R&esumo'
          '&Sem Resumo'
          'S'#243' Res&umo')
        ParentFont = False
        TabOrder = 11
      end
      object gbxNumeracao: TGroupBox
        Left = 249
        Top = 12
        Width = 164
        Height = 52
        Caption = ' NUMERA'#199#195'O '
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 3
        object gbxLivro: TGroupBox
          Left = 1
          Top = 14
          Width = 55
          Height = 36
          Caption = ' LIVRO '
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 0
          object edtLivro: TEditNumero
            Left = 2
            Top = 10
            Width = 50
            Height = 23
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            MaxLength = -1
            ParentFont = False
            TabOrder = 0
            Mascara = False
            TipoMascara = tmGERAL
            NrDecimal = 0
            Decimais = False
            Negativo = False
            Tamanho = 4
          end
        end
        object gbxPagina: TGroupBox
          Left = 54
          Top = 14
          Width = 55
          Height = 36
          Caption = ' P'#193'GINA '
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 1
          object edtPagina: TEditNumero
            Left = 2
            Top = 10
            Width = 50
            Height = 23
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            MaxLength = -1
            ParentFont = False
            TabOrder = 0
            Mascara = False
            TipoMascara = tmGERAL
            NrDecimal = 0
            Decimais = False
            Negativo = False
            Tamanho = 9
          end
        end
        object gbxMaximo: TGroupBox
          Left = 107
          Top = 14
          Width = 55
          Height = 36
          Caption = ' M'#193'XIMO '
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 2
          object edtMaximo: TEditNumero
            Left = 2
            Top = 10
            Width = 50
            Height = 23
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            MaxLength = -1
            ParentFont = False
            TabOrder = 0
            Mascara = False
            TipoMascara = tmGERAL
            NrDecimal = 0
            Decimais = False
            Negativo = False
            Tamanho = 9
          end
        end
      end
      object gbxSubConta: TGroupBox
        Left = 433
        Top = 362
        Width = 172
        Height = 36
        Caption = ' CONTAS CONT'#193'BEIS '
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 13
        object lblA: TLabel
          Left = 80
          Top = 16
          Width = 7
          Height = 15
          Caption = 'a'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
        end
        object edtSubContaInicial: TEditNumero
          Left = 2
          Top = 10
          Width = 71
          Height = 23
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = -1
          ParentFont = False
          TabOrder = 0
          Mascara = True
          Alignment = taLeftJustify
          TipoMascara = tmGERAL
          NrDecimal = 2
          Decimais = False
          Negativo = False
          Tamanho = 6
        end
        object edtSubContaFinal: TEditNumero
          Left = 95
          Top = 10
          Width = 73
          Height = 23
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = -1
          ParentFont = False
          TabOrder = 1
          Mascara = True
          Alignment = taLeftJustify
          TipoMascara = tmGERAL
          NrDecimal = 2
          Decimais = False
          Negativo = False
          Tamanho = 6
        end
      end
      object gbxAgrupamento: TGroupBox
        Left = 150
        Top = 218
        Width = 141
        Height = 139
        Caption = ' AGRUPAMENTO '
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 9
        object ckbAgruparGrupoFilial: TCheckBox
          Left = 10
          Top = 11
          Width = 124
          Height = 25
          Caption = 'Grupo de Filial'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 0
        end
        object ckbAgruparFilial: TCheckBox
          Left = 10
          Top = 31
          Width = 124
          Height = 25
          Caption = 'Filiais'
          Checked = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          State = cbChecked
          TabOrder = 1
        end
        object ckbAgruparClasseProduto: TCheckBox
          Left = 10
          Top = 71
          Width = 124
          Height = 25
          Caption = 'Classe de Produto'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 2
        end
        object ckbAgruparGrupoProduto: TCheckBox
          Left = 10
          Top = 91
          Width = 124
          Height = 25
          Caption = 'Grupo de Produto'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 3
        end
        object ckbAgruparProduto: TCheckBox
          Left = 10
          Top = 111
          Width = 124
          Height = 25
          Caption = 'Produto'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 4
        end
        object ckbAgruparLocalizacao: TCheckBox
          Left = 10
          Top = 51
          Width = 124
          Height = 25
          Caption = 'Localiza'#231#227'o'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 5
        end
      end
      object gbxGrupos: TGroupBox
        Left = 6
        Top = 406
        Width = 614
        Height = 60
        Caption = ' CONSIDERAR AS COLUNAS DE ESTOQUE '
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 14
        object sbnMarcarColunas: TSpeedButton
          Left = 588
          Top = 10
          Width = 22
          Height = 22
          Hint = 'Marcar Agentes'
          Flat = True
          Glyph.Data = {
            
            DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
          NumGlyphs = 2
          OnClick = sbnMarcarColunasClick
        end
        object sbnDesmarcarColunas: TSpeedButton
          Left = 588
          Top = 34
          Width = 22
          Height = 22
          Hint = 'Desmarcar Agentes'
          Flat = True
          Glyph.Data = {
            
            DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
          NumGlyphs = 2
          OnClick = sbnDesmarcarColunasClick
        end
        object clbColunas: TCheckListBox
          Left = 2
          Top = 9
          Width = 584
          Height = 47
          Columns = 4
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ItemHeight = 15
          Items.Strings = (
            'Em Estoque'
            'Reservado'
            'Tr'#226'nsito'
            'Demonstra'#231#227'o'
            'Conserto'
            'Danificada'
            'Rerserva Pr'#233'via'
            'Estoque F'#237'sico')
          ParentFont = False
          Style = lbOwnerDrawFixed
          TabOrder = 0
        end
      end
      object gbxPeriodo: TGroupBox
        Left = 420
        Top = 12
        Width = 151
        Height = 52
        Caption = 'DATAS PARA OS TERMOS'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 2
        object gbxAbertura: TGroupBox
          Left = 1
          Top = 14
          Width = 75
          Height = 36
          Caption = 'ABERTURA'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 0
          object edtDataInicial: TEditData
            Left = 2
            Top = 10
            Width = 70
            Height = 23
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            MaxLength = 10
            ParentFont = False
            TabOrder = 0
            Minimo = 37353
            Maximo = 0
          end
        end
        object gbxFechamento: TGroupBox
          Left = 74
          Top = 14
          Width = 75
          Height = 36
          Caption = 'FECHAMENTO'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 1
          object edtDataFinal: TEditData
            Left = 2
            Top = 10
            Width = 70
            Height = 23
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            MaxLength = 10
            ParentFont = False
            TabOrder = 0
            Minimo = 37353
            Maximo = 0
          end
        end
      end
      object ckbComPrecodeVenda: TCheckBox
        Left = 8
        Top = 288
        Width = 138
        Height = 26
        Caption = 'Com Pre'#231'o de Venda'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 7
      end
      object gbxCSTIcms: TGroupBox
        Left = 9
        Top = 363
        Width = 417
        Height = 36
        Caption = ' CST ICMS - C'#211'DIGO DA SITUA'#199#195'O TRIBUT'#193'RIA '
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 12
        object sbnCOnsultaIncidencia: TSpeedButton
          Left = 37
          Top = 11
          Width = 23
          Height = 22
          Hint = 'Procurar Classes'
          Flat = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          Glyph.Data = {
            
            DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
          NumGlyphs = 2
          ParentFont = False
          OnClick = sbnCOnsultaIncidenciaClick
        end
        object edfIncidencia: TtecDbEditFind
          Left = 2
          Top = 10
          Width = 33
          Height = 23
          Alignment = taLeftJustify
          DataField = 'codigo'
          DataSource = dtmrelatorioinventario.dsrProcuraIncidencia
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          Group = 'Incidencia'
          MaxLength = 3
          Maximo = 0
          Minimo = 37353
          Adicional = 0
          Opcional = True
          ParentFont = False
          TabOrder = 0
          PermitirZero = False
          PermitirNulo = False
          Operacao = opATRIBUICAO
          Parameter = 'incidencia'
          ActiveSetControls = True
          DenyInsert = True
          NoSetControls = <>
          SetControls = <>
        end
        object dtxIncidencia: TtecDBText
          Left = 61
          Top = 10
          Width = 353
          Height = 23
          TabStop = False
          Color = clBtnFace
          DataField = 'descricao'
          DataSource = dtmrelatorioinventario.dsrProcuraIncidencia
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 1
          Alignment = taLeftJustify
        end
      end
      object gbxTiposProduto: TGroupBox
        Left = 6
        Top = 474
        Width = 615
        Height = 84
        Caption = ' TIPOS DE PRODUTOS '
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 15
        object sbnMarcarTipodeProdutos: TSpeedButton
          Left = 589
          Top = 11
          Width = 22
          Height = 22
          Hint = 'Marcar Agentes'
          Flat = True
          Glyph.Data = {
            
            DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
          NumGlyphs = 2
          OnClick = sbnMarcarTipodeProdutosClick
        end
        object sbnDesmarcarTipodeProdutos: TSpeedButton
          Left = 589
          Top = 34
          Width = 22
          Height = 22
          Hint = 'Desmarcar Agentes'
          Flat = True
          Glyph.Data = {
            
            DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
          NumGlyphs = 2
          OnClick = sbnDesmarcarTipodeProdutosClick
        end
        object clbTiposProdutos: TCheckListBox
          Left = 2
          Top = 11
          Width = 585
          Height = 70
          Columns = 3
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ItemHeight = 15
          Items.Strings = (
            '00 - Mercadoria para revenda'
            '01 - Mat'#233'ria-prima'
            '02 - Embalagem'
            '03 - Produto em processo'
            '04 - Produto acabado'
            '05 - Subproduto'
            '06 - Produto intermedi'#225'rio'
            '07 - Material de uso e consumo'
            '08 - Ativo imobilizado'
            '09 - Servi'#231'os'
            '10 - Outros insumos'
            '99 - Outros')
          ParentFont = False
          Style = lbOwnerDrawFixed
          TabOrder = 0
        end
      end
      object gbxCompostos: TGroupBox
        Left = 8
        Top = 316
        Width = 135
        Height = 38
        Caption = ' COMPOSTOS '
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 8
        object cbbCompostos: TComboBox
          Left = 5
          Top = 12
          Width = 126
          Height = 23
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ItemHeight = 15
          ItemIndex = 0
          ParentFont = False
          TabOrder = 0
          Text = 'Com compostos'
          Items.Strings = (
            'Com compostos'
            'Sem compostos'
            'Somente compostos')
        end
      end
      object pgcProdutos: TtecPageControl
        Left = 8
        Top = 155
        Width = 609
        Height = 54
        ActivePage = tstProdutos
        TabOrder = 5
        TabWidth = 90
        OnChange = pgcProdutosChange
        object tstProdutos: TTabSheet
          Caption = ' &Produto'
          ImageIndex = 2
          inline fraConsultaProduto: TfraConsultaCodigo
            Left = 0
            Top = 0
            Width = 601
            Height = 27
            HorzScrollBar.Range = 529
            VertScrollBar.Range = 23
            Align = alTop
            AutoScroll = False
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            ParentFont = False
            TabOrder = 0
            inherited sbnProcura: TSpeedButton
              Left = 136
            end
            inherited dtxDescricao: TtecDBText
              Left = 159
              Width = 714
              DataField = 'descricao'
              DataSource = fraConsultaProduto.dsrProcuraProdutos
            end
            inherited edfCodigo: TtecDBFindLookup
              Width = 135
              DataaFieldInterno = 'caracteristica'
              DataaFieldVisual = 'caracteristicavisual'
              DataField = 'caracteristicavisual'
              DataSource = fraConsultaProduto.dsrProcuraProdutos
              Group = 'Produto'
              MaxLength = 16
              OnFound = fraConsultaProdutoedfCodigoFound
              Operacao = opPESQUISA
              Parameter = 'caracteristicavisual'
              ActiveSetControls = False
              DenyInsert = True
              NoSetControls = <>
              SetControls = <>
            end
          end
        end
        object tstGrupodeProdutos: TTabSheet
          Caption = ' &Grupo'
          ImageIndex = 1
          object sbnConsultaGrupoProdutos: TSpeedButton
            Left = 62
            Top = 1
            Width = 23
            Height = 23
            Hint = 'Procurar grupos de Produtos'
            Flat = True
            Glyph.Data = {
              
              DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
            NumGlyphs = 2
            OnClick = sbnConsultaGrupoProdutosClick
          end
          object edfGrupoProduto: TtecDbEditFind
            Left = 2
            Top = 1
            Width = 60
            Height = 23
            Alignment = taLeftJustify
            CharCase = ecUpperCase
            DataField = 'codigo'
            DataSource = dtmrelatorioinventario.dsrProcuraGrupoProdutos
            Group = 'Grupo'
            MaxLength = 3
            Maximo = 0
            Minimo = 37353
            Adicional = 0
            Opcional = True
            OnFound = edfGrupoProdutoFound
            TabOrder = 0
            PermitirZero = False
            PermitirNulo = False
            Operacao = opATRIBUICAO
            Parameter = 'codigo'
            ActiveSetControls = False
            DenyInsert = True
            NoSetControls = <>
            SetControls = <>
          end
          object dtxGrupoProdutos: TtecDBText
            Left = 86
            Top = 1
            Width = 370
            Height = 24
            TabStop = False
            Color = clBtnFace
            DataField = 'descricao'
            DataSource = dtmrelatorioinventario.dsrProcuraGrupoProdutos
            ReadOnly = True
            TabOrder = 1
            Alignment = taLeftJustify
          end
        end
        object tstClasses: TTabSheet
          Caption = ' Cla&sse'
          ImageIndex = 3
          object sbnConsultaClasse: TSpeedButton
            Left = 62
            Top = 1
            Width = 23
            Height = 23
            Hint = 'Procurar Classes'
            Flat = True
            Glyph.Data = {
              
              DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
            NumGlyphs = 2
            OnClick = sbnConsultaClasseClick
          end
          object edfClasse: TtecDbEditFind
            Left = 2
            Top = 1
            Width = 60
            Height = 23
            Alignment = taLeftJustify
            DataField = 'codigo'
            DataSource = dtmrelatorioinventario.dsrProcuraClasses
            Group = 'Classe'
            MaxLength = 3
            Maximo = 0
            Minimo = 37353
            Adicional = 0
            Opcional = True
            OnFound = edfIncidenciaFound
            TabOrder = 0
            PermitirZero = False
            PermitirNulo = False
            Operacao = opATRIBUICAO
            Parameter = 'codigo'
            ActiveSetControls = False
            DenyInsert = True
            NoSetControls = <>
            SetControls = <>
          end
          object dtxClasse: TtecDBText
            Left = 85
            Top = 1
            Width = 370
            Height = 24
            TabStop = False
            Color = clBtnFace
            DataField = 'descricao'
            DataSource = dtmrelatorioinventario.dsrProcuraClasses
            ReadOnly = True
            TabOrder = 1
            Alignment = taLeftJustify
          end
        end
        object tstlocalizacao: TTabSheet
          Caption = '&Localiza'#231#227'o'
          ImageIndex = 3
          object edtLocalizacao: TEditTexto
            Left = 2
            Top = 1
            Width = 453
            Height = 24
            MaxLength = -1
            TabOrder = 0
            CharCasenoDefault = False
            Alignment = taLeftJustify
          end
        end
      end
      object stxRelatorio: TStaticText
        Left = 13
        Top = 12
        Width = 60
        Height = 16
        Caption = ' RELAT'#211'RIO'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 16
      end
      object stxCusto: TStaticText
        Left = 16
        Top = 218
        Width = 39
        Height = 16
        Caption = ' CUSTO'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 17
      end
      object stxOrdenacao: TStaticText
        Left = 305
        Top = 219
        Width = 66
        Height = 16
        Caption = ' ORDENA'#199#195'O'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 18
      end
      object stxResumo: TStaticText
        Left = 305
        Top = 278
        Width = 47
        Height = 16
        Caption = ' RESUMO'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 19
      end
    end
  end
  object ecvValida: TtecEditionControlValidation
    EditionControl = <
      item
        Control = edtData
      end
      item
        Control = edtMaximo
      end
      item
        Control = edtPagina
      end
      item
        Control = edtDataFinal
      end
      item
        Control = edtDataInicial
      end>
    Left = 224
    Top = 8
  end
  object svdExportarExcel: TSaveDialog
    DefaultExt = 'xls'
    Filter = '*.xls'
    Title = 'Exportar Relat'#243'rio Curva ABC para Excel'
    Left = 256
    Top = 8
  end
end
