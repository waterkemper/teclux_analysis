unit dmdefinicaomontador;

interface

uses
  SysUtils, Classes, dmbasico, DB, cpdatasource, ZQuery, ZPgSqlQuery,
  cpquery, clparametrossistema,
  ctconstantes, dmtecsoft, biblio, ZTransact;

type
  TdtmDefinicaoMontador = class(TdtmBasico)
    ...
    qryMontadorSelecao: TtecQuery;
    dsrMontadorSelecao: TtecDataSource;
    qryMontadorSelecaocodigo: TIntegerField;
    qryMontadorSelecaonome: TStringField;
    qryMontadorSelecaotipo: TStringField;
    ...
    qryRequisicoesmarcar: TBooleanField;
    ...
    qryRequisicoesvalor: TFloatField;
  ...
  private
    ...
    FRequisicaoSemMontador: Boolean;
    FOnScrollLinhaColunaGradeRequisicoesItens: TNotifyEvent;
    ...
    function GetColunadaGradeRequisicoesItens: String;
    function GetLinhadaGradeRequisicoesItens: String;
  ...
  public
    ...
    property RequisicaoSemMontador: Boolean read FRequisicaoSemMontador write FRequisicaoSemMontador;
    ...
    procedure GerarDefinicaoMontadorRequisicoes;

    property OnScrollLinhaColunaGradeRequisicoesItens    : TNotifyEvent read FOnScrollLinhaColunaGradeRequisicoesItens write FOnScrollLinhaColunaGradeRequisicoesItens;
    property LinhadaGradeRequisicoesItens: String read GetLinhadaGradeRequisicoesItens;
    property ColunadaGradeRequisicoesItens: String read GetColunadaGradeRequisicoesItens;
...
var
  dtmDefinicaoMontador: TdtmDefinicaoMontador;

implementation

{$R *.dfm}
const
 WhereFiliaisMontagem = 41;
 WhereFiliaisVenda    = 42;
 WhereCliente         = 43;
 WhereMontador        = 44;

{ TdtmDefinicaoMontador }

procedure TdtmDefinicaoMontador.AbreConsultaCliente;
begin
 ExisteCliente('nome', '0')
end;

procedure TdtmDefinicaoMontador.AbreTabelaPesquisa(
...
constructor TdtmDefinicaoMontador.Create(AOwner: TComponent);
begin
  inherited;
  ...
  qryMontadorSelecao.tag       := ctTabelas;
  ...
  qryMontadorSelecao.Params[1].AsString := 'U';
...
function TdtmDefinicaoMontador.ExisteCliente(NomeCampo: String;
...
function TdtmDefinicaoMontador.ExisteFilial(NomeCampo: String;
  Value: Variant): Boolean;
begin
  Result:=ExisteCodigo(qryConsultaFiliais, Nomecampo, Value);
end;

function TdtmDefinicaoMontador.ExisteGrupoFilial(NomeCampo: String;
  Value: Variant): Boolean;
begin
  Result:=ExisteCodigo(qryConsultaGrupoFiliais, Nomecampo, Value);
end;

function TdtmDefinicaoMontador.ExisteMontador(NomeCampo: String;
  Value: Variant): Boolean;
begin
  Result:=ExisteCodigo(qryConsultaMontador, Nomecampo, Value);
end;

procedure TdtmDefinicaoMontador.FechaTabelaPesquisa(
  TipoPesquisa: TipoProcuraRequisicoes);
begin
  case TipoPesquisa of
   tpRequisicoesFiliaisdeMontagem,
...
function TdtmDefinicaoMontador.GetConsultaClientes: TZDataSet;
begin
result:=qryConsultaClientes;
end;

function TdtmDefinicaoMontador.GetConsultaFiliais: TZDataSet;
begin
result:=qryConsultaFiliais;
end;

function TdtmDefinicaoMontador.GetConsultaGrupoFiliais: TZDataSet;
begin
result:=qryConsultaGrupoFiliais;
end;

function TdtmDefinicaoMontador.GetConsultaMontadores: TZDataSet;
begin
result := qryConsultaMontador;
end;

function TdtmDefinicaoMontador.GetTotalRegistros: Integer;
begin
  result := qryRequisicoes.RecordCount;
end;

procedure TdtmDefinicaoMontador.Selecionar(
  TipoPesquisa: TipoProcuraRequisicoes);
begin
case TipoPesquisa of
 ...
 begin
  ...
  RefazConsulta(qryMontadorSelecao,[0,1], [qryConsultaMontadorcodigo.AsVariant,

                                           qryConsultaMontadortipo.AsVariant]);

  qryMontadorSelecao.Params[1].AsString:=qryConsultaMontadortipo.AsString;
...
procedure TdtmDefinicaoMontador.SetCliente(const Value: String);
begin
 FCliente := Value;
 if Value <> '' then
...
procedure TdtmDefinicaoMontador.SetDataFinal(const Value: string);
begin
  FDataFinal := Value;
  ...
  qryRequisicoes.parambyname('DataInicial').asstring:=FDataInicial;
  qryRequisicoes.parambyname('DataFinal').asstring:=FDataFinal;

end;

procedure TdtmDefinicaoMontador.SetDataInicial(const Value: string);
begin
  FDataInicial := Value;
end;

procedure TdtmDefinicaoMontador.SetFilialdeMontagem(const Value: String);
begin
  FFilialdeMontagem := Value;
  if Value <> '' then
    qryRequisicoes.Sql[WhereFiliaisMontagem]:=' and (r.filialMontagem = ' + Value + ')'
  else
    qryRequisicoes.Sql[WhereFiliaisMontagem]:='';
end;

procedure TdtmDefinicaoMontador.SetFilialdeVenda(const Value: String);
begin
  FFilialdeVenda := Value;
  if Value <> '' then
    qryRequisicoes.Sql[WhereFiliaisVenda]:=
    ' and ((select c.filialvenda from contratos c where c.numero='+
    '(select df.contrato from dadosfiscais df where df.numero=r.dadofiscal)'+
    ')= ' + Value + ')'
  else
    qryRequisicoes.Sql[WhereFiliaisVenda]:='';
end;

procedure TdtmDefinicaoMontador.SetGrupoFilialdeMontagem(
  const Value: String);
begin
...
procedure TdtmDefinicaoMontador.SetGrupoFilialdeVenda(const Value: String);
begin
  ...
  if (Value <> '') then
   qryRequisicoes.Sql[WhereFiliaisVenda] :=
    ' and ((select c.filialvenda from contratos c where c.numero='+
    '(select df.contrato from dadosfiscais df where df.numero=r.dadofiscal)'+
    ') in (select filial from filiaisgruposfiliais where grupo = ' + Value + '))';
end;

procedure TdtmDefinicaoMontador.SetMontador(const Value: String);
begin
  FMontador := Value;
  if FRequisicaoSemMontador then
    qryRequisicoes.Sql[WhereMontador]:=' and (r.montador is null)'
  else
   if Value <> '' then
...
procedure TdtmDefinicaoMontador.SetCodigoMontador(const Value: string);
begin
  FCodigoMontador := Value;
  if value<>'' then
   FTipoMontador:=qryMontadorSelecaotipo.AsString
  else
   FTipoMontador:='';
end;

procedure TdtmDefinicaoMontador.qryRequisicoesAfterScroll(
  DataSet: TDataSet);
begin
  inherited;
  if not qryRequisicoes.ControlsDisabled then
  begin
   ReFazConsulta(qryRequisicoesItens, [0], [qryRequisicoesrequisicao.AsVariant]);
   RefazConsulta(qryProdutosContratos, [0], [qryRequisicoescontrato.AsVariant]);
   RefazConsulta(qryProdutosDadosFiscais, [0], [qryRequisicoesdadofiscal.AsVariant]);
   RefazConsulta(qryClientesRequisicoes, [0,1], [qryRequisicoescliente.AsVariant,
                                     qryRequisicoestipocliente.AsVariant]);
  end;
end;

procedure TdtmDefinicaoMontador.GerarDefinicaoMontadorRequisicoes;
begin
  try
   qryRequisicoes.DisableControls;
   qryRequisicoes.First;
   while not qryRequisicoes.Eof do
   begin
    if qryRequisicoesmarcar.AsBoolean then
    begin
     qryRequisicoes.Edit;
     qryRequisicoesmontador.asstring:=CodigoMontador;
     qryRequisicoestipomontador.asstring:=TipoMontador;
     qryRequisicoes.Post;
    end;
    qryRequisicoes.Next;
   end;
   Perpetrar([qryRequisicoes]);
  finally
   qryRequisicoes.EnableControls;
  end;
end;

procedure TdtmDefinicaoMontador.MarcarSelecionados(Marcando,
  Todos: Boolean);
begin
  MarcarRegistros(qryRequisicoes,
                  qryRequisicoesmarcar,
                  qryRequisicoesvalor,
                  Marcando,
                  Todos);
end;

function TdtmDefinicaoMontador.GetColunadaGradeRequisicoesItens: String;
begin
  result := PrimeiraLetraEmMaiuscula(qryRequisicoesItenscoluna.AsString)
end;

function TdtmDefinicaoMontador.GetLinhadaGradeRequisicoesItens: String;
begin
  result := PrimeiraLetraEmMaiuscula(qryRequisicoesItenslinha.AsString)
end;

procedure TdtmDefinicaoMontador.qryRequisicoesItensAfterScroll(
  DataSet: TDataSet);
begin
  inherited;
  if Assigned(FOnScrollLinhaColunaGradeRequisicoesItens) then
   FOnScrollLinhaColunaGradeRequisicoesItens(DataSet);

end;

function TdtmDefinicaoMontador.GetColunadaGradeProdutosContratos: String;
begin
  result := PrimeiraLetraEmMaiuscula(qryProdutosContratoscoluna.AsString)
end;

function TdtmDefinicaoMontador.GetLinhadaGradeProdutosContratos: String;
begin
  result := PrimeiraLetraEmMaiuscula(qryProdutosContratoslinha.AsString)
end;

procedure TdtmDefinicaoMontador.qryProdutosContratosAfterScroll(
  DataSet: TDataSet);
begin
  inherited;
...
function TdtmDefinicaoMontador.GetColunadaGradeProdutosDadosFiscais: String;
begin
  result := PrimeiraLetraEmMaiuscula(qryProdutosDadosFiscaiscoluna.AsString)
end;

function TdtmDefinicaoMontador.GetLinhadaGradeProdutosDadosFiscais: String;
begin
  result := PrimeiraLetraEmMaiuscula(qryProdutosDadosFiscaislinha.AsString)
end;

procedure TdtmDefinicaoMontador.qryProdutosDadosFiscaisAfterScroll(
  DataSet: TDataSet);
begin
  inherited;
