inherited frmRelatorioRequisicoesporMontador: TfrmRelatorioRequisicoesporMontador
  Left = 203
  Top = 172
  ActiveControl = pgcFilialGrupoFilial
  Caption = 'Relat'#243'rio de Requisi'#231#245'es de Montagens'
  ClientHeight = 341
  ClientWidth = 792
  FormStyle = fsMDIChild
  Visible = True
  PixelsPerInch = 96
  TextHeight = 16
  inherited pnlBarra: TToolBar
    Width = 792
  end
  inherited pnlFundoJanela: TPanel
    Top = 41
    Width = 792
    Height = 300
    object gbxCliente: TGroupBox
      Left = 5
      Top = 91
      Width = 380
      Height = 57
      Caption = 'Cliente'
      TabOrder = 1
      object sbnProcuraCliente: TSpeedButton
        Left = 74
        Top = 24
        Width = 23
        Height = 23
        Hint = 'Procurar Cliente'
        Flat = True
        Glyph.Data = {
          
          DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
        NumGlyphs = 2
        OnClick = sbnProcuraClienteClick
      end
      object dtxCliente: TtecDBText
        Left = 102
        Top = 24
        Width = 267
        Height = 24
        TabStop = False
        Color = clBtnFace
        DataField = 'nome'
        DataSource = dtmRelatorioRequisicoesporMontador.dsrCliente
        ReadOnly = True
        TabOrder = 0
        Alignment = taLeftJustify
      end
      object edfCliente: TtecDbEditFind
        Left = 8
        Top = 24
        Width = 65
        Height = 24
        Alignment = taLeftJustify
        DataField = 'codigo'
        DataSource = dtmRelatorioRequisicoesporMontador.dsrCliente
        Group = 'Cliente'
        MaxLength = 6
        Maximo = 0
        Minimo = 37353
        Adicional = 0
        Opcional = True
        TabOrder = 1
        PermitirZero = False
        PermitirNulo = False
        Operacao = opATRIBUICAO
        Parameter = 'codigo'
        ActiveSetControls = False
        DenyInsert = True
        NoSetControls = <>
        SetControls = <>
      end
    end
    object gbxPeriodo: TGroupBox
      Left = 397
      Top = 8
      Width = 256
      Height = 54
      Caption = 'Per'#237'odo de Abertura'
      TabOrder = 3
      object lblVencimentoInicial: TLabel
        Left = 10
        Top = 26
        Width = 33
        Height = 16
        Alignment = taRightJustify
        Caption = 'Inicial'
      end
      object lblVencimentoFinal: TLabel
        Left = 137
        Top = 26
        Width = 28
        Height = 16
        Alignment = taRightJustify
        Caption = 'Final'
      end
      object edtDataInicial: TEditData
        Left = 45
        Top = 22
        Width = 75
        Height = 24
        MaxLength = 10
        TabOrder = 1
        Minimo = 37353
        Maximo = 0
      end
      object edtDataFinal: TEditData
        Left = 169
        Top = 22
        Width = 75
        Height = 24
        MaxLength = 10
        TabOrder = 0
        Minimo = 37353
        Maximo = 0
      end
    end
    object gbxMontador: TGroupBox
      Left = 5
      Top = 159
      Width = 380
      Height = 57
      Caption = 'Montador'
      TabOrder = 2
      object sbnProcuraMontador: TSpeedButton
        Left = 66
        Top = 24
        Width = 23
        Height = 23
        Hint = 'Procurar Montador'
        Flat = True
        Glyph.Data = {
          
          DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
        NumGlyphs = 2
        OnClick = sbnProcuraMontadorClick
      end
      object dtxMontador: TtecDBText
        Left = 94
        Top = 24
        Width = 275
        Height = 24
        TabStop = False
        Color = clBtnFace
        DataField = 'nome'
        DataSource = dtmRelatorioRequisicoesporMontador.dsrMontador
        ReadOnly = True
        TabOrder = 0
        Alignment = taLeftJustify
      end
      object edfMontador: TtecDbEditFind
        Left = 8
        Top = 24
        Width = 57
        Height = 24
        Alignment = taLeftJustify
        DataField = 'codigo'
        DataSource = dtmRelatorioRequisicoesporMontador.dsrMontador
        Group = 'Montador'
        MaxLength = 6
        Maximo = 0
        Minimo = 37353
        Adicional = 0
        Opcional = True
        TabOrder = 1
        PermitirZero = False
        PermitirNulo = False
        Operacao = opATRIBUICAO
        Parameter = 'codigo'
        ActiveSetControls = False
        DenyInsert = True
        NoSetControls = <>
        SetControls = <>
      end
    end
    object rgpResumo: TRadioGroup
      Left = 661
      Top = 191
      Width = 124
      Height = 104
      Caption = 'Resumo'
      ItemIndex = 0
      Items.Strings = (
        'Com R&esumo'
        '&Sem Resumo'
        'S'#243' Res&umo')
      TabOrder = 10
    end
    object rgpSituacaoPagto: TRadioGroup
      Left = 397
      Top = 174
      Width = 124
      Height = 121
      Caption = 'Pagamento'
      ItemIndex = 3
      Items.Strings = (
        '&em Aberto'
        '&Parcial'
        '&Fechada'
        '&Todos')
      TabOrder = 6
    end
    object rgpSituacaodaMontagem: TRadioGroup
      Left = 533
      Top = 174
      Width = 120
      Height = 121
      Caption = 'Montagem'
      ItemIndex = 3
      Items.Strings = (
        '&em Aberto'
        '&Parcial'
        '&Fechada'
        '&Todos')
      TabOrder = 7
    end
    object pgcFilialGrupoFilial: TtecPageControl
      Left = 5
      Top = 8
      Width = 377
      Height = 75
      ActivePage = tstFilial
      TabOrder = 0
      OnChange = pgcFilialGrupoFilialChange
      object tstFilial: TTabSheet
        Caption = 'Filia&l'
        object sbnConsultaFilial: TSpeedButton
          Left = 67
          Top = 12
          Width = 23
          Height = 23
          Hint = 'Procurar Filiais'
          Flat = True
          Glyph.Data = {
            
            DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
          NumGlyphs = 2
          OnClick = sbnConsultaFilialClick
        end
        object edfFilial: TtecDbEditFind
          Left = 6
          Top = 11
          Width = 60
          Height = 24
          Alignment = taLeftJustify
          DataField = 'codigo'
          DataSource = dtmRelatorioRequisicoesporMontador.dsrFiliais
          Group = 'Filial'
          MaxLength = 3
          Maximo = 0
          Minimo = 37353
          Adicional = 0
          Opcional = True
          TabOrder = 0
          PermitirZero = False
          PermitirNulo = False
          Operacao = opATRIBUICAO
          Parameter = 'codigo'
          ActiveSetControls = False
          DenyInsert = True
          NoSetControls = <>
          SetControls = <>
        end
        object dtxFilial: TtecDBText
          Left = 94
          Top = 12
          Width = 267
          Height = 24
          TabStop = False
          Color = clBtnFace
          DataField = 'nome'
          DataSource = dtmRelatorioRequisicoesporMontador.dsrFiliais
          ReadOnly = True
          TabOrder = 1
          Alignment = taLeftJustify
        end
      end
      object tstGrupoFilial: TTabSheet
        Caption = '&Grupo Filial'
        ImageIndex = 1
        object sbnConsultaGrupoFilial: TSpeedButton
          Left = 67
          Top = 12
          Width = 23
          Height = 23
          Hint = 'Procurar grupos de filiais'
          Flat = True
          Glyph.Data = {
            
            DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
          NumGlyphs = 2
          OnClick = sbnConsultaGrupoFilialClick
        end
        object edfGrupoFilial: TtecDbEditFind
          Left = 6
          Top = 11
          Width = 60
          Height = 23
          Alignment = taLeftJustify
          DataField = 'codigo'
          DataSource = dtmRelatorioRequisicoesporMontador.dsrGrupoFiliais
          Group = 'GrupoFilial'
          MaxLength = 3
          Maximo = 0
          Minimo = 37353
          Adicional = 0
          Opcional = True
          TabOrder = 0
          PermitirZero = False
          PermitirNulo = False
          Operacao = opATRIBUICAO
          Parameter = 'codigo'
          ActiveSetControls = False
          DenyInsert = True
          NoSetControls = <>
          SetControls = <>
        end
        object dtxGrupoFilial: TtecDBText
          Left = 91
          Top = 12
          Width = 270
          Height = 21
          TabStop = False
          Color = clBtnFace
          DataField = 'descricao'
          DataSource = dtmRelatorioRequisicoesporMontador.dsrGrupoFiliais
          ReadOnly = True
          TabOrder = 1
          Alignment = taLeftJustify
        end
      end
    end
    object gbxAgrupamento: TGroupBox
      Left = 661
      Top = 80
      Width = 124
      Height = 110
      Caption = 'Agrupamento'
      TabOrder = 9
      object ckbAgruparGrupoFilial: TCheckBox
        Left = 6
        Top = 16
        Width = 115
        Height = 25
        Caption = '&Grupo de Filial'
        TabOrder = 0
      end
      object ckbAgruparFilial: TCheckBox
        Left = 6
        Top = 45
        Width = 115
        Height = 25
        Caption = '&Filial'
        Checked = True
        State = cbChecked
        TabOrder = 1
      end
      object ckbAgruparMontador: TCheckBox
        Left = 6
        Top = 74
        Width = 115
        Height = 25
        Caption = '&Montador'
        Checked = True
        State = cbChecked
        TabOrder = 2
      end
    end
    object gbxItensRequisicoes: TGroupBox
      Left = 661
      Top = 8
      Width = 124
      Height = 65
      Caption = 'Itens'
      TabOrder = 8
      object ckbRequisicoesItensMontagemFechada: TCheckBox
        Left = 6
        Top = 15
        Width = 115
        Height = 24
        Caption = 'Montados'
        TabOrder = 0
      end
      object ckbRequisicoesItensPagtoFechado: TCheckBox
        Left = 6
        Top = 38
        Width = 99
        Height = 24
        Caption = 'Pagos'
        TabOrder = 1
      end
    end
    object gbxPeriodoPagamento: TGroupBox
      Left = 397
      Top = 117
      Width = 256
      Height = 54
      Caption = 'Per'#237'odo de Pagamento'
      TabOrder = 5
      object Label1: TLabel
        Left = 10
        Top = 26
        Width = 33
        Height = 16
        Alignment = taRightJustify
        Caption = 'Inicial'
      end
      object Label2: TLabel
        Left = 137
        Top = 26
        Width = 28
        Height = 16
        Alignment = taRightJustify
        Caption = 'Final'
      end
      object edtDataInicialPagamento: TEditData
        Left = 45
        Top = 22
        Width = 75
        Height = 24
        MaxLength = 10
        TabOrder = 1
        Minimo = 37353
        Maximo = 0
      end
      object edtDataFinalPagamento: TEditData
        Left = 169
        Top = 22
        Width = 75
        Height = 24
        MaxLength = 10
        TabOrder = 0
        Minimo = 37353
        Maximo = 0
      end
    end
    object GroupBox1: TGroupBox
      Left = 397
      Top = 63
      Width = 256
      Height = 54
      Caption = 'Per'#237'odo de Fechamento'
      TabOrder = 4
      object Label3: TLabel
        Left = 10
        Top = 26
        Width = 33
        Height = 16
        Alignment = taRightJustify
        Caption = 'Inicial'
      end
      object Label4: TLabel
        Left = 137
        Top = 26
        Width = 28
        Height = 16
        Alignment = taRightJustify
        Caption = 'Final'
      end
      object edtDataInicialFechamento: TEditData
        Left = 45
        Top = 22
        Width = 75
        Height = 24
        MaxLength = 10
        TabOrder = 1
        Minimo = 37353
        Maximo = 0
      end
      object edtDataFinalFechamento: TEditData
        Left = 169
        Top = 22
        Width = 75
        Height = 24
        MaxLength = 10
        TabOrder = 0
        Minimo = 37353
        Maximo = 0
      end
    end
  end
end
