...
WITH 
  ...
  select
	 ...
	 cast(''C'' as char(3)) as tipoconta,
         True as totalizar,
         cast(''Licitações Ganhas - Venda''  as varchar(100)) as conta,
...
from licitacoes l
  ...
  select
		 ...
		 cast(''C'' as char(3)) as tipoconta,
                 True as totalizar,
         cast(''Custo''  as varchar(100)) as conta,
  ...
  Lucro_Bruto_Prejuizo AS 
    ...
    (select L_G_V.ano_mes,
	      cast(''RT'' as char(3)) as tipoconta,
              True as totalizar,
          cast(''LUCRO BRUTO / PREJUIZO''  as varchar(100)) as conta,      
  ...
  select 
       ...
       cast(''C'' as char(3)) as tipoconta,
       False as totalizar,
       cast(''EMISSÃO''  as varchar(100)) as conta,
  ...
  (
	...
	select 
		   ...
		   cast(''C'' as char(3)) as tipoconta,
                   True as totalizar,
		   cast(''Faturamento de Licitações (notas emitidas)''  as varchar(100)) as conta,      
  ...
  (
	...
	select 
		   ...
		   cast(''C'' as char(3)) as tipoconta,
                   True as totalizar,
		   cast(''Faturamento Sem Licitações (notas emitidas)''  as varchar(100)) as conta,      
   ...
   (select faturamento.ano_mes, 
           ...
           True as totalizar, 
	       cast(''TOTAL FATURAMENTO'' as varchar(100)) as conta, 
	...
	select 
		   ...
		   cast(''C'' as char(3)) as tipoconta,
                   True as totalizar,
		   cast(''Custo dos Equipamentos sobre o Faturamento de Licitações (notas emitidas)''  as varchar(100)) as conta,      
		   cast(14 as smallint) as ordem,
	...
	select 
		   ...
		   cast(''C'' as char(3)) as tipoconta,
                   True as totalizar,
		   cast(''Custo dos Equipamentos sobre o Faturamento sem Licitações (notas emitidas)''  as varchar(100)) as conta,      
   ...
   (select Total_Custo_Eqptos_Faturamento.ano_mes, 
           cast(''T'' as char(3)) as tipoconta,
           True as totalizar, 
	       cast(''TOTAL CUSTO EQUIPAMENTO SOBRE O FATURAMENTO'' as varchar(100)) as conta, 
	...
	select 
		   ...
		   cast(''C'' as char(3)) as tipoconta,
                   True as totalizar, 
		   cast(''Fretes sob Licitações (notas emitidas)''  as varchar(100)) as conta,      
	...
	select
		   ...
		   cast(''C'' as char(3)) as tipoconta,
                   True as totalizar,
		   cast(''Fretes sem Licitações (notas emitidas)''  as varchar(100)) as conta,
   ...
   (
	...
	select
		   ...
		   cast(''C'' as char(3)) as tipoconta,
                   True as totalizar,
		   cast(''Comissão Sobre Licitações ''||u.nome  as varchar(100)) as conta,
	...
	(
	  select ano_mes, 
	         cast(''T'' as char(3)) as tipoconta,
                 True as totalizar,
	         cast(''TOTAL COMISSÕES SOBRE LICITAÇÕES''  as varchar(100)) as conta,
	...
	(
		   ...
		   cast(''C'' as char(3)) as tipoconta,
                   True as totalizar,
		   cast(''Comissão Sem Licitações ''||u.nome  as varchar(100)) as conta,
	...
	(
	  ...
	  select ano_mes, 
	         cast(''T'' as char(3)) as tipoconta,
                 True as totalizar,
	         cast(''TOTAL COMISSÕES SEM LICITAÇÕES''  as varchar(100)) as conta,
	...
	SELECT 
		   ...
		   cast(''C'' as char(3)) as tipoconta,
                   True as totalizar,
		   cast(''Impostos sobre Faturamento de Licitações - ICMS''  as varchar(100)) as conta,      
	...
	SELECT 
		   ...
		   cast(''C'' as char(3)) as tipoconta,
                   True as totalizar,
		   cast(''Impostos sobre Faturamento de Licitações - IPI''  as varchar(100)) as conta,      
	...
	SELECT
		   ...
		   cast(''C'' as char(3)) as tipoconta,
                   True as totalizar,
		   cast(''Impostos sobre Faturamento de Licitações - PIS''  as varchar(100)) as conta,
	...
	SELECT
		   ...
		   cast(''C'' as char(3)) as tipoconta,
                   True as totalizar,
		   cast(''Impostos sobre Faturamento de Licitações - COFINS''  as varchar(100)) as conta,
	...
	SELECT 
		   ...
		   cast(''C'' as char(3)) as tipoconta,
                   True as totalizar,
		   cast(''TOTAL IMPOSTOS SOBRE FATURAMENTO DE LICITAÇÕES''  as varchar(100)) as conta,      
	...
	SELECT 
		   ...
		   cast(''C'' as char(3)) as tipoconta,
                   True as totalizar,
		   cast(''Impostos sobre Faturamento sem Licitações - ICMS''  as varchar(100)) as conta,      
	...
	SELECT 
		   ...
		   cast(''C'' as char(3)) as tipoconta,
                   True as totalizar,
		   cast(''Impostos sobre Faturamento sem Licitações - IPI''  as varchar(100)) as conta,      
		   cast(43 as smallint) as ordem, 
	...
	SELECT 
		   ...
		   cast(''C'' as char(3)) as tipoconta,
                   True as totalizar,
		   cast(''Impostos sobre Faturamento sem Licitações - PIS''  as varchar(100)) as conta,      
	...
	SELECT 
		   ...
		   cast(''C'' as char(3)) as tipoconta,
                   True as totalizar,
		   cast(''Impostos sobre Faturamento sem Licitações - COFINS''  as varchar(100)) as conta,      
	...
	SELECT 
		   ...
		   cast(''C'' as char(3)) as tipoconta,
                   True as totalizar,
		   cast(''TOTAL IMPOSTOS SOBRE FATURAMENTO SEM LICITAÇÕES''  as varchar(100)) as conta,      
		   cast(48 as smallint) as ordem,
...
/*      and d.autorizado = true*/
      ...
      and e.descricao = ''PREVISÃO DE CRÉDITOS DE IMPOSTOS''
  )  as d
/*  where trim(d.descricao) = ''Investimentos'' */
  ...
  order by to_char(d.datavencto, ''yyyy-MM''), d.descricao
   ...
   (select sel.ano_mes, 
           cast(''T'' as char(3)) as tipoconta,
           True as totalizar,
	       cast(''TOTAL DESPESAS'' as varchar(100)) as conta, 
		   ...
		   (	    
			   ...
			   (select ano_mes, total from Total_Impostos_sobre_Faturamento_de_Licitacoes) union all
                           (select ano_mes, total from pg_eventos_Credito_Previsao_Impostos) union all
   ...
   (select sel.ano_mes, 
           cast(''RT'' as char(3)) as tipoconta,
           True as totalizar,
	       cast(''LUCRO DO EXERCÍCIO'' as varchar(100)) as conta, 
  ...
  select 
       ...
       cast(''C'' as char(3)) as tipoconta,
       False as totalizar,
       cast(''CAIXA''  as varchar(100)) as conta,      
    ...
    (
      ...
      select to_char(mon_last,''YYYY-MM'') as ano_mes,
              cast(''C'' as char(3)) as tipoconta,
              false as totalizar,
      	      cast(''A Receber'' as varchar(100)) as conta,
    ...
    (
           ...
           (
               ...
               (
                  ...
                  where
                     ...
                     (
                      /*  t.datapagto is null */
                        (t.datapagto is null or (t.datapagto between
        ...
        select to_char(mon_last,''YYYY-MM'') as ano_mes,
		      cast(''C'' as char(3)) as tipoconta,
                      False as totalizar,
      	      cast(''À Pagar'' as varchar(100)) as conta, 
  ...
  (
      ...
      /*       c.banco, b.nome, c.agencia, ag.nome, c.conta, c.digito,*/
	         cast(''C'' as char(3)) as tipoconta,
              false as totalizar,   
             cast(b.nome||'' ''||''Ag. ''||ag.nome||'' ''||cast(c.conta as varchar)||''-''||cast(c.digito as varchar) as varchar(100)) as conta,
  ...
  (
     select ano_mes, 
			...
			cast(''T'' as char(3)) as tipoconta,
                        false as totalizar,
            cast(''TOTAL SALDOS BANCÁRIOS E CAIXA CERON'' as varchar(100)) as conta,
  ...
  (
        ...
        /*           d.valorvencto */
            ...
            where dp.emissao <= periodo.mon_last
              and (d.datapagto is null or (d.datapagto between periodo.mon_first and periodo.mon_last))
              and e.incluirnadregerencial
              and dp.previsao = false
              and d.autorizado = true
              and trim(e.descricao) = ''Empréstimos Bancos''
  ...
  order by to_char(d.datavencto, ''yyyy-MM''), d.descricao
    ...
    (
      ...
      select ano_mes,
	       cast(''C'' as char(3)) as tipoconta,
               False as totalizar,
           CAST(''Estoque'' as varchar(100)) as conta,
    ...
    (
        ...
        select to_char(max(periodo.mon_last), ''yyyy-MM'')  as ano_mes,
		              cast(''C'' as char(3)) as tipoconta,
                      False as totalizar,        
                      cast(''Importação (PEDIDOS JÁ PAGOS POR ANTECIPAÇÃO)'' as varchar(100)) as conta, 
    ...
    (
        ...
        Select to_char(p.datapagto, ''yyyy-MM'') as ano_mes,
              ...
              True as totalizar,
      	      cast(''RECEBIMENTOS MÊS'' as varchar(100)) as conta, 
    ...
    (
                  ...
                  Select to_char(t.datapagto, ''yyyy-MM'') as ano_mes,
                            cast(''DT'' as char(3)) as tipoconta,
                            True as totalizar,
                            cast(''PAGAMENTOS MÊS'' as varchar(100)) as conta, 
  ...
  select tipoconta, totalizar, conta, sum(ordem) as ordem, ano_mes, sum(Total) as Total
  from
  ...
  (
     ...
     (
        ...
        (
          ...
          */
          ...
          (select  ano_mes,
                   cast(''C'' as char(3)) as tipoconta,
                   True as totalizar,
                   cast(''Imposto (Sob Faturamento Sem Abatimentos)'' as varchar(100)) as conta,
                   cast(49 as smallint) as ordem,
                   sum(total) as Total
            from
            (
              (select ano_mes, total from Total_Impostos_sobre_Faturamento_de_Licitacoes)                     UNION ALL
              (select ano_mes, total from Total_Impostos_sobre_Faturamento_sem_Licitacoes)
          ...
          (select * from pg_eventos_Credito_Previsao_Impostos) UNION ALL

          (select  ano_mes,
                   cast(''T'' as char(3)) as tipoconta,
                   True as totalizar,
                   cast(''Total Impostos'' as varchar(100)) as conta,
            ...
            (
              (select ano_mes, total from Total_Impostos_sobre_Faturamento_de_Licitacoes)                     UNION ALL
              (select ano_mes, total from Total_Impostos_sobre_Faturamento_sem_Licitacoes)   union all
              (select ano_mes, total from pg_eventos_Credito_Previsao_Impostos)
          ...
          (select  ano_mes,
                   cast(''RT'' as char(3)) as tipoconta,
                   false as totalizar,
                   cast(''TOTAL LÍQUIDO'' as varchar(100)) as conta,
        ...
        select tipoconta,
			   ...
			   sum(case when totalizar then Total else cast(null as numeric(11,2)) end) as Total
         from
        (
