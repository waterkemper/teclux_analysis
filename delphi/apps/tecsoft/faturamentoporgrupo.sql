;(
 ...
 CASE WHEN (COALESCE(TotalLiquidoProdutos,0)<>0) THEN
   (((valorliquidoproduto+ValorAcrescProduto) * 100) / TotalLiquidoProdutos)
 ELSE CAST('0.00' AS numeric) END as Percentual,

 CASE WHEN (COALESCE(lucroporproduto,0)<>0) THEN
   (lucroporproduto)*100/lucro
 ELSE cast('0.00' as numeric) END as percentuallucro,

 CASE WHEN (COALESCE(custo,0)<>0) THEN
   (valorliquidoproduto+ValorAcrescProduto)/custo
 ELSE cast('0.00' as numeric) END as percentualmargem

 FROM
 (
 SELECT VendaseDevolucoes.*,
 CASE WHEN VendasEDevolucoes.ValorBrutodoProduto<> 0 THEN
    ((SUM(VendaseDevolucoes.ValorDescontodoProduto)*100)/SUM(VendasEDevolucoes.ValorBrutodoProduto))
    ELSE cast('0.00' as numeric) END as PercDescontodoProduto,
 ...
 SUM(TotalClasses.ValorBrutodoProduto -
     TotalClasses.ValorDescontodoProduto +
     TotalClasses.ValorAcrescProduto) - SUM(TotalClasses.custodaclasse) as lucro,
...
(
 SELECT
 CASE WHEN :AgruparFilial THEN filial ELSE cast(null as integer) END as filial,
 codigoclasse,
 nomeclasse,
 SUM(quantidade) as quantidade,
 SUM(custo) as custo,
...
(
select produtos.*,
 CASE WHEN NOT ContratoeAPrazo THEN (ValorBrutodoProduto-ValorDescontodoProduto)+ValorAcrescProduto ELSE 0 END AS ValoraVistadoProduto,
 CASE WHEN ContratoeAPrazo THEN (ValorBrutodoProduto-ValorDescontodoProduto)+ValorAcrescProduto ELSE 0 END AS ValoraPrazodoProduto
from
(
...
/*
         ...
         (((coalesce(frete,0)+coalesce(seguro,0))*(produtos.quantidade*precovenda)) /
         ((vvalorvista+coalesce(desconto,0)-coalesce(frete,0)-coalesce(seguro,0)) -
         ...
         (((vValorPrazo-vValorVista)*(produtos.quantidade*precovenda)) /
         ((vvalorvista+coalesce(desconto,0)-coalesce(frete,0)-coalesce(seguro,0)) -
         ...
         */
         ...
         (SELECT EXISTS (SELECT p.contrato
                         ...
                         WHERE p.contrato = vcontrato
                           and p.datavencto > vfaturamento)) as ContratoeaPrazo
from
(
...
(
 ...
 from ((dadosfiscais df
       ...
       join ((produtosdadosfiscais pdf
               left join movimentos m on pdf.movimento = m.numero)
               join (produtos p
...
(
   SELECT produtos.filial,
         ...
         ((produtos.quantidade*precovenda) +
         (((coalesce(frete,0)+coalesce(seguro,0))*(produtos.quantidade*precovenda)) /
         (vvalorvista+coalesce(desconto,0)-coalesce(frete,0)-coalesce(seguro,0)) )) AS ValorBrutodoProduto,

         ((coalesce(desconto,0)*(produtos.quantidade*precovenda)) /
         (vvalorvista+coalesce(desconto,0)-coalesce(frete,0)-coalesce(seguro,0))) as ValorDescontodoProduto,

         (((vValorPrazo-vValorVista)*(produtos.quantidade*precovenda)) /
         (vvalorvista+coalesce(desconto,0)-coalesce(frete,0)-coalesce(seguro,0))) as ValorAcrescProduto,
         */
...
(
   SELECT produtos.filial,
         ...
         (((coalesce(frete,0)+coalesce(seguro,0))*(qtfaturada*precovenda)) /
         ((vvalorvista+coalesce(desconto,0)-coalesce(frete,0)-coalesce(seguro,0)) -
           totalservico))) AS ValorBrutodoProduto,
...
(
 ...
 WHERE ct.faturamento BETWEEN(:DataInicial) and (:DataFinal)
   ...
   and case when ct.os then ct.tipoequipamento in (1,2) and not (coalesce(ct.os_garantia,false) and ct.os_garantia_status='A') and not coalesce(ct.os_cortesia,false) else true end
/*
   and ((ct.situacao <> 'N') or
        (ct.situacao = 'N' and exists (select cd.contrato
...
*/
   ...
   and ((ct.situacao = 'F') or
        (ct.situacao = 'C' and not exists (select df.numero from dadosfiscais df where df.contrato = ct.numero and df.situacao = 'C')) or
        (ct.situacao = 'N' and exists (select cd.contrato
	                               from contratosdevolvidos cd
			   	       where cd.contrato = ct.numero
					 and cd.produto = pc.produto
...
order by pc.contrato, pc.produto, pc.filial
) as produtos left join (select df.contrato, pdf.produto, pdf.filial,
                          ...
                          from ((dadosfiscais df
                                ...
                                on p.caracteristica=c.codigo)
                               on pdf.produto=p.codigo)
                             on df.numero = pdf.dadofiscal), produtoscontratos pc
			   ...
			   %WhereFornecedor
                         ...
                         group by df.contrato, pdf.produto, pdf.filial
...
(
 select devolucoes.filial, codigoclasse, nomeclasse, devolucoes.quantidade, - abs(m.valor) as custo, - abs(devolucoes.creditotroca) as creditotroca,
        codigogrupo, nomegrupo, valorbrutodoproduto,
        Valordescontodoproduto,valoracrescproduto,contratoeaprazo,
        case when not ContratoeAPrazo then (ValorBrutodoProduto-ValorDescontodoProduto)+ValorAcrescProduto else 0 end as ValoraVistadoProduto,
        case when ContratoeAPrazo then (ValorBrutodoProduto-ValorDescontodoProduto)+ValorAcrescProduto else 0 end as ValoraPrazodoProduto
 from
 (
 ...
 from (((contratosdevolvidos cd
       join (contratos ct join notaspag np on ct.numero = np.contrato)
 ...
 ) as devolucoes join movimentos m on devolucoes.vcodigonota = m.codigonota
...
(
 ...
 select devolucoes.filial, codigoclasse, nomeclasse, devolucoes.quantidade, - abs(m.valor) as custo, 0.00 as creditotroca,
        codigogrupo, nomegrupo, valorbrutodoproduto,
        Valordescontodoproduto,valoracrescproduto,contratoeaprazo,
        case when not ContratoeAPrazo then (ValorBrutodoProduto-ValorDescontodoProduto)+ValorAcrescProduto else 0 end as ValoraVistadoProduto,
        case when ContratoeAPrazo then (ValorBrutodoProduto-ValorDescontodoProduto)+ValorAcrescProduto else 0 end as ValoraPrazodoProduto
...
(
 select devolucoes.filial, codigoclasse, nomeclasse, devolucoes.quantidade,
          - abs((CASE WHEN coalesce((EmEstoque + Reservado    +
                               Transito  + Demonstracao +
                               Conserto  + Danificada   +
                                ReservaPrevia),0)=0 THEN 0
           ELSE (financeiro/(EmEstoque + Reservado    +
                               Transito  + Demonstracao +
                               Conserto  + Danificada   +
                                ReservaPrevia))*devolucoes.quantidade
           END)) as custo,
   ...
   SELECT produtos.filial,
          ...
          - abs((CASE WHEN coalesce((EmEstoque + Reservado    +
           ...
           ELSE (financeiro/(EmEstoque + Reservado    +
                               Transito  + Demonstracao +
                               Conserto  + Danificada   +
                                ReservaPrevia))*produtos.quantidade
           END)) as custo,
		   produtos.creditotroca,
          produtos.codigogrupo,
          produtos.nomegrupo,
 ...
 WHERE  ct.numero = pc.contrato
   ...
   and case when ct.os then ct.tipoequipamento in (1,2) and not (coalesce(ct.os_garantia,false) and ct.os_garantia_status='A') and not coalesce(ct.os_cortesia,false) else true end
			   ...
			   AND ct.faturamento BETWEEN(:DataInicial) and (:DataFinal)
                           AND (coalesce(ct.valorvista,0)<>0)
                           and ct.origem IS NULL
                           and case when ct.os then ct.tipoequipamento in (1,2) else true end
...
/*FINAL TOTAL DEVOLUCOES*/
 ...
 group by filial, codigoclasse, nomeclasse, codigogrupo, nomegrupo, quantidade,
