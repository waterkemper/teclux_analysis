CREATE OR REPLACE FUNCTION public.Calcular_Saldo_Adiantamento (
  pDocumentoPag integer, pUltimodia date
)
RETURNS numeric AS
$body$
DECLARE
 ...
 vSaldoDocumento numeric;
 rDocumentosPag record;
BEGIN
      ...
      for rDocumentosPag in
        ...
        where (dp.fornecedor, dp.tipofornecedor) IN
              (select dp1.fornecedor, dp.tipofornecedor
               ...
               where dp1.numero = pDocumentoPag)
           and coalesce(dp.adiantamento,false)	 
        order by dp.emissao, dp.numero
	  loop 
      
	    if vSaldoAbatimento >= rDocumentosPag.valor then
		  vSaldoAbatimento :=  coalesce(vSaldoAbatimento,0) - coalesce(rDocumentosPag.valor,0);
		  vSaldoDocumento := 0;
		else 
		  vSaldoDocumento := coalesce(rDocumentosPag.valor,0) - coalesce(vSaldoAbatimento,0);
 		  vSaldoAbatimento := 0;	
		end if;
        
		if rDocumentosPag.numero = pDocumentoPag then
		  exit;
        end if;  
      end loop;
	  
  end if;  
  RETURN vSaldoDocumento;
END;
$body$
LANGUAGE 'plpgsql';

ALTER FUNCTION public.Calcular_Saldo_Adiantamento (pDocumentoPag integer, pUltimoDia Date)
  OWNER TO tecsoft;



/*
Considerações: descontos concedidos/recebidos nos pagamentos e recebimentos não estão entrando
Devoluções não são tratadas

Custo_Eqptos_Faturamento esta pegando o valordaultimacompra e se não tiver peg o financeiro, porém da tabela atual,
  tabela estoques, e não o da data do faturamento.
Já o estoque pega a ultima entrada (conforme inventário), senão o financeiro. 
...
WITH 
  ...
  where l.datavenda between ''2023-07-01'' and current_date
  ...
  (
  select 
       to_char(cast(''2023-07-01'' as date), ''yyyy-MM'') as ano_mes,
  ...
  (
   ...
   (
	...
	(
	  select ano_mes, cast(''TOTAL COMISSÕES SOBRE LICITAÇÕES''  as varchar(100)) as descricao, 
	...
	(
	  ...
	  select ano_mes, cast(''TOTAL COMISSÕES SEM LICITAÇÕES''  as varchar(100)) as descricao, 
  ...
  from
    ...
    where d.datavencto between ''2023-07-01'' and current_date    
  ...
  select 
    ...
    (
      ...
      (        
          ...
          FROM   generate_series(date ''2023-07-01'', current_date, interval ''1 day'') d
  ...
  order by to_char(d.datavencto, ''yyyy-MM''), d.descricao
    ...
    (
        ...
        (
        select sel.*,
               Calcular_Saldo_Adiantamento(sel.numero, current_date) as Saldo   
        from
        (       
        select sel.*,
...
group by descricao, ano_mes, ordem
...
--select sum(valorvencto) from parcelas where datavencto between '2024-06-01' and '2024-06-30'

--select sum(valorpagto) from parcelas where datapagto between '2024-06-01' and '2024-06-30'



--select * from eventos --
--where tipomovimentacao = 'S'
--where descricao ilike '%INVE%'


--select * from contas

--select saldobanco(555, '2024-06-30','C')