...
(
 ...
 select classe,
   ...
   case when not ContratoeAPrazo then
    (ValorBrutodoProduto-ValorDescontodoProduto)+ValorAcrescProduto
 ...
 select
      ...
      ((pc.quantidade*pc.precovenda) +
        ...
        ratearimpostoretidoproduto_contratos (pc.contrato, pc.produto)
   ...
   from (((contratos ct
		      ...
		      join ((produtoscontratos pc join (produtos p join (caracteristicas c join classes cl
   ...
   where not pc.brinde and
             ct.faturamento between(:DataInicial) and (:DataFinal) and
 ...
 (
  select
   ...
   from ((((dadosfiscais df
         ...
         join vfornecedores vf on df.cliente = vf.codigo and df.tipocliente = vf.tipo)
                ...
                join (servicoscontratos sc
                      join (usuarios u
                            left join GruposUsuarios gu
                             on u.grupo = gu.codigo)
                      on sc.vendedor=u.codigo)
               on df.contrato = sc.contrato and sdf.servico = sc.servico)
 ...
 (
   ...
   (
    ...
    from ((contratosdevolvidos cd
           ...
           on cd.contrato = pc.contrato and cd.produto=pc.produto)

    where cd.devolucao between(:DataInicial) and (:DataFinal) and cd.tipo = 'D'
   ...
   (
  	...
  	select c.classe,
        ...
        ((pc.quantidade*pc.precovenda) + coalesce(pc.frete,0) + coalesce(pc.seguro,0) -
