...
type
  ...
  TdtmCadastroContratos = class(TdtmBasico)
    ...
    qryProdutosCompostostribcodigoean: TStringField;
    qryContratosorcamento: TIntegerField;
    ...
    qryContratosPossuiNF: TBooleanField;
    ...
    qryProdutosContratosvicmsufdest: TFloatField;
    qryProdutosContratosvicmsufremet: TFloatField;
    qryContratosemitir_nfce: TBooleanField;
    qryContratospercentualdescontogeralautorizado: TFloatField;
    qryContratosentreganaloja: TBooleanField;
    qryContratosusuarioentregaloja: TIntegerField;
    qryContratosnomeusuarioentregaloja: TStringField;
    qryDadosFiscaislocalretirada_pessoatipo: TStringField;
    qryDadosFiscaislocalentrega_pessoatipo: TStringField;
    qryContratosdata_situacao_entrega: TDateTimeField;
    qryAtualizar_AliquotaIPI: TtecQuery;
    qryContratosValorPrazoComRetencoes: TCurrencyField;
    qryContratosestrangeiro: TBooleanField;
    qryDadosFiscaisestrangeiro: TBooleanField;
    qryFornecedoresestrangeiro: TBooleanField;
    qryProdutosContratosAtribuidosipibasecalculo: TFloatField;
    qryProdutosContratosAtribuidosvaloripi: TFloatField;
    qryProdutosNotaEmitidaipibasecalculo: TFloatField;
    qryProdutosNotaEmitidavaloripi: TFloatField;
...
//    procedure qryNotaseCuponsdoContratoAfterScroll(DataSet: TDataSet);
    ...
    procedure qryServicosContratosBeforeOpen(DataSet: TDataSet);
    procedure qryProdutosContratosAfterEdit(DataSet: TDataSet);
  private
...
//    vDadoFiscalOrigem : integer;
    ...
    FPrecisaRecalcularParcelas: Boolean;
...
//    procedure MensagemBloqueioContrato;
 //   function MensagemBloqueioContratoBloqueado: boolean;
    ...
    function ValidarProdutosContratosLotes(vOwner : TComponent; Todos: Boolean; Exibirmsg: Boolean = false): Boolean;
    procedure VerificarSaldoProdutoListaCasamento(DesligarControles: Boolean = true);
    function  GravarServicoContrato: Boolean;
    procedure ImprimirContrato(OrdenacaoProdutos: String = ''; SimplesMeiaPagina: Boolean = false; ExibirFotodosProdutos: Boolean = false);
...
//    property  QtdadeDadosFiscais: Integer read FQtdadeDadosFiscais;
    ...
    property  PrecisaRecalcularParcelas : Boolean read FPrecisaRecalcularParcelas write SetPrecisaRecalcularParcelas;
    property  ListaPrecisaRecalcularParcelas: TStringList read fListaPrecisaRecalcularParcelas write fListaPrecisaRecalcularParcelas;
    ...
    procedure Abrir_qryNotaseCuponsdoContrato(forced : Boolean = false);
    procedure Abrir_qryNotasFrete;
    procedure Abrir_qryNotasDevolucoesContrato(forced : Boolean = false);
...
begin
  ...
  //      end;
    ...
    begin
      qryContratos.edit;
      qryContratosPossuiNF.asBoolean := true;
      qryContratos.post;
      Abrir_qryNotaseCuponsdoContrato(true);
    end;
  end;
...
begin
  ...
  if  (qryFornecedoresnome.AsString        <> qryContratosnome.AsString) or
      ...
      (qryFornecedoresestrangeiro.AsBoolean <> qryContratosestrangeiro.AsBoolean) or
  ...
  begin
    ...
    qryFornecedoresestrangeiro.AsBoolean := qryContratosestrangeiro.AsBoolean;
...
begin
  ...
  begin
    ...
    frmVisualizarProdutosContratosSeries := TfrmVisualizarProdutosContratosSeries.create(self, self, dsrProdutosContratosSeries);

    with frmVisualizarProdutosContratosSeries do
    begin
      frmVisualizarProdutosContratosSeries.ExisteProdutosContratosSeriesNaoInformada := self.ExisteProdutosContratosSeriesNaoInformada;
      frmVisualizarProdutosContratosSeries.ExisteProdutosContratosSeriesDuplicada := self.ExisteProdutosContratosSeriesDuplicada;
...
var
  ...
  ValorDescontoItem  : Real;

  SomenteBrinde: Boolean;
  TotProdutoAnt, vProduto: Real;
begin
  ...
  try
    ...
    begin
      ...
      begin
        ...
        vProduto      := vProduto + (QtdadeTotal * Preco);
      end
      else
    ...
    if parsistema.PermitirAlterarBrindenoContrato then
      ...
      begin
        ...
        AdicionarListaPrecisaRecalcularParcelas('Itens possuem somente brindes');
      end
      else
      ...
      if (qryParcelas.recordcount = 1) then
        ...
        begin
          ...
          qryparcelas.delete;
          PrecisaRecalcularParcelas := true;
          AdicionarListaPrecisaRecalcularParcelas('Parcela única e sem valor de vencimento');
        end;

    if Not FExisteMontagem then
       qryContratosmontagemfilial.Clear;
  finally
    vProduto := RoundTo(vProduto,-2);
    qryContratossubtotalprodutos.AsFloat:= vProduto;

    qryContratostotalprodutos.AsFloat := vProduto - FTotalValorDescontoItem;
    ...
    if (ParSistema.CalcularSubstituicaoTributarianoContrato or contribipi) and
       {(qryContratospessoatipo.AsString = 'J') and}
    ...
    begin
      ...
      begin
        ...
        {para o cálculo da retenção de produtos é utlizado o valor do ipi, assim só pode ser calculado depois  de caluclar os impostos}
...
{     qryContratosaliquotaicmsproprio.AsFloat   := 0;}
    ...
    if (SituacaoContrato in ([scORCADO, scRESERVADO])) and not ViaSite  and not qryprodutoscontratos.readonly then
      if (TotProdutoAnt > 0) and (roundTo(TotProdutoAnt,-2) <> roundto(((vProduto - fTotalValorDescontoItem - vValorRateadoCupomDesconto) + TotalBrinde),-2)) then
      begin

        PrecisaRecalcularParcelas := true;

        AdicionarListaPrecisaRecalcularParcelas(format('Total produto anterior: %f diferente do atual %f', [
          roundTo(TotProdutoAnt,-2),
          roundto(((vProduto - fTotalValorDescontoItem - vValorRateadoCupomDesconto) + TotalBrinde),-2) ]));
      end;
  end
end;
...
var
  ...
  Pos: TBookmark;
  SQLNAOCOMPOSTO, SQLCOMPOSTO, SQLLotes: String;
...
//    qryProdutosContratos.DisableControls;
    ...
    try
      ...
      begin
        ...
        begin
          if qryProdutosContratoscomposto.asboolean then
            SQLCOMPOSTO := SQLCOMPOSTO + '((e.produto = ' + qryProdutosContratosproduto.AsString + ') and ' +
                         '(e.filial = ' + qryProdutosContratosfilial.AsString   + ')) or '
          else
            SQLNAOCOMPOSTO := SQLNAOCOMPOSTO + '((m1.produto = ' + qryProdutosContratosproduto.AsString + ') and ' +
      ...
      qryCancelarMov.Macrobyname('SQLCOMPOSTO').asString := ' and False ';
      qryCancelarMov.Macrobyname('SQLNAOCOMPOSTO').asString := ' and False ';

      if SQLNAOCOMPOSTO <> '' then
      begin
        Delete(SQLNAOCOMPOSTO, Length(SQLNAOCOMPOSTO) - 3, 4);
        qryCancelarMov.Macrobyname('SQLNAOCOMPOSTO').asString := ' and ' + SQLNAOCOMPOSTO;
      end;

      if SQLCOMPOSTO <> '' then
      begin
        Delete(SQLCOMPOSTO, Length(SQLCOMPOSTO) - 3, 4);
        qryCancelarMov.Macrobyname('SQLCOMPOSTO').asString := ' and ' + SQLCOMPOSTO;
      end;
                ...
                begin
                  ...
                  begin
                    ...
                    begin
                      if MovExtra then
                      begin
                        qryMovimentosquantidade.AsCurrency := qryCancelarMovfuturo.AsCurrency;
                        qryMovimentostipomovimento.AsString := 'SQU'
                      end
                      else
                      begin
                        ...
                        begin
                          qryMovimentosquantidade.AsCurrency := Qtdade - qryCancelarMovfuturo.AsCurrency;
...
begin
  ...
  begin
    ...
    begin
      ...
      if not qryContratosestrangeiro.IsNull then
        qryCopiarContrato.FieldByName('estrangeiro').AsBoolean   := qryContratosestrangeiro.AsBoolean;
...
begin
  ...
  begin
    ...
    if (field = qryContratosestado) or
       (field = qryContratoscidade) or
       (field = qryContratosbairro) or

       (field = qryContratosentestado) or
       (field = qryContratosentcidade) or
       (field = qryContratosentbairro) then
    ...
    begin
      ...
      if not field.isnull and not field.Value then
        qryContratosemitir_nfce.AsBoolean := false
      else
      if CondicaoEmissorNFCE  then
        qryContratosemitir_nfce.AsBoolean := parsistema.PadraoNotaEletronicaVendaNFCe;
    ...
    begin
      {para o cálculo da retenção de produtos é utlizado o valor do ipi, assim
       só pode ser calculado depois  de caluclar os impostos}
//      if qryProcuraClientereterimpprodutos.asBoolean then
//         CalcularTotalImpostoRetidoProdutos;
    ...
    //      qryProcuraConjuge.Sql[4] := 'or (conjuge = ' + qryContratoscliente.AsString + ')';
        ...
        if CondicaoEmissorNFCE and qryContratosvendaconsumidorfinal.AsBoolean then
          qryContratosemitir_nfce.AsBoolean := parsistema.PadraoNotaEletronicaVendaNFCe;
...
{      if ParSistema.CalcularSubstituicaoTributarianoContrato then
      ...
      begin
        ...
        begin
          PrecisaRecalcularParcelas := true;
          AdicionarListaPrecisaRecalcularParcelas('Preço por cargo e alteração do campo cliente');
        end;
      end;
    end
    else
    ...
    begin
      ...
      begin
        PrecisaRecalcularParcelas := true;
        AdicionarListaPrecisaRecalcularParcelas(format('Total crédito de troca anterior: %f diferente do atual %f', [
          CreditoTrocaAnt, qrycontratoscreditotroca.AsCurrency]));
      end;
    end
    else
    ...
    begin
      ...
      begin
        CalcularRateioDescontoProdutos;
        CalcularValorTotalProdutos;
//        dsrContratosDataChange(qryContratos, qryContratostotalprodutos);
      end;
      CalcularValorAVista;
    ...
    begin
      ...
      begin
        PrecisaRecalcularParcelas := true;
        AdicionarListaPrecisaRecalcularParcelas(format('Total desconto cashback anterior: %f diferente do atual %f', [
          desconto_cashbackAnt, qryContratosdesconto_cashback.AsCurrency]));
...
begin
  ...
  begin
    try
      ...
      {Verifica se o formulario de cadastro do contrato esta aberto e é o mesmo do
       que esta sendo gravado}
      Frm := TfrmCadastroContratos.Referencia;
      if Assigned(Frm) then
        TfrmCadastroContratos(Frm).LigarDesligarTemporizadordeBloqueio_(false);
      ...
      begin
        ...
        { Reabre o contrato gravado que esteja com seu cadastro aberto}
        if Result then
        begin
          if Assigned(Frm) then
          begin
            TfrmCadastroContratos(Frm).SelecionarContrato(NumeroContrato);
            TfrmCadastroContratos(Frm).LigarDesligarTemporizadordeBloqueio_(true);
          end;
...
begin
  ...
  if (SituacaoContrato in [scNOTAFISCAL, scNOTAPARCIAL]) then
    ...
    begin
      ...
      case vRetornoMensagem of
        smbYES    : result := True;
        smbNO     : begin result := False; abort; end;
        smbCancel : begin result := false; abort; end; //exit;
      end;
    end;

  if result then
  Result := ValidarTotaisdoContrato(true);

  if result then
    result := ValidarProdutosContratosLotes(nil, true, true);

  if result then
    result := qryProdutosContratos.CheckRequiredFields(true, false, true, FormCadastro, true, false);

  if result then
  begin
    Result := not PrecisaRecalcularParcelas;
    if not result then
      MensagemErro(format('O contrato %s precisa recalcular as parcelas.'+chr(13)+
                   ListaPrecisaRecalcularParcelas.Text,[qryContratosnumero.asString]));
  end;
  ...
  begin
    ...
    qryContratospessoanumero.Required   := ParSistema.CPF_CNPJObrigatorio and
          not qryContratosestrangeiro.asBoolean;

    result := qryContratos.CheckRequiredFields(false, true, true, FormCadastro, true, true);
    if result then
    begin
      ...
      if ExisteConjuge                        and
         (qryContratoscivil.AsString <> 'C')  and (qryContratoscivil.AsString <> 'O') then
      begin
      ...
      if Result then
        if ((qryContratospessoatipo.AsString = 'F') and (qryContratossexo.AsString = 'E'))  or
           ((qryContratospessoatipo.AsString = 'J') and (qryContratossexo.AsString <> 'E')) then
        begin
      ...
      if Result then
        if ((qryContratosconjuge.AsInteger <> 0) and
            (qryContratoscliente.AsInteger  = qryContratosconjuge.AsInteger)) then
        begin
      ...
      begin
        ...
        if FSomenteBrindes and
          not parsistema.PermitirAlterarBrindenoContrato then
        begin
          MensagemAviso(ctSOMENTEBRINDENOCONTRATO);
          Result := False
        end
        else
        if ((qryParcelas.RecordCount = 0) and
        ...
        if FExisteMontagem and qryContratosmontagemfilial.IsNull then
        begin
          MensagemAviso(format(ctFILIALMONTAGEMNULA,[qryContratosNumero.AsString]));
          Result := False
        end
        else
        if (SituacaoContrato <= scFATURADO) and
                    ...
                    not ViaSite { via site permite duplicar cnpj/cpj}  then
        begin
          ...
          try
            ...
            begin
              ...
              begin
                ...
                Result := qryConjuge.CheckRequiredFields(false, true, true, FormCadastro, true, true);
              end;
            end;
      ...
      //                    ErroNota := ernNENHUMA;
                      ...
                      (* NÃO EXISTE OPERAÇÃO COM TEF DIRETAMENTE NO CONTRATO
                      if ComTEF and Result then begin
                        ...
                        Result := PagamentoComTEF(NroParcela, qryContratosvalorprazo.AsCurrency,
                                                  ...
                                                  CredQtdadeParcelas, DebQtdadeParcelas, CDCQtdadeParcelas, {
                                                  qryContratosorcamento.asinteger, qryContratosnumero.asString,}
...
//                    dtmImprimeFiscal:=nil;
      ...
      RefazConsultaPorNome(qryQualidade_Venda,['contrato'],[qryContratosnumero.asString], false);
    end
    else
      Result := False;
  end;
...
procedure TdtmCadastroContratos.ImprimirContrato(OrdenacaoProdutos: String; SimplesMeiaPagina: Boolean; ExibirFotodosProdutos: Boolean);
begin
  ...
  begin
    ...
    begin
      ...
      try
        ...
        ImpressaoContrato.ImprimirContratos(quotedstr(qryContratosnumero.AsString), SimplesMeiaPagina, ExibirFotodosProdutos);
      finally
              ...
              begin
                ...
                while Not qryProdutosDadosFiscais.Eof do begin
                  if qryProdutosDadosFiscaisdadofiscal.AsInteger = qryDadosFiscaisnumero.AsInteger then begin
                    ...
                    qryMovimentos.Edit;
                    qryMovimentosreferencia.AsString := 'CT ' + qryContratosnumero.AsString + ' Cupom ' + IntToStr(NumeroCupom);
                  end;
...
{  try}
  ...
  ReFazConsultaPorNome(qryServicosContratos,['FilialBase','contrato'],
         [FilialBase, qryContratosnumero.AsString]);
...
//  DescontoProduto : Currency;
  ...
  begin
    ...
    if qryProdutosContratosvaloripi.AsCurrency > 0 then
      qryProdutosDadosFiscaisvaloripi.AsCurrency := ROUNDTO(qryProdutosContratosvaloripi.Asfloat * Qtdade / qryProdutosContratosquantidade.Asfloat,-2);
    if qryProdutosContratosipibasecalculo.AsCurrency > 0 then
      qryProdutosDadosFiscaisipibasecalculo.AsCurrency := ROUNDTO(qryProdutosContratosipibasecalculo.Asfloat * Qtdade / qryProdutosContratosipibasecalculo.Asfloat,-2);
    ...
    begin
      ...
      begin
        ...
        qryProdutosContratosAtribuidosvaloripi.AsCurrency := qryProdutosContratosAtribuidosvaloripi.AsCurrency +
                                                         qryProdutosDadosFiscaisvaloripi.AsCurrency;

        qryProdutosContratosAtribuidosipibasecalculo.AsCurrency := qryProdutosContratosAtribuidosipibasecalculo.AsCurrency +
                                                         qryProdutosDadosFiscaisipibasecalculo.AsCurrency;
      ...
      begin
        ...
        qryProdutosDadosFiscaisipibasecalculo.AsCurrency := qryProdutosContratosipibasecalculo.AsCurrency - qryProdutosContratosAtribuidosipibasecalculo.AsCurrency;
        qryProdutosDadosFiscaisvaloripi.AsCurrency := qryProdutosContratosvaloripi.AsCurrency - qryProdutosContratosAtribuidosvaloripi.AsCurrency;
  ...
  begin
    ...
    begin
      ...
      if qryProdutosContratosipibasecalculo.AsCurrency > 0 then
        qryProdutosDadosFiscaisipibasecalculo.AsCurrency     := qryProdutosContratosipibasecalculo.AsCurrency - qryProdutosNotaEmitidaipibasecalculo.AsCurrency;

      if qryProdutosContratosvaloripi.AsCurrency > 0 then
        qryProdutosDadosFiscaisvaloripi.AsCurrency     := qryProdutosContratosvaloripi.AsCurrency - qryProdutosNotaEmitidavaloripi.AsCurrency;
...
//  qryProdutosDadosFiscaisprodutodescricao.AsString := qryProdutosContratosdescricao.AsString;
  ...
  if not qryprodutoscontratosanvisa.isnull then
    qryProdutosDadosFiscaisdescricaoproduto.AsString := Trim(qryProdutosDadosFiscaisdescricaoproduto.AsString) + ' ANVISA - '+qryprodutoscontratosanvisa.asString;
  ...
  begin
    ...
    TotalIPIBaseCalculoInc := TotalIPIBaseCalculoInc + qryProdutosDadosFiscaisipibasecalculo.asCurrency;
    TotalValorIPIInc := TotalValorIPIInc + qryProdutosDadosFiscaisvaloripi.asCurrency;
...
{  end;}
  ...
  begin
    ...
    begin
      ...
      qryProdutosDadosFiscaisincidencia.AsString       := CSTSaida(qryContratosestado.AsString,
                                                                     ...
                                                                     qryProcuraClienteOrgaoPublico.asBoolean);
{
    ...
    begin
      ...
      {
      qryProdutosDadosFiscaisvaloripi.AsCurrency     :=
        roundto((( ((Qtdade * qryProdutosDadosFiscaisprecovenda.AsFloat) -

                     (qryProdutosDadosFiscaisdesconto.asFloat +
                      qryProdutosDadosFiscaisvalordescontoitem.asFloat) +

                      qryProdutosDadosFiscaisseguro.AsFloat +
                      qryProdutosDadosFiscaisfrete.AsFloat


                     ) * qryProdutosDadosFiscaisaliquotaipi.AsCurrency)) / 100, -2);
      }
    end;
...
{    DescontoProduto := 0;
    ...
    qryProdutosDadosFiscaistribcodigoean.AsString   := qryProdutosContratostribcodigoean.AsString;
...
//  Total,
  ...
  begin
    ...
    if CondicaoEmissorNFCE and qryContratosemitir_nfce.asBoolean then
      result := false;
...
//        Pos := qryProdutosContratos.GetBookmark;
        ...
        qryProdutosContratos.sortbyfield('numero'); ///garantir que a nota siga a mesma sequencia do contrato
                      ...
                      begin
                        ...
                        begin
                          ...
                          begin
                            ...
                            NovoMovimento(qryProdutosContratosproduto.AsLargeInt, qryProdutosContratosfilial.AsInteger);
                            qryMovimentosquantidade.AsCurrency    := qryProdutosReservasquantidade.AsCurrency;
                            QtdadePrevia                         := qryProdutosReservasquantidade.AsCurrency;
                            qryMovimentostipomovimento.AsString  := 'SPV';
                            ...
                            begin
                              ...
                              if result then
                                qryMovimentosreferencia.AsString := 'CT ' + qryContratosnumero.AsString +
                          ...
                          begin
                            ...
                            begin
                              ...
                              if result then
                                qryMovimentostipomovimento.AsString  := qryNaturezasPadrao_.fieldbyname('tipomovimento').asString;
                            end
                            else
                            if eHGarantia then
                            begin
                              ...
                              qryMovimentostipomovimento.AsString  := 'RFV';
                            ...
                            begin
                              result :=  VerificarSerieFilial(ComServico);
                              if result then
                                 qryMovimentosreferencia.AsString := ifthen(AbrindoOs, 'OS ', 'CT ') + qryContratosnumero.AsString +
                                                 ' NF ' + {SerieSugestao} qrySeriesFiliaisProdutosvalor.asString + '-' +
                                                 qrySeriesFiliaisProdutosnumeroinicial.AsString;
                            end;
                          ...
                          if (Qtdade + QtdadePrevia) > 0 then  {aki}
                            result := IncluirProdutosDadosFiscais(Qtdade + QtdadePrevia, qryMovimentosnumero.AsInteger, ComCartao);
...
{              ------------------------------}
                if SituacaoProduto = stNAOCOMPOSTO then  //Lotes não possuem composto
                begin
  ...
  //              begin
                      ...
                      begin
                        ...
                        begin
                          ...
                          begin
                            ...
                            if (Qtdade + QtdadePrevia) > 0 then
                              result := IncluirProdutosDadosFiscais(Qtdade + QtdadePrevia,  //3
                                          qryMovimentosnumero.AsInteger,
      ...
      //                  SelecionarDadosNaturezaPadrao('SAIDA - GARANTIA', nil, qryProdutosDadosFiscais, qryContratos, Contrato);
      //                  qryMovimentostipomovimento.AsString  := qryNaturezasPadrao_.fieldbyname('tipomovimento').asString;
                        result := IncluirProdutosDadosFiscais(qryProdutosContratosLotesquantidade.asFloat, 0, false, qryProdutosContratosLoteslote.aslargeint); //4
...
//  Brinde := TipoNota = tnBRINDE;
  ...
  qryDadosFiscaisestrangeiro.AsBoolean   := qryContratosestrangeiro.AsBoolean;
  ...
  qryDadosFiscaiscidadeibgefilialemitente.asString :=  CodigoCidadeIBGEFilialBase;
  qryDadosFiscaiscnpjfilialemitente.asString := cnpjfilialbase;
  ...
  qryDadosFiscaiscontribicmsemitente.asBoolean := qryContratoscontribicmsemitente.asBoolean;
  ...
  begin
    ...
    qryDadosFiscaislocalentrega_pessoatipo.AsString   := qryContratospessoatipo.AsString;
...
//  qryDadosFiscaisaliquotacreditoicms.AsFloat:= qryProcuraFilialcreditoicms.AsFloat;
  ...
  {    qryDadosFiscaistotalbaseicmssubstituicao.AsFloat  := RoundToD(TotalBaseSubstituicao,2);
    ...
    qryDadosFiscaisvaloripi.AsFloat           := TotalValorIPIInc;
...
//    qryDadosFiscaisnatureza.AsString := 'SIMPLES REMESSA';
  ...
  {    (NotaSubstituicao(ifthen(RegimeTributario=1,qryProdutosDadosFiscaiscsosn.AsString,qryProdutosDadosFiscaisincidencia.AsString)) aqui}
    ...
    begin
      ...
      begin
//        result := SelecionarDadosNaturezaPadrao('NOTAS DIVERSAS',nil, nil, qryDadosFiscais, Contrato);  aqui
...
//      MensagemBloqueioContrato;
    ...
    ReadOnly := (SituacaoContrato > scRESERVADO) or
                Bloqueado or
                ((qryContratosindpres.AsInteger = 2) and
                  not ViaSite and  {Quando pela internet indpres=2 e ViaSite = Pagamento pelo retorno comprado no site}
                  not (UsuarioLogin.Administrador or UsuarioLogin.GerenteVendas));
...
begin
  ...
  qryContratosrestipo.AsString      := 'P';
  qryContratosreftipo.AsString      := 'P';
  qryContratosemitir_nfce.asBoolean := CondicaoEmissorNFCE;
  FExisteMontagem                   := False;
  FExisteEntrega                    := False;
  FPermiteDesconto                  := True;
...
//  ReFazConsulta(qryConjuge,[0],[0]);
  ...
  if parsistema.obsnfenoContrato.text<>'' then
    qryContratosmontagemobs.asString := parsistema.obsnfenoContrato.text
...
//  if ParSistema.ValordaQuantidade>0 then
  ...
  begin
    ...
    if not qryContratosdataentrega.isnull then
      qryProdutosContratosdataentrega.asDateTime := qryContratosdataentrega.asVariant;

    if not qryContratoshoraentrega.isnull then
...
(*
  ...
  *)
    ...
    if (qryContratosnumero.AsString <> '') then
      ReFazConsultaPorNome(qryProdutosEntregar,['contrato','primogenito'],[qryContratosnumero.AsString, qryContratosprimogenito.asString])
    else
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if qryProdutosContratos.UpdateStatus = usDeleted then
            qryMovimentosquantidade.AsCurrency := qryMovimentosquantidade.AsCurrency - QuantidadeOriginal
          else
...
{            qryMovimentosquantidade.AsCurrency := qryMovimentosquantidade.AsCurrency - QuantidadeLoteOriginal}
          ...
          begin
            ...
            begin
              ...
              qryMovimentosquantidade.AsCurrency := qryMovimentosquantidade.AsCurrency + qryProdutosContratosLotesquantidade.AsCurrency;
          ...
          *)
          ...
          begin
            ...
            begin
              ...
              if qryContratossituacao.AsString <> qryContratossituacao.OldValue then
                qryMovimentosquantidade.AsCurrency := qryProdutosContratosLotesquantidade.AsCurrency;
          end
          else
            qryMovimentosquantidade.AsCurrency := qryProdutosContratosLotesquantidade.AsCurrency;
    ...
    begin
      ...
      if qryMovimentostipomovimento.AsString = '' then  // CASO O MOVIMENTO NAO TENHA SIDO ALTERADO
      begin
...
//            then
            begin
              ...
              begin
                ...
                begin
                  QtdadeReservado                     := qryMovimentosquantidade.AsCurrency;
                  qryMovimentostipomovimento.AsString := 'TPE';
                  if Futuro > 0 then
                    if qryMovimentosquantidade.AsCurrency > Futuro then
                    begin
                      NovoMovimentoExtra(Futuro, 'SQU');
                      QtdadeReservado                    := qryMovimentosquantidade.AsCurrency - Futuro;
                      qryMovimentosquantidade.AsCurrency := qryMovimentosquantidade.AsCurrency - Futuro
                    end
                    else
                    begin
                      qryMovimentostipomovimento.AsString := 'SQU';
              ...
              begin
                if qryProdutosContratosreserva.AsInteger = 0 then
                begin
                  if EmEstoque < qryMovimentosquantidade.AsCurrency then
                  begin
                    ...
                    begin
                      ...
                      begin
                        NovoMovimentoExtra(qryMovimentosquantidade.AsCurrency - EmEstoque, 'AQU');
                        qryMovimentostipomovimento.AsString := 'TER';
                        qryMovimentosquantidade.AsCurrency := EmEstoque;
                      end;
                    end
                    else
                    begin
                ...
                begin
                  ...
                  if qryMovimentosquantidade.AsCurrency = qryProdutosReservassaldo.AsCurrency then
                  begin
                    Movimentar := False;
                  ...
                  begin
                    ...
                    QtdadeReservado                     := qryMovimentosquantidade.AsCurrency;
                    Qtdade                              := qryMovimentosquantidade.AsCurrency;
                    qryMovimentosquantidade.AsCurrency  := qryProdutosReservassaldo.AsCurrency - qryMovimentosquantidade.AsCurrency;
                    AtualizarReserva                    := True;
                  end
                  else
                  begin
                    Qtdade := qryMovimentosquantidade.AsCurrency - qryProdutosReservassaldo.AsCurrency;
...
//                    if ((qryProdutosContratosvendasemestoque.AsString <> 'S') or not parsistema.PermitirVendaSemEstoque) and not ViaSite then
    ...
    begin
      ...
      begin
        ...
        qryMovimentosquantidade.AsCurrency  := MovExtras[a].Quantidade;
      ...
      begin
        ...
        qryMovimentosquantidade.AsCurrency  := MovCopiar[a].Quantidade;
...
//  if ParSistema.MostrarSimilaresContrato then
  ...
  ReFazConsulta(dtmCadastroContratosAuxiliar.qryConsultaReservasProduto,[1,2],[IntToStr(qryProcuraProdutoProduto.AsLargeInt),
                                                  qryProcuraProdutofilial.asinteger {qryProcuraFilialProdutocodigo.AsInteger erro encontrato na conversao delphi ce }]);
...
//  qryContratos.ReadOnly         := Value;
  ...
  begin
    ...
    qryContratosemitir_nfce.readonly := false;
    ...
    qryContratosfornecedorfrete.readonly := False;
...
begin
  ...
  if CondicaoEmissorNFCE and qryContratosemitir_nfce.asBoolean {qryContratosvendaconsumidorfinal.asBoolean and (qryContratosestado.asString = EstadofilialBase) and
...
begin
  ...
  if self.owner is TfrmCadastroContratos then
    ...
    begin
      ...
      begin
        tstObservacoesVenda_.TabVisible := trim((self.qryProcuraProdutoobservacoesvenda.asstring))<>'';
        if tstObservacoesVenda_.TabVisible then
           pgcExtraProdutos_.ActivePage := tstObservacoesVenda_;
        pgcExtraProdutos_Change(nil);
      end;
    end;
...
begin
  ...
  qryContratosValorPrazoComRetencoes.asCurrency :=
     qryContratosValorPrazo.asCurrency +
     qryContratosTotalImpostoRetidoGeral.asCurrency;
...
begin
  if not qryConjuge.readonly then
  begin
...
begin
  ...
  begin
    ...
    begin
      ...
      qryProdutosDadosFiscaisCompostos.FieldbyName('tribcodigoean').AsString   := qryProdutosCompostostribcodigoean.AsString;

      qryProdutosDadosFiscaisCompostos.FieldbyName('incidencia').AsString   := CSTSaida(qryContratosestado.AsString,
                                                                                        qryProdutosCompostoscomponente.AsString,
                                                                                        qryContratospessoatipo.AsString,
                                                                                        qryContratosvendaconsumidorfinal.asboolean,
                                                                                        qryContratoscontribicms.asboolean,
...
//    IPIBaseCalculoRateado := qryProdutosDadosFiscaisIPIBaseCalculo.asCurrency;
//    ValorIPIRateado := qryProdutosDadosFiscaisValorIPI.asCurrency;
    ...
    begin
      ...
      if not qryProdutosCompostosanvisa.isnull then
        qryProdutosDadosFiscaisCompostos.FieldbyName('descricao').AsString := qryProdutosDadosFiscaisCompostos.FieldbyName('descricao').AsString+ ' ANVISA - '+qryprodutoscompostosanvisa.asString;
...

{
      ...
      begin
        ...
        qryProdutosDadosFiscaisCompostos.FieldbyName('incidencia').AsString  :=
           CSTSaida(qryContratosestado.AsString,
                    qryProdutosCompostoscomponente.AsString,
                    qryContratospessoatipo.AsString,
                    qryContratosvendaconsumidorfinal.asboolean,
                    qryContratoscontribicms.asboolean,
             ...
             qryProcuraClienteNoSimples.asBoolean, qryProcuraClienteOrgaoPublico.asBoolean);
{
      ...
      begin
        ...
        qryProdutosDadosFiscaisCompostos.FieldbyName('ipibasecalculo').AsCurrency :=
           qryProdutosDadosFiscaisCompostos.FieldbyName('quantidade').AsCurrency *
           qryProdutosDadosFiscaisCompostos.FieldbyName('precovenda').AsFloat;
...
//      qryProdutosDadosFiscaisCompostos.FieldbyName('fatorsubstituicao').AsFloat := qryProdutosCompostosfatorsubstituicao.AsFloat;
      ...
      if (qryProdutosDadosFiscaisfrete.AsCurrency     > 0) or
         ...
         (qryProdutosDadosFiscaisacrescimo.AsCurrency > 0) {  ipi dos compostos é diferente...cada componente tem seus valores...que ~pode não fechar com o que esta no contrato
         {(qryProdutosDadosFiscaisipibasecalculo.AsCurrency > 0) or
         (qryProdutosDadosFiscaisvaloripi.AsCurrency > 0)} then
      begin
        if FreteRateado > 0 then
        begin
          if Cont = t then
          ...
          begin
            qryProdutosDadosFiscaisCompostos.FieldbyName('frete').AsCurrency :=

            Truncar(((qryProdutosDadosFiscaisCompostos.FieldbyName('precovenda').AsCurrency * qryProdutosDadosFiscaisCompostos.FieldbyName('quantidade').AsCurrency) / (qryProdutosContratosprecovenda.AsCurrency * Qtdade)) * qryProdutosDadosFiscaisfrete.AsCurrency,2);
...
begin
  ...
  if (CondicaoEmissorNFCE and qryContratosemitir_nfce.asBoolean and not assigned(ecfpadrao)) then
  begin
...
begin
  inherited;
//  qryProdutosContratosaliquotaipi_1.value := qryProdutosContratosaliquotaipi.value;
...
begin
  ...
  begin
    frmVisualizarProdutosObservacoes := TfrmVisualizarProdutosObservacoes.create(self, self, dsrProdutosContratosObservacoes);
...
begin
  inherited;
  qryAtualizar_AliquotaIPI.parambyname('contrato').asString := qryContratosNumero.asstring;
  qryAtualizar_AliquotaIPI.execSQl;
  perpetrar([]);

  qryProdutosContratos.ParamByName('filialbase').AsInteger := FilialBase;
  qryProdutosContratos.ParamByName('pContribuinteOrigem').AsBoolean := qryContratoscontribicmsemitente.asBoolean;
  qryProdutosContratos.ParamByName('pContribuinteDestino').AsBoolean := qryContratoscontribicms.asBoolean;
  qryProdutosContratos.ParamByName('pVendaConsumidorFinal').AsBoolean := qryContratosvendaconsumidorfinal.asBoolean;
  ...
  begin
    ...
    begin
      ...
      try
        ...
        qryProdutosContratos.SortByField('numero');
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if qryImpostosRetidosProdutosminimo.AsCurrency <= (qryContratostotalprodutos.AsCurrency +
                                                             qryContratostotalipi.ascurrency)  then
          begin
            {
            if ((qryImpostosRetidosdescricao.AsString = 'ISS') or
                (qryImpostosRetidosdescricao.AsString = 'ISSQN'))  then
...
//              vValor2 := Truncar(((qryImpostosRetidosProdutostaxa.AsCurrency/100)*qryContratostotalprodutos.AsCurrency),2);
              vValor2 := CalcularArredondamentoImpostosRetidosProdutos(((qryImpostosRetidosProdutostaxa.AsCurrency/100)*
                 ({qryContratostotalprodutos.AsCurrency +}   TotalProdutos +
                  qryContratostotalipi.ascurrency
                  )));
    ...
    if qryContratosimpostoretidoproduto.AsCurrency <> TotalImpostoRetido then
    ...
    if qryImpostosRetidosProdutosmarcar.AsBoolean then
      if qryImpostosRetidosProdutosminimo.AsCurrency <= (qryContratostotalprodutos.AsCurrency +
          ...
          qryImpostosRetidosProdutosvalorimpostoretido.AsCurrency :=
             CalcularArredondamentoImpostosRetidosProdutos((qryImpostosRetidosProdutostaxa.AsCurrency/100)*
                    ({qryContratostotalprodutos.AsCurrency}  totalprodutos +
                     qryContratostotalipi.AsCurrency));
  ...
  inherited;
{
  ...
  begin
    Abrir_qryNotasFrete;
    Abrir_qryNotasDevolucoesContrato;
    Abrir_qryNotaseCuponsdoContrato;
...
function TdtmCadastroContratos.ValidarTotaisdoContrato(Exibirmsg: Boolean = false): Boolean;
var
...
var
  ...
  begin
    ...
    begin
      ...
      if ((qryContratosvalorvista.AsFloat - qryContratosfrete.AsFloat - qryContratosseguro.asFloat - qryContratosTotalServicos.asCurrency) +
        ...
        result := ((qryContratosvalorvista.AsFloat -
                               qryContratosfrete.AsFloat -
                               qryContratosseguro.asFloat -
                               qryContratosTotalServicos.asCurrency -
                               0.01) + qryContratosDesconto_Cashback.AsCurrency);
...
begin
  ...
  begin
    ...
    begin
      ...
      if result then
      begin
        if TotalContrato  <  (qryContratosfrete.AsCurrency +
                             qryContratosseguro.AsCurrency +
                             qryContratostotalservicos.asCurrency +
                             0.01) then
        begin
          MensagemErro(format(ctVALORCASHBACKMAIORPRODUTOS,[Desconto_CashBack,
                  ...
                  (TotalContrato   -
                    ...
                    (qryContratosfrete.AsCurrency +
                     qryContratosseguro.AsCurrency +
                     qryContratostotalservicos.asCurrency)  + Desconto_CashBack) ]));
...
{  vTotal : Currency;}
  ...
  vNovoPercentualDescontoGeral {, vNovoPercentualDescontoFinanceiro} : Currency;
  Usuario: TtecUsuarios;
...
begin
  ...
  begin
            {
    vNovoPercentualDescontoFinanceiro := 0;
    ...
    vTotalDesconto := qryContratosdescontogeral.AsCurrency;
    vNovoPercentualDescontoGeral := Truncar(PercentualDoValorSobreTotal(vTotal, vTotalDesconto), 2);

    {
    if ComDescontoFinanceiro then
      vNovoPercentualDesconto := vNovoPercentualDescontoFinanceiro
    else
      vNovoPercentualDesconto := vNovoPercentualDescontoGeral;
    }

    if  (vNovoPercentualDescontoGeral > Truncar(ParSistema.PercentualDescontoFrenteCaixa, 2)) and

        ({(ComDescontoFinanceiro and (vNovoPercentualDescontoFinanceiro > qryContratosdescontofinanceiroautorizado.asCurrency)) or}
         ({not ComDescontoFinanceiro and} (vNovoPercentualDescontogeral > qryContratospercentualdescontogeralautorizado.asCurrency))) then
    begin
      {
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            qryContratospercentualdescontogeralautorizado.asCurrency := vNovoPercentualDescontoGeral;
                                               {
            if qryContratosvalorprazo.asCurrency <>0 then
            begin
              vTotalDesconto := vTotal -  qryContratosvalorprazo.asCurrency;
...
function TdtmCadastroContratos.ValidarProdutosContratosLotes(vOwner : TComponent;
  Todos: Boolean; Exibirmsg: Boolean = false): Boolean;
var
...
//      if SituacaoContrato <> scORCADO then
  ...
  if not result and Exibirmsg then
    MensagemErro('Erro na validação dos Lotes');
...
begin
  ...
  fCondicaoEmissorNFCE := fCondicaoEmissorNFCE and
            (qryContratosvendaconsumidorfinal.asBoolean and
              ...
              and not (qryContratosindpres.AsInteger=2);
...
begin
 if self.owner is TfrmCadastroContratos then
   ...
   begin
     with TfrmCadastroContratos(self.owner).frmCadastroProdutosContrato do
     if (pgcExtraProdutos_.ActivePage = tstProdutosporCliente_) and
        tstProdutosporCliente_.tabvisible then
...
begin
    try
      ...
      qryProdutosContratosLotes.disablecontrols;
      dsrProdutosContratosLotes.enabled := false;
      ...
      qryProdutosContratosLotes.enablecontrols;
      dsrProdutosContratosLotes.enabled := true;
      qryProdutosContratosLotes.LinkFields := 'produto=produto;filial=filial';
      qryProdutosContratosLotes.MasterSource := dsrProdutosContratos;
...
procedure TdtmCadastroContratos.qryServicosContratosBeforeOpen(
  DataSet: TDataSet);
begin
  inherited;
  qryServicosContratos.parambyname('FilialBase').asInteger := FilialBase;
end;

procedure TdtmCadastroContratos.qryProdutosContratosAfterEdit(
  DataSet: TDataSet);
begin
  inherited;
;
...
procedure TdtmCadastroContratos.Atribuircalc_valorcashback;
var
 vRecno : integer;

begin
  qryAtualizarCalc_ValorCashBack.parambyname('contrato').asString :=
    qryContratosnumero.asString;
  ...
  vRecno := qryProdutosContratos.recno;

  qryProdutosContratos.AfterOpen := nil;
  qryProdutosContratos.close;
  qryProdutosContratos.open;
  qryProdutosContratos.AfterOpen := qryProdutosContratosAfterOpen;

  qryProdutosContratos.recno := vrecno;

end;

procedure TdtmCadastroContratos.RefazConsulta_limites_entregas_regioes;
begin
  ...
  if IncluindoouEditandoProduto then
    ...
    begin
      RefazConsultaPorNome(qrylimites_entregas_regioes,
           ...
           [qryProdutosContratosdataentrega.asVariant,
            qryProdutosContratosperiodoentrega.asinteger,

            qryContratosestado.AsVariant,
            qryContratoscidade.AsVariant,
            qryContratosbairro.AsVariant,

            qryContratosentestado.AsVariant,
            qryContratosentcidade.AsVariant,
            qryContratosentbairro.AsVariant], true);
...
function TdtmCadastroContratos.FormCadastro: TComponent;
begin
  if self.Owner.ClassName = 'TfrmCadastroContratos' then
    result := self.Owner
  else
    result := nil;
end;

procedure TdtmCadastroContratos.SetPrecisaRecalcularParcelas(
  const Value: Boolean);
begin
  FPrecisaRecalcularParcelas := Value;
  if not Value then
  begin
    if assigned(ListaPrecisaRecalcularParcelas) then
    begin
      ListaPrecisaRecalcularParcelas.free;
      ListaPrecisaRecalcularParcelas := nil;
    end
  end;
end;

procedure TdtmCadastroContratos.AdicionarListaPrecisaRecalcularParcelas(
  Descricao: string);
begin
  if not assigned(ListaPrecisaRecalcularParcelas) then
    ListaPrecisaRecalcularParcelas := TStringList.create;

  if ListaPrecisaRecalcularParcelas.IndexOf(Descricao) = -1 then
    ListaPrecisaRecalcularParcelas.Append(Descricao);

end;



function TdtmCadastroContratos.PercentualdoCashBackSobreLiqProdutos(valordescontocashback: Currency): Currency;
var
    vLiquidoProdutos: Currency;
    vpercentual: Currency;
begin
  vLiquidoProdutos := (TotalContrato   -
                         (qryContratosfrete.AsCurrency +
                          qryContratosseguro.AsCurrency +
                          qryContratostotalservicos.asCurrency)  + desconto_cashback);
...
function TdtmCadastroContratos.ValordoPercentualSobreCashBackSobreLiqProdutos: Currency;
var
    vLiquidoProdutos: Currency;
    vValor: Currency;
begin
...
procedure TdtmCadastroContratos.Abrir_qryNotaseCuponsdoContrato(forced : Boolean);
  procedure refazer;
  begin
...
begin
  ...
  if self.owner is TfrmCadastroContratos then
    with TfrmCadastroContratos(self.owner) do
    begin
...
procedure TdtmCadastroContratos.Abrir_qryNotasFrete;
begin
  if self.owner is TfrmCadastroContratos then
    with TfrmCadastroContratos(self.owner) do
    begin
      if pgcContratos.activepage = tstNotasFiscais then
        RefazConsultaPorNome(dtmCadastroContratosAuxiliar.qryNotasFrete,['contrato', 'primogenito'],
      [qryContratosnumero.asstring, qryContratosPrimogenito.asVariant]);
    end;
end;

procedure TdtmCadastroContratos.Abrir_qryNotasDevolucoesContrato(forced : Boolean);
  procedure refazer;
  begin
    RefazConsultaPorNome(dtmCadastroContratosAuxiliar.qryNotasDevolucoesContrato,['contrato', 'primogenito'],
       [qryContratosnumero.asstring, qryContratosPrimogenito.asVariant]);
  end;
begin
  if forced then
...
function TdtmCadastroContratos.RegistrarEntregaNaLoja: Boolean;
begin
  ...
  try
    ...
    if dtmCadastroContratosAuxiliar.qryNotaseCuponsdoContrato.recordcount = 0 then
      Abrir_qryNotaseCuponsdoContrato(true);

    if dtmCadastroContratosAuxiliar.qryNotaseCuponsdoContrato.Locate('situacao', 'N',[]) then
    begin
      ...
      begin
        ...
        dtmCadastroContratosAuxiliar.qryEntregasnotafilial.AsString := dtmCadastroContratosAuxiliar.qryNotaseCuponsdoContratofilial.AsString;
        dtmCadastroContratosAuxiliar.qryEntregasnotaserie.AsString := dtmCadastroContratosAuxiliar.qryNotaseCuponsdoContratoserie.AsString;
        dtmCadastroContratosAuxiliar.qryEntregasnotanumero.AsString := dtmCadastroContratosAuxiliar.qryNotaseCuponsdoContratonumero.AsString;
        ...
        qryContratos.Edit;
        qryContratosusuarioentregaloja.AsInteger :=  UsuarioAutAux.CodigoUsuario;
        qryContratosnomeusuarioentregaloja.AsString :=  UsuarioAutAux.NomeUsuario;
        qryContratosdata_situacao_entrega.asDateTime := dtmCadastroContratosAuxiliar.qryEntregasdata_hora.asDatetime;
        qryContratossituacao_entrega.asString := 'ENTREGUE';

        qryContratos.post;

        qryContratos.readonly := True;

        result := perpetrar([qryContratos, dtmCadastroContratosAuxiliar.qryEntregas]);
      end;
    end
    else
    begin
...
function TdtmCadastroContratos.GetContratoAtual: Boolean;
begin
  fContratoAtual := qryContratosNumero.asString = qryContratoscontrato_atual.asString;
  Result := fContratoAtual;
end;

procedure TdtmCadastroContratos.EqualizarDescontoGeralcomIPI;
var
  nxdif : integer;
...
begin
  ...
  begin
    vStateAnt := qrycontratos.state;

    while abs((qryContratosdescontogeral.AsCurrency -
              qryContratostotalipi.AsCurrency))<>0 do
    begin
      if not (qryContratos.state in [dsedit, dsinsert]) then
        qryContratos.edit;
      qryContratosdescontogeral.AsCurrency := qryContratostotalipi.AsCurrency;

      if not (vStateAnt in [dsedit, dsinsert]) then
        qryContratos.Post;

      if (abs((qryContratosdescontogeral.AsCurrency -
              qryContratostotalipi.AsCurrency))>0) and

         (abs((qryContratosdescontogeral.AsCurrency -
              qryContratostotalipi.AsCurrency)) <= 0.02) then
         inc(nxdif);
