inherited frmEmissaoNotasFiscais: TfrmEmissaoNotasFiscais
  Left = 95
  Top = 144
  ActiveControl = dbgSelecionados
  Caption = 'Emiss'#227'o Notas Fiscais'
  ClientHeight = 504
  ClientWidth = 758
  Color = clBtnFace
  FormStyle = fsMDIChild
  PixelsPerInch = 75
  inherited pnlBarra: TPanel
    Width = 758
  end
  inherited tblBarra: TToolBar
    Width = 758
    TabOrder = 2
    inherited sbnProcurar: TSpeedButton
      Left = 162
      Visible = False
    end
    object sbnEmissaoNotasFiscais: TSpeedButton [2]
      Left = 4
      Top = 5
      Width = 75
      Height = 35
      Hint = 'Emiss'#227'o de notas fiscais'
      Caption = 'Emitir F7'
      Flat = True
      Glyph.Data = {
        
        DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
      Layout = blGlyphTop
      NumGlyphs = 2
      Spacing = 0
      OnClick = sbnEmissaoNotasFiscaisClick
    end
  end
  object pnlFundoJanela: TPanel
    Left = 0
    Top = 44
    Width = 758
    Height = 460
    Align = alClient
    BevelOuter = bvNone
    BorderWidth = 3
    TabOrder = 1
    object gbxEmissaoNotasFiscais: TGroupBox
      Left = 3
      Top = 3
      Width = 752
      Height = 454
      Align = alClient
      TabOrder = 0
      object gbxFilial: TGroupBox
        Left = 6
        Top = 9
        Width = 369
        Height = 42
        TabOrder = 0
        object lblFilial: TLabel
          Left = 8
          Top = 12
          Width = 27
          Height = 15
          Alignment = taRightJustify
          Caption = 'Filial'
        end
        object lblDescricaoFilial: TLabel
          Left = 102
          Top = 9
          Width = 260
          Height = 23
          AutoSize = False
          BorderStyle = bsSingle
        end
        object lblCodigoFilial: TLabel
          Left = 41
          Top = 9
          Width = 60
          Height = 23
          Alignment = taRightJustify
          AutoSize = False
          BorderStyle = bsSingle
        end
      end
      object dbgSelecionados: TtecDBGrid
        Left = 6
        Top = 82
        Width = 740
        Height = 369
        DataSource = dtmEmissaoNotasFiscais.dsrContratosEmitirNotas
        Options = [dgTitles, dgIndicator, dgColLines, dgRowLines, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        TabOrder = 1
        TitleFont.Color = clBlack
        TitleFont.Height = 13
        TitleFont.Name = 'helvetica'
        TitleFont.Pitch = fpVariable
        TitleFont.Style = []
        OnDblClick = dbgSelecionadosDblClick
        OnKeyDown = dbgSelecionadosKeyDown
        RowWrap = 60
        DoubleRowColor = True
        Columns = <
          item
            Expanded = False
            FieldName = 'numero'
            Title.Alignment = taCenter
            Title.Caption = 'Contrato'
            Width = 110
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'nomecliente'
            Title.Alignment = taCenter
            Title.Caption = 'Nome do Cliente'
            Width = 220
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'data'
            Title.Alignment = taCenter
            Title.Caption = 'Emiss'#227'o'
            Width = 60
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'filialvenda'
            Title.Alignment = taCenter
            Title.Caption = 'Fil.'
            Width = 30
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'valorprazo'
            Title.Alignment = taCenter
            Title.Caption = 'Valor'
            Width = 80
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'nomeavalista'
            Title.Alignment = taCenter
            Title.Caption = 'Nome do Avalista '
            Width = 165
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'selecionar'
            Title.Alignment = taCenter
            Title.Caption = 'Sel.'
            Width = 30
            Visible = True
          end>
      end
      object gbxContratos: TGroupBox
        Left = 425
        Top = 2
        Width = 133
        Height = 75
        Caption = 'Contratos'
        TabOrder = 2
        object lblQtdeContratos: TLabel
          Left = 33
          Top = 19
          Width = 27
          Height = 15
          Alignment = taRightJustify
          Caption = 'Total'
        end
        object lblQtdeMarcados: TLabel
          Left = 4
          Top = 46
          Width = 56
          Height = 15
          Alignment = taRightJustify
          Caption = 'Marcados'
        end
        object edtQtdeContratos: TEditNumero
          Left = 64
          Top = 15
          Width = 60
          Height = 23
          TabStop = False
          Alignment = taRightJustify
          Color = clBtnFace
          ReadOnly = True
          TabOrder = 2
          Mascara = False
          NrDecimal = 2
          Decimais = False
          Negativo = False
          Tamanho = 6
        end
        object edtQtdeMarcados: TEditNumero
          Left = 64
          Top = 42
          Width = 60
          Height = 23
          TabStop = False
          Alignment = taRightJustify
          Color = clBtnFace
          ReadOnly = True
          TabOrder = 3
          Mascara = False
          NrDecimal = 2
          Decimais = False
          Negativo = False
          Tamanho = 6
        end
      end
      object gbxValores: TGroupBox
        Left = 562
        Top = 2
        Width = 182
        Height = 75
        Caption = 'Valores'
        TabOrder = 3
        object lblTotalContratos: TLabel
          Left = 34
          Top = 19
          Width = 27
          Height = 15
          Alignment = taRightJustify
          Caption = 'Total'
        end
        object lblValoresSelecao: TLabel
          Left = 5
          Top = 46
          Width = 56
          Height = 15
          Alignment = taRightJustify
          Caption = 'Marcados'
        end
        object edtTotalContratos: TEditNumero
          Left = 65
          Top = 15
          Width = 110
          Height = 23
          TabStop = False
          Alignment = taRightJustify
          Color = clBtnFace
          ReadOnly = True
          TabOrder = 2
          Mascara = True
          NrDecimal = 2
          Decimais = True
          Negativo = False
          Tamanho = 9
        end
        object edtTotalMarcados: TEditNumero
          Left = 65
          Top = 42
          Width = 110
          Height = 23
          TabStop = False
          Alignment = taRightJustify
          Color = clBtnFace
          ReadOnly = True
          TabOrder = 3
          Mascara = True
          NrDecimal = 2
          Decimais = True
          Negativo = False
          Tamanho = 9
        end
      end
      object ckbSelecionarTodos: TCheckBox
        Left = 6
        Top = 58
        Width = 203
        Height = 16
        Caption = '&Selecionar Todos os Contratos'
        TabOrder = 4
        OnClick = ckbSelecionarTodosClick
      end
      object ckbNotaOpcional: TCheckBox
        Left = 237
        Top = 51
        Width = 140
        Height = 31
        Caption = 'Sempre Emitir Nota'
        TabOrder = 5
      end
    end
  end
end
