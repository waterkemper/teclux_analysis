inherited frmProdutosaEntregar: TfrmProdutosaEntregar
  Left = 600
  Top = 238
  Width = 801
  Height = 368
  HorzScrollBar.Range = 761
  VertScrollBar.Range = 323
  ActiveControl = dbgProdutos
  Caption = 'Produtos a entregar'
  Position = poScreenCenter
  PixelsPerInch = 96
  TextHeight = 16
  object lblMensagem: TLabel
    Left = 16
    Top = 256
    Width = 75
    Height = 15
    Caption = 'lblMensagem'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clRed
    Font.Height = -12
    Font.Name = 'Luxi Sans'
    Font.Pitch = fpVariable
    Font.Style = []
    ParentFont = False
  end
  object lblLegenda: TLabel
    Left = 4
    Top = 232
    Width = 306
    Height = 16
    Alignment = taCenter
    Caption = '<Ctrl Barra de Espa'#231'o / Duplo clique> Altera Entrega'
  end
  object dbgProdutos: TDBGrid
    Left = 8
    Top = 0
    Width = 750
    Height = 233
    DataSource = dtmCadastroContratos.dsrProdutosContratosEntregar
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -11
    Font.Name = 'helvetica'
    Font.Pitch = fpVariable
    Font.Style = []
    Options = [dgTitles, dgIndicator, dgColLines, dgRowLines, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
    ParentFont = False
    TabOrder = 0
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clBlack
    TitleFont.Height = -13
    TitleFont.Name = 'helvetica'
    TitleFont.Pitch = fpVariable
    TitleFont.Style = []
    OnDblClick = dbgProdutosDblClick
    OnKeyDown = dbgProdutosKeyDown
    Columns = <
      item
        Expanded = False
        FieldName = 'filial'
        Title.Alignment = taCenter
        Title.Caption = 'Fil.'
        Width = 20
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'produto'
        Title.Alignment = taCenter
        Title.Caption = 'Item'
        Width = 135
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'descricaoproduto'
        Title.Alignment = taCenter
        Title.Caption = 'Descri'#231#227'o'
        Width = 310
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'valorgrade1'
        Title.Caption = 'Linha'
        Width = 62
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'valorgrade2'
        Title.Caption = 'Coluna'
        Width = 62
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'quantidade'
        Title.Alignment = taCenter
        Title.Caption = 'Qtde.'
        Width = 35
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'precovenda'
        Title.Alignment = taCenter
        Title.Caption = 'Pre'#231'o'
        Width = 80
        Visible = True
      end
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'entrega'
        Title.Alignment = taCenter
        Title.Caption = 'E'
        Width = 20
        Visible = True
      end>
  end
  object bbnOK: TBitBtn
    Left = 320
    Top = 242
    Width = 86
    Height = 25
    Caption = 'OK'
    Default = True
    ModalResult = 1
    TabOrder = 1
    Glyph.Data = {
      
      0000}
    NumGlyphs = 2
  end
  object bbnCancelar: TBitBtn
    Left = 416
    Top = 242
    Width = 86
    Height = 25
    Cancel = True
    Caption = '&Cancelar'
    ModalResult = 2
    TabOrder = 2
    Glyph.Data = {
      
      0000}
    NumGlyphs = 2
  end
  object gbxFiltroEntrega: TGroupBox
    Left = 509
    Top = 236
    Width = 121
    Height = 73
    Caption = 'Filtragem'
    TabOrder = 3
    object ckbComEntrega: TCheckBox
      Left = 8
      Top = 16
      Width = 100
      Height = 30
      Caption = 'Com Entrega'
      Checked = True
      State = cbChecked
      TabOrder = 0
      OnClick = ckbComEntregaClick
    end
    object ckbSemEntrega: TCheckBox
      Left = 8
      Top = 40
      Width = 100
      Height = 30
      Caption = 'Sem Entrega'
      TabOrder = 1
      OnClick = ckbComEntregaClick
    end
  end
end
