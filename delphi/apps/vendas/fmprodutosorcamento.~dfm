inherited frmProdutosOrcamento: TfrmProdutosOrcamento
  Left = 415
  Top = 55
  Action = actHabilitarControles
  ActiveControl = mmoObsProdVenda
  Caption = 'Cadastro de Produtos do Or'#231'amento'
  ClientHeight = 685
  ClientWidth = 1018
  PixelsPerInch = 96
  TextHeight = 16
  inherited pnlBarra: TPanel
    Width = 485
    inherited bvlBotoesEd: TBevel
      Left = 201
    end
  end
  inherited pnlTopMenu: TPanel
    Width = 1018
    TabOrder = 2
    inherited tblBarra: TToolBar
      Width = 793
    end
  end
  object pnlFundoJanela: TPanel
    Left = 0
    Top = 45
    Width = 1018
    Height = 640
    Align = alClient
    BevelOuter = bvNone
    BorderWidth = 3
    TabOrder = 1
    object gbxFundoJanela: TGroupBox
      Left = 3
      Top = 3
      Width = 1012
      Height = 634
      Align = alClient
      TabOrder = 0
      object Label6: TLabel
        Left = 874
        Top = 320
        Width = 33
        Height = 14
        Alignment = taRightJustify
        Caption = 'TOTAL'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
      end
      object Label1: TLabel
        Left = 709
        Top = 320
        Width = 30
        Height = 14
        Alignment = taRightJustify
        Caption = 'QTDE.'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
      end
      object lblGrupo: TLabel
        Left = 20
        Top = 323
        Width = 34
        Height = 12
        Alignment = taRightJustify
        Caption = 'GRUPO'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
      end
      object Label5: TLabel
        Left = 5
        Top = 347
        Width = 53
        Height = 12
        Alignment = taRightJustify
        Caption = 'PROMO'#199#195'O'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
      end
      object gbxProduto: TGroupBox
        Left = 7
        Top = 46
        Width = 849
        Height = 36
        Caption = 'PRODUTO'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 1
        object sbnConsultaProduto: TSpeedButton
          Left = 138
          Top = 10
          Width = 23
          Height = 23
          Hint = 'Procura produto'
          Flat = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          Glyph.Data = {
            
            DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
          NumGlyphs = 2
          ParentFont = False
          OnClick = sbnConsultaProdutoClick
        end
        object sbnVisualizarFoto: TSpeedButton
          Left = 164
          Top = 9
          Width = 24
          Height = 24
          Hint = 'Visualizar a imagem relacionada no site da empresa'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          Glyph.Data = {
            
            7F7F007F7F007F7F007F7F007F7F007F7F007F7F007F7F007F7F}
          ParentFont = False
          OnClick = sbnVisualizarFotoClick
        end
        object flkCodigoProduto: TtecDBFindLookup
          Left = 2
          Top = 10
          Width = 135
          Height = 24
          Alignment = taLeftJustify
          DataaFieldInterno = 'produto'
          DataaFieldVisual = 'produtovisual'
          DataField = 'produtodigitado'
          DataSource = dtmOrcamentos.dsrProdutosOrcamentos
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = 18
          Maximo = 0
          Minimo = 37353
          Adicional = 0
          Opcional = True
          OnEnter = flkCodigoProdutoEnter
          OnExit = flkCodigoProdutoExit
          OnFound = flkCodigoProdutoFound
          OnKeyDown = flkCodigoProdutoKeyDown
          OnMessage = flkCodigoProdutoMessage
          ParentFont = False
          TabOrder = 0
          PermitirZero = False
          PermitirNulo = False
          Operacao = opATRIBUICAO
          DataFieldTotal = 'quantidade'
          LookupaFieldinterno = 'produto'
          LookupaFieldVisual = 'produtovisual'
          LookupField = 'produtovisual'
          LookupSource = dtmOrcamentos.dsrProcuraProduto
          LookupQueryParameter = 'produtovisual'
          LookupParametersLabel = 'Item de Produto'
          LookupParameters = 'produtovisual'
          LocateParameters = 'produtovisual'
          Parameter = ' '
          ActiveSetControls = False
          DenyInsert = True
          NoSetControls = <>
          SetControls = <>
          LookupParameter = 'produtovisual'
        end
        object dtxDescricaoProduto: TtecDBText
          Left = 191
          Top = 9
          Width = 652
          Height = 24
          TabStop = False
          Color = clBtnFace
          DataField = 'descricaolc'
          DataSource = dtmOrcamentos.dsrProdutosOrcamentos
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 1
          Alignment = taLeftJustify
        end
      end
      object gbxQuantidade: TGroupBox
        Left = 7
        Top = 10
        Width = 159
        Height = 36
        Caption = 'QUANTIDADE'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        object edtQuantidade: TDBEditNumero
          Left = 3
          Top = 10
          Width = 86
          Height = 24
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = -1
          ParentFont = False
          TabOrder = 0
          Text = 'edtQuantidade'
          OnExit = edtQuantidadeExit
          OnKeyDown = edtQuantidadeKeyDown
          Mascara = False
          Alignment = taLeftJustify
          TipoMascara = tmQUANTIDADE
          NrDecimal = 0
          Decimais = False
          Negativo = False
          Tamanho = 0
          DataField = 'quantidade'
          DataSource = dtmOrcamentos.dsrProdutosOrcamentos
        end
        object dtxUnidade: TtecDBText
          Left = 91
          Top = 10
          Width = 65
          Height = 24
          TabStop = False
          Color = clBtnFace
          DataField = 'unidade'
          DataSource = dtmOrcamentos.dsrProdutosOrcamentos
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 1
          Alignment = taLeftJustify
        end
      end
      object gbxFilial: TGroupBox
        Left = 6
        Top = 81
        Width = 351
        Height = 36
        Caption = 'FILIAL'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 2
        object sbnConsultaFilial: TSpeedButton
          Left = 39
          Top = 10
          Width = 23
          Height = 23
          Hint = 'Procurar filial'
          Flat = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          Glyph.Data = {
            
            DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
          NumGlyphs = 2
          ParentFont = False
          OnClick = sbnConsultaFilialClick
        end
        object flkFilial: TtecDBFindLookup
          Left = 3
          Top = 10
          Width = 35
          Height = 24
          Alignment = taLeftJustify
          DataField = 'filial'
          DataSource = dtmOrcamentos.dsrProdutosOrcamentos
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = 3
          Maximo = 0
          Minimo = 37353
          Adicional = 0
          Opcional = True
          OnEnter = flkFilialEnter
          OnFound = flkFilialFound
          OnMessage = flkFilialMessage
          ParentFont = False
          TabOrder = 0
          PermitirZero = False
          PermitirNulo = False
          Operacao = opATRIBUICAO
          LookupField = 'codigo'
          LookupSource = dtmOrcamentos.dsrProcuraFilialProduto
          LookupQueryParameter = 'codigo'
          Parameter = ' '
          ActiveSetControls = False
          DenyInsert = True
          NoSetControls = <>
          SetControls = <>
          LookupParameter = 'codigo'
        end
        object dtxNomeFilial: TtecDBText
          Left = 63
          Top = 10
          Width = 284
          Height = 24
          TabStop = False
          Color = clBtnFace
          DataField = 'nome'
          DataSource = dtmOrcamentos.dsrProcuraFilialProduto
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 1
          Alignment = taLeftJustify
        end
      end
      object gbxEmEstoque: TGroupBox
        Left = 747
        Top = 15
        Width = 110
        Height = 38
        Caption = 'EM ESTOQUE'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 7
        object dtxEmEstoque: TtecDBText
          Left = 4
          Top = 10
          Width = 101
          Height = 24
          TabStop = False
          Color = clBtnFace
          DataField = 'emestoque'
          DataSource = dtmOrcamentos.dsrProdutoEstoque
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 0
          Alignment = taLeftJustify
        end
      end
      object GroupBox1: TGroupBox
        Left = 5
        Top = 118
        Width = 350
        Height = 195
        Caption = 'PRE'#199'O DO PRODUTO'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 3
        object lblPrecoNormal: TLabel
          Left = 21
          Top = 22
          Width = 39
          Height = 12
          Caption = 'NORMAL'
          Visible = False
        end
        object Label3: TLabel
          Left = 28
          Top = 45
          Width = 32
          Height = 12
          Alignment = taRightJustify
          Caption = 'PRE'#199'O'
        end
        object Label4: TLabel
          Left = 5
          Top = 70
          Width = 55
          Height = 12
          Alignment = taRightJustify
          Caption = 'DESCRI'#199#195'O'
        end
        object sbnAlterarPreco: TSpeedButton
          Left = 189
          Top = 38
          Width = 93
          Height = 23
          Caption = '&ALTERAR PRE'#199'O'
          Flat = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          OnClick = sbnAlterarPrecoClick
        end
        object lblPromocao: TLabel
          Left = 189
          Top = 16
          Width = 156
          Height = 18
          Caption = 'Produto em promo'#231#227'o'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clRed
          Font.Height = -15
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = [fsBold]
          ParentFont = False
          Layout = tlCenter
          Visible = False
        end
        object pnlValorDescontoItem: TPanel
          Left = 2
          Top = 109
          Width = 346
          Height = 21
          Align = alBottom
          BevelOuter = bvNone
          TabOrder = 0
          Visible = False
          object edtValorDescontoItem: TDBEditNumero
            Left = 176
            Top = -1
            Width = 89
            Height = 23
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            MaxLength = -1
            ParentFont = False
            TabOrder = 1
            Mascara = True
            TipoMascara = tmGERAL
            NrDecimal = 2
            Decimais = True
            Negativo = False
            Tamanho = 9
            DataField = 'valordescontoitem'
            DataSource = dtmOrcamentos.dsrProdutosOrcamentos
          end
          object StaticText1: TStaticText
            Left = 4
            Top = 5
            Width = 100
            Height = 16
            Alignment = taRightJustify
            Caption = 'DESCONTO  DO ITEM'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -9
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            ParentFont = False
            TabOrder = 2
          end
          object StaticText2: TStaticText
            Left = 112
            Top = 5
            Width = 12
            Height = 16
            Alignment = taCenter
            Caption = '%'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -9
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            ParentFont = False
            TabOrder = 3
          end
          object edtpercentualvalordescontoitem: TDBEditNumero
            Left = 132
            Top = -1
            Width = 44
            Height = 23
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            MaxLength = -1
            ParentFont = False
            TabOrder = 0
            Mascara = True
            TipoMascara = tmGERAL
            NrDecimal = 2
            Decimais = True
            Negativo = False
            Tamanho = 9
            DataField = 'percentualvalordescontoitem'
            DataSource = dtmOrcamentos.dsrProdutosOrcamentos
          end
        end
        object edtPreco: TDBEditNumero
          Left = 64
          Top = 38
          Width = 121
          Height = 24
          TabStop = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = -1
          ParentFont = False
          TabOrder = 1
          Text = 'edtPreco'
          OnExit = edtPrecoExit
          Mascara = True
          Alignment = taLeftJustify
          TipoMascara = tmGERAL
          NrDecimal = 2
          Decimais = True
          Negativo = False
          Tamanho = 9
          DataField = 'precovenda'
          DataSource = dtmOrcamentos.dsrProdutosOrcamentos
        end
        object edtDescricaoPrecoVenda: TDBEditTexto
          Left = 65
          Top = 62
          Width = 280
          Height = 24
          DataField = 'descricaoprecovenda'
          DataSource = dtmordemservico.dsrProdutosOrdemServico
          Enabled = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = -1
          ParentFont = False
          TabOrder = 2
          OnExit = edtDescricaoPrecoVendaExit
          CharCasenoDefault = False
          Alignment = taLeftJustify
        end
        object dtxPrecoProduto: TtecDBText
          Left = 64
          Top = 14
          Width = 121
          Height = 24
          TabStop = False
          Color = clBtnFace
          DataField = 'preconormal'
          DataSource = dtmOrcamentos.dsrProdutoEstoque
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 3
          Alignment = taLeftJustify
        end
        object dtxPrecoVenda: TtecDBText
          Left = 64
          Top = 38
          Width = 121
          Height = 24
          TabStop = False
          Color = clBtnFace
          DataField = 'precovenda'
          DataSource = dtmOrcamentos.dsrProdutosOrcamentos
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 4
          Alignment = taLeftJustify
        end
        object pnlDescontoCupomDesconto: TPanel
          Left = 2
          Top = 130
          Width = 346
          Height = 21
          Align = alBottom
          BevelOuter = bvNone
          TabOrder = 5
          object Label2: TLabel
            Left = 32
            Top = 5
            Width = 89
            Height = 12
            Alignment = taRightJustify
            Caption = 'CUPOM DESCONTO'
          end
          object edtdescontoCupomDesconto: TDBEditNumero
            Left = 132
            Top = -1
            Width = 133
            Height = 23
            TabStop = False
            Color = clBtnFace
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            MaxLength = -1
            ParentFont = False
            ReadOnly = True
            TabOrder = 0
            Mascara = True
            TipoMascara = tmGERAL
            NrDecimal = 2
            Decimais = True
            Negativo = False
            Tamanho = 9
            DataField = 'valordescontoitemcupomdesconto'
            DataSource = dtmOrcamentos.dsrProdutosOrcamentos
          end
        end
        object pnlDescontoRateado: TPanel
          Left = 2
          Top = 88
          Width = 346
          Height = 21
          Align = alBottom
          BevelOuter = bvNone
          TabOrder = 6
          object Label7: TStaticText
            Left = 18
            Top = 5
            Width = 103
            Height = 16
            Alignment = taRightJustify
            Caption = 'DESCONTO RATEADO'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -9
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            ParentFont = False
            TabOrder = 1
          end
          object edtDescontoRateado: TDBEditNumero
            Left = 132
            Top = -1
            Width = 133
            Height = 23
            TabStop = False
            Color = clBtnFace
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            MaxLength = -1
            ParentFont = False
            ReadOnly = True
            TabOrder = 0
            Mascara = True
            TipoMascara = tmGERAL
            NrDecimal = 2
            Decimais = True
            Negativo = False
            Tamanho = 9
            DataField = 'descontogeral'
            DataSource = dtmOrcamentos.dsrProdutosOrcamentos
          end
        end
        object pnlcalc_valorcashback: TPanel
          Left = 2
          Top = 172
          Width = 346
          Height = 21
          Align = alBottom
          BevelOuter = bvNone
          TabOrder = 7
          object Label8: TLabel
            Left = 24
            Top = 5
            Width = 97
            Height = 12
            Alignment = taRightJustify
            Caption = 'CASH BACK GERADO'
          end
          object DBEditNumero1: TDBEditNumero
            Left = 132
            Top = -1
            Width = 133
            Height = 23
            TabStop = False
            Color = clBtnFace
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            MaxLength = -1
            ParentFont = False
            ReadOnly = True
            TabOrder = 0
            Mascara = True
            TipoMascara = tmGERAL
            NrDecimal = 2
            Decimais = True
            Negativo = False
            Tamanho = 9
            DataField = 'calc_valorcashback'
            DataSource = dtmOrcamentos.dsrProdutosOrcamentos
          end
        end
        object pnldesconto_cashback: TPanel
          Left = 2
          Top = 151
          Width = 346
          Height = 21
          Align = alBottom
          BevelOuter = bvNone
          TabOrder = 8
          object Label9: TLabel
            Left = 2
            Top = 5
            Width = 119
            Height = 12
            Alignment = taRightJustify
            Caption = 'CASH BACK APROPRIADO'
          end
          object Edtesconto_cashback: TDBEditNumero
            Left = 132
            Top = -1
            Width = 133
            Height = 23
            TabStop = False
            Color = clBtnFace
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            MaxLength = -1
            ParentFont = False
            ReadOnly = True
            TabOrder = 0
            Mascara = True
            TipoMascara = tmGERAL
            NrDecimal = 2
            Decimais = True
            Negativo = False
            Tamanho = 9
            DataField = 'desconto_cashback'
            DataSource = dtmOrcamentos.dsrProdutosOrcamentos
          end
        end
      end
      object gbxVendedor: TGroupBox
        Left = 4
        Top = 400
        Width = 353
        Height = 37
        Caption = 'VENDEDOR'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 4
        object sbnProcurarVendedor: TSpeedButton
          Left = 65
          Top = 11
          Width = 23
          Height = 23
          Hint = 'Procura vendedores'
          Flat = True
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          Glyph.Data = {
            
            DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
          NumGlyphs = 2
          ParentFont = False
          OnClick = sbnProcurarVendedorClick
        end
        object flkVendedor: TtecDBFindLookup
          Left = 4
          Top = 11
          Width = 60
          Height = 24
          Alignment = taLeftJustify
          DataField = 'vendedor'
          DataSource = dtmOrcamentos.dsrProdutosOrcamentos
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          MaxLength = 6
          Maximo = 0
          Minimo = 37353
          Adicional = 0
          Opcional = True
          ParentFont = False
          TabOrder = 0
          PermitirZero = False
          PermitirNulo = False
          Operacao = opATRIBUICAO
          LookupField = 'codigo'
          LookupSource = dtmOrcamentos.dsrProcuraVendedorProduto
          LookupQueryParameter = 'codigo'
          Parameter = ' '
          ActiveSetControls = False
          DenyInsert = False
          NoSetControls = <>
          SetControls = <>
          LookupParameter = 'codigo'
        end
        object dtxNomeVendedor: TtecDBText
          Left = 89
          Top = 10
          Width = 260
          Height = 24
          TabStop = False
          Color = clBtnFace
          DataField = 'nome'
          DataSource = dtmOrcamentos.dsrProcuraVendedorProduto
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 1
          Alignment = taLeftJustify
        end
      end
      object gbxDepositos: TGroupBox
        Left = 864
        Top = 8
        Width = 140
        Height = 81
        Caption = 'DEP'#211'SITOS'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 8
        object dbgDepositos: TtecDBGrid
          Left = 2
          Top = 11
          Width = 134
          Height = 75
          TabStop = False
          Color = clBtnFace
          DataSource = dtmOrcamentos.dsrEstoque
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'MS Sans Serif'
          Font.Pitch = fpVariable
          Font.Style = []
          Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
          ParentFont = False
          ReadOnly = True
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clBlack
          TitleFont.Height = -9
          TitleFont.Name = 'helvetica'
          TitleFont.Pitch = fpVariable
          TitleFont.Style = []
          RowWrap = 60
          Large = False
          DoubleRowColor = False
          TitleMinHeight = 100
          CellHeights = 100
          StrippedColor = 16054260
          CanDelete = False
          CanNotInsertFromGrid = False
          PostOnEnter = False
          PostOnSetUpDown = False
          DenySort = False
          DefaultRowHeight = 16
          ExibirNumerodaLinha = False
          NaoAtribuirDadosaTabelaaoDigitar = False
          ShowWhenFieldInvisible = False
          CanClickWhenEditing = False
          Columns = <
            item
              Expanded = False
              FieldName = 'filial'
              Title.Alignment = taCenter
              Title.Caption = 'FILIAL'
              Width = 40
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'emestoque'
              Title.Alignment = taCenter
              Title.Caption = 'ESTOQUE'
              Width = 60
              Visible = True
            end>
        end
      end
      object rgpMontagem: TtecDBRadioGroup
        Left = 6
        Top = 366
        Width = 110
        Height = 32
        Caption = 'MONTAGEM'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 5
        TabStop = False
        DataField = 'montagem'
        DataSource = dtmOrcamentos.dsrProdutosOrcamentos
        object tecRadioButton1: TtecRadioButton
          Left = 9
          Top = 13
          Width = 48
          Height = 17
          Caption = 'SIM'
          TabOrder = 0
          Value = 'S'
        end
        object tecRadioButton2: TtecRadioButton
          Left = 59
          Top = 13
          Width = 46
          Height = 17
          Caption = 'N'#195'O'
          TabOrder = 1
          Value = 'N'
        end
      end
      object tecDBRadioGroup2: TtecDBRadioGroup
        Left = 120
        Top = 366
        Width = 110
        Height = 32
        Caption = 'ENTREGAR'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 6
        TabStop = False
        DataField = 'entrega'
        DataSource = dtmOrcamentos.dsrProdutosOrcamentos
        object tecRadioButton3: TtecRadioButton
          Left = 9
          Top = 13
          Width = 48
          Height = 17
          Caption = 'SIM'
          TabOrder = 0
          Value = 'S'
        end
        object tecRadioButton4: TtecRadioButton
          Left = 59
          Top = 13
          Width = 46
          Height = 17
          Caption = 'N'#195'O'
          TabOrder = 1
          Value = 'N'
        end
      end
      object dtxTotalProdutos: TtecDBText
        Left = 923
        Top = 317
        Width = 69
        Height = 24
        TabStop = False
        Color = clBtnFace
        DataField = 'totalprodutos'
        DataSource = dtmOrcamentos.dsrOrcamentos
        ReadOnly = True
        TabOrder = 9
        Alignment = taLeftJustify
      end
      object dtxQtdeProdutos: TtecDBText
        Left = 752
        Top = 317
        Width = 69
        Height = 24
        TabStop = False
        Color = clBtnFace
        DataField = 'totalqtdeprodutos'
        DataSource = dtmOrcamentos.dsrOrcamentos
        ReadOnly = True
        TabOrder = 10
        Alignment = taLeftJustify
      end
      object pnlGradeProdutos: TPanel
        Left = 359
        Top = 89
        Width = 654
        Height = 224
        BevelOuter = bvSpace
        BorderStyle = bsSingle
        Caption = 'pnlGradeProdutos'
        TabOrder = 11
        object dbgProdutosOrcamentos: TtecDBGrid
          Left = 1
          Top = 1
          Width = 648
          Height = 218
          TabStop = False
          Align = alClient
          BorderStyle = bsNone
          Color = clBtnFace
          DataSource = dtmOrcamentos.dsrProdutosOrcamentos
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'MS Sans Serif'
          Font.Pitch = fpVariable
          Font.Style = []
          Options = [dgTitles, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
          ParentFont = False
          ReadOnly = True
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clBlack
          TitleFont.Height = -13
          TitleFont.Name = 'helvetica'
          TitleFont.Pitch = fpVariable
          TitleFont.Style = []
          RowWrap = 60
          Large = False
          DoubleRowColor = False
          MsgDelete = 'o PRODUTO DO OR'#199'AMENTO'
          TitleMinHeight = 100
          CellHeights = 100
          StrippedColor = 16054260
          CanDelete = False
          CanNotInsertFromGrid = True
          PostOnEnter = False
          PostOnSetUpDown = False
          DenySort = True
          DefaultRowHeight = 17
          ExibirNumerodaLinha = False
          NaoAtribuirDadosaTabelaaoDigitar = True
          ShowWhenFieldInvisible = False
          CanClickWhenEditing = False
          Columns = <
            item
              Expanded = False
              FieldName = 'produtovisual'
              Title.Alignment = taCenter
              Title.Caption = 'C'#243'digo'
              Width = 110
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'descricaolc'
              Title.Alignment = taCenter
              Title.Caption = 'Descri'#231#227'o do Produto'
              Width = 266
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'quantidade'
              Title.Alignment = taCenter
              Title.Caption = 'Qtde.'
              Width = 55
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'precovenda'
              Title.Alignment = taCenter
              Title.Caption = 'Pre'#231'o'
              Width = 77
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'valordescontoitem'
              Title.Alignment = taCenter
              Title.Caption = 'Desc.'
              Width = 50
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'total'
              Title.Alignment = taCenter
              Title.Caption = 'Total'
              Width = 65
              Visible = True
            end>
        end
      end
      object pgcExtraProdutos: TPageControl
        Left = 2
        Top = 436
        Width = 1008
        Height = 196
        ActivePage = tstObservacoesVenda
        Align = alBottom
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -13
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 12
        OnChange = pgcExtraProdutosChange
        OnDrawTab = pgcExtraProdutosDrawTab
        object tstObservacoesVenda: TTabSheet
          Caption = 'Observa'#231#245'es do produtos para a venda'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          object mmoObsProdVenda: TtecDBMemo
            Left = 0
            Top = 0
            Width = 1000
            Height = 165
            Align = alClient
            DataField = 'observacoesvenda'
            DataSource = dtmOrcamentos.dsrProcuraProduto
            ReadOnly = True
            ScrollBars = ssVertical
            TabOrder = 0
          end
        end
        object tstProdutosSimilares: TTabSheet
          Tag = 1
          Caption = 'Produtos Similares'
          ImageIndex = 1
          object dbgSimilares: TtecDBGrid
            Left = 0
            Top = 0
            Width = 1000
            Height = 165
            Align = alClient
            DataSource = dtmOrcamentos.dsrProdutosSimilares
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -13
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
            ParentFont = False
            ReadOnly = True
            TabOrder = 0
            TitleFont.Charset = DEFAULT_CHARSET
            TitleFont.Color = clBlack
            TitleFont.Height = -13
            TitleFont.Name = 'helvetica'
            TitleFont.Pitch = fpVariable
            TitleFont.Style = []
            OnDblClick = dbgSimilaresDblClick
            RowWrap = 60
            Large = False
            DoubleRowColor = False
            TitleMinHeight = 100
            CellHeights = 100
            StrippedColor = 16054260
            CanDelete = False
            CanNotInsertFromGrid = False
            PostOnEnter = False
            PostOnSetUpDown = False
            DenySort = False
            DefaultRowHeight = 20
            ExibirNumerodaLinha = False
            NaoAtribuirDadosaTabelaaoDigitar = False
            ShowWhenFieldInvisible = False
            CanClickWhenEditing = False
            Columns = <
              item
                Expanded = False
                FieldName = 'codigo'
                Width = 109
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'descricao'
                Width = 168
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'valorgrade1'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'valorgrade2'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'filial'
                Width = 31
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'emestoque'
                Width = 55
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'reservado'
                Title.Caption = 'Reserv.'
                Width = 55
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'valorproduto'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'similares'
                Width = 45
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'grupo'
                Title.Caption = 'Grupo'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'descricaogrupo'
                Title.Caption = 'Descri'#231#227'o do Grupo'
                Width = 157
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'promocao'
                Title.Caption = 'Promo'#231#227'o'
                Width = 63
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'descricaopromocao'
                Title.Caption = 'Descri'#231#227'o da Promo'#231#227'o'
                Width = 223
                Visible = True
              end>
          end
        end
        object tstPrecosCargos: TTabSheet
          Caption = 'Pre'#231'os por Cargos'
          ImageIndex = 2
          object dbgprecoscargos: TtecDBGrid
            Left = 0
            Top = 0
            Width = 1000
            Height = 165
            Align = alClient
            DataSource = dtmOrcamentos.dsrPrecosCargos
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'Arial'
            Font.Pitch = fpVariable
            Font.Style = []
            ParentFont = False
            TabOrder = 0
            TitleFont.Charset = DEFAULT_CHARSET
            TitleFont.Color = clBlack
            TitleFont.Height = -10
            TitleFont.Name = 'helvetica'
            TitleFont.Pitch = fpVariable
            TitleFont.Style = []
            RowWrap = 60
            Large = False
            DoubleRowColor = True
            TitleMinHeight = 100
            CellHeights = 100
            StrippedColor = 16054260
            CanDelete = False
            CanNotInsertFromGrid = False
            PostOnEnter = False
            PostOnSetUpDown = False
            DenySort = False
            DefaultRowHeight = 19
            ExibirNumerodaLinha = False
            NaoAtribuirDadosaTabelaaoDigitar = False
            ShowWhenFieldInvisible = False
            CanClickWhenEditing = False
            Columns = <
              item
                Expanded = False
                FieldName = 'codigo'
                Title.Alignment = taCenter
                Title.Caption = 'CARGO'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'descricao'
                Title.Alignment = taCenter
                Title.Caption = 'DESCRI'#199#195'O CARGO'
                Width = 260
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'valorproduto'
                Title.Alignment = taCenter
                Title.Caption = 'VALOR'
                Width = 100
                Visible = True
              end>
          end
        end
        object tstEstoque: TTabSheet
          Caption = 'Estoque'
          ImageIndex = 3
          object dbgEstoque: TtecDBGrid
            Left = 0
            Top = 0
            Width = 1000
            Height = 165
            Align = alClient
            DataSource = dtmOrcamentos.dsrConsultaEstoques
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -11
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
            ParentFont = False
            ReadOnly = True
            TabOrder = 0
            TitleFont.Charset = DEFAULT_CHARSET
            TitleFont.Color = clBlack
            TitleFont.Height = -13
            TitleFont.Name = 'helvetica'
            TitleFont.Pitch = fpVariable
            TitleFont.Style = []
            RowWrap = 60
            Large = False
            DoubleRowColor = True
            TitleMinHeight = 100
            CellHeights = 100
            StrippedRows = 2
            StrippedColor = 16054260
            CanDelete = False
            CanNotInsertFromGrid = False
            PostOnEnter = False
            PostOnSetUpDown = False
            DenySort = False
            DefaultRowHeight = 18
            ExibirNumerodaLinha = False
            NaoAtribuirDadosaTabelaaoDigitar = False
            ShowWhenFieldInvisible = False
            CanClickWhenEditing = False
            Columns = <
              item
                Expanded = False
                FieldName = 'filial'
                Title.Alignment = taCenter
                Title.Caption = 'FILIAL'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Width = 37
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'emestoque'
                Title.Alignment = taCenter
                Title.Caption = 'EM ESTOQUE'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'reservado'
                Title.Alignment = taCenter
                Title.Caption = 'RESERVADO'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'reservaprevia'
                Title.Alignment = taCenter
                Title.Caption = 'RES. PREVIA'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'demonstracao'
                Title.Alignment = taCenter
                Title.Caption = 'DEMONS.'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'conserto'
                Title.Alignment = taCenter
                Title.Caption = 'CONSERTO'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Width = 57
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'transito'
                Title.Alignment = taCenter
                Title.Caption = 'TR'#194'NSITO'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Width = 53
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'danificada'
                Title.Alignment = taCenter
                Title.Caption = 'DANIFICADA'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'futuro'
                Title.Alignment = taCenter
                Title.Caption = 'FUTURO'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Width = 50
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'estoquevenda'
                Title.Caption = 'E. VENDA'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Width = 85
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'estoquetotal'
                Title.Caption = 'E. TOTAL'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'quantidadefilial'
                Title.Alignment = taCenter
                Title.Caption = 'PEDIDOS'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Width = 53
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'csosn'
                Title.Caption = 'CSOSN'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'incidencia'
                Title.Caption = 'CST'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'customedio'
                Title.Alignment = taCenter
                Title.Caption = 'CUSTO M'#201'DIO'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Width = 71
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'precosemicms'
                Title.Alignment = taCenter
                Title.Caption = 'SEM ICMS'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'precocomicms'
                Title.Alignment = taCenter
                Title.Caption = 'COM ICMS'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'valorultimacompra'
                Title.Alignment = taCenter
                Title.Caption = 'ULT. COMPRA'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'ultimacompra_com_icms'
                Title.Alignment = taCenter
                Title.Caption = 'UC.+ICMS'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Visible = True
              end>
          end
        end
        object tstPedidos: TTabSheet
          Caption = 'Pedidos'
          ImageIndex = 4
          object dbgProdutosPedidos: TtecDBGrid
            Left = 0
            Top = 0
            Width = 1000
            Height = 165
            Align = alClient
            DataSource = dtmOrcamentos.dsrConsultaProdutosPedidos
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'Arial'
            Font.Pitch = fpVariable
            Font.Style = []
            ParentFont = False
            ReadOnly = True
            TabOrder = 0
            TitleFont.Charset = DEFAULT_CHARSET
            TitleFont.Color = clBlack
            TitleFont.Height = -10
            TitleFont.Name = 'helvetica'
            TitleFont.Pitch = fpVariable
            TitleFont.Style = []
            RowWrap = 60
            Large = False
            DoubleRowColor = True
            TitleMinHeight = 100
            CellHeights = 100
            StrippedColor = 16054260
            CanDelete = False
            CanNotInsertFromGrid = False
            PostOnEnter = False
            PostOnSetUpDown = False
            DenySort = False
            DefaultRowHeight = 19
            ExibirNumerodaLinha = False
            NaoAtribuirDadosaTabelaaoDigitar = False
            ShowWhenFieldInvisible = False
            CanClickWhenEditing = False
            Columns = <
              item
                Alignment = taCenter
                Expanded = False
                FieldName = 'pedido'
                Title.Alignment = taCenter
                Title.Caption = 'PEDIDO'
                Width = 70
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'filialentrega'
                Title.Alignment = taCenter
                Title.Caption = 'FILIAL'
                Width = 36
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'emissao'
                Title.Alignment = taCenter
                Title.Caption = 'EMISS'#195'O'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'fornecedor'
                Title.Alignment = taCenter
                Title.Caption = 'FORNCEDOR'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'nomefornecedor'
                Title.Caption = 'NOME DO FORNECEDOR'
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'quantidadependente'
                Title.Alignment = taCenter
                Title.Caption = 'QUANTIDADE'
                Width = 75
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'entrega'
                Title.Alignment = taCenter
                Title.Caption = 'PREVIS'#195'O ENTREGA'
                Width = 115
                Visible = True
              end>
          end
        end
        object tstFornecedores: TTabSheet
          Caption = 'Fornecedores'
          ImageIndex = 5
          object dbgFornecedor: TtecDBGrid
            Left = 0
            Top = 0
            Width = 1000
            Height = 165
            Align = alClient
            DataSource = dtmOrcamentos.dsrFornecedoresProdutos
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -12
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
            ParentFont = False
            TabOrder = 0
            TitleFont.Charset = DEFAULT_CHARSET
            TitleFont.Color = clBlack
            TitleFont.Height = -12
            TitleFont.Name = 'helvetica'
            TitleFont.Pitch = fpVariable
            TitleFont.Style = []
            RowWrap = 60
            Large = False
            DoubleRowColor = True
            MsgDelete = 'o FORNECEDOR'
            TitleMinHeight = 100
            CellHeights = 100
            StrippedColor = 16054260
            CanDelete = False
            CanNotInsertFromGrid = False
            PostOnEnter = False
            PostOnSetUpDown = False
            DenySort = False
            DefaultRowHeight = 19
            ExibirNumerodaLinha = False
            NaoAtribuirDadosaTabelaaoDigitar = False
            ShowWhenFieldInvisible = False
            CanClickWhenEditing = False
            Columns = <
              item
                Expanded = False
                FieldName = 'fornecedor'
                Title.Alignment = taCenter
                Title.Caption = 'C'#211'DIGO'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Width = 50
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'razao'
                Title.Alignment = taCenter
                Title.Caption = 'NOME OU RAZ'#195'O SOCIAL'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Width = 286
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'referencia'
                Title.Alignment = taCenter
                Title.Caption = 'REFER'#202'NCIA'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Width = 100
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'data'
                Title.Alignment = taCenter
                Title.Caption = 'DATA'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Width = 70
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'preco'
                Title.Alignment = taCenter
                Title.Caption = 'PRE'#199'O'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Width = 70
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'ipi'
                Title.Alignment = taCenter
                Title.Caption = '% IPI'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Width = 30
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'precocomipi'
                Title.Alignment = taCenter
                Title.Caption = 'PRE'#199'O + IPI'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Width = 70
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'dataultimaentrada'
                Title.Alignment = taCenter
                Title.Caption = #218'LTIMA DATA'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Width = 70
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'precoultimaentrada'
                Title.Alignment = taCenter
                Title.Caption = 'PRE'#199'O'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Width = 70
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'ipiultimaentrada'
                Title.Alignment = taCenter
                Title.Caption = '% IPI'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Width = 30
                Visible = True
              end
              item
                Expanded = False
                FieldName = 'precoipiultimaentrada'
                Title.Alignment = taCenter
                Title.Caption = 'PRE'#199'O + IPI'
                Title.Font.Charset = DEFAULT_CHARSET
                Title.Font.Color = clBlack
                Title.Font.Height = -9
                Title.Font.Name = 'helvetica'
                Title.Font.Pitch = fpVariable
                Title.Font.Style = []
                Width = 70
                Visible = True
              end>
          end
        end
        object tstGradeProdutos: TTabSheet
          Caption = 'Grade de Produtos'
          ImageIndex = 6
          object dbgGrades: TtecDBGrid
            Left = 0
            Top = 0
            Width = 1000
            Height = 165
            Align = alClient
            DataSource = dtmOrcamentos.dsrGradesProdutos
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -11
            Font.Name = 'helvetica'
            Font.Pitch = fpVariable
            Font.Style = []
            Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
            ParentFont = False
            ReadOnly = True
            TabOrder = 0
            TitleFont.Charset = DEFAULT_CHARSET
            TitleFont.Color = clBlack
            TitleFont.Height = -13
            TitleFont.Name = 'helvetica'
            TitleFont.Pitch = fpVariable
            TitleFont.Style = []
            OnDrawColumnCell = dbgGradesDrawColumnCell
            OnDblClick = dbgGradesDblClick
            FixedCols = 1
            RowWrap = 60
            Large = True
            DoubleRowColor = True
            TitleMinHeight = 50
            CellHeights = 258
            StrippedColor = 16054260
            CanDelete = False
            CanNotInsertFromGrid = False
            PostOnEnter = False
            PostOnSetUpDown = False
            DenySort = False
            DefaultRowHeight = 46
            ExibirNumerodaLinha = False
            NaoAtribuirDadosaTabelaaoDigitar = False
            ShowWhenFieldInvisible = False
            CanClickWhenEditing = False
            GravarConfiguracoes = False
          end
        end
      end
      object dtxCodigoGrupo: TtecDBText
        Left = 61
        Top = 317
        Width = 52
        Height = 23
        TabStop = False
        Color = clBtnFace
        DataField = 'grupo'
        DataSource = dtmOrcamentos.dsrProcuraProduto
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        ReadOnly = True
        TabOrder = 13
        Alignment = taLeftJustify
      end
      object dtxDescricaoGrupo: TtecDBText
        Left = 115
        Top = 317
        Width = 239
        Height = 23
        TabStop = False
        Color = clBtnFace
        DataField = 'descricaogrupo'
        DataSource = dtmOrcamentos.dsrProcuraProduto
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        ReadOnly = True
        TabOrder = 14
        Alignment = taLeftJustify
      end
      object dtxcodigopromocao: TtecDBText
        Left = 61
        Top = 341
        Width = 52
        Height = 23
        TabStop = False
        Color = clBtnFace
        DataField = 'codigopromocao'
        DataSource = dtmOrcamentos.dsrProcuraProduto
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        ReadOnly = True
        TabOrder = 15
        Alignment = taLeftJustify
      end
      object dtxDescricaoPromocao: TtecDBText
        Left = 115
        Top = 341
        Width = 294
        Height = 23
        TabStop = False
        Color = clBtnFace
        DataField = 'descricaopromocao'
        DataSource = dtmOrcamentos.dsrProcuraProduto
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        ReadOnly = True
        TabOrder = 16
        Alignment = taLeftJustify
      end
      object gbxLembrarEm: TGroupBox
        Left = 232
        Top = 362
        Width = 633
        Height = 37
        Caption = 
          ' DATA ENTREGA ------ PER'#205'ODO ------------------HORA-------------' +
          '-----------------------------------------------------LIMITE POR ' +
          'REGI'#195'O---|---TOTAL AGENDADO'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 17
        object sbnVisualizarEntregasProdutos: TSpeedButton
          Left = 375
          Top = 3
          Width = 50
          Height = 32
          Hint = 'Visualizar entregas agendadas'
          Glyph.Data = {
            
            F0EEEEEEF8F8F8F8F8F8F8F8F8EEEEEEEEEEEEF8F8F8FFFFFF00}
          Transparent = False
          OnClick = sbnVisualizarEntregasProdutosClick
        end
        object edttpHoraEntrega: TAdvDBDateTimePicker
          Left = 200
          Top = 10
          Width = 57
          Height = 23
          Date = 43019.366192129630000000
          Format = 'HH:mm'
          Time = 43019.366192129630000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          Kind = dkTime
          ParentFont = False
          TabOrder = 2
          AutoTab = True
          BorderStyle = bsSingle
          Ctl3D = True
          DateTime = 43019.366192129630000000
          TimeFormat = 'HH:mm'
          Version = '1.2.0.1'
          LabelFont.Charset = DEFAULT_CHARSET
          LabelFont.Color = clWindowText
          LabelFont.Height = -11
          LabelFont.Name = 'MS Sans Serif'
          LabelFont.Style = []
          DataField = 'horaentrega'
          DataSource = dtmOrcamentos.dsrProdutosOrcamentos
        end
        object edttpDataEntrega: TDBPlannerDatePicker
          Left = 3
          Top = 11
          Width = 96
          Height = 23
          AllowNumericNullValue = True
          EmptyTextFocused = True
          Flat = False
          LabelFont.Charset = DEFAULT_CHARSET
          LabelFont.Color = clWindowText
          LabelFont.Height = -11
          LabelFont.Name = 'MS Sans Serif'
          LabelFont.Style = []
          Lookup.Separator = ';'
          Color = clWindow
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          ReadOnly = False
          TabOrder = 0
          Visible = True
          Version = '1.4.0.9'
          ButtonStyle = bsButton
          ButtonWidth = 16
          Etched = False
          Glyph.Data = {
            }
          DataField = 'dataentrega'
          DataSource = dtmOrcamentos.dsrProdutosOrcamentos
          HideCalendarAfterSelection = True
          object TDBPlannerCalendar
            Left = 0
            Top = 0
            Width = 180
            Height = 180
            EventDayColor = clBlack
            EventMarkerColor = clYellow
            EventMarkerShape = evsCircle
            BackgroundPosition = bpTiled
            BevelOuter = bvNone
            BorderWidth = 1
            Look = lookFlat
            DateDownColor = clNone
            DateHoverColor = clNone
            DayFont.Charset = DEFAULT_CHARSET
            DayFont.Color = clWindowText
            DayFont.Height = -11
            DayFont.Name = 'MS Sans Serif'
            DayFont.Style = []
            WeekFont.Charset = DEFAULT_CHARSET
            WeekFont.Color = clWindowText
            WeekFont.Height = -11
            WeekFont.Name = 'MS Sans Serif'
            WeekFont.Style = []
            WeekName = 'Wk'
            TextColor = clBlack
            SelectColor = clTeal
            SelectFontColor = clWhite
            InActiveColor = clGray
            HeaderColor = clNone
            FocusColor = clHighlight
            InversColor = clTeal
            WeekendColor = clRed
            NameOfDays.Monday = 'seg'
            NameOfDays.Tuesday = 'ter'
            NameOfDays.Wednesday = 'qua'
            NameOfDays.Thursday = 'qui'
            NameOfDays.Friday = 'sex'
            NameOfDays.Saturday = 's'#225'b'
            NameOfDays.Sunday = 'dom'
            NameOfMonths.January = 'jan'
            NameOfMonths.February = 'fev'
            NameOfMonths.March = 'mar'
            NameOfMonths.April = 'abr'
            NameOfMonths.May = 'mai'
            NameOfMonths.June = 'jun'
            NameOfMonths.July = 'jul'
            NameOfMonths.August = 'ago'
            NameOfMonths.September = 'set'
            NameOfMonths.October = 'out'
            NameOfMonths.November = 'nov'
            NameOfMonths.December = 'dez'
            NameOfMonths.UseIntlNames = True
            MinDate.Day = 18
            MinDate.Month = 10
            MinDate.Year = 2017
            MinDate.Use = True
            StartDay = 7
            TodayFormat = '"Today" DDD/mm, YYYY'
            Day = 18
            Month = 10
            Year = 2017
            TabOrder = 0
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -11
            Font.Name = 'MS Sans Serif'
            Font.Style = []
            CaptionColor = clNone
            CaptionTextColor = clBlack
            LineColor = clGray
            Line3D = True
            GradientStartColor = clWhite
            GradientEndColor = clBtnFace
            GradientDirection = gdVertical
            MonthGradientStartColor = clNone
            MonthGradientEndColor = clNone
            MonthGradientDirection = gdHorizontal
            HintPrevYear = 'Previous Year'
            HintPrevMonth = 'Previous Month'
            HintNextMonth = 'Next Month'
            HintNextYear = 'Next Year'
            Version = '1.9.2.1'
            Active = True
            AutoUpdate = True
            DataSource = dtmCadastroAtendimentos.dsrAtendimento
          end
        end
        object ckbDBDefinirEntregaParaTodos: TPDBCheck
          Left = 261
          Top = 15
          Width = 103
          Height = 19
          Cursor = crArrow
          Hint = 'Atualizar os demais itens com os dados da entrega atual'
          Version = 
            'Version 1.50, Copyright '#169' 2000-2001 by Peric, E-mail: pericddn@p' +
            'tt.yu'
          DataField = 'definirdadosentregaparatodos'
          DataSource = dtmOrcamentos.dsrOrcamentos
          Caption = 'Definir para todos'
          HotTrack = False
          ImageType = igCheckBox
          ShowHandCursor = False
          ColorSipleMargine = clBlack
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          OnClick = ckbDBDefinirEntregaParaTodosClick
          TabOrder = 3
        end
        object DBLPeriodo: TDBLookupComboPlus
          Left = 99
          Top = 10
          Width = 100
          Height = 24
          DataField = 'periodoentrega'
          DataSource = dtmOrcamentos.dsrProdutosOrcamentos
          LookupSource = dtmOrcamentos.dsrPeriodosEntrega
          LookupDisplay = 'descricao'
          LookupField = 'codigo'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ListColor = clWindow
          ListFont.Charset = DEFAULT_CHARSET
          ListFont.Color = clBlack
          ListFont.Height = -12
          ListFont.Name = 'helvetica'
          ListFont.Pitch = fpVariable
          ListFont.Style = []
          ListCursor = crDefault
          ButtonCursor = crDefault
          ParentFont = False
          TabOrder = 1
        end
        object dtxLimiteporRegiao: TtecDBText
          Left = 428
          Top = 12
          Width = 98
          Height = 23
          TabStop = False
          Color = clBtnFace
          DataField = 'quantidade'
          DataSource = dtmOrcamentos.dsrlimites_entregas_regioes
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 4
          Alignment = taCenter
        end
        object dtxAgendamentos: TtecDBText
          Left = 532
          Top = 12
          Width = 98
          Height = 23
          TabStop = False
          Color = clBtnFace
          DataField = 'entregas'
          DataSource = dtmOrcamentos.dsrTotalAgendado
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          ReadOnly = True
          TabOrder = 5
          Alignment = taCenter
        end
      end
    end
  end
  object aclHabilitar: TActionList
    Left = 424
    Top = 8
    object actHabilitarControles: TAction
      Caption = 'actHabilitarControles'
      OnUpdate = actHabilitarControlesUpdate
    end
  end
  object Timer1: TTimer
    OnTimer = Timer1Timer
    Left = 546
    Top = 90
  end
end
