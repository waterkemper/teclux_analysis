inherited frmrelatoriocustomercadoriasvendidas: Tfrmrelatoriocustomercadoriasvendidas
  Left = 283
  Top = 15
  ActiveControl = fraListaFiliais1
  Caption = 'Relat'#243'rio de Custo das Mercadorias Vendidas'
  ClientHeight = 721
  ClientWidth = 839
  FormStyle = fsMDIChild
  Visible = True
  PixelsPerInch = 96
  TextHeight = 16
  inherited pnlBarra: TToolBar
    Width = 839
  end
  inherited pnlFundoJanela: TPanel
    Top = 40
    Width = 839
    Height = 681
    Align = alClient
    object gbxCustoMercadoriasVendidas: TGroupBox
      Left = 3
      Top = 3
      Width = 833
      Height = 675
      Align = alClient
      TabOrder = 0
      object pgcProdutos: TtecPageControl
        Left = 224
        Top = 79
        Width = 489
        Height = 87
        ActivePage = tstItemProduto
        TabOrder = 3
        TabWidth = 90
        OnChange = pgcProdutosChange
        object tstItemProduto: TTabSheet
          Caption = '  &Item Produto'
          object sbnConsultaItemProdutoInicial: TSpeedButton
            Left = 183
            Top = 3
            Width = 23
            Height = 23
            Hint = 'Procurar Item de Produtos'
            Flat = True
            Glyph.Data = {
              
              DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
            NumGlyphs = 2
            OnClick = sbnConsultaItemProdutoInicialClick
          end
          object sbnConsultaItemProdutoFinal: TSpeedButton
            Left = 183
            Top = 30
            Width = 23
            Height = 23
            Hint = 'Procurar Item de Produtos'
            Flat = True
            Glyph.Data = {
              
              DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
            NumGlyphs = 2
            OnClick = sbnConsultaItemProdutoFinalClick
          end
          object lblItemProdutoInicial: TLabel
            Left = 8
            Top = 10
            Width = 33
            Height = 16
            Caption = 'Inicial'
          end
          object lblItemProdutoFinal: TLabel
            Left = 8
            Top = 34
            Width = 28
            Height = 16
            Caption = 'Final'
          end
          object edfItemProdutoInicial: TtecDbEditFind
            Left = 46
            Top = 3
            Width = 136
            Height = 24
            Alignment = taLeftJustify
            DataField = 'codigo'
            DataSource = dtmrelatoriocustomercadoriasvendidas.dsrItemProdutosInicial
            Group = 'ItemProdutoInicial'
            MaxLength = 18
            Maximo = 0
            Minimo = 37353
            Adicional = 0
            Opcional = True
            TabOrder = 0
            PermitirZero = False
            PermitirNulo = False
            Operacao = opATRIBUICAO
            Parameter = 'codigo'
            ActiveSetControls = False
            DenyInsert = True
            NoSetControls = <>
            SetControls = <>
          end
          object dtxItemProdutoInicial: TtecDBText
            Left = 206
            Top = 3
            Width = 267
            Height = 24
            TabStop = False
            Color = clBtnFace
            DataField = 'descricao'
            DataSource = dtmrelatoriocustomercadoriasvendidas.dsrItemProdutosInicial
            ReadOnly = True
            TabOrder = 1
            Alignment = taLeftJustify
          end
          object edfItemProdutoFinal: TtecDbEditFind
            Left = 46
            Top = 30
            Width = 136
            Height = 24
            Alignment = taLeftJustify
            DataField = 'codigo'
            DataSource = dtmrelatoriocustomercadoriasvendidas.dsrItemProdutosFinal
            Group = 'ItemProdutoFinal'
            MaxLength = 18
            Maximo = 0
            Minimo = 37353
            Adicional = 0
            Opcional = True
            TabOrder = 2
            PermitirZero = False
            PermitirNulo = False
            Operacao = opATRIBUICAO
            Parameter = 'codigo'
            ActiveSetControls = False
            DenyInsert = True
            NoSetControls = <>
            SetControls = <>
          end
          object dtxItemProdutoFinal: TtecDBText
            Left = 206
            Top = 30
            Width = 267
            Height = 24
            TabStop = False
            Color = clBtnFace
            DataField = 'descricao'
            DataSource = dtmrelatoriocustomercadoriasvendidas.dsrItemProdutosFinal
            ReadOnly = True
            TabOrder = 3
            Alignment = taLeftJustify
          end
        end
        object tstProdutos: TTabSheet
          Caption = ' &Produto'
          ImageIndex = 2
          object sbnConsultaProdutosInicial: TSpeedButton
            Left = 167
            Top = 3
            Width = 23
            Height = 23
            Hint = 'Procurar Produtos'
            Flat = True
            Glyph.Data = {
              
              DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
            NumGlyphs = 2
            OnClick = sbnConsultaProdutosInicialClick
          end
          object sbnConsultaProdutosFinal: TSpeedButton
            Left = 167
            Top = 30
            Width = 23
            Height = 23
            Hint = 'Procurar Produto Final'
            Flat = True
            Glyph.Data = {
              
              DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
            NumGlyphs = 2
            OnClick = sbnConsultaProdutosFinalClick
          end
          object lblProdutoFinal: TLabel
            Left = 8
            Top = 34
            Width = 28
            Height = 16
            Caption = 'Final'
          end
          object lblProdutoInicial: TLabel
            Left = 8
            Top = 10
            Width = 33
            Height = 16
            Caption = 'Inicial'
          end
          object edfProdutoInicial: TtecDbEditFind
            Left = 46
            Top = 3
            Width = 120
            Height = 23
            Alignment = taLeftJustify
            DataField = 'codigo'
            DataSource = dtmrelatoriocustomercadoriasvendidas.dsrProdutosInicial
            Group = 'ProdutoInicial'
            MaxLength = 16
            Maximo = 0
            Minimo = 37353
            Adicional = 0
            Opcional = True
            TabOrder = 0
            PermitirZero = False
            PermitirNulo = False
            Operacao = opATRIBUICAO
            Parameter = 'codigo'
            ActiveSetControls = False
            DenyInsert = True
            NoSetControls = <>
            SetControls = <>
          end
          object dtxProdutoInicial: TtecDBText
            Left = 191
            Top = 3
            Width = 282
            Height = 24
            TabStop = False
            Color = clBtnFace
            DataField = 'descricao'
            DataSource = dtmrelatoriocustomercadoriasvendidas.dsrProdutosInicial
            ReadOnly = True
            TabOrder = 1
            Alignment = taLeftJustify
          end
          object edfProdutoFinal: TtecDbEditFind
            Left = 46
            Top = 30
            Width = 120
            Height = 23
            Alignment = taLeftJustify
            DataField = 'codigo'
            DataSource = dtmrelatoriocustomercadoriasvendidas.dsrProdutosFinal
            Group = 'ProdutoFinal'
            MaxLength = 16
            Maximo = 0
            Minimo = 37353
            Adicional = 0
            Opcional = True
            TabOrder = 3
            PermitirZero = False
            PermitirNulo = False
            Operacao = opATRIBUICAO
            Parameter = 'codigo'
            ActiveSetControls = False
            DenyInsert = True
            NoSetControls = <>
            SetControls = <>
          end
          object dtxProdutoFinal: TtecDBText
            Left = 191
            Top = 30
            Width = 282
            Height = 24
            TabStop = False
            Color = clBtnFace
            DataField = 'descricao'
            DataSource = dtmrelatoriocustomercadoriasvendidas.dsrProdutosFinal
            ReadOnly = True
            TabOrder = 2
            Alignment = taLeftJustify
          end
        end
        object tstGrupodeProdutos: TTabSheet
          Caption = ' &Grupo'
          ImageIndex = 1
          object sbnConsultaGrupoProdutoInicial: TSpeedButton
            Left = 107
            Top = 3
            Width = 22
            Height = 23
            Hint = 'Procurar grupos de Produtos'
            Flat = True
            Glyph.Data = {
              
              DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
            NumGlyphs = 2
            OnClick = sbnConsultaGrupoProdutoInicialClick
          end
          object sbnConsultaGrupoProdutoFinal: TSpeedButton
            Left = 107
            Top = 30
            Width = 23
            Height = 23
            Hint = 'Procurar grupos de Produtos'
            Flat = True
            Glyph.Data = {
              
              DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
            NumGlyphs = 2
            OnClick = sbnConsultaGrupoProdutoFinalClick
          end
          object lblGrupoProdutoInicial: TLabel
            Left = 8
            Top = 10
            Width = 33
            Height = 16
            Caption = 'Inicial'
          end
          object lblGrupoProdutoFinal: TLabel
            Left = 8
            Top = 34
            Width = 28
            Height = 16
            Caption = 'Final'
          end
          object edfGrupoProdutoInicial: TtecDbEditFind
            Left = 45
            Top = 3
            Width = 60
            Height = 24
            Alignment = taLeftJustify
            CharCase = ecUpperCase
            DataField = 'codigo'
            DataSource = dtmrelatoriocustomercadoriasvendidas.dsrGrupoProdutosInicial
            Group = 'GrupoInicial'
            MaxLength = 3
            Maximo = 0
            Minimo = 37353
            Adicional = 0
            Opcional = True
            TabOrder = 0
            PermitirZero = False
            PermitirNulo = False
            Operacao = opATRIBUICAO
            Parameter = 'codigo'
            ActiveSetControls = False
            DenyInsert = True
            NoSetControls = <>
            SetControls = <>
          end
          object dtxGrupoProdutoInicial: TtecDBText
            Left = 130
            Top = 3
            Width = 343
            Height = 24
            TabStop = False
            Color = clBtnFace
            DataField = 'descricao'
            DataSource = dtmrelatoriocustomercadoriasvendidas.dsrGrupoProdutosInicial
            ReadOnly = True
            TabOrder = 1
            Alignment = taLeftJustify
          end
          object edfGrupoProdutoFinal: TtecDbEditFind
            Left = 46
            Top = 30
            Width = 60
            Height = 24
            Alignment = taLeftJustify
            CharCase = ecUpperCase
            DataField = 'codigo'
            DataSource = dtmrelatoriocustomercadoriasvendidas.dsrGrupoProdutosFinal
            Group = 'GrupoFinal'
            MaxLength = 3
            Maximo = 0
            Minimo = 37353
            Adicional = 0
            Opcional = True
            TabOrder = 2
            PermitirZero = False
            PermitirNulo = False
            Operacao = opATRIBUICAO
            Parameter = 'codigo'
            ActiveSetControls = False
            DenyInsert = True
            NoSetControls = <>
            SetControls = <>
          end
          object dtxGrupoProdutoFinal: TtecDBText
            Left = 131
            Top = 30
            Width = 342
            Height = 24
            TabStop = False
            Color = clBtnFace
            DataField = 'descricao'
            DataSource = dtmrelatoriocustomercadoriasvendidas.dsrGrupoProdutosFinal
            ReadOnly = True
            TabOrder = 3
            Alignment = taLeftJustify
          end
        end
        object tstClasses: TTabSheet
          Caption = ' Cla&sse'
          ImageIndex = 3
          object sbnConsultaClasseInicial: TSpeedButton
            Left = 107
            Top = 3
            Width = 23
            Height = 23
            Hint = 'Procurar Classes'
            Flat = True
            Glyph.Data = {
              
              DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
            NumGlyphs = 2
            OnClick = sbnConsultaClasseInicialClick
          end
          object sbnConsultaClasseFinal: TSpeedButton
            Left = 107
            Top = 30
            Width = 23
            Height = 23
            Hint = 'Procurar Classe Final'
            Flat = True
            Glyph.Data = {
              
              DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
            NumGlyphs = 2
            OnClick = sbnConsultaClasseFinalClick
          end
          object lblClasseProdutoInicial: TLabel
            Left = 8
            Top = 10
            Width = 33
            Height = 16
            Caption = 'Inicial'
          end
          object lblClasseProdutoFinal: TLabel
            Left = 8
            Top = 34
            Width = 28
            Height = 16
            Caption = 'Final'
          end
          object edfClasseInicial: TtecDbEditFind
            Left = 46
            Top = 3
            Width = 60
            Height = 23
            Alignment = taLeftJustify
            DataField = 'codigo'
            DataSource = dtmrelatoriocustomercadoriasvendidas.dsrClassesInicial
            Group = 'ClasseInicial'
            MaxLength = 3
            Maximo = 0
            Minimo = 37353
            Adicional = 0
            Opcional = True
            TabOrder = 0
            PermitirZero = False
            PermitirNulo = False
            Operacao = opATRIBUICAO
            Parameter = 'codigo'
            ActiveSetControls = False
            DenyInsert = True
            NoSetControls = <>
            SetControls = <>
          end
          object dtxClasseInicial: TtecDBText
            Left = 131
            Top = 3
            Width = 342
            Height = 24
            TabStop = False
            Color = clBtnFace
            DataField = 'descricao'
            DataSource = dtmrelatoriocustomercadoriasvendidas.dsrClassesInicial
            ReadOnly = True
            TabOrder = 1
            Alignment = taLeftJustify
          end
          object edfClasseFinal: TtecDbEditFind
            Left = 46
            Top = 30
            Width = 60
            Height = 23
            Alignment = taLeftJustify
            DataField = 'codigo'
            DataSource = dtmrelatoriocustomercadoriasvendidas.dsrClassesFinal
            Group = 'ClasseFinal'
            MaxLength = 3
            Maximo = 0
            Minimo = 37353
            Adicional = 0
            Opcional = True
            TabOrder = 2
            PermitirZero = False
            PermitirNulo = False
            Operacao = opATRIBUICAO
            Parameter = 'codigo'
            ActiveSetControls = False
            DenyInsert = True
            NoSetControls = <>
            SetControls = <>
          end
          object dtxClasseFinal: TtecDBText
            Left = 131
            Top = 30
            Width = 342
            Height = 24
            TabStop = False
            Color = clBtnFace
            DataField = 'descricao'
            DataSource = dtmrelatoriocustomercadoriasvendidas.dsrClassesFinal
            ReadOnly = True
            TabOrder = 3
            Alignment = taLeftJustify
          end
        end
      end
      object gbxPeriodo: TGroupBox
        Left = 634
        Top = 209
        Width = 190
        Height = 73
        Caption = 'Faturamento entre'
        TabOrder = 5
        object lblDataInicial: TLabel
          Left = 9
          Top = 24
          Width = 64
          Height = 16
          Alignment = taRightJustify
          Caption = 'Data Inicial'
        end
        object lblDataFinal: TLabel
          Left = 14
          Top = 48
          Width = 59
          Height = 16
          Alignment = taRightJustify
          Caption = 'Data Final'
        end
        object edtDataInicial: TEditData
          Left = 89
          Top = 20
          Width = 80
          Height = 24
          MaxLength = 10
          TabOrder = 0
          Minimo = 37353
          Maximo = 0
        end
        object edtDataFinal: TEditData
          Left = 89
          Top = 44
          Width = 80
          Height = 24
          MaxLength = 10
          TabOrder = 1
          Minimo = 37353
          Maximo = 0
        end
      end
      object gbxProdutos: TGroupBox
        Left = 224
        Top = 208
        Width = 129
        Height = 153
        Caption = 'Produtos'
        TabOrder = 6
        object ckbProdutoEmLinha: TCheckBox
          Left = 8
          Top = 13
          Width = 97
          Height = 25
          Caption = '&Em Linha'
          Checked = True
          State = cbChecked
          TabOrder = 0
          OnClick = ckbProdutoEmLinhaClick
        end
        object ckbProdutoForadeLinha: TCheckBox
          Left = 8
          Top = 35
          Width = 113
          Height = 25
          Caption = '&Fora de Linha'
          Checked = True
          State = cbChecked
          TabOrder = 1
          OnClick = ckbProdutoForadeLinhaClick
        end
        object ckbProdutoComMontagem: TCheckBox
          Left = 8
          Top = 57
          Width = 113
          Height = 25
          Caption = '&Com Montagem'
          Checked = True
          State = cbChecked
          TabOrder = 2
          OnClick = ckbProdutoComMontagemClick
        end
        object ckbProdutoSemMontagem: TCheckBox
          Left = 8
          Top = 80
          Width = 113
          Height = 25
          Caption = '&Sem Montagem'
          Checked = True
          State = cbChecked
          TabOrder = 3
          OnClick = ckbProdutoSemMontagemClick
        end
        object ckbProdutoNaoBrinde: TCheckBox
          Left = 8
          Top = 125
          Width = 89
          Height = 25
          Caption = '&N'#227'o Brinde'
          Checked = True
          State = cbChecked
          TabOrder = 4
          OnClick = ckbProdutoNaoBrindeClick
        end
        object ckbProdutoBrinde: TCheckBox
          Left = 8
          Top = 102
          Width = 89
          Height = 25
          Caption = '&Brinde'
          Checked = True
          State = cbChecked
          TabOrder = 5
          OnClick = ckbProdutoBrindeClick
        end
      end
      object gbxListar: TGroupBox
        Left = 634
        Top = 283
        Width = 190
        Height = 78
        Caption = 'Listar'
        TabOrder = 12
        object ckbListarVendas: TCheckBox
          Left = 8
          Top = 15
          Width = 153
          Height = 25
          Caption = 'Vendas do Periodo'
          Checked = True
          State = cbChecked
          TabOrder = 0
        end
        object ckbListarDevolucoesdoPeriodo: TCheckBox
          Left = 8
          Top = 33
          Width = 177
          Height = 25
          Caption = 'S'#243' Devolu'#231#245'es do Per'#237'odo'
          Checked = True
          State = cbChecked
          TabOrder = 1
        end
        object ckbListarDevolucoesAnteriores: TCheckBox
          Left = 8
          Top = 51
          Width = 169
          Height = 25
          Caption = 'Devolu'#231#245'es Anteriores'
          Checked = True
          State = cbChecked
          TabOrder = 2
        end
      end
      object rgpOrdenacao: TRadioGroup
        Left = 487
        Top = 207
        Width = 143
        Height = 113
        Caption = 'Ordena'#231#227'o'
        ItemIndex = 0
        Items.Strings = (
          'Data e &Nr. Nota'
          '&Data e Produto'
          '&Produto e Data'
          '&Vendedor e Data')
        TabOrder = 8
      end
      object rgpTotalizacao: TRadioGroup
        Left = 360
        Top = 312
        Width = 121
        Height = 105
        Caption = 'Totaliza'#231#227'o'
        ItemIndex = 3
        Items.Strings = (
          '&Produtos'
          '&Grupos'
          '&Classes'
          '&Nr. Nota')
        Constraints.MaxHeight = 125
        Constraints.MaxWidth = 128
        TabOrder = 9
      end
      object rgpTipodeResumo: TRadioGroup
        Left = 634
        Top = 368
        Width = 190
        Height = 49
        Caption = 'Tipo de Resumo'
        ItemIndex = 0
        Items.Strings = (
          'Total por Datas'
          'Somente Totais')
        TabOrder = 10
      end
      object rgpResumo: TRadioGroup
        Left = 487
        Top = 328
        Width = 143
        Height = 89
        Caption = 'Resumo'
        ItemIndex = 0
        Items.Strings = (
          'Com R&esumo'
          '&Sem Resumo'
          'S'#243' Res&umo')
        TabOrder = 11
      end
      object gbxMostrar: TGroupBox
        Left = 224
        Top = 364
        Width = 129
        Height = 53
        Caption = 'Mostrar'
        TabOrder = 13
        object ckbMostrarPlanos: TCheckBox
          Left = 8
          Top = 16
          Width = 97
          Height = 25
          Caption = 'Planos'
          TabOrder = 0
        end
      end
      object gbxAgrupamento: TGroupBox
        Left = 360
        Top = 208
        Width = 121
        Height = 93
        Caption = 'Agrupamento'
        TabOrder = 7
        object ckbAgruparGrupoFilial: TCheckBox
          Left = 6
          Top = 19
          Width = 107
          Height = 15
          Caption = '&Grupo de Filial'
          TabOrder = 0
        end
        object ckbAgruparFilial: TCheckBox
          Left = 6
          Top = 37
          Width = 107
          Height = 15
          Caption = '&Filial'
          Checked = True
          State = cbChecked
          TabOrder = 1
        end
        object ckbAgruparCliente: TCheckBox
          Left = 4
          Top = 74
          Width = 115
          Height = 15
          Caption = '&Cliente'
          TabOrder = 3
        end
        object ckbAgruparConceitos: TCheckBox
          Left = 5
          Top = 56
          Width = 92
          Height = 15
          Caption = 'Conc&eitos'
          TabOrder = 2
        end
      end
      object gbxClientes: TGroupBox
        Left = 224
        Top = 8
        Width = 489
        Height = 69
        Caption = 'Clientes'
        TabOrder = 14
        object sbnConsultaClienteFinal: TSpeedButton
          Left = 123
          Top = 42
          Width = 23
          Height = 23
          Hint = 'Procurar grupos de Produtos'
          Flat = True
          Glyph.Data = {
            
            DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
          NumGlyphs = 2
          OnClick = sbnConsultaClienteFinalClick
        end
        object sbnConsultaClienteInicial: TSpeedButton
          Left = 123
          Top = 15
          Width = 23
          Height = 23
          Hint = 'Procurar grupos de Produtos'
          Flat = True
          Glyph.Data = {
            
            DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
          NumGlyphs = 2
          OnClick = sbnConsultaClienteInicialClick
        end
        object lblInicial: TLabel
          Left = 8
          Top = 19
          Width = 33
          Height = 16
          Caption = 'Inicial'
        end
        object lblFinal: TLabel
          Left = 8
          Top = 43
          Width = 28
          Height = 16
          Caption = 'Final'
        end
        object dtxClienteInicial: TtecDBText
          Left = 147
          Top = 15
          Width = 334
          Height = 24
          TabStop = False
          Color = clBtnFace
          DataField = 'nome'
          DataSource = dtmrelatoriocustomercadoriasvendidas.dsrClienteInicial
          ReadOnly = True
          TabOrder = 0
          Alignment = taLeftJustify
        end
        object dtxClienteFinal: TtecDBText
          Left = 147
          Top = 42
          Width = 334
          Height = 24
          TabStop = False
          Color = clBtnFace
          DataField = 'nome'
          DataSource = dtmrelatoriocustomercadoriasvendidas.dsrClienteFinal
          ReadOnly = True
          TabOrder = 2
          Alignment = taLeftJustify
        end
        object edfClienteInicial: TtecDbEditFind
          Left = 48
          Top = 15
          Width = 73
          Height = 24
          Alignment = taLeftJustify
          DataField = 'codigo'
          DataSource = dtmrelatoriocustomercadoriasvendidas.dsrClienteInicial
          Group = 'ClienteInicial'
          MaxLength = 9
          Maximo = 0
          Minimo = 37353
          Adicional = 0
          Opcional = True
          TabOrder = 1
          PermitirZero = False
          PermitirNulo = False
          Operacao = opATRIBUICAO
          Parameter = 'codigo'
          ActiveSetControls = False
          DenyInsert = True
          NoSetControls = <>
          SetControls = <>
        end
        object edfClienteFinal: TtecDbEditFind
          Left = 48
          Top = 42
          Width = 73
          Height = 24
          Alignment = taLeftJustify
          DataField = 'codigo'
          DataSource = dtmrelatoriocustomercadoriasvendidas.dsrClienteFinal
          Group = 'ClienteFinal'
          MaxLength = 9
          Maximo = 0
          Minimo = 37353
          Adicional = 0
          Opcional = True
          TabOrder = 3
          PermitirZero = False
          PermitirNulo = False
          Operacao = opATRIBUICAO
          Parameter = 'codigo'
          ActiveSetControls = False
          DenyInsert = True
          NoSetControls = <>
          SetControls = <>
        end
      end
      object rgpSelecaoClientes: TRadioGroup
        Left = 721
        Top = 8
        Width = 106
        Height = 69
        Caption = '&Sele'#231#227'o'
        ItemIndex = 1
        Items.Strings = (
          'C'#243'digo'
          'Descri'#231#227'o')
        Constraints.MaxWidth = 290
        Constraints.MinWidth = 90
        TabOrder = 15
      end
      object rgpSelecaoProduto: TRadioGroup
        Left = 720
        Top = 96
        Width = 106
        Height = 70
        Caption = '&Sele'#231#227'o'
        ItemIndex = 1
        Items.Strings = (
          'C'#243'digo'
          'Descri'#231#227'o')
        Constraints.MaxWidth = 290
        Constraints.MinWidth = 90
        TabOrder = 16
      end
      inline fraListaFiliais1: TfraListaFiliais
        Left = 5
        Top = 8
        Width = 215
        Height = 225
        TabOrder = 0
        inherited gbxFiliais: TGroupBox
          Width = 215
          Height = 225
          inherited sbnMarcarFiliais: TSpeedButton
            Left = 161
            Top = 198
            OnClick = fraListaFiliais1sbnMarcarFiliaisClick
          end
          inherited sbnDesmarcarFiliais: TSpeedButton
            Left = 185
            Top = 198
          end
          inherited clbFiliais: TCheckListBox
            Width = 204
            Height = 176
            OnClickCheck = fraListaFiliais1clbFiliaisClickCheck
          end
        end
      end
      inline fraListaGruposFiliais1: TfraListaGruposFiliais
        Left = 4
        Top = 243
        Width = 217
        Height = 174
        TabOrder = 1
        inherited gbxGrupodeFiliais: TGroupBox
          Width = 217
          Height = 174
          inherited sbnmarcarGrupodeFiliais: TSpeedButton
            Left = 164
            Top = 146
            OnClick = fraListaGruposFiliais1sbnmarcarGrupodeFiliaisClick
          end
          inherited sbnDesmarcarGrupodeFiliais: TSpeedButton
            Left = 188
            Top = 146
          end
          inherited clbGrupodeFiliais: TCheckListBox
            Width = 205
            Height = 124
            OnClickCheck = fraListaGruposFiliais1clbGrupodeFiliaisClickCheck
          end
        end
      end
      object gbxMaquinas: TGroupBox
        Left = 0
        Top = 419
        Width = 353
        Height = 118
        Caption = ' REGI'#213'ES'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        ParentFont = False
        TabOrder = 2
        inline fraSelecaoAleatoriaRegioes: TfraSelecaoAleatoria
          Left = 2
          Top = 14
          Width = 349
          Height = 102
          HorzScrollBar.Range = 26
          Align = alClient
          AutoScroll = False
          TabOrder = 0
          inherited Splitter1: TSplitter
            Left = 349
            Height = 87
          end
          inherited dbgSelecaoAleatoria: TtecDBGrid
            Width = 318
            Height = 87
            Font.Height = -12
            Options = [dgEditing, dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgCancelOnExit]
            OnDblClick = fraSelecaoAleatoriaRegioesdbgSelecaoAleatoriaDblClick
            OnKeyDown = fraSelecaoAleatoriaRegioesdbgSelecaoAleatoriaKeyDown
          end
          inherited pnlLabels: TPanel
            Left = 318
            Height = 87
            inherited sbnProcura: TSpeedButton
              OnClick = fraSelecaoAleatoriaRegioessbnProcuraClick
            end
          end
          inherited pnlAbaixo: TPanel
            Top = 87
            Width = 349
            Font.Pitch = fpVariable
            inherited lblOrientacao: TLabel
              Font.Pitch = fpVariable
            end
            inherited pnlAbaixoDireita: TPanel
              Left = 318
              Font.Pitch = fpVariable
            end
          end
          inherited qrySelecaoAleatoria: TtecQuery
            AfterOpen = fraSelecaoAleatoriaRegioesqrySelecaoAleatoriaAfterOpen
            Sql.Strings = (
              'select r.*'
              'from regioes r'
              'where false')
          end
        end
      end
      object gbxConceitos: TGroupBox
        Left = 359
        Top = 416
        Width = 254
        Height = 121
        Caption = '&Conceitos'
        TabOrder = 17
        object sbnMarcarConceitos: TSpeedButton
          Left = 224
          Top = 18
          Width = 22
          Height = 22
          Hint = 'Marcar Conceitos'
          Flat = True
          Glyph.Data = {
            
            DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
          NumGlyphs = 2
        end
        object sbnDesmarcarConceitos: TSpeedButton
          Left = 224
          Top = 42
          Width = 22
          Height = 22
          Hint = 'Desmarcar Conceitos'
          Flat = True
          Glyph.Data = {
            
            DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
          NumGlyphs = 2
        end
        object clbConceitos: TCheckListBox
          Left = 8
          Top = 17
          Width = 209
          Height = 96
          ItemHeight = 15
          Style = lbOwnerDrawVariable
          TabOrder = 0
        end
      end
      object gbxFornecedor: TGroupBox
        Left = 224
        Top = 168
        Width = 603
        Height = 37
        Caption = 'FORNECEDOR'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -9
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 4
        inline fraConsultaFornecedor: TfraConsultaCodigo
          Left = 3
          Top = 11
          Width = 597
          Height = 22
          HorzScrollBar.Range = 631
          HorzScrollBar.Visible = False
          VertScrollBar.Range = 23
          VertScrollBar.Visible = False
          AutoScroll = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'helvetica'
          Font.Pitch = fpVariable
          Font.Style = []
          ParentFont = False
          TabOrder = 0
          inherited dtxDescricao: TtecDBText
            Left = 89
            Top = -1
            Width = 509
            DataField = 'nome'
            DataSource = fraConsultaFornecedor.dsrProcuraCliente
          end
          inherited edfCodigo: TtecDBFindLookup
            DataField = 'codigo'
            DataSource = fraConsultaFornecedor.dsrProcuraCliente
            Group = 'FORNECEDOR'
            MaxLength = 6
            OnFound = nil
            Operacao = opPESQUISA
            Parameter = 'codigo'
            ActiveSetControls = False
            DenyInsert = True
            NoSetControls = <>
            SetControls = <>
          end
        end
      end
      object gbxListaCondicionalVendedores: TGroupBox
        Left = 0
        Top = 536
        Width = 615
        Height = 136
        Caption = 'Vendedores'
        TabOrder = 18
        inline fraSelecaoAleatoriavendedores: TfraSelecaoAleatoriavendedores
          Left = 2
          Top = 18
          Width = 611
          Height = 116
          Align = alClient
          TabOrder = 0
          inherited fraSelecaoAleatoriavendedores: TfraSelecaoAleatoria
            Width = 611
            Height = 116
            inherited Splitter1: TSplitter
              Left = 576
              Height = 101
            end
            inherited dbgSelecaoAleatoria: TtecDBGrid
              Width = 576
              Height = 101
            end
            inherited pnlLabels: TPanel
              Left = 579
              Height = 101
            end
            inherited pnlAbaixo: TPanel
              Top = 101
              Width = 611
              Font.Pitch = fpVariable
              inherited lblOrientacao: TLabel
                Font.Pitch = fpVariable
              end
              inherited pnlAbaixoDireita: TPanel
                Left = 580
                Font.Pitch = fpVariable
              end
            end
          end
        end
      end
    end
  end
end
