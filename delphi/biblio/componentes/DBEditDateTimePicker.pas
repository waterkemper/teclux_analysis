...
uses
  Windows, Messages, SysUtils, Classes, Controls, ComCtrls, DBCtrls, Forms, DB,
  CommCtrl, DateUtils, MaskUtils, Dialogs, ctconstantes, Graphics, biblio, variants;
...
type
  ...
  published
    ...
    property ConfirmDateBlank  : boolean read fConfirmDateBlank write fConfirmDateBlank;
  end;
...
begin
    ...
    begin
	    ...
	    begin
        	...
        	begin
	            // leave it with empty mask until field update
	        end;
	    end;
	   	inherited;
    end
...
procedure TDateTimePickerDBEdit.DoEnter;
begin
  inherited;
  if Focused then
...
procedure TDateTimePickerDBEdit.DoExit;
begin
  inherited;
  Color:= clWindow;
end;
...
begin
    ...
    else                {Gedovar Duthevicz 14/06/2019 }
    if key = #13 then  {ao dar enter componente esta colocando a tabela erroneamente em edição, anulando para evitar}
      key := #0;
...
procedure TDBEditDateTimePicker.DoExit;
begin
  inherited;
  if ConfirmDateBlank and (somentenumero(DBEdit.Text) = '') then
    if MensagemConfirmacao('A data esta em branco. Confirma?') = smbCancel then
      self.setfocus;
end;
