...
type
  ...
  protected
    ...
    function  FindQueryByParam(qr: TtecQuery; Param: String {TrocarLargeInt: boolean = false}): Boolean;
...
//    procedure SetParent(const Value: TWinControl); override; CLX_TO_VCL
  ...
  public
    ...
    property  Query: TtecQuery read GetQuery;
...
//    FLookupParameterDefault: String;
    ...
    function  GetLookupQuery: TtecQuery;
  ...
  public
    ...
    constructor Create(AOwner: TComponent); override;
    destructor  Destroy; override;
    property LookupQuery: TtecQuery read GetLookupQuery;
...
var
  a, Count: Integer;
  Ctrl: TtecFindCustom;
  QueryInsert: TtecQuery;
begin
  Result := stUndefined;
  QueryInsert := TtecQuery.Create(Owner);
...
//      if operacao = opATRIBUICAO then {Verificar a razão pois esta incompativel com as pesquisas dos relatórios}
  ...
  else if ForeignFound = stFound then begin
    SetControl(True);
    if Group = ''  then
      UpdateData
    else
...
function TtecFindCustom.FindQueryByParam(qr: TtecQuery; Param: String {TrocarLargeInt: boolean = false}): Boolean;
var
  Str: String;
  Par: TParam;
  a,vParIndex: Integer;
    ...
    if Assigned(Par) and (Par.Index > -1) then begin
      for a := 0 to qr.ParamCount - 1 do begin
        ...
        begin
          ...
          ftString, ftUnknown : qr.Params[a].AsString := Trim(Str);
      ...
      try
        ...
        begin
          {if qr.Params[Par.Index].DataType in [ftString, ftFixedChar, ftWideString] then
          begin}
//            if ((ANSIUpperCase(qr.Params[Par.Index].AsString) <> ANSIUpperCase(Trim(Str))) and
//                ((qr.RecordCount = 0) or (group <> ''))) then
              ...
              begin
                case FDataLink.Field.DataType of
                  ...
                  ftLargeInt            : begin
                                            ...
                                            ftString, ftUnknown :  qr.Params[Par.Index].Asstring :=   ANSIUpperCase(Trim(Str));
                                            ftlargeint: begin
...
function TtecFindCustom.GetQuery: TtecQuery;
begin

  if Assigned(FDataLink.DataSet) then
    Result := TtecQuery(FDataLink.DataSet)
  else
    Result := nil;
end;
...
begin
  ...
  if (FDataLink.Field.DataType in [ftstring])
     and (Key in [#32..#255])
...
begin
  if Group = '' then
    ...
    FindCustomList.Items[FIndexFormList, Group].OnFound := Value
end;
...
function TtecDBFindLookup.GetLookupQuery: TtecQuery;
begin
  if Assigned(FDataLinkLookup.DataSet) then
    Result := TtecQuery(FDataLinkLookup.DataSet)
  else
    Result := nil
end;
...
begin
  ...
  if (Operation = opRemove) and (AComponent <> Self) then
  begin
    if Assigned(FDataLink) and (AComponent = LookupSource) then
      LookupSource := nil;
  end
end;
...
begin
  ...
  begin
    ...
    if (LookupParameters <> '') and
       (LookupQuery.ParamByName(value).DataType = ftString) and
