...
procedure TDBEditLCPFCNPJ.Change;
var
  NC:    byte;
  Novo:  string;
begin
...
constructor TDBEditLCPFCNPJ.Create(AOwner: TComponent);
begin
  inherited;
  FTipo:= '';
  MaxLength := 14;
...
function TDBEditLCPFCNPJ.Criticar(Mostrar: Boolean): Boolean;
...
procedure TDBEditLCPFCNPJ.DoChangeTipo;
begin
  if Assigned(OnChangeTipo) then
    OnChangeTipo(Self)
end;

procedure TDBEditLCPFCNPJ.DoEnter;
begin
  inherited;
  if Focused then
...
procedure TDBEditLCPFCNPJ.DoExit;
begin
  inherited;
  Color:= clWindow;
...
procedure TDBEditLCPFCNPJ.KeyDown(var Key: Word; Shift: TShiftState);
begin
  inherited;
  If (Key in [32..255])
...
procedure TDBEditLCPFCNPJ.Loaded;
begin
//  inherited Loaded;
//    Field.EditMask:= MascaraPessoaJuridica;
end;

procedure TDBEditLCPFCNPJ.Resize;
begin
...
procedure TDBEditLCPFCNPJ.SetTipo(const Value: String);
begin
