...
type
  TTipoMascara = (tMascaraContabil, tMascaraBanco, tMascaraReferencial);
type
  ...
  public
    ...
    property SetandoValor: Boolean read FSetandoValor write FSetandoValor;
  published
...
begin
   ...
   if assigned(datasource) and Assigned(Field) and not SetandoValor then
   begin
     SetandoValor := true;

     Text := FormatMaskText(field.EditMask, text);
     if datasource.state in [dsedit, dsinsert] then
       if (field.value<>trim(Trocar(text,'. ','')) ) and
...
//           Nivel := parsistema.mascaracontabil[1].indexof(copy(text,1,1));
           ...
           begin
             ...
             if MaxLength <> length(ParSistema.mascaracontabil[2].Strings[nivel]) then
...
begin
  ...
  begin
    case TipodeMascara of
      ...
      tMascaraReferencial : field.EditMask:= '9.99.99.99.99.99.99;1;_';
    end;
  end;
end;
