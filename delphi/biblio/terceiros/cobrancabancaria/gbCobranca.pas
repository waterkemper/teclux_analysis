...
uses
   Windows, classes, SysUtils, Graphics, extctrls, gbLogoBanco, Forms
   {$IFDEF VER150}
...
type
   ...
   {Opção de LogoMarca ou Nome do Banco}
   TLogoBanco = (lbLogoMarca, lbNomeBanco);
   {Opção de LogoMarca em Cor ou PB}
   TCorLogo = (clPB, clCOR);
   ...
   private
      ...
      FTecUsodoBanco: String;
      FTecEspecieDocumento: String; {Indica quem emite o boleto: banco ou cliente}
      fLogoBanco: TLogoBanco; {Indica se vai aparecer a Logomarca ou o nome do banco}
      fCorLogo: TCorLogo;
//      fLog: String;
...
//      procedure   EnviarPorEMail(Host, LoginUsuario : string; Porta :integer; Assunto : string; Mensagem : TStringList);
   ...
   published
      ...
      property TecUsodoBanco: String read FTecUsodoBanco write FTecUsodobanco;
      property TecEspecieDocumento: String read FTecEspecieDocumento write FTecEspecieDocumento;
      property LogoBanco : TLogoBanco read fLogoBanco write fLogoBanco;
      property CorLogo : TCorLogo read fCorLogo write fCorLogo;
   end;
...
uses gbCob001, gbCob021, gbCob033, gbCob038, gbCob104, gbCob151, gbCob237, gbCob244, gbCob275, gbCob291, gbCob320, gbCob341, gbCob347, gbCob353, gbCob356, gbCob389, gbCob394, gbCob399, gbCob409, gbCob422, gbCob479, gbCob745;
...
begin
   ...
   fLogoBanco := lbNomeBanco;
   fCorLogo := clPB;
//   fLog:='c:\temp\logboleto.txt';
end;
...
//   WriteToTXT(flog,'dignossonumero='+result);
end;
...
{
   WriteToTXT(flog,'banco='+acodigobanco);
   WriteToTXT(flog,'moeda='+acodigomoeda);
   WriteToTXT(flog,'fator='+afatorvencimento);
   WriteToTXT(flog,'valor='+avalordocumento);
   ...
   begin
      ...
      TRY
         ...
         if @GetCampoLivreCodigoBarra <> nil then
            ACampoLivre :=  GetCampoLivreCodigoBarra(Self)// + DigitoNossoNumero
         else
      ...
      Raise Exception.CreateFmt('Os boletos para o banco %s não estão disponíveis',[ACodigoBanco]);
//   WriteToTXT(flog,'campolivre='+acampolivre);
...
//   WriteToTXT(flog,'codigobarras='+acodigobarras);
//   WriteToTXT(flog,'digitocodigobarras='+adigitocodigobarras);
   Result.Codigo := ACodigoBanco + ACodigoMoeda + ADigitoCodigoBarras + AFatorVencimento + AValorDocumento + ACampoLivre;
   //Copy(ACodigoBarras,1,4) + ADigitoCodigoBarras + Copy(ACodigoBarras,5,length(ACodigoBarras)-4);
//   WriteToTXT(flog,'result.codigo='+Result.Codigo);
...
var
   ...
   Ind: Integer;
begin
  if fLogoBanco = lbLogoMarca then
  begin
    Frm := TfrmLogoBanco.Create(Application);
    try
      Img := TImage.Create(nil);
      Img.Height := 38;
      Img.Width  := 149;

      Bco := '000' + Cedente.ContaBancaria.Banco.Codigo;
      Bco := Copy(Bco, Length(Bco)-2, 3);
      if FCorLogo = clPB then
        Bco := 'imgP' + Bco
      else
        Bco := 'imgC' + Bco;

      for Ind := 0 to Frm.ComponentCount-1 do begin
       if Frm.Components[Ind].Name = Bco then begin
          Img.Picture.Assign(TImage(Frm.Components[Ind]).Picture);
          Break;
       end;
      end;
    finally
   ...
   AEspecieDocumento := TecEspecieDocumento;
   ...
   begin
      ...
      if fLogoBanco = lbLogomarca then
        imgLogo1.Picture.Assign(Img.Picture);
      imgLogo1.Enabled    := fLogoBanco = lbLogomarca;
      txtNomeBanco.Enabled:= Not(fLogoBanco = lbLogomarca);
      ...
      txtUsoBanco.Caption := FTecUsodoBanco;
...
(*
      ...
      if fLogoBanco = lbLogomarca then
        imgLogo2.Picture.Assign(Img.Picture);
      imgLogo2.Enabled    := fLogoBanco = lbLogomarca;
      txtNomeBanco2.Enabled:= Not(fLogoBanco = lbLogomarca);
      ...
      txtUsoBanco2.Caption := FTecUsodoBanco;
...
*)
      ...
      if fLogoBanco = lbLogomarca then
        imgLogo3.Picture.Assign(Img.Picture);
      imgLogo3.Enabled    := fLogoBanco = lbLogomarca;
      txtNomeBanco3.Enabled:= Not(fLogoBanco = lbLogomarca);
      ...
      txtUsoBanco3.Caption := FTecUsodoBanco;
...
//      WriteToTXT(flog,'linhadigitavel='+codigobarra.linhadigitavel);
