inherited frmFCI: TfrmFCI
  Tag = 1
  Left = 406
  Top = 156
  Caption = 'FCI - Fichario de Conte'#250'do de Importa'#231#227'o'
  ClientHeight = 583
  ClientWidth = 1466
  Font.Color = clWindowText
  Font.Height = -12
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Visible = True
  PixelsPerInch = 96
  TextHeight = 15
  inherited pnlBarra: TPanel
    Top = 98
    Width = 991
    Height = 0
  end
  inherited pnlTopMenu: TPanel
    Width = 1466
    inherited tblBarra: TToolBar
      Width = 1465
      inherited sbnProcurar: TSpeedButton
        Visible = False
      end
      inherited sbnAjuda: TSpeedButton
        Action = actHabilitar
      end
      object sbnGerar: TSpeedButton
        Left = 158
        Top = 2
        Width = 97
        Height = 35
        Hint = 'gerar dados FCI'
        Caption = 'Gerar dados F7'
        Flat = True
        Glyph.Data = {
          
          DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
        Layout = blGlyphTop
        NumGlyphs = 2
        Spacing = 0
        OnClick = sbnGerarClick
      end
      object sbnExportar: TSpeedButton
        Left = 255
        Top = 2
        Width = 130
        Height = 35
        Hint = 'gerar dados FCI'
        Caption = 'Gerar arquivo TXT F8'
        Flat = True
        Glyph.Data = {
          
          DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
        Layout = blGlyphTop
        NumGlyphs = 2
        Spacing = 0
        OnClick = sbnExportarClick
      end
    end
  end
  object gbxArquivo: TGroupBox
    Left = 0
    Top = 542
    Width = 1466
    Height = 41
    Align = alBottom
    Caption = ' ARQUIVO DE REMESSA '
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'helvetica'
    Font.Pitch = fpVariable
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 2
    object lblArquivo: TLabel
      Left = 5
      Top = 16
      Width = 557
      Height = 18
      AutoSize = False
      Caption = 'FCI_DDMMYYY_N.TXT'
      Color = clWhite
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentColor = False
      ParentFont = False
    end
    object sbnArquivo: TSpeedButton
      Left = 566
      Top = 13
      Width = 81
      Height = 22
      Caption = 'Arqui&vo '
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      Glyph.Data = {
        
        5555575FFF755555555557000075555555555577775555555555}
      NumGlyphs = 2
      ParentFont = False
      OnClick = sbnArquivoClick
    end
    object ckbMarcarTodos: TCheckBox
      Left = 928
      Top = 16
      Width = 140
      Height = 17
      Caption = 'Marcar todos os PN'#39's'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      OnClick = ckbMarcarTodosClick
    end
  end
  object gbxDados: TGroupBox
    Left = 185
    Top = 40
    Width = 1281
    Height = 502
    Align = alClient
    Caption = ' DADOS PARA REMESSA '
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'helvetica'
    Font.Pitch = fpVariable
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 3
    object dbgfciRetorno: TtecDBGrid
      Left = 2
      Top = 16
      Width = 1277
      Height = 484
      Align = alClient
      DataSource = dtmFCI.dsrFCIRetornoProdutos
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -9
      TitleFont.Name = 'helvetica'
      TitleFont.Pitch = fpVariable
      TitleFont.Style = []
      OnDrawColumnCell = dbgfciRemessaDrawColumnCell
      RowWrap = 60
      Large = False
      DoubleRowColor = False
      TitleMinHeight = 100
      CellHeights = 100
      StrippedColor = 16054260
      CanDelete = False
      PostOnEnter = False
      DenySort = False
      DefaultRowHeight = 19
      ExibirNumerodaLinha = False
      Columns = <
        item
          Expanded = False
          FieldName = 'pn'
          Title.Alignment = taCenter
          Title.Caption = 'PN'
          Width = 40
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'codigovisual'
          Title.Alignment = taCenter
          Title.Caption = 'C'#211'DIGO'
          Width = 160
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'descricao'
          Title.Alignment = taCenter
          Title.Caption = 'DESCRI'#199#195'O'
          Width = 200
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ncm'
          Title.Alignment = taCenter
          Title.Caption = 'NCM'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'gtin'
          Title.Alignment = taCenter
          Title.Caption = 'GTIN'
          Width = 55
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'unidade'
          Title.Alignment = taCenter
          Title.Caption = 'UNIDADE'
          Width = 50
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'valorsaida'
          Title.Alignment = taCenter
          Title.Caption = 'PRE'#199'O DE VENDA'
          Width = 100
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'valorimportacao'
          Title.Alignment = taCenter
          Title.Caption = 'VALOR IMPORTA'#199#195'O'
          Width = 100
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'percimportacao'
          Title.Alignment = taCenter
          Title.Caption = 'PERCENTUAL'
          Width = 80
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'codigo_fci'
          Title.Alignment = taCenter
          Title.Caption = 'C'#211'DIGO FCI'
          Width = 120
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'in_validacao_ficha'
          Title.Alignment = taCenter
          Title.Caption = 'INDICADOR DE VALIDA'#199#195'O'
          Width = 150
          Visible = True
        end>
    end
    object dbgfciRemessa: TtecDBGrid
      Left = 106
      Top = 49
      Width = 1277
      Height = 483
      DataSource = dtmFCI.dsrDadosFCI
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -9
      TitleFont.Name = 'helvetica'
      TitleFont.Pitch = fpVariable
      TitleFont.Style = []
      OnDrawColumnCell = dbgfciRemessaDrawColumnCell
      RowWrap = 60
      Large = False
      DoubleRowColor = False
      TitleMinHeight = 100
      CellHeights = 100
      StrippedColor = 16054260
      CanDelete = False
      PostOnEnter = False
      DenySort = False
      DefaultRowHeight = 19
      ExibirNumerodaLinha = False
      Columns = <
        item
          Expanded = False
          Width = 0
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'marcar'
          Title.Alignment = taCenter
          Title.Caption = 'MARCAR'
          Width = 45
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'pn'
          Title.Alignment = taCenter
          Width = 40
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'codigovisual'
          Title.Alignment = taCenter
          Width = 160
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'descricao'
          Title.Alignment = taCenter
          Width = 200
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'ncm'
          Title.Alignment = taCenter
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'gtin'
          Title.Alignment = taCenter
          Title.Caption = 'GTIN'
          Width = 55
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'unidade'
          Title.Alignment = taCenter
          Title.Caption = 'UNIDADE'
          Width = 50
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'precovenda'
          Title.Alignment = taCenter
          Width = 100
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'valorimportacao'
          Title.Alignment = taCenter
          Width = 100
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'percimportacao'
          Title.Alignment = taCenter
          Title.Caption = 'PERCENTUAL'
          Width = 80
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DescricaoErro'
          Title.Alignment = taCenter
          Title.Caption = 'DESCRI'#199#195'O DO ERRO'
          Width = 340
          Visible = True
        end>
    end
  end
  object pnlEsquerda: TPanel
    Left = 0
    Top = 40
    Width = 185
    Height = 502
    Align = alLeft
    BevelOuter = bvNone
    TabOrder = 4
    object rgpOperacao: TRadioGroup
      Left = 0
      Top = 0
      Width = 185
      Height = 57
      Align = alTop
      Caption = 'Opera'#231#227'o'
      ItemIndex = 0
      Items.Strings = (
        'Remessa'
        'Retorno')
      TabOrder = 0
      OnClick = rgpOperacaoClick
    end
    object gbxPNs: TGroupBox
      Left = 0
      Top = 57
      Width = 185
      Height = 445
      Align = alClient
      Caption = ' PN'#180's '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 1
      inline fraSelecaoAleatoriaPN: TfraSelecaoAleatoria
        Left = 2
        Top = 16
        Width = 181
        Height = 427
        HorzScrollBar.Range = 26
        Align = alClient
        AutoScroll = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Helvetica'
        Font.Pitch = fpVariable
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 0
        inherited dbgSelecaoAleatoria: TtecDBGrid
          Width = 142
          Height = 427
          Align = alLeft
          Font.Height = -12
          Font.Name = 'helvetica'
          Font.Style = [fsBold]
          PostOnEnter = True
          DefaultRowHeight = 19
          Columns = <
            item
              Expanded = False
              FieldName = 'codigo'
              Title.Alignment = taCenter
              Title.Caption = 'PART NUMBER'
              Title.Font.Charset = DEFAULT_CHARSET
              Title.Font.Color = clBlack
              Title.Font.Height = -9
              Title.Font.Name = 'Helvetica'
              Title.Font.Pitch = fpVariable
              Title.Font.Style = []
              Width = 101
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'descricao'
              ReadOnly = True
              Title.Caption = 'Descri'#231#227'o'
              Width = 0
              Visible = True
            end>
        end
        inherited pnlLabels: TPanel
          Left = 153
          Width = 28
          Height = 427
          inherited sbnProcura: TSpeedButton
            Left = -1
          end
          inherited sbnExcluir: TSpeedButton
            Left = -1
          end
          inherited sbnExcluirTodos: TSpeedButton
            Left = -1
          end
          inherited sbnIncluirItem: TSpeedButton
            Left = -1
          end
        end
        inherited qrySelecaoAleatoria: TtecQuery
          Sql.Strings = (
            'SELECT  Codigo,'
            '         descricao'
            'FROM produtos'
            'WHERE false')
        end
      end
    end
  end
  object dlgArquivoaExportar: TSaveDialog
    Title = 'Save As'
    Left = 440
  end
  object aclHabilitar: TActionList
    Left = 608
    object actHabilitar: TAction
      Caption = 'actHabilitar'
      OnUpdate = actHabilitarUpdate
    end
  end
end
