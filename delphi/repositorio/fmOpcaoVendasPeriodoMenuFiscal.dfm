inherited frmOpcaoVendasPeriodoMenuFiscal: TfrmOpcaoVendasPeriodoMenuFiscal
  Left = 482
  Top = 174
  ActiveControl = rbnOpcaoVendasPeriodo
  BorderIcons = [biSystemMenu, biMinimize]
  Caption = 'Vendas do Per'#237'odo'
  ClientHeight = 303
  ClientWidth = 255
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  KeyPreview = False
  OldCreateOrder = False
  PixelsPerInch = 96
  TextHeight = 13
  object rbnOpcaoVendasPeriodo: TRadioGroup
    Left = 44
    Top = 0
    Width = 146
    Height = 73
    Caption = 'Selecione o Formato'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Arial'
    Font.Pitch = fpVariable
    Font.Style = []
    ItemIndex = 0
    Items.Strings = (
      '&Conv'#234'nio 57/95'
      '&Ato COTEPE/ICMS 09/08')
    ParentFont = False
    TabOrder = 0
    OnClick = rbnOpcaoVendasPeriodoClick
  end
  object bbnOK: TBitBtn
    Left = 34
    Top = 252
    Width = 80
    Height = 25
    Caption = '&OK'
    Default = True
    TabOrder = 1
    TabStop = False
    OnClick = bbnOKClick
    Glyph.Data = {
      
      0000}
    NumGlyphs = 2
  end
  object bbnCancelar: TBitBtn
    Left = 121
    Top = 252
    Width = 80
    Height = 25
    Caption = '&Cancelar'
    TabOrder = 2
    TabStop = False
    Kind = bkCancel
  end
  object gbxFaixas: TGroupBox
    Left = 45
    Top = 81
    Width = 142
    Height = 73
    Caption = 'PER'#205'ODOs'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -12
    Font.Name = 'Helvetica'
    Font.Pitch = fpVariable
    Font.Style = []
    ParentFont = False
    TabOrder = 3
    object lblDe: TLabel
      Left = 8
      Top = 23
      Width = 14
      Height = 15
      Caption = 'de'
    end
    object lblAte: TLabel
      Left = 8
      Top = 47
      Width = 17
      Height = 15
      Caption = 'at'#233
    end
    object pnlCupons: TPanel
      Left = 34
      Top = 15
      Width = 86
      Height = 53
      BevelOuter = bvNone
      TabOrder = 1
      object edtCupomInicial: TEditNumero
        Left = 2
        Top = 2
        Width = 80
        Height = 23
        MaxLength = -1
        TabOrder = 0
        Mascara = True
        Alignment = taLeftJustify
        TipoMascara = tmGERAL
        NrDecimal = 0
        Decimais = False
        Negativo = False
        Tamanho = 9
      end
      object edtCupomFinal: TEditNumero
        Left = 2
        Top = 27
        Width = 80
        Height = 23
        MaxLength = -1
        TabOrder = 1
        Mascara = True
        Alignment = taLeftJustify
        TipoMascara = tmGERAL
        NrDecimal = 0
        Decimais = False
        Negativo = False
        Tamanho = 9
      end
    end
    object pnlDatas: TPanel
      Left = 34
      Top = 15
      Width = 86
      Height = 53
      BevelOuter = bvNone
      TabOrder = 0
      object edtPeriodoInicial: TEditData
        Left = 2
        Top = 2
        Width = 80
        Height = 23
        TabOrder = 0
        Minimo = 37353
        Maximo = 0
      end
      object edtPeriodoFinal: TEditData
        Left = 2
        Top = 27
        Width = 80
        Height = 23
        TabOrder = 1
        Minimo = 37353
        Maximo = 0
      end
    end
  end
  object GroupBox1: TGroupBox
    Left = 45
    Top = 177
    Width = 142
    Height = 52
    Caption = 'Data Posi'#231#227'o Estoque'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -12
    Font.Name = 'Helvetica'
    Font.Pitch = fpVariable
    Font.Style = []
    ParentFont = False
    TabOrder = 4
    object Panel1: TPanel
      Left = 34
      Top = 15
      Width = 86
      Height = 32
      BevelOuter = bvNone
      TabOrder = 1
      object EditNumero1: TEditNumero
        Left = 2
        Top = 2
        Width = 80
        Height = 23
        MaxLength = -1
        TabOrder = 0
        Mascara = True
        Alignment = taLeftJustify
        TipoMascara = tmGERAL
        NrDecimal = 0
        Decimais = False
        Negativo = False
        Tamanho = 9
      end
    end
    object Panel2: TPanel
      Left = 34
      Top = 15
      Width = 86
      Height = 28
      BevelOuter = bvNone
      TabOrder = 0
      object EdtDataInventario: TEditData
        Left = 2
        Top = 2
        Width = 80
        Height = 23
        TabOrder = 0
        Minimo = 37353
        Maximo = 0
      end
    end
  end
end
