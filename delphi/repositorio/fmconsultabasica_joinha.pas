...
begin
{
  ...
  if source.dataset.findfield(NomeCampo) <> nil then
    TZDataSet(source.dataset).SortByField(NomeCampo);
...
//    Source.DataSet := nil;
    ...
    dbgConsulta.datasource.dataset := nil;
    Source.DataSet := nil;
    Source.DataSet := value;
    dbgConsulta.datasource.dataset := Source.DataSet;
...
{    dbgConsulta.DataSource.Enabled := false; problema cadastro endereço}
    ...
    dbgConsulta.datasource.dataset := DataSet;
...
//  DataSet.Refresh;
  AtualizarQuantidadeRegistros(DataSet);
