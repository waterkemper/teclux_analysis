unit fmimprimeboletografico;

interface

uses
  SysUtils, Types, Classes, Graphics, Controls, Forms, Dialogs,
  StdCtrls, RLReport, RLBarcode, RLPrintDialog, RLHTMLFilter,
  RLFilters, RLPDFFilter, RLConsts, ACBrBoleto, ACBrBase, clparametrossistema,
  RLPreviewForm, ACBrBoletoConversao;
...
type
  ...
  TfrmImprimeBoletoGrafico = class(TForm)
...
//    RLPrintDialogSetup1: TRLPrintDialogSetup;
    lblLocalPagto2: TRLLabel;
    txtLocalPagamento4: TRLLabel;
    RLPreviewSetup1: TRLPreviewSetup;
    procedure FormCreate(Sender: TObject);
    procedure LayoutBoletoBeforePrint(Sender: TObject;
      var PrintIt: Boolean);
    procedure LayoutBoletoDataCount(Sender: TObject;
      var DataCount: Integer);
    procedure LayoutBoletoDataRecord(Sender: TObject; RecNo,
...
var
  frmImprimeBoletoGrafico: TfrmImprimeBoletoGrafico;
...
var
  frBoletoFortes : TfrmImprimeBoletoGrafico;
  RLFiltro : TRLCustomSaveFilter;
begin
  inherited Imprimir;    // Executa verificações padroes

  frBoletoFortes := TfrmImprimeBoletoGrafico.Create(Self);
  try
     with frBoletoFortes do
     begin
        DirLogo := ParSistema.DiretorioImagens;
...
function TfrmImprimeBoletoGrafico.GetACBrTitulo: TACBrTitulo;
begin
  Result := fBoletoFC.ACBrBoleto.ListadeBoletos[ fIndice ] ;
end;

procedure TfrmImprimeBoletoGrafico.FormCreate(Sender: TObject);
begin
  fIndice   := 0 ;
  fBoletoFC := TACBrBoletoFCFortes(Owner) ;  // Link para o Pai
end;

procedure TfrmImprimeBoletoGrafico.LayoutBoletoBeforePrint(
  Sender: TObject; var PrintIt: Boolean);
begin
  fIndice := 0 ;
end;

procedure TfrmImprimeBoletoGrafico.LayoutBoletoDataCount(
  Sender: TObject; var DataCount: Integer);
begin
  DataCount := fBoletoFC.ACBrBoleto.ListadeBoletos.Count ;
end;

procedure TfrmImprimeBoletoGrafico.LayoutBoletoDataRecord(
  Sender: TObject; RecNo, CopyNo: Integer; var Eof: Boolean;
  var RecordAction: TRLRecordAction);
begin
  fIndice := RecNo - 1 ;
...
procedure TfrmImprimeBoletoGrafico.RLBand1BeforePrint(Sender: TObject;
  var PrintIt: Boolean);
Var
   NossoNum,
...
begin
  ...
  begin
    ...
    AgenciaCodigoCedente := Banco.MontarCampoCodigoCedente(Titulo);

//    fBoletoFC.CarregaLogo( imgBanco2.Picture, Banco.Numero );  gg
    txtNumeroBanco2.Caption         := IntToStrZero(Banco.Numero, 3)+ '-' +
                                       IntToStrZero(Banco.Digito, 1);
//    lblLocalPagto1.Caption          := Titulo.LocalPagamento1;   gg
//    lblLocalPagto2.Caption          := Titulo.LocalPagamento2; gg
    txtDataVencimento2.Caption      := FormatDateTime('dd/mm/yyyy', Titulo.Vencimento);
    txtNomeCedente2.Caption         := Cedente.Nome;
    txtCodigoCedente2.Caption       := AgenciaCodigoCedente;
...
procedure TfrmImprimeBoletoGrafico.RLBand2BeforePrint(Sender: TObject;
  var PrintIt: Boolean);
var
...
begin
  ...
  begin
    ...
    txtLocalPagamento3.Caption    := lblLocalPagto1.Caption;
    txtLocalPagamento4.Caption    := lblLocalPagto2.Caption;
