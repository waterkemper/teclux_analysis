object fraCartaCorrecaoEletronica: TfraCartaCorrecaoEletronica
  Left = 0
  Top = 0
  Width = 661
  Height = 225
  TabOrder = 0
  object StaticText1: TStaticText
    Left = 0
    Top = 0
    Width = 661
    Height = 12
    Align = alTop
    Alignment = taCenter
    Caption = 'CARTA DE CORRE'#199#195'O ELETR'#212'NICA'
    Color = 14408667
    Constraints.MaxHeight = 12
    Constraints.MinHeight = 12
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentColor = False
    ParentFont = False
    TabOrder = 1
  end
  object tblBarraCadastro: TToolBar
    Left = 0
    Top = 12
    Width = 661
    Height = 35
    ButtonHeight = 30
    ButtonWidth = 75
    Caption = 'tblBarra'
    Color = clBtnFace
    Constraints.MaxHeight = 35
    Constraints.MinHeight = 35
    EdgeBorders = [ebTop, ebBottom]
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -12
    Font.Name = 'helvetica'
    Font.Pitch = fpVariable
    Font.Style = []
    ParentColor = False
    ParentFont = False
    TabOrder = 0
    DesignSize = (
      661
      31)
    object sbnIncluir: TSpeedButton
      Left = 0
      Top = 2
      Width = 107
      Height = 30
      Anchors = [akTop, akRight]
      Constraints.MaxHeight = 30
      Constraints.MinHeight = 30
      Caption = '  Nova'
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -12
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      Glyph.Data = {
        
        DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
      NumGlyphs = 2
      ParentFont = False
      Spacing = 0
      OnClick = sbnIncluirClick
    end
    object sbnSalvar: TSpeedButton
      Left = 107
      Top = 2
      Width = 107
      Height = 30
      Hint = 'Grava a Altera'#231#227'o ou Inclus'#227'o na Tabela'
      Anchors = [akTop, akRight]
      Constraints.MaxHeight = 30
      Constraints.MinHeight = 30
      Caption = '  Gravar'
      Enabled = False
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -12
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      Glyph.Data = {
        
        7F7F7F7F7F7F7F7F7F7F7F7F7F7F7F7F7F7F7F7FC3C3C3C3C3C3}
      NumGlyphs = 2
      ParentFont = False
      Spacing = 0
      OnClick = sbnSalvarClick
    end
    object sbnImprmirNota: TSpeedButton
      Left = 214
      Top = 2
      Width = 107
      Height = 30
      Hint = 'Reimprimir a Nota => F7'
      Constraints.MaxHeight = 30
      Constraints.MinHeight = 30
      Caption = '  Imprimir'
      Flat = True
      Glyph.Data = {
        
        DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC000000}
      Spacing = 0
      OnClick = sbnImprmirNotaClick
    end
    object sbnEnviarNfe: TSpeedButton
      Left = 321
      Top = 2
      Width = 104
      Height = 30
      Hint = 'Enviar or'#231'amento para o cliente'
      Caption = '   Enviar F12'
      Flat = True
      Glyph.Data = {
        
        DCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDCDC}
      NumGlyphs = 2
      Spacing = 0
      OnClick = sbnEnviarNfeClick
    end
  end
  object pnlFundo: TPanel
    Left = 0
    Top = 47
    Width = 661
    Height = 178
    Align = alClient
    BorderWidth = 2
    Color = 14408667
    TabOrder = 2
    DesignSize = (
      661
      178)
    object sbnAux: TSpeedButton
      Left = 321
      Top = 2
      Width = 8
      Height = 7
      Action = Action1
      Anchors = [akTop, akRight]
      Flat = True
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -12
      Font.Name = 'helvetica'
      Font.Pitch = fpVariable
      Font.Style = []
      NumGlyphs = 2
      ParentFont = False
      Spacing = 0
    end
    object pnlCartaCorrecao: TPanel
      Left = 3
      Top = 3
      Width = 257
      Height = 172
      Align = alLeft
      BevelOuter = bvNone
      Caption = 'pnlCartaCorrecao'
      TabOrder = 0
      object dbgCartaCorrecao: TtecDBGrid
        Left = 0
        Top = 0
        Width = 257
        Height = 172
        Align = alClient
        DataSource = dsrCartasCorrecao
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'helvetica'
        Font.Pitch = fpVariable
        Font.Style = []
        Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
        ParentFont = False
        ReadOnly = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clBlack
        TitleFont.Height = -13
        TitleFont.Name = 'helvetica'
        TitleFont.Pitch = fpVariable
        TitleFont.Style = []
        RowWrap = 60
        Large = False
        DoubleRowColor = True
        TitleMinHeight = 100
        CellHeights = 100
        StrippedColor = 16054260
        CanDelete = False
        CanNotInsertFromGrid = False
        PostOnEnter = False
        PostOnSetUpDown = False
        DenySort = False
        DefaultRowHeight = 19
        ExibirNumerodaLinha = False
        NaoAtribuirDadosaTabelaaoDigitar = False
        ShowWhenFieldInvisible = False
        CanClickWhenEditing = False
        Columns = <
          item
            Expanded = False
            FieldName = 'nseqevento'
            Title.Alignment = taCenter
            Title.Caption = 'N'#186
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clBlack
            Title.Font.Height = -9
            Title.Font.Name = 'helvetica'
            Title.Font.Pitch = fpVariable
            Title.Font.Style = []
            Width = 25
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'numprotocolonfe'
            Title.Alignment = taCenter
            Title.Caption = 'PROTOCOLO'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clBlack
            Title.Font.Height = -9
            Title.Font.Name = 'helvetica'
            Title.Font.Pitch = fpVariable
            Title.Font.Style = []
            Width = 120
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'dhevento'
            Title.Alignment = taCenter
            Title.Caption = 'DATA'
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clBlack
            Title.Font.Height = -9
            Title.Font.Name = 'helvetica'
            Title.Font.Pitch = fpVariable
            Title.Font.Style = []
            Width = 70
            Visible = True
          end>
      end
    end
    object gbxCorrecao: TGroupBox
      Left = 260
      Top = 3
      Width = 398
      Height = 172
      Align = alClient
      Caption = 'TEXTO PARA CORRE'#199#195'O DA NFE'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -9
      Font.Name = 'HELVETICA'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      object mmoXCorrecao: TtecDBMemo
        Left = 2
        Top = 14
        Width = 394
        Height = 156
        Align = alClient
        DataField = 'texto'
        DataSource = dsrCartasCorrecao
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        OnKeyPress = mmoXCorrecaoKeyPress
      end
    end
  end
  object spcCartasCorrecaoProximoCodigo: TtecQuery
    Tag = -1
    Database = dtmTecSoft.dbaTecSoft
    Transaction = dtmTecSoft.tstTecSoft
    CachedUpdates = False
    ShowRecordTypes = [ztModified, ztInserted, ztUnmodified]
    Options = [doAutoFillDefs, doUseRowId]
    LinkOptions = [loAlwaysResync]
    Constraints = <>
    ExtraOptions = [poTextAsMemo, poOidAsBlob]
    Macros = <>
    Sql.Strings = (
      'select nextval('#39'cartas_correcao_proximocodigo'#39') as codigo')
    RequestLive = False
    Left = 408
    Top = 40
    object spcCartasCorrecaoProximoCodigocodigo: TLargeintField
      FieldName = 'codigo'
    end
  end
  object spcCartasCorrecaoProximaSequencia: TtecQuery
    Tag = -1
    Database = dtmTecSoft.dbaTecSoft
    Transaction = dtmTecSoft.tstTecSoft
    CachedUpdates = False
    ShowRecordTypes = [ztModified, ztInserted, ztUnmodified]
    Options = [doAutoFillDefs, doUseRowId]
    LinkOptions = [loAlwaysResync]
    Constraints = <>
    ExtraOptions = [poTextAsMemo, poOidAsBlob]
    Macros = <>
    Sql.Strings = (
      
        'select cast(cartas_correcao_proximasequencia(:chv_nfe) as varcha' +
        'r(2)) as sequencia')
    RequestLive = False
    Left = 440
    Top = 40
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'chv_nfe'
        ParamType = ptUnknown
      end>
    object spcCartasCorrecaoProximaSequenciasequencia: TStringField
      FieldName = 'sequencia'
      Size = 2
    end
  end
  object qryCartasCorrecao: TtecQuery
    Tag = -1
    Database = dtmTecSoft.dbaTecSoft
    Transaction = dtmTecSoft.tstTecSoft
    CachedUpdates = True
    ShowRecordTypes = [ztModified, ztInserted, ztUnmodified]
    Options = [doAutoFillDefs]
    LinkOptions = [loAlwaysResync]
    Constraints = <>
    AfterInsert = qryCartasCorrecaoAfterInsert
    ExtraOptions = [poTextAsMemo, poOidAsBlob]
    Macros = <>
    Sql.Strings = (
      'select *'
      'from cartas_correcao'
      'WHERE chv_nfe=:chv_nfe'
      'ORDER BY nseqevento desc')
    RequestLive = True
    Left = 472
    Top = 40
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'chv_nfe'
        ParamType = ptUnknown
      end>
    object qryCartasCorrecaocodigo: TIntegerField
      DisplayLabel = 'C'#243'digo'
      FieldName = 'codigo'
      Required = True
    end
    object qryCartasCorrecaotiponota: TStringField
      DisplayLabel = 'Tipo de Nota'
      FieldName = 'tiponota'
      Required = True
      Size = 1
    end
    object qryCartasCorrecaocodigonota: TIntegerField
      DisplayLabel = 'Codigo da Nota / Dado Fiscal'
      FieldName = 'codigonota'
      Required = True
    end
    object qryCartasCorrecaochv_nfe: TStringField
      DisplayLabel = 'Chave da NFE'
      FieldName = 'chv_nfe'
      Required = True
      Size = 44
    end
    object qryCartasCorrecaotexto: TStringField
      DisplayLabel = 'Texto da Corre'#231#227'o'
      FieldName = 'texto'
      Size = 1000
    end
    object qryCartasCorrecaoambiente: TIntegerField
      DisplayLabel = 'Ambiente'
      FieldName = 'ambiente'
      Required = True
    end
    object qryCartasCorrecaodhevento: TDateTimeField
      DisplayLabel = 'Data do Evento'
      FieldName = 'dhevento'
      Required = True
    end
    object qryCartasCorrecaotpevento: TIntegerField
      DisplayLabel = 'Tipo do Evento'
      FieldName = 'tpevento'
      Required = True
    end
    object qryCartasCorrecaonseqevento: TIntegerField
      DisplayLabel = 'Sequ'#234'ncia do Evento'
      FieldName = 'nseqevento'
      Required = True
    end
    object qryCartasCorrecaoverevento: TStringField
      DisplayLabel = 'Vers'#227'o do Evento'
      FieldName = 'verevento'
      Required = True
      Size = 1
    end
    object qryCartasCorrecaonumprotocolonfe: TStringField
      FieldName = 'numprotocolonfe'
      Size = 15
    end
  end
  object dsrCartasCorrecao: TtecDataSource
    DataSet = qryCartasCorrecao
    Left = 504
    Top = 48
  end
  object qryUltimaCartaCorrecao: TtecQuery
    Tag = -1
    Database = dtmTecSoft.dbaTecSoft
    Transaction = dtmTecSoft.tstTecSoft
    CachedUpdates = False
    ShowRecordTypes = [ztModified, ztInserted, ztUnmodified]
    Options = [doAutoFillDefs, doUseRowId]
    LinkOptions = [loAlwaysResync]
    Constraints = <>
    ExtraOptions = [poTextAsMemo, poOidAsBlob]
    Macros = <>
    Sql.Strings = (
      'select cc.*,'
      ''
      '       case when cc.tiponota='#39'S'#39' then'
      '         (select vf.nome'
      '          from vfornecedores vf'
      
        '          where (vf.codigo,vf.tipo) in (select df.cliente, df.ti' +
        'pocliente'
      '                                        from dadosfiscais df'
      
        '                                        where df.numero = cc.cod' +
        'igonota))'
      '       else'
      '         (select vf.nome'
      '          from vfornecedores vf'
      
        '          where (vf.codigo,vf.tipo) in (select np.fornecedor, np' +
        '.tipofornecedor'
      '                                        from notaspag np'
      
        '                                        where np.codigo = cc.cod' +
        'igonota))'
      '       end as nome,'
      ''
      '       case when cc.tiponota='#39'S'#39' then'
      '         (select vf.email'
      '          from vfornecedores vf'
      
        '          where (vf.codigo,vf.tipo) in (select df.cliente, df.ti' +
        'pocliente'
      '                                        from dadosfiscais df'
      
        '                                        where df.numero = cc.cod' +
        'igonota))'
      '       else'
      '         (select vf.email'
      '          from vfornecedores vf'
      
        '          where (vf.codigo,vf.tipo) in (select np.fornecedor, np' +
        '.tipofornecedor'
      '                                        from notaspag np'
      
        '                                        where np.codigo = cc.cod' +
        'igonota))'
      '       end as email,'
      ''
      '       case when cc.tiponota='#39'S'#39' then'
      '         (select df.nrdocumento'
      '          from dadosfiscais df'
      '          where df.numero = cc.codigonota)'
      '       else'
      '         (select np.numero'
      '          from notaspag np'
      '          where np.codigo = cc.codigonota)'
      '       end as nrdocumento,'
      ''
      '       case when cc.tiponota='#39'S'#39' then'
      '         (select n.serie'
      
        '          from dadosfiscais df join notas n on df.numero = n.dad' +
        'ofiscal'
      '          where df.numero = cc.codigonota)'
      '       else'
      '         (select np.serie'
      '          from notaspag np'
      '          where np.codigo = cc.codigonota)'
      '       end as serie,'
      ''
      '       case when cc.tiponota='#39'S'#39' then'
      '         (select df.filialemissao'
      '          from dadosfiscais df'
      '          where df.numero = cc.codigonota)'
      '       else'
      '         (select np.filial'
      '          from notaspag np'
      '          where np.codigo = cc.codigonota)'
      '       end as filial,'
      ''
      '       case when cc.tiponota='#39'S'#39' then'
      '         (select df.valortotal'
      '          from dadosfiscais df'
      '          where df.numero = cc.codigonota)'
      '       else'
      '         (select np.valornota'
      '          from notaspag np'
      '          where np.codigo = cc.codigonota)'
      '       end as valor'
      ''
      'from cartas_correcao cc'
      ''
      'WHERE cc.chv_nfe= :chv_nfe'
      ''
      'ORDER BY cc.nseqevento desc limit 1')
    RequestLive = False
    Left = 536
    Top = 40
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'chv_nfe'
        ParamType = ptUnknown
      end>
    object qryUltimaCartaCorrecaocodigo: TIntegerField
      FieldName = 'codigo'
    end
    object qryUltimaCartaCorrecaotiponota: TStringField
      FieldName = 'tiponota'
      Size = 1
    end
    object qryUltimaCartaCorrecaocodigonota: TIntegerField
      FieldName = 'codigonota'
    end
    object qryUltimaCartaCorrecaochv_nfe: TStringField
      FieldName = 'chv_nfe'
      Size = 44
    end
    object qryUltimaCartaCorrecaotexto: TStringField
      FieldName = 'texto'
      Size = 512
    end
    object qryUltimaCartaCorrecaoambiente: TIntegerField
      FieldName = 'ambiente'
    end
    object qryUltimaCartaCorrecaodhevento: TDateTimeField
      FieldName = 'dhevento'
    end
    object qryUltimaCartaCorrecaotpevento: TIntegerField
      FieldName = 'tpevento'
    end
    object qryUltimaCartaCorrecaonseqevento: TIntegerField
      FieldName = 'nseqevento'
    end
    object qryUltimaCartaCorrecaoverevento: TStringField
      FieldName = 'verevento'
      Size = 1
    end
    object qryUltimaCartaCorrecaonumprotocolonfe: TStringField
      FieldName = 'numprotocolonfe'
      Size = 15
    end
    object qryUltimaCartaCorrecaonome: TStringField
      FieldName = 'nome'
      Size = 50
    end
    object qryUltimaCartaCorrecaoemail: TStringField
      FieldName = 'email'
      Size = 50
    end
    object qryUltimaCartaCorrecaonrdocumento: TIntegerField
      FieldName = 'nrdocumento'
    end
    object qryUltimaCartaCorrecaoserie: TStringField
      FieldName = 'serie'
      Size = 3
    end
    object qryUltimaCartaCorrecaofilial: TIntegerField
      FieldName = 'filial'
    end
    object qryUltimaCartaCorrecaovalor: TFloatField
      FieldName = 'valor'
    end
  end
  object ActionList1: TActionList
    Left = 552
    Top = 16
    object Action1: TAction
      Caption = 'Action1'
      OnUpdate = Action1Update
    end
  end
end
