...
procedure Tfradbcgcoucpf.AlteraTipoPessoa;
begin
...
constructor Tfradbcgcoucpf.Create(AOwner: TComponent);
begin
  inherited;
...
procedure Tfradbcgcoucpf.edtCPFCNPJChangeTipo(Sender: TObject);
begin
  AlteraTipoPessoa;
end;

procedure Tfradbcgcoucpf.edtCPFCNPJKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    Key:= #0;
end;

procedure Tfradbcgcoucpf.rbnJuridicaClick(Sender: TObject);
begin
...
procedure Tfradbcgcoucpf.rbnFisicaClick(Sender: TObject);
begin
  if not rgbTipoPessoa.ReadOnly then
  begin
