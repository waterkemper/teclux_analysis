...
**Correção (2026-07-24, achado no ticket 20 do mapa de investigação)**: `minimo`/`maximo` só são editáveis quando **exatamente uma filial** está marcada no filtro (`qryComprasTotalminimo.readonly := not SomenteUmaFilialMarcada`, `dmconsultacompras.pas:3650-3651` — `SomenteUmaFilialMarcada` é `true` só quando a lista de filiais marcadas tem exatamente um item, nem zero nem várias). Faz sentido: o grid principal pode agregar várias filiais numa única linha (a depender do checkbox "Usar Tabela Redutora" — ver `11-speckit-prompt-correcao-agrupamento-produtos-total.md`); editar mínimo/máximo só é seguro quando há exatamente uma filial em jogo na linha. **Correção à proposta abaixo**: os campos `minimo`/`maximo` devem ficar **somente leitura quando o filtro atual não tiver exatamente uma filial selecionada** (nenhuma, ou mais de uma) — não apenas sempre editáveis.
