...
Crie a especificação funcional completa para uma nova funcionalidade chamada **Ficha de Produtos**, localizada no contexto de Estoques do ERP tecLUX.

## Regra fundamental sobre as fontes

Você não possui acesso ao código Delphi e **não deve investigar, procurar, solicitar, citar ou depender de arquivos Delphi**. Não inclua tarefas de “analisar o legado”, “validar no Delphi” ou “comparar com o Delphi”. O levantamento já foi concluído e o contrato abaixo é a fonte autoritativa e autocontida.

Investigue somente o estado atual do Laravel para confirmar os nomes e contratos concretos de componentes, serviços, rotas, Policies, repositories, lookups, grids e testes que devem ser reutilizados. O objetivo dessa inspeção é integrar-se ao Laravel existente, não redescobrir requisitos. Se o Laravel divergir do contrato funcional abaixo, registre a divergência e especifique a adaptação necessária; não procure confirmação em outro código-fonte.

Não faça nova entrevista. Não deixe requisitos pendentes para “investigação posterior no Delphi”. Produza a spec a partir deste briefing.

## Orientações para a especificação

- Trate a primeira entrega como consulta estritamente somente leitura.
- Preserve a separação entre Ficha de Produtos, Cadastro de Produtos, Análise de Estoques, Movimentação Diária e Contratos.
- Priorize reuso dos componentes e serviços Laravel já existentes.
- Defina requisitos verificáveis e independentes de nomes de arquivos.
- Inclua fluxos felizes, validações, estados vazios, falhas, autorização, confidencialidade financeira, paginação, desempenho, observabilidade e acessibilidade.
- Inclua critérios de aceite para backend e frontend, mas não prescreva código-fonte.
- Registre explicitamente os comportamentos rejeitados que não podem ser reproduzidos.
- Use o vocabulário ERP do contrato.
- Considere a spec pronta para posterior planejamento e implementação por agentes sem contexto desta conversa.
...
Produza uma especificação completa e implementável que contenha, no mínimo:

1. objetivo e fronteiras da funcionalidade;
2. atores e capacidades;
3. histórias/cenários funcionais priorizados;
4. requisitos funcionais numerados;
5. requisitos não funcionais;
6. modelo conceitual dos filtros e resultados;
7. estados da interface;
8. autorização de módulo, Filial e dados financeiros;
9. contratos observáveis de Movimentos, Lotes, opening balance e Contratos;
10. critérios de aceite verificáveis;
11. estratégia de testes no seam HTTP/Inertia e seams puros necessários;
12. dependências e reuso do Laravel atual;
13. comportamentos explicitamente fora do escopo ou proibidos;
14. riscos de desempenho e medidas de otimização que preservem a geração síncrona obrigatória.

Não gere tarefas de implementação nesta etapa. Não crie requisitos de escrita. Não remeta o leitor ao Delphi, ao dossiê Wayfinder ou a esta conversa: a especificação gerada também deve permanecer autocontida.
```
