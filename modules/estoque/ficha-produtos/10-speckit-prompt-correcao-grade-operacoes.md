# Prompt para /speckit.specify — Correção da grade e operações da Ficha de Produtos

```text
/speckit.specify

Crie uma especificação funcional completa e implementável para a correção **Ficha de Produtos — Grade e Operações**, no contexto de Estoques do ERP tecLUX.

## Regra fundamental sobre as fontes

Você não possui acesso ao código Delphi e **não deve investigar, procurar, solicitar, citar ou depender de arquivos Delphi**. Não crie tarefas para analisar legado, validar no Delphi ou comparar implementações. O levantamento já foi concluído e o contrato autocontido abaixo é a fonte autoritativa.

Inspecione somente o Laravel atual para localizar nomes e contratos concretos de componentes, catálogos, DTOs, serviços, repositories, rotas, capacidades e testes que devem ser reutilizados. Essa inspeção serve apenas para integrar a correção ao módulo já implementado. Se o Laravel divergir do contrato abaixo, trate como lacuna a corrigir; não transforme a divergência em requisito pendente e não procure confirmação em outra fonte.

Não faça nova entrevista. Não recrie a Ficha de Produtos. Não gere tarefas de implementação nesta etapa. Produza a spec a partir deste briefing.

## Orientações obrigatórias

- A especificação deve seguir as seções Problem Statement, Solution, User Stories, Implementation Decisions, Testing Decisions, Out of Scope e Further Notes.
- Inclua uma lista extensa e numerada de histórias de usuário.
- Converta o contrato em requisitos funcionais e critérios de aceite verificáveis.
- Preserve a geração principal síncrona e paginada e a natureza somente leitura.
- Trate a grade como extrato histórico cuja ordem de linhas é semântica e imutável.
- Priorize reuso das peças Laravel atuais e evite criar seams desnecessários.
- Use como seam principal de teste o comportamento público HTTP/Inertia/JSON da Ficha; use testes de componente para estados visuais e acessibilidade e testes puros somente onde a costura principal não observar adequadamente catálogo/grupos.
- Não inclua caminhos específicos de arquivos ou código-fonte na spec; nomes concretos podem mudar.
- Registre explicitamente comportamentos proibidos e tudo que deve permanecer sem regressão.
- A spec resultante deve ser autocontida e não remeter ao Delphi, ao Wayfinder, ao dossiê ou a esta conversa.

## Contrato funcional e técnico autoritativo
...
A especificação deve conter, no mínimo:

1. problema e resultado esperado do ponto de vista do usuário;
2. atores, capacidades e confidencialidade financeira;
3. histórias de usuário extensas e priorizáveis;
4. requisitos funcionais numerados para catálogos, visão padrão, botões, preferências, busca, exportação e ordem do extrato;
5. requisitos não funcionais de desempenho, acessibilidade, segurança e observabilidade;
6. contrato observável das perspectivas Movimentos e Lotes;
7. estados dos botões combináveis e sua relação com preferências permanentes;
8. autorização financeira server-side em catálogo, payload, preferências, busca e exportação;
9. critérios de aceite e cenários negativos;
10. estratégia de testes no seam HTTP/Inertia/JSON e nos componentes de página;
11. dependências e reuso do Laravel atual;
12. plano de preservação dos filtros, lookups, geração síncrona, snapshots, saldo anterior, cálculos e histórico contratual;
13. comportamentos fora de escopo e proibidos;
14. riscos de paginação/exportação e como manter a ordem canônica.

Não produza implementação ou tickets nesta etapa.
```
